...
module Dummy = struct
  ...
  let get_entry_from_pid pid list =
    try Some (List.find (fun entry -> entry.d_pid = Some pid) list) with
    | _ ->
        None


  let get_entry_from_minor minor list =
    try Some (List.find (fun entry -> entry.d_minor = Some minor) list) with
    | _ ->
        None
        ...
        let list =
          List.map
            (fun e ->
              ...
              then { e with d_state = Some "0"; d_args = Some args }
              else e )
            list
        in
    ...
    Mutex.execute d_lock (fun () ->
        ...
        let list =
          List.map
            (fun e ->
              if e.d_pid = Some t.tapdisk_pid && e.d_minor = Some t.minor
              then { e with d_state = Some "0x2"; d_args = None }
              else e )
            list
        in
        write_dummy_tapdisk_list ctx list )
        ...
        List.filter_map
          (fun e ->
            ...
            | Some m, Some p, Some s, Some t ->
                if t.tapdisk_pid = p && t.minor = m
                then Some ({ tapdisk_pid = p; minor = m }, s, args)
                else None
            | _ ->
                None )
          list )
...
let attach ctx pid minor =
  if ctx.dummy
  then Dummy.attach ctx pid minor
  else
    let _ =
      invoke_tap_ctl
        ctx
...
let _open ctx t leaf_path driver =
  if ctx.dummy
  then Dummy._open ctx t leaf_path driver
  else
    ignore
      (invoke_tap_ctl
         ctx
...
let close ctx t =
  if ctx.dummy
  then Dummy.close ctx t
  else ignore (invoke_tap_ctl ctx "close" (args t))


let pause ctx t =
  if ctx.dummy
  then Dummy.pause ctx t
  else ignore (invoke_tap_ctl ctx "pause" (args t))


let unpause ctx t leaf_path driver =
  if ctx.dummy
  then Dummy.unpause ctx t leaf_path driver
  else
    ignore
      (invoke_tap_ctl
         ctx
...
let detach ctx t =
  if ctx.dummy
  then Dummy.detach ctx t
  else ignore (invoke_tap_ctl ctx "detach" (args t))


let free ctx minor =
  if ctx.dummy
  then Dummy.free ctx minor
  else ignore (invoke_tap_ctl ctx "free" [ "-m"; string_of_int minor ])


let list ?t ctx =
  if ctx.dummy
  then Dummy.list ?t ctx
  else
...
let stats ctx t =
  if ctx.dummy
  then Dummy.stats t
  else Stats.t_of_rpc (Jsonrpc.of_string (invoke_tap_ctl ctx "stats" (args t)))
