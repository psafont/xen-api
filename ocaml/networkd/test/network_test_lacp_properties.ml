...
let test_lacp_aggregation_key arg () =
  ...
  check_string_list
    "lacp_aggregation_key_correct_iface_props"
    correct_iface_props
    per_iface_props
...
let test_lacp_aggregation_key_vsctl arg () =
  ...
  Alcotest.(
    check
      pass
      "lacp_aggregation_key is passed to ovs-vsctl command"
      true
      (List.exists
  ...
  List.iter
    (fun arg ->
      Alcotest.(
        check
          bool
          "key=value argument pairs can't have missing values"
          true
