...
let default_error_handler script args stdout stderr status =
  ...
  in
  error
    "Call '%s %s' exited badly: %s [stdout = '%s'; stderr = '%s']"
    script
    (String.concat " " args)
    message
    stdout
...
let check_n_run
    ?(on_error = default_error_handler) ?(log = true) run_func script args =
  try
...
module Sysfs = struct
  let list_drivers () =
    try Array.to_list (Sys.readdir "/sys/bus/pci/drivers") with
    | _ ->
  ...
  let read_one_line file =
    ...
    | exn ->
        error
          "Error in read one line of file: %s, exception %s\n%s"
          file
  ...
  let assert_exists dev =
    if not @@ exists dev
    then raise (Network_error (Interface_does_not_exist dev))
  ...
  let get_features dev =
    try Some (int_of_string (read_one_line (getpath dev "features"))) with
    | _ ->
        None
  ...
  let set_multicast_snooping bridge value =
    ...
    | _ ->
        warn
          "Could not %s IGMP-snooping on bridge %s"
  ...
  let get_child_vfs_sysfs_paths dev =
    ...
    | _ ->
        Result.Error
          ( Vf_sysfs_path_not_found
          , "Can not get child vfs sysfs paths for " ^ dev )


  let device_index_of_vf parent_dev pcibuspath =
    try
      let open Rresult.R.Infix in
      get_child_vfs_sysfs_paths parent_dev
      >>= fun paths ->
      let group =
        List.find
  ...
  let unbind_child_vfs dev =
    ...
    let unbind vf_path =
      ...
      if driver_name = ""
      then Result.Ok () (* not bind to any driver, Ok *)
      else (
        debug
          "unbinding %s from driver %s at %s"
          vf_path
          driver_name
          vf_pcibuspath ;
        ...
        | _ ->
            Result.Error
              ( Fail_to_unbind_from_driver
              , Printf.sprintf
                  "%s: VF Fail to be unbound from driver %s"
                  vf_path
                  driver_name ) )
    in
    get_child_vfs_sysfs_paths dev
    >>= fun paths ->
  ...
  let set_sriov_numvfs dev num_vfs =
    ...
    try
      ...
      if get_sriov_numvfs dev = num_vfs
      then Result.Ok ()
...
module Ip = struct
  ...
  let link_set_mtu dev mtu =
    try ignore (link_set dev [ "mtu"; string_of_int mtu ]) with
    | Network_error (Script_error _) ->
  ...
  let get_ipv6_interface_id dev =
    ...
    let rec modified_bytes ac i = function
      ...
      | head :: tail ->
          ...
          then modified_bytes (254 :: 255 :: head :: ac) 3 tail
          else modified_bytes (head :: ac) (i + 1) tail
    in
  ...
  let get_ipv6_link_local_addr dev =
    ...
    let rec to_string ac i = function
      | [] ->
          ac
      | hd :: tl ->
          let separator = if i = 0 || i mod 2 = 1 then "" else ":" in
  ...
  let set_ip_addr dev (ip, prefixlen) =
    ...
    let broadcast =
      ...
      if String.contains addr '.' then [ "broadcast"; "+" ] else []
    in
    try ignore (call ([ "addr"; "add"; addr; "dev"; dev ] @ broadcast)) with
    | _ ->
        ()
  ...
  let set_ipv6_link_local_addr dev =
    ...
    try ignore (call [ "addr"; "add"; addr; "dev"; dev; "scope"; "link" ]) with
    | _ ->
        ()
  ...
  let create_vlan interface vlan =
    if not (Sysfs.exists (vlan_name interface vlan))
    then
      ignore
        (call
           [ "link"
  ...
  let set_vf_mac dev index mac =
    try
      debug
        "Setting VF MAC address for dev: %s, index: %d, MAC: %s"
        dev
        index
        mac ;
...
module Linux_bonding = struct
  ...
  let add_bond_master name =
    ...
    if is_bond_device name
    then debug "Bond master %s already exists, not creating" name
    else (
      debug "Adding bond master %s" name ;
      try Sysfs.write_one_line bonding_masters ("+" ^ name) with
      | _ ->
      ...
      let rec destroy retries =
        ...
        | _ ->
            ...
            then (
              Thread.delay 0.5 ;
              destroy (retries - 1) )
            else error "Failed to remove bond master %s" name
      in
      destroy 10
    else error "Bond master %s does not exist; cannot destroy it" name


  let get_bond_slaves master =
    let path = Sysfs.getpath master "bonding/slaves" in
    let slaves = Sysfs.read_one_line path in
    if slaves = "" then [] else Astring.String.cuts ~empty:false ~sep:" " slaves
  ...
  let set_bond_slaves master slaves =
    ...
    else error "Bond %s does not exist; cannot set slaves" master
  ...
  let with_slaves_removed master f =
    ...
    else error "Bond %s does not exist; cannot remove/add slaves" master
  ...
  let set_bond_properties master properties =
    ...
    else error "Bond %s does not exist; cannot set properties" master
end
...
module Dhclient : sig
  ...
  let ensure_running ?(ipv6 = false) interface options =
    if not (is_running ~ipv6 interface)
    then
...
module Proc = struct
  ...
  let get_bond_slave_mac name slave =
    ...
    if List.mem_assoc slave macs then List.assoc slave macs else raise Not_found
  ...
  module Make (Cli : Cli_S) = struct
    ...
    let bridge_to_vlan name =
      try
        ...
        let vlan =
          vsctl ~log:false [ "br-to-vlan"; name ]
          |> String.trim
          |> int_of_string
        in
        Some (parent, vlan)
        ...
        (fun interface ->
          let do_workaround =
            ...
            | None ->
              ...
              | Some driver ->
                  if List.mem driver no_vlan_workaround_drivers
                  then false
                  else Sysfs.has_vlan_accel interface )
          in
    ...
    let create_bridge
      ...
      let fail_mode_arg =
        if vlan = None
        then [ "--"; "set"; "bridge"; name; "fail_mode=" ^ fail_mode ]
        else []
      in
      let external_id_arg =
      ...
      let del_old_arg =
        let real_bridge_exists () =
          ...
          | _ ->
              false
        in
        if vlan <> None && real_bridge_exists ()
        then
    ...
    let make_bond_properties name properties =
      ...
      let mode_args =
        let mode =
          if List.mem_assoc "mode" properties
          then List.assoc "mode" properties
          else "balance-slb"
        in
        let halgo =
          if List.mem_assoc "hashing-algorithm" properties
        ...
        then
          ...
          ::
          ( if halgo = "src_mac"
          then [ "bond_mode=balance-slb" ]
          else if halgo = "tcpudp_ports"
          then [ "bond_mode=balance-tcp" ]
          else (
            debug
      ...
      let get_prop_legacy (prop, ovs_key) =
        ...
        then
          ...
          then (
            ...
            [] )
          else if prop = "use_carrier"
          then [ (ovs_key ^ "=" ^ if value' > 0 then "carrier" else "miimon") ]
          else [ ovs_key ^ "=" ^ string_of_int value' ]
        else []
    ...
    let set_mtu interface mtu =
      vsctl
        [ "set"; "interface"; interface; Printf.sprintf "mtu_request=%d" mtu ]
  end
...
module Brctl = struct
  ...
  let create_port bridge name =
    if not (List.mem name (Sysfs.bridge_to_interfaces bridge))
    then ignore (call [ "addif"; bridge; name ])


  let destroy_port bridge name =
    if List.mem name (Sysfs.bridge_to_interfaces bridge)
    then ignore (call [ "delif"; bridge; name ])
  ...
  let is_param_array driver param_name =
    ...
    | _ ->
        Error
          ( Other
          , Printf.sprintf
              "Failed to determine if VF param of driver '%s' is an array"
              driver )
end
  ...
  *)
  ...
  let get_maxvfs driver config =
    let get_default_maxvfs config =
      try Some (List.assoc "VFs-maxvfs-by-default" config |> int_of_string) with
      | _ ->
          None
    in
    let get_user_defined_maxvfs config =
      try Some (List.assoc "VFs-maxvfs-by-user" config |> int_of_string) with
      | _ ->
          None
    in
  ...
  let config_sriov driver vf_param maxvfs =
    let open Rresult.R.Infix in
    Modinfo.is_param_array driver vf_param
    >>= fun is_array ->
    ...
    | false, false ->
        ...
        in
        write_conf_file driver (new_conf @ [ new_option_line ])
        >>= fun () -> Dracut.rebuild_initrd ()
    | true, false ->
        Result.Ok ()
