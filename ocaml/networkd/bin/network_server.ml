...
let write_config () =
  try Network_config.write_config !config with
  | Network_config.Write_error ->
      ()
...
module Sriov = struct
  ...
  let config_sriov ~enable dev =
    ...
    | Some vf_param ->
        ...
        Modprobe.config_sriov driver vf_param numvfs
        >>= fun _ ->
        if numvfs = Sysfs.get_sriov_numvfs dev
        then Ok Modprobe_successful
        else Ok Modprobe_successful_requires_reboot
    | None ->
        ...
        then
          ...
          match
            ...
            >>= fun maxvfs -> maxvfs |> Sysfs.set_sriov_numvfs dev
          with
          | Ok _ ->
  ...
  let make_vf_conf_internal pcibuspath mac vlan rate =
    ...
    in
    let open Rresult.R.Infix in
    Sysfs.parent_device_of_vf pcibuspath
    >>= fun dev ->
    Sysfs.device_index_of_vf dev pcibuspath
    >>= fun index ->
    ...
    (* In order to ensure the Networkd to be idempotent, configuring VF with no
       ...
       means to reset them *)
    config_or_otherwise_reset
      (Ip.set_vf_vlan dev index)
      (fun () -> Ip.set_vf_vlan dev index 0)
      vlan
    >>= fun () ->
    config_or_otherwise_reset
      (Ip.set_vf_rate dev index)
...
module Interface = struct
  ...
  let is_up dbg name =
    Debug.with_thread_associated
      dbg
      (fun () -> if List.mem name (Sysfs.list ()) then Ip.is_up name else false)
      ()
  ...
  let set_ipv4_gateway _ dbg ~name ~address =
    Debug.with_thread_associated
      ...
      (fun () ->
        debug
          "Configuring IPv4 gateway for %s: %s"
          name
          (Unix.string_of_inet_addr address) ;
        update_config
          name
          { (get_config name) with ipv4_gateway = Some address } ;
        if !config.gateway_interface = None
           || !config.gateway_interface = Some name
        then (
          debug "%s is the default gateway interface" name ;
          Ip.set_gateway name address )
        else debug "%s is NOT the default gateway interface" name )
      ()
  ...
  let set_ipv6_conf _ dbg ~name ~conf =
    Debug.with_thread_associated
      dbg
      (fun () ->
        if Proc.get_ipv6_disabled ()
        then warn "Not configuring IPv6 address for %s (IPv6 is disabled)" name
        else (
          debug
            "Configuring IPv6 address for %s: %s"
            name
          ...
          | None6 ->
              if List.mem name (Sysfs.list ())
              then (
                if Dhclient.is_running ~ipv6:true name
                then ignore (Dhclient.stop ~ipv6:true name) ;
  ...
  let set_ipv6_gateway _ dbg ~name ~address =
    Debug.with_thread_associated
      ...
      (fun () ->
        if Proc.get_ipv6_disabled ()
        then warn "Not configuring IPv6 gateway for %s (IPv6 is disabled)" name
        else (
          debug
            "Configuring IPv6 gateway for %s: %s"
            name
            (Unix.string_of_inet_addr address) ;
          update_config
            name
            { (get_config name) with ipv6_gateway = Some address } ;
          if !config.gateway_interface = None
             || !config.gateway_interface = Some name
          then (
            debug "%s is the default gateway interface" name ;
  ...
  let set_ipv4_routes _ dbg ~name ~routes =
    Debug.with_thread_associated
      ...
      (fun () ->
        debug
          "Configuring IPv4 static routes for %s: %s"
          name
          (String.concat
             ", "
             (List.map
  ...
  let set_dns _ dbg ~name ~nameservers ~domains =
    Debug.with_thread_associated
      ...
      (fun () ->
        update_config
          name
          { (get_config name) with dns = (nameservers, domains) } ;
        debug
          "Configuring DNS for %s: nameservers: [%s]; domains: [%s]"
          name
        ...
        then (
          ...
          let domains' =
            if domains <> []
            then [ "search " ^ String.concat " " domains ]
            else []
          in
          let nameservers' =
            List.map
          ...
          Xapi_stdext_unix.Unixext.write_string_to_file
            resolv_conf
            (String.concat "\n" lines ^ "\n") )
        else debug "%s is NOT the DNS interface" name )
      ()
  ...
  let set_mtu _ dbg ~name ~mtu =
    Debug.with_thread_associated
      ...
      (fun () ->
        ...
        | Openvswitch ->
          ( try ignore (Ovs.set_mtu name mtu) with
          | _ ->
  ...
  let set_ethtool_settings _ dbg ~name ~params =
    Debug.with_thread_associated
      ...
      (fun () ->
        debug
          "Configuring ethtool settings for %s: %s"
          name
  ...
  let set_ethtool_offload _ dbg ~name ~params =
    Debug.with_thread_associated
      ...
      (fun () ->
        debug
          "Configuring ethtool offload settings for %s: %s"
          name
          (String.concat ", " (List.map (fun (k, v) -> k ^ "=" ^ v) params)) ;
        let add_defaults =
          List.filter
  ...
  let set_persistent dbg name value =
    Debug.with_thread_associated
      ...
      (fun () ->
        debug
          "Making interface %s %spersistent"
          name
  ...
  let make_config dbg conservative config =
    Debug.with_thread_associated
      ...
      (fun () ->
        ...
        in
        let config =
          if need_enic_workaround ()
          then
            List.fold_left
              (fun accu (name, interface) ->
                if Sysfs.is_physical name
                ...
                else (name, interface) :: accu )
              []
              config
          else config
        in
        debug
          "** Configuring the following interfaces: %s%s"
...
module Bridge = struct
  ...
  let determine_backend () =
    try
      ...
      | backend ->
          warn
            "Network backend unknown (%s). Falling back to Open vSwitch."
            backend ;
          backend_kind := Openvswitch
    with
    | _ ->
  ...
  let destroy_existing_vlan_ovs_bridge dbg wanted_bridge (parent, vlan) =
    let vlan_bridges =
      ...
      in
      if raw <> ""
      then Astring.String.cuts ~empty:false ~sep:"\n" (String.trim raw)
      else []
    in
    ...
    List.iter
      (fun bridge ->
        ...
        then
          ...
          if List.mem vlan_device ifaces_on_bridge
          then (
            debug "Destroying existing bridge %s" bridge ;
    ...
    Debug.with_thread_associated
      ...
      (fun () ->
        ...
        | Openvswitch ->
            let fail_mode =
              if not
                   (List.mem_assoc "vswitch-controller-fail-mode" other_config)
              then "standalone"
              else
                let mode =
                  List.assoc "vswitch-controller-fail-mode" other_config
                in
                if mode = "secure" || mode = "standalone"
                then (
                  ( try
                      if mode = "secure" && Ovs.get_fail_mode name <> "secure"
                      then add_default := name :: !add_default
                    with
                  | _ ->
                      () ) ;
                  mode )
            ...
            let vlan_bug_workaround =
              if List.mem_assoc "vlan-bug-workaround" other_config
              then Some (List.assoc "vlan-bug-workaround" other_config = "true")
              else None
            in
            let external_id =
              if List.mem_assoc "network-uuids" other_config
              then
                Some
            ...
            ignore
              (Ovs.create_bridge
                 ...
                 vlan_bug_workaround
                 name ) ;
            if igmp_snooping = Some true && not old_igmp_snooping
            then Ovs.inject_igmp_query ~name
        | Bridge ->
            ...
            | Some (parent, vlan) ->
                ...
                List.iter
                  (fun (device, vlan', parent') ->
                    ...
                    then raise (Network_error (Vlan_in_use (parent, vlan))) )
                  (Proc.get_vlans ()) ;
            ...
            then (
              ...
              List.iter
                (fun dev ->
                  ...
                  Interface.bring_down dbg dev ;
                  if Linux_bonding.is_bond_device dev
                  then Linux_bonding.remove_bond_master dev ;
                  if ( Astring.String.is_prefix ~affix:"eth" dev
                     || Astring.String.is_prefix ~affix:"bond" dev )
                     && String.contains dev '.'
                  then (
                    ignore (Ip.destroy_vlan dev) ;
                    let n = String.length dev in
                    if String.sub dev (n - 2) 2 = ".0"
                       && need_enic_workaround ()
                    then
                      let vlan_base = String.sub dev 0 (n - 2) in
                      if Linux_bonding.is_bond_device vlan_base
                      then
  ...
  let add_basic_port
      dbg bridge name { interfaces; bond_mac; bond_properties; _ } =
    match !backend_kind with
    | Openvswitch ->
        ( match interfaces with
        | [ iface ] ->
        ...
        if List.mem bridge !add_default
        then
          let mac =
            match bond_mac with
            | None ->
  ...
  let add_port dbg bond_mac bridge name interfaces bond_properties kind =
    Debug.with_thread_associated
      ...
      (fun () ->
        ...
        let ports =
          if List.mem_assoc name config.ports
          then List.remove_assoc name config.ports
          else config.ports
        in
        ...
        debug
          "Adding %s port %s to bridge %s with interface(s) %s%s"
  ...
  let make_config dbg conservative config =
    Debug.with_thread_associated
      ...
      (fun () ->
        let vlans_go_last
            (_, { vlan = vlan_of_a; _ }) (_, { vlan = vlan_of_b; _ }) =
          if vlan_of_a = None && vlan_of_b = None
          then 0
          else if vlan_of_a <> None && vlan_of_b = None
          then 1
          else if vlan_of_a = None && vlan_of_b <> None
          then -1
          else 0
        ...
        let config =
          ...
          then (
            ...
            in
            debug
              "Ensuring the following persistent bridges are up: %s"
              (String.concat
                 ", "
              ...
              List.filter_map
                (function
                  | _, { vlan = Some (parent, _); _ } ->
                      if not (List.mem_assoc parent persistent_config)
                      then Some (parent, List.assoc parent config)
                      else None
                  | _ ->
                      None )
                persistent_config
        ...
        let exec f = if conservative then try f () with _ -> () else f () in
        debug
          "** Configuring the following bridges: %s%s"
          (String.concat ", " (List.map (fun (name, _) -> name) config))
          (if conservative then " (best effort)" else "") ;
        List.iter
          (function
            | ( bridge_name
              , ( { ports; vlan; bridge_mac; igmp_snooping; other_config; _ } as
                c ) ) ->
...
module PVS_proxy = struct
  ...
  let do_call call =
    try Jsonrpc_client.with_rpc ~path:!path ~call () with
    | e ->
  ...
  let remove_site _dbg uuid =
    ...
    let call =
      Rpc.
        ...
        ; params =
            [ Dict
                [ ("site_uuid", Rpcmarshal.marshal Rpc.Types.string.ty uuid) ]
            ]
        ; is_notification = false
        }
    in
...
let on_startup () =
  ...
  Debug.with_thread_associated
    ...
    (fun () ->
      ...
      | e ->
          debug
            "Error while configuring networks on startup: %s\n%s"
            (Printexc.to_string e)
