...
let test_valid_key key_name () =
  ...
  | Error (`Msg (_, msg)) ->
      Alcotest.fail
        (Format.asprintf
           "Valid key could not be validated: %a"
...
let test_valid_cert ~kind cert time pkey =
  ...
  | Error (`Msg (_, msg)) ->
      Alcotest.fail
        (Format.asprintf
           "Valid certificate could not be validated: %a"
           Fmt.(Dump.list string)
           msg )
...
let load_pkcs8 name =
  ...
  |> Rresult.R.reword_error (fun (`Msg msg) ->
         `Msg
           (Printf.sprintf
              "Could not load private key with name '%s': %s"
              name
              msg ) )
...
let valid_leaf_cert_tests =
  ...
  List.map
    (fun (name, pkey_leaf_name, time, digest) ->
      let cert_test =
        load_pkcs8 pkey_leaf_name
        >>= fun pkey_leaf ->
        sign_leaf_cert host_name digest pkey_leaf
        >>| fun cert ->
...
let test_invalid_leaf_cert
  ...
  let test_cert =
    load_pkcs8 pkey_leaf_name
    >>= fun pkey_leaf ->
    load_pkcs8 pkey_expected_name
    >>= fun pkey_expected ->
    sign_leaf_cert host_name digest pkey_leaf
    >>| fun cert ->
    test_invalid_leaf_cert
      cert
      (time_of_rfc3339 time)
      pkey_expected
      error
      reason
  in
      ...
      List.fold_left
        (fun (pkey_sign, chain_result) pkey ->
          let result =
            chain_result
            >>| fun chain ->
            sign_cert host_name ~pkey_sign `SHA256 pkey
            >>| fun cert -> cert :: chain
          in
