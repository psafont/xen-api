...
let test_valid_cert_chain cert time =
  match validate_certificate_chain cert with
  | Ok _ ->
      ()
  | Error (`Msg (_, msg)) ->
      Alcotest.fail
...
let test_corrupt_cert cert error reason =
  match load_server_certificate cert with
  | Ok _ ->
      Alcotest.fail "Corrupt certificate was parsed into a valid certificate"
  | Error (`Msg msg) ->
      Alcotest.(check @@ pair string @@ list string)
        "Error must match" (error, reason) msg
...
let test_corrupt_leaf_cert (cert_name, pkey_name, time, error, reason) =
  let test_cert =
    ...
    let test () = test_corrupt_cert cert_name error reason in
    test
  in
