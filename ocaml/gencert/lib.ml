...
let validate_private_key pkcs8_private_key =
  ...
  |> R.reword_error (fun (`Msg err_msg) ->
         ...
         if Astring.String.is_prefix ~affix:"multi-prime RSA" err_msg
         then `Msg (server_certificate_key_rsa_multi_not_supported, [])
         else if Astring.String.is_prefix ~affix:unknown_algorithm err_msg
         then
           `Msg
...
let assert_not_expired ~now certificate ~error_not_yet ~error_expired =
  ...
  if Ptime.is_earlier ~than:not_before now
  then Error (`Msg (error_not_yet, [ to_string now; to_string not_before ]))
  else if Ptime.is_later ~than:not_after now
  then Error (`Msg (error_expired, [ to_string now; to_string not_after ]))
  else Ok certificate


let _validate_not_expired
    ~now (blob : string) ~error_invalid ~error_not_yet ~error_expired =
  pem_of_string blob ~error_invalid
  >>= fun cert -> assert_not_expired ~now cert ~error_not_yet ~error_expired
...
let validate_certificate kind pem now private_key =
  ...
  | Leaf ->
      _validate_not_expired
        ~now
        pem
        ~error_invalid:server_certificate_invalid
...
let install_server_certificate
    ?(pem_chain = None) ~pem_leaf ~pkcs8_private_key ~server_cert_path =
  let now = Ptime_clock.now () in
  validate_private_key pkcs8_private_key
  >>= fun priv ->
  ...
  Option.fold
    ...
    ~some:(fun pem_chain ->
      ...
      >>= fun _ignored -> Ok [ pkcs8_private_key; pem_leaf; pem_chain ] )
    pem_chain
