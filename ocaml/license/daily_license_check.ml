...
type result =
  | Good
  | Expiring of string list
  | Expired of string list
...
let get_hosts all_license_params threshold =
  List.fold_left
    (fun acc (name_label, license_params) ->
      ...
      if expiry < threshold then name_label :: acc else acc )
    []
    all_license_params
...
let check_license now pool_license_state all_license_params =
  ...
  if days <= 0.
  then Expired (get_hosts all_license_params now)
  else if days <= 30.
  then Expiring (get_hosts all_license_params (now +. seconds_per_30_days))
  else Good
...
let execute rpc session pool result =
  ...
  | Expired hosts ->
      let body =
        Printf.sprintf
          "The licenses of the following hosts have expired: %s"
          (String.concat ", " hosts)
      in
