...
let vdi_resize_test rpc session_id sr_info () =
  Qt.VDI.with_new rpc session_id sr_info.Qt.sr (fun vdi ->
      ...
      then Alcotest.fail "The final size should be >= the requested size" )
...
let vdi_bad_introduce rpc session_id sr_info () =
  Qt.VDI.with_any rpc session_id sr_info (fun vdi ->
      ...
      ( try
          print_endline
            (Printf.sprintf
               "Introducing a VDI with a duplicate UUID (%s)"
               vdir.API.vDI_uuid ) ;
          let (_ : API.ref_VDI) =
            Client.Client.VDI.introduce
              ~rpc
              ~session_id
              ~uuid:vdir.API.vDI_uuid
              ~name_label:"bad uuid"
              ~name_description:""
              ~sR:vdir.API.vDI_SR
              ~_type:vdir.API.vDI_type
              ~sharable:false
              ~read_only:false
              ~other_config:[]
              ...
              ~is_a_snapshot:false
              ~snapshot_time:Xapi_stdext_date.Date.never
      ...
      | Api_errors.Server_error (_, _) as e ->
          Printf.printf
            "API error caught as expected: %s\n"
            (Printexc.to_string e) ) ;
      try
        print_endline
          (Printf.sprintf
             "Introducing a VDI with a duplicate location (%s)"
        ...
        let (_ : API.ref_VDI) =
          Client.Client.VDI.introduce
            ...
            ~read_only:false
            ~other_config:[]
            ~location:vdir.API.vDI_location
            ~xenstore_data:[]
            ~sm_config:[]
            ~managed:true
            ~virtual_size:0L
...
let check_vdi_snapshot rpc session_id vdi =
  ...
  Qt.VDI.with_destroyed rpc session_id snapshot_vdi (fun () ->
      ...
      Qt.Time.(check (of_field r.API.vDI_snapshot_time))
        ...
        ~before:snapshot_finish ;
      Alcotest.(check bool)
        "VDI.is_a_snapshot of must be true for snapshot VDI"
        true
        r.API.vDI_is_a_snapshot ;
      Alcotest.(check bool)
        "VDI.snapshot_of must not be null for snapshot VDI"
        true
    ...
    Xapi_stdext_pervasives.Pervasiveext.finally
      (fun () ->
        try
          ...
          print_endline
            (Printf.sprintf
               "XenAPI reports size: %Ld; dom0 reports size: %Ld"
               size_should_be
               size_dom0 ) ;
          if size_should_be <> size_dom0
          then
            Alcotest.fail
        ...
        | Not_this_host ->
            print_endline
              "Skipping size check: disk is plugged into another host" )
      (fun () ->
        Client.Client.VBD.unplug rpc session_id vbd ;
        Client.Client.VBD.destroy rpc session_id vbd )
  in
