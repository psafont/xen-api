...
let event_next_unblocking_test rpc session_id () =
  ...
  let (_ : Thread.t) =
    Thread.create
      (fun () ->
        ( try ignore (Client.Client.Event.next rpc session_id) with
        | e ->
            print_endline
              (Printf.sprintf
...
let event_next_test rpc session_id () =
  ...
  ( try Client.Client.Pool.remove_from_other_config rpc session_id pool key with
  | _ ->
      () ) ;
  let (_ : Thread.t) =
    Thread.create
  ...
  Thread.delay 1. ;
  Alcotest.(check bool)
    "failed to see pool.other_config change"
    true
...
let object_level_event_test rpc session_id () =
  ...
  ( try Client.Client.VM.remove_from_other_config rpc session_id vm_a key with
  | _ ->
      () ) ;
  ( try Client.Client.VM.remove_from_other_config rpc session_id vm_b key with
  | _ ->
      () ) ;
  let (_ : Thread.t) =
    Thread.create
      (fun () ->
        ...
        while not (Mutex.execute m (fun () -> !finished)) do
          ...
          List.iter
            (fun event ->
              if event.Event_types.reference <> Ref.string_of vm_a
              then (
                print_endline
                  (Printf.sprintf
                     "event on %s which we aren't watching"
                     event.reference ) ;
                Mutex.execute m (fun () ->
                    ...
                    Alcotest.fail
                      (Printf.sprintf
                         "got unexpected event (new token = %s)"
                         !token ) ) ) )
            events.events ;
          token := events.token ;
  ...
  Mutex.execute m (fun () ->
      ...
      then
        Alcotest.(check bool)
          "failed to see object-level event change"
          true
          !finished
      else Alcotest.fail "test failed" )
...
let event_message_test rpc session_id () =
  ...
  print_endline
    (Printf.sprintf
       "Finding messages for object: %s"
       (Client.Client.Pool.get_uuid rpc session_id pool) ) ;
  let messages =
    Client.Client.Message.get
      ~rpc
      ~session_id
      ~cls
      ~obj_uuid
  ...
  let _, f =
    List.fold_left
      (fun (set, failed) ev ->
        ...
        then (set, true)
        else (StringSet.add reference set, failed) )
