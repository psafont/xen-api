...
let vm_import
    ?(metadata_only = false) ?(preserve = false) ?sr rpc session_id filename =
  let sr_uuid =
  ...
  with_setup rpc session_id sr vm_template (fun vm ->
      ...
      List.iter
        (fun sr_info ->
          ...
          if List.length vbds <> List.length by_device
          then Alcotest.fail "Wrong number of VBDs after import" ;
          List.iter
            (fun vbd ->
              ...
              | "1" ->
                  ...
                  then
                    Alcotest.fail
                      (Printf.sprintf
                         "Device %s should not vary in VDIness"
                         all.API.vBD_userdevice )
              | _ ->
                  Alcotest.fail
                    (Printf.sprintf
                       "Unhandled device number: %s"
                       all.API.vBD_userdevice ) )
            vbds ;
