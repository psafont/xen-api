...
let sr_probe_test rpc session_id sr_info () =
  ...
  | pbd :: _ ->
      ...
      if not (List.mem srr.API.sR_uuid all_uuids)
      then
        Alcotest.fail
          (Printf.sprintf
...
let sr_set_name_test rpc session_id sr_info () =
  ...
  Xapi_stdext_pervasives.Pervasiveext.finally
    (fun () ->
      Client.Client.SR.set_name_label
        ~rpc
        ~session_id
        ~sr
        ~value:"test_name_label" ;
      Client.Client.SR.set_name_description
        ~rpc
        ~session_id
        ~sr
...
let tests () =
  ...
  ; [ ("sr_probe_test", `Slow, sr_probe_test) ]
    |> conn
    |> sr
         SR.(all |> has_capabilities [ Sr_capabilities.sr_probe ] |> can_unplug)
