...
let cli_cmd args =
  ...
  | Forkhelpers.Spawn_internal_error (log, output, Unix.WEXITED n) ->
      Alcotest.failf
        "CLI failed: exit code=%d output=[%s] log=[%s]"
        n
        output
        log
  | Forkhelpers.Spawn_internal_error (log, output, _) ->
      Alcotest.failf
        "CLI failed: exit code unkown; output=[%s] log=[%s]"
        output
        log
  | e ->
...
module Test = struct
  let assert_raises_match exception_match fn =
    ...
    with
    | failure ->
        if not (exception_match failure) then raise failure else ()
...
module Time = struct
  ...
  let check t ~after ~before =
    Alcotest.(check bool)
      (Printf.sprintf
         "Time %s should be between %s and %s (+-1s)"
         (pp t)
         (pp before)
         (pp after) )
      true
...
module VDI = struct
  ...
  let with_any rpc session_id sr_info f =
    if List.mem `vdi_create sr_info.allowed_operations
    then with_new rpc session_id sr_info.sr f
    else
      let vdis =
