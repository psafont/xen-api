...
let gen_pm_ds state cpu_id state_id (derive_value : int64) =
  ...
  , Ds.ds_make
      ...
      ~description:
        (Printf.sprintf
           "Proportion of time CPU %d spent in %c-state %d"
           cpu_id
           state_letter
           state_id )
...
let get_cpu_averages () : int64 list =
  ...
  let match_fun s =
    ...
    then Some (Int64.of_string (Str.matched_group 1 s))
    else None
  in
  Utils.exec_cmd
    (module Process.D)
  ...
  let _nbelt, accsmall, accbig =
    List.fold_left
      (fun (nbelt, accsmall, accbig) elt ->
        if nbelt < n
        then (nbelt + 1, elt :: accsmall, accbig)
        else (1, [ elt ], List.rev accsmall :: accbig) )
      (0, [], [])
      l
  in
...
let generate_state_dss state_kind =
  ...
  D.debug
    "Found %d states; with %d CPUs this means %d states per CPU"
    (List.length states)
    !nr_cpu
    states_by_cpu ;
  try
...
let generate_cpu_averages () =
  ...
  try
    List.mapi (fun cpu_id time -> gen_pm_cpu_averages cpu_id time) averages
  with
  | _ ->
      []
