...
let get_squeezed_data domid =
  let get_current_value ~label current_values =
    try IntMap.find domid !current_values with
    | _ ->
        if domid <> 0
        then
          D.warn
            "Couldn't find cached %s value for domain %d, using 0"
            label
            domid ;
        0L
  in
...
let generate_squeezed_dss () =
  ...
  [ ( Rrd.Host
    , Ds.ds_make
        ...
        ~value:(Rrd.VT_Int64 memory_reclaimed)
        ~ty:Rrd.Gauge
        ~default:true
        ~units:"B"
        () )
  ; ( Rrd.Host
    , Ds.ds_make
        ~name:"memory_reclaimed_max"
        ~description:"Host memory that could be reclaimed by squeezed"
        ~value:(Rrd.VT_Int64 memory_possibly_reclaimed)
        ~ty:Rrd.Gauge
        ~default:true
        ~units:"B"
        () )
