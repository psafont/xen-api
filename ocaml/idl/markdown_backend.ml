...
let pad_right x max_width =
  ...
  if String.length x < max_width
  then x ^ String.make (max_width - length) ' '
  else x
...
let markdown_section_of_message
    printer obj ~is_class_deprecated ~is_class_removed x =
  let is_event_from =
  ...
  printer "" ;
  if List.exists is_removal_marker x.msg_lifecycle || is_class_removed
  then (
    printer "**This message is removed.**" ;
    printer "" )
  else if List.exists is_deprecation_marker x.msg_lifecycle
          || is_class_deprecated
  then (
  ...
  printer "```" ;
  let result =
    if is_event_from then "<event batch>" else of_ty_opt_verbatim x.msg_result
  in
  printer
    (sprintf
       "%s %s (%s)"
...
let print_field_table_of_obj printer ~is_class_deprecated ~is_class_removed x =
  ...
  printer "" ;
  if x.contents = []
  then printer ("Class " ^ escape x.name ^ " has no fields.")
  else (
    printer
    ...
    printer
      "|:-------------------|:-------------------|:--------------|:---------------------------------------|" ;
    let print_field_content
        printer
        ({ release; qualifier; ty; field_description = description } as y) =
      let wired_name = Datamodel_utils.wire_name_of_field y in
      let descr =
        ( if List.exists is_removal_marker y.lifecycle || is_class_removed
        then "**Removed**. "
        else if List.exists is_deprecation_marker y.lifecycle
                || is_class_deprecated
        then "**Deprecated**. "
        else "" )
        ^ escape description
    ...
    if String.lowercase_ascii x.name = "event"
    then
      printer
        (sprintf
           "|%s|`%s`|%s|%s|"
...
let of_obj printer x =
  ...
  then (
    printer
      (sprintf
         "Class %s has no additional RPCs associated with it."
         (escape x.name) ) ;
    printer "" )
  else
    x.messages
...
let print_classes api io =
  ...
  printer
    "# API Reference - Types and Classes\n\n\
     ...
     |:-------------------|:---------------------------------------------------------------------|" ;
  let get_descr obj =
    ( if List.exists is_removal_marker obj.obj_lifecycle
    then "**Removed**. "
    else if List.exists is_deprecation_marker obj.obj_lifecycle
    then "**Deprecated**. "
    else "" )
