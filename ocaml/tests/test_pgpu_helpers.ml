...
module GetRemainingCapacity = Generic.MakeStateful (struct
  ...
  let tests =
    `QuickAndAutoDocumented
      ...
      ; ( ( { default_k2 with resident_VGPU_types = [ k240q; k240q; k240q ] }
          , k240q )
        , 1L )
      ...
      ; ( ({ default_k1 with scheduled_VGPU_types = [ k100 ] }, passthrough_gpu)
        , 0L )
      ; ( ({ default_k1 with scheduled_VGPU_types = [ passthrough_gpu ] }, k100)
        , 0L )
      ; ( ( { default_k1 with scheduled_VGPU_types = [ passthrough_gpu ] }
          , passthrough_gpu )
        , 0L )
      ; (({ default_k2 with scheduled_VGPU_types = [ k200; k200 ] }, k200), 6L)
      ; ( ({ default_k2 with scheduled_VGPU_types = [ k200 ] }, passthrough_gpu)
        , 0L )
      ; ( ({ default_k2 with scheduled_VGPU_types = [ passthrough_gpu ] }, k200)
        , 0L )
