...
let test_network_event_loop ~no_nbd_networks_at_start () =
  ...
  let assert_update_firewall_raised () =
    if !fail_firewall_update && Threadext.Delay.wait wait_hdl_fail timeout
    then
      Alcotest.fail
  ...
  let pif1 =
    Test_common.make_pif
      ...
      ()
  in
  ( if not no_nbd_networks_at_start
  then
    let (_ : Thread.t) = start_event_loop () in
    () ) ;
  assert_received_params
    "network1 is now connected to this host with pif1"
    [ "bridge1" ] ;
  ...
  in
  assert_received_params
    "network3 is now connected to this host by pif3"
  ...
  assert_received_params
    "network3's only PIF connected to this host has been removed"
    [ "bridge2" ] ;
  (* Remove network2 *)
  received_params := None ;
  fail_firewall_update := true ;
