...
let test_create_internal () =
  ...
  let sriov, logical_PIF =
    Xapi_network_sriov.create_internal
      ~__context
      ~physical_PIF
      ~physical_rec
      ~network
...
let test_require_operation_on_pci_device_not_attached_not_need_reboot () =
  ...
  Alcotest.(check bool)
    ...
    false
    (Xapi_network_sriov_helpers.require_operation_on_pci_device
       ~__context
       ~sriov
       ~self:sriov_logical_PIF )
...
let test_require_operation_on_pci_device_unknown () =
  ...
  let sriov_logical_PIF, sriov =
    ...
    in
    Db.Network_sriov.set_requires_reboot ~__context ~self:sriov ~value:false ;
    Db.Network_sriov.set_configuration_mode
      ~__context
      ~self:sriov
      ~value:`unknown ;
    (sriov_logical_PIF, sriov)
...
let create_physical_pif_with_driver
    ~__context ~host ~network ?(driver_name = "") () =
  let physical_PIF = T.create_physical_pif ~__context ~host ~network () in
  let pci =
...
let create_modprobe_sriov_logical_pif_with_driver
    ~__context ~host ~network ~driver_name ~currently_attached ~requires_reboot
    =
  let physical_PIF =
...
let test_require_operation_on_pci_device_modprobe_2 () =
  ...
  let _ =
    let network = T.make_network ~__context ~bridge:"xapi0" () in
    create_modprobe_sriov_logical_pif_with_driver
      ~__context
      ~host
      ~network
      ~driver_name
