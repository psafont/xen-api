...
let make_vm ~__context ?(name_label = "name_label")
    ...
    ?(tags = []) ?(blocked_operations = []) ?(appliance = Ref.null)
    ?(start_delay = 0L) ?(snapshot_schedule = Ref.null)
  ...
  Xapi_vm.create ~__context ~name_label ~name_description ~user_version
    ...
    ~ha_always_run ~ha_restart_priority ~tags ~blocked_operations ~appliance
