...
let assert_raises_api_error
    (code : string) ?(args : string list option) (f : unit -> 'a) : unit =
  try
    f () ;
    Alcotest.fail
      (Printf.sprintf "Function didn't raise expected API error %s" code)
  with
  | Api_errors.Server_error (c, a) ->
      Alcotest.check
        Alcotest.string
        "Function raised unexpected API error"
        code
        c ;
      ( match args with
      | None ->
      ...
      | Some args ->
          Alcotest.(check (list string))
            "Function raised API error with unexpected args"
            args
            a )
...
let make_localhost ~__context ?(features = Features.all_features) () =
  ...
  Db.Host.set_cpu_info
    ~__context
    ~self:!Xapi_globs.localhost_ref
    ~value:default_cpu_info ;
  Db.Host.remove_from_software_version
    ~__context
    ~self:!Xapi_globs.localhost_ref
    ~key:"network_backend" ;
  Db.Host.add_to_software_version
    ~__context
    ~self:!Xapi_globs.localhost_ref
    ~key:"network_backend"
    ~value:Network_interface.(string_of_kind Openvswitch) ;
  Create_misc.ensure_domain_zero_records
    ~__context
...
let make_test_database
    ?(conn = Mock.Database.conn) ?(reuse = false) ?features () =
...
let make_vm
    ...
    ?(affinity = Ref.null)
    ?(memory_target = 500L)
    ?(memory_static_max = 1000L)
    ?(memory_dynamic_max = 500L)
    ?(memory_dynamic_min = 500L)
...
let make_bond
    ...
    ~master
    ?(other_config = [])
    ?(primary_slave = Ref.null)
    ?(mode = `activebackup)
    ?(properties = [])
    ?(auto_update_mac = false)
    () =
  Db.Bond.create
    ~__context
    ~ref
...
let make_pool
    ...
    ?(allowed_operations = [])
    ?(other_config =
      [ Xapi_globs.memory_ratio_hvm; Xapi_globs.memory_ratio_pv ])
...
let make_sm
    ...
    ?(capabilities = [])
    ?(features = default_sm_features)
    ?(configuration = [])
    ?(other_config = [])
    ?(driver_filename = "/dev/null")
...
let make_vdi
    ...
    ?(snapshot_time = API.Date.never)
    ?(tags = [])
    ?(allow_caching = true)
    ?(on_boot = `persist)
    ?(metadata_of_pool = Ref.make ())
...
let make_session
    ...
    ?(this_host = Ref.null)
    ?(this_user = Ref.null)
    ?(last_active = API.Date.never)
    ?(pool = false)
    ?(other_config = [])
    ?(is_local_superuser = false)
    ?(subject = Ref.null)
    ?(validation_time = API.Date.never)
    ?(auth_user_sid = "")
    ?(auth_user_name = "")
    ?(rbac_permissions = [])
    ?(parent = Ref.null)
...
let create_physical_pif
    ~__context ~host ?network ?(bridge = "xapi0") ?(managed = true) () =
  let network =
    match network with
    | Some network ->
        network
...
let make_cluster_and_cluster_host
    ~__context
    ?(ref = Ref.make ())
    ?(uuid = make_uuid ())
    ?(cluster_token = "")
