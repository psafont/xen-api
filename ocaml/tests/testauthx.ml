      ...
      ( try
          print_endline
            (List.fold_left
               conc
               ""
               (AuthX.methods.query_group_membership "u999") )
        with
      | Not_found ->
          print_endline "subject Not_found, as expected." ;
          hr
            "TEST 2d. AuthX.methods.query_group_membership a999 (unknown \
             subject)" ;
          ( try
              print_endline
                (List.fold_left
                   conc
                   ""
                   (AuthX.methods.query_group_membership "a999") )
            with
          | Not_found ->
              print_endline "subject Not_found, as expected." ;
              hr
                "TEST 2e. AuthX.methods.query_group_membership 999 (unknown \
                 subject)" ;
              ( try
                  print_endline
                    (List.fold_left
              ...
              | Not_found ->
                  ...
                  hr
                    ...
                    ^ " (a user)" ) ;
                  let infolist1 =
                    AuthX.methods.query_subject_information userid
                  in
                  ...
                  hr
                    ...
                    ^ " (a group)" ) ;
                  let infolist1 =
                    AuthX.methods.query_subject_information agroup
                  in
                  for i = 0 to List.length infolist1 - 1 do
                    let print_elems (e1, e2) = print_endline (e1 ^ ": " ^ e2) in
                    print_elems (List.nth infolist1 i)
                  ...
                  hr
                    "TEST 3c. AuthX.methods.query_subject_information u999 \
                     (unknown subject)" ;
                  ( try
                      let infolist1 =
                        AuthX.methods.query_subject_information "u999"
                      in
                      for i = 0 to List.length infolist1 - 1 do
                        let print_elems (e1, e2) =
                          print_endline (e1 ^ ": " ^ e2)
                        in
                        print_elems (List.nth infolist1 i)
                      done
                  ...
                  | Not_found ->
                      ...
                      (* should return an error not_found *)
                      hr
                        "TEST 3d. AuthX.methods.query_subject_information a999 \
                         (unknown subject)" ;
                      ( try
                          let infolist1 =
                            AuthX.methods.query_subject_information "a999"
                          in
                          for i = 0 to List.length infolist1 - 1 do
                            let print_elems (e1, e2) =
                              print_endline (e1 ^ ": " ^ e2)
                      ...
                      | Not_found ->
                          ...
                          (* should return an error not_found *)
                          hr
                            "TEST 3e. AuthX.methods.query_subject_information \
                             999 (unknown subject)" ;
                          ( try
                              let infolist1 =
                                AuthX.methods.query_subject_information "999"
                              in
                              for i = 0 to List.length infolist1 - 1 do
                                let print_elems (e1, e2) =
                                  print_endline (e1 ^ ": " ^ e2)
                          ...
                          | Not_found ->
                              ...
                              hr
                                ( "TEST 4. \
                                   AuthX.methods.authenticate_username_password "
                                ^ username ) ;
                              print_endline
                                (AuthX.methods.authenticate_username_password
                                   username
