...
let test_clusterd_rpc ~__context call =
  ...
  | "create", _ ->
      Rpc.
        { success = true
        ; contents = Rpc.String test_token
...
let create_cluster
    ...
    ?(test_clusterd_rpc = test_clusterd_rpc)
    ?(token_timeout = 1.)
...
let test_enable () =
  ...
  ( match
      Xapi_clustering.find_cluster_host
        ~__context
        ~host:Helpers.(get_localhost ~__context)
    with
  | Some self ->
...
let make_cluster_host
    ~__context ~cluster ?(network = Test_common.make_network ~__context ()) () =
...
let test_get_network_fails () =
  ...
  | None ->
      Alcotest.failf
        "No cluster_host found on localhost %s"
        (Ref.string_of host) ) ;
  Alcotest.check_raises
