    ...
    `QuickAndAutoDocumented
      ...
      ; (* A pool of edition2 hosts, of which two have expiry dates. *)
        ( [ { license_params = []; edition = "edition2" }
          ; { license_params = [ ("expiry", f2d2s 500.0) ]
            ; edition = "edition2"
            }
          ; { license_params = [ ("expiry", f2d2s 350.0) ]
            ; edition = "edition2"
            }
          ]
      ...
      ; (* A pool of hosts, some edition2 (with different expiry dates) and some edition1 (no expiry). *)
        ( [ { license_params = [ ("expiry", f2d2s 5000.0) ]
            ; edition = "edition2"
            }
          ; { license_params = []; edition = "edition1" }
          ; { license_params = [ ("expiry", f2d2s 6000.0) ]
            ; edition = "edition2"
            }
          ]
        , ("edition1", "never") )
