...
let format_good datestring =
  (("host", "hostname"), date_of datestring),
  ("host", None)
...
let format_expiring (datestring, days, alert) =
  let ppf = "The TLS server certificate of host hostname expires in " ^
    (string_of_int days) ^ " days (expires on %s)"
  in
  _format (datestring, ppf, alert)

let format_expired datestring =
  let ppf = "The TLS server certificate of host hostname expired on %s" in
  _format (datestring, ppf, Api_messages.host_server_certificate_expiring_07)
