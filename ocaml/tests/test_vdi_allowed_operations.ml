...
let run_assert_equal_with_vdi
    ~__context ?(ha_enabled = false) ?sm_fun ?vdi_fun op exc =
  ...
  let _, _ =
    setup_test
      ...
      ~vdi_fun:(fun vdi_ref ->
        ...
        Xapi_vbd_helpers.assert_operation_valid
          ~__context
          ~self:vbd_ref
          ~op:`plug )
      ()
...
let test_cbt =
  ...
  let for_cbt_enable_disable =
    for_vdi_operations [ `enable_cbt; `disable_cbt ]
  in
...
let test_operations_restricted_during_rpu =
  ...
  let test_update_allowed_operations () =
    ...
    Alcotest.(check bool)
      "update_allowed_operations should exclude `enable_cbt during RPU"
      false
...
let test_update_allowed_operations () =
  ...
  Alcotest.(check Alcotest_comparators.vdi_operations_set)
    "update_allowed_operations should be correct"
    ok_ops
    allowed_operations ;
