...
let test_parser () =
  ...
  check_records
    "Invalid records must be discarded and stop the parser"
    (Ok [])
    (parse_string invalid_string) ;
  check_records
    "Two records with same name is valid input"
    (Ok baseboard_two_record)
    (parse_string baseboard_two_string) ;
  check_records
    "Arrays must be parsed as multi-line values"
    (Ok with_array)
...
let test_baseboard () =
  check_values
    "Baseboard values must have empty values when input is empty"
    baseboard_empty
...
let test_oem () =
  check_values
    "OEM default values must be used when input is empty"
    oem_empty
    (get_oem_strings @@ fun _ _ -> []) ;
  check_values
    "OEM default values must be listed first"
    oem
