...
let make_config
    host_num host_base host_extra vm_num vm_base vm_extra num_failures =
  ...
  let config =
    { hosts; vms; placement; total_hosts = host_num; num_failures }
  in
...
let prove_plan_is_possible_via_counterexample_search
  ...
  in
  Printf.printf
    "Trying %d (out of %Ld) combinations %s\n"
...
let try_impossible_cases () =
  ...
  List.iter
    (fun h ->
      ...
      if h.plan_always_possible config
      then failwith "plan_always_possible shouldn't return true" ;
      try
    ...
    for n = 1 to n' do
      for r = 1 to r' do
        ...
        then (
          ...
          if always
          then ignore (prove_plan_is_possible_via_counterexample_search h c) ;
          let time = Unix.gettimeofday () -. start in
...
let _ =
  ...
  try_possible_cases () ;
  if !graph <> ""
  then check_planning_performance !graph !graph_n !graph_r !graph_i
