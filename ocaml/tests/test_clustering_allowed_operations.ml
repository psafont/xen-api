...
let test_pool_cluster_create_not_allowed_when_cluster_exists () =
  ...
  assert_true
    "Pool.allowed_operations should not contain 'cluster_create'"
...
let test_pool_cluster_create_allowed () =
  ...
  let allowed_ops = Db.Pool.get_allowed_operations ~__context ~self in
  assert_true
    "Pool.allowed_operations should contain 'cluster_create'"
...
let test_all_cluster_ops_allowed_when_no_cluster_ops_in_progress () =
  ...
  List.iter
    (fun op ->
      let msg =
        Printf.sprintf
          "Cluster.allowed_operations should contain '%s'"
...
let test_cluster_host_disable_allowed () =
  ...
  assert_true
    "Cluster_host.allowed_operations should contain 'disable'"
    (List.mem `disable allowed_ops) ;
  assert_true
    "Cluster_host.allowed_operations should contain 'enable'"
...
let test_cluster_host_ops_not_allowed_during_cluster_host_op () =
  ...
  Xapi_cluster_host_helpers.with_cluster_host_operation
    ...
    (fun () ->
      ...
      in
      assert_true
        "Cluster_host.allowed_operations should be empty"
        (allowed_ops = []) )
