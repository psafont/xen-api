...
let string_of_host { memory_total; name_label; vms } =
  Printf.sprintf
    "{memory_total = %Ld; name_label = %S; vms = [%s]}"
    memory_total
    name_label
...
let setup ~__context { master; slaves; ha_host_failures_to_tolerate; cluster } =
  ...
  if cluster > 0
  then Test_common.make_cluster_and_cluster_host ~__context () |> ignore ;
  ...
  Db.Pool.set_ha_host_failures_to_tolerate
    ~__context
    ~self:pool
...
module PlanForNFailures = Generic.MakeStateful (struct
  ...
  let extract_output __context pool =
    ...
    Xapi_ha_vm_failover.plan_for_n_failures
      ~__context
      ~all_protected_vms
