...
let listener rpc session queue =
  ...
  let update_snapshot r other_config =
    ...
    if Hashtbl.mem snapshot r
    then debug "Update an entry of the snapshot table: %s" r
    else debug "Add a new entry to the snapshot table: %s" r ;
...
let sender rpc session (delay, msg, queue) =
  ...
  let proceed alert =
    if was_broken alert.pbd alert.scsi_id || alert.current <> alert.max
    then (
      debug
        "Adding '%s' to the temp buffer as was_broken=%b and is_broken=%b"
        (to_string alert)
      ...
      debug
        "Ignoring '%s' as was_broken=%b and is_broken=%b"
        (to_string alert)
        (was_broken alert.pbd alert.scsi_id)
        (alert.current <> alert.max) ;
    ...
    with_global_lock (fun () ->
        ...
        then (
          ...
          Buffer.add_string
            tmp
            (Printf.sprintf
               "Events received during the last %.0f seconds:\n"
               delay ) ;
