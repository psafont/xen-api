...
let rec cmdtable_data : (string * cmd_spec) list =
  ...
  ; ( "pool-crl-uninstall"
    , { reqd = [ "name" ]
      ; optn = []
      ; help =
          "Uninstall a pool-wide TLS CA-issued Certificate Revocation List."
  ...
  ; ( "pool-enable-client-certificate-auth"
    , { reqd = [ "name" ]
      ...
      ; implementation =
          No_fd Cli_operations.pool_enable_client_certificate_auth
      ; flags = []
      } )
  ...
  ; ( "vm-install"
    , { reqd = [ "new-name-label" ]
      ; optn =
          [ "sr-name-label"; "sr-uuid"; "template"; "copy-bios-strings-from" ]
      ; help =
...
*)
  ...
  ; ( "pool-ha-compute-vm-failover-plan"
    , { reqd = [ "host-uuids" ]
      ; optn = []
      ; help =
          "Compute a VM failover plan assuming the given set of hosts fail."
...
*)
  ...
  ; ( "vmss-create"
    , { reqd = [ "name-label"; "type"; "frequency" ]
      ; optn =
          [ "name-description"; "enabled"; "schedule:"; "retained-snapshots" ]
...
let convert_switch switch =
  try List.assoc switch switch_map with
  | Not_found as e ->
...
let get_reqd_param cmd p =
  try List.assoc p cmd.params with
  | Not_found as e ->
...
let parse_commandline arg_list =
  try
    ...
    if String.contains cmdname '='
    then raise (ParseError "command name is missing") ;
...
let rio_help printer minimal cmd =
  ...
  let cmds =
    List.filter
      ...
      cmd.params
  in
  if List.length cmds > 0
  then List.iter docmd (List.map fst cmds)
  else
    let cmds =
    ...
    then
      ...
      if minimal
      then printer (Cli_printer.PList cmds)
      else (
