...
let get_param params param ~default =
  if List.mem_assoc param params then List.assoc param params else default
...
let waiter printer rpc session_id params task =
    ...
    (fun () ->
      (if List.mem_assoc "progress" params then wait_with_progress_bar else wait)
        printer
        rpc
        session_id
        task )
...
let get_chunks fd =
  ...
  let rec f bytes_read =
    ...
    | Blob (Chunk len) ->
        ...
        if bytes_read > Constants.max_cli_upload_bytes
        then
          failwith
            (Printf.sprintf
...
let diagnostic_timing_stats printer rpc session_id params =
  let table_of_host host =
    ...
    @
    try Client.Host.get_diagnostic_timing_stats rpc session_id host with
    | e ->
        [ ("Error", Api_errors.to_string e) ]
  in
...
let create_vbd_and_plug_with_other_config
    rpc
    session_id
    vm
    vdi
  ...
  let vbd =
    Client.VBD.create
      ...
      ~unpluggable
      ~empty:false
      ~qos_algorithm_type:qtype
      ~qos_algorithm_params:qparams
      ~other_config
      ~device:""
      ~currently_attached:false
  in
  try Client.VBD.plug rpc session_id vbd with
  | Api_errors.Server_error (_, _) as e ->
      debug "VBD created but not hotplugged: %s" (Api_errors.to_string e)


let create_vbd_and_plug
    rpc session_id vm vdi device_name bootable rw cd unpluggable qtype qparams =
  create_vbd_and_plug_with_other_config
    rpc
    session_id
    vm
    vdi
...
let get_field_type fieldname record =
  ...
  then Normal
  else if (* New 'normal' behaviour is to split map name from key by the separator ':' *)
...
let select_fields params records default_params =
  ...
  if params = []
  then List.map (fun record -> record.fields) records
  else
    List.map
      (fun record ->
...
let print_field x =
  let append =
    if x.get_set <> None
    then (* Set *)
      if x.add_to_set = None then " (SRO)" else " (SRW)"
    else if x.get_map <> None
    then (* map *)
      if x.add_to_map = None then " (MRO)" else " (MRW)"
    else if x.set = None
    then " ( RO)"
    else " ( RW)"
  in
...
let with_specified_database rpc session_id params f =
  ...
  if use_db_vdi && use_db_file
  then failwith "xapi can query a DB vdi or a DB file, but not both." ;
  let session_id =
    if use_db_vdi
    then
    ...
    then
      ...
      Client.Session.create_from_db_file
        ~rpc
        ~session_id
...
let make_param_funs
  ...
  let p_get printer rpc session_id params : unit =
    with_specified_database rpc session_id params (fun session_id ->
        ...
        if List.mem_assoc "param-key" params
        then
          let key = List.assoc "param-key" params in
          match x.get_map with
          | Some f ->
              let result =
                try List.assoc key (f ()) with
                | _ ->
  ...
  let p_remove (printer : printer) rpc session_id params =
    ...
    | { get_set = Some g; remove_from_set = Some f } ->
        if List.mem param_key (g ())
        then f param_key
        else failwith (Printf.sprintf "Key %s is not in the set" param_key)
    | { get_map = Some g; remove_from_map = Some f } ->
        if List.mem_assoc param_key (g ())
        then f param_key
        else failwith (Printf.sprintf "Key %s is not in map" param_key)
...
let message_create printer rpc session_id params =
  ...
  let priority =
    try Int64.of_string (List.assoc "priority" params) with
    | _ ->
        failwith "Priority field should be an integer"
  in
  let name = List.assoc "name" params in
  let uuid, cls =
    if List.mem_assoc "vm-uuid" params
    then (List.assoc "vm-uuid" params, `VM)
    else if List.mem_assoc "pool-uuid" params
    then (List.assoc "pool-uuid" params, `Pool)
    else if List.mem_assoc "sr-uuid" params
    then (List.assoc "sr-uuid" params, `SR)
    else if List.mem_assoc "host-uuid" params
    then (List.assoc "host-uuid" params, `Host)
    else
      raise
...
let pool_ha_compute_hypothetical_max_host_failures_to_tolerate
    printer rpc session_id params =
  ...
  in
  if List.length vms <> List.length pri
  then
    failwith
  ...
  in
  let n =
    Client.Pool.ha_compute_hypothetical_max_host_failures_to_tolerate
      rpc
      session_id
      (List.combine vms pri)
  in
  printer (Cli_printer.PList [ Int64.to_string n ])
    ...
    List.map
      (fun (vm, result) ->
        ...
        , if List.mem_assoc "host" result
          ...
          else if List.mem_assoc "error_code" result
          then List.assoc "error_code" result
          else "UNKNOWN" ) )
      plan
  in
  ...
  | e ->
      marshal
        ...
        (Command
           (PrintStderr
              (Printf.sprintf
                 "Host.ha_xapi_healthcheck threw exception: %s\n"
            ...
            (List.map
               (fun pbd ->
                 try
                   ...
                   in
                   if other_hosts_than_me = [] then [ sr ] else []
                 with
                 | _ ->
                     [] )
               pbds )
...
let pool_emergency_transition_to_master printer rpc session_id params =
  let force = get_bool_param params "force" in
  if (not (Pool_role.is_master ())) || force
  then (
...
let pool_install_ca_certificate fd printer rpc session_id params =
  ...
  match get_client_file fd filename with
  | Some cert ->
      Client.Pool.install_ca_certificate
        ~rpc
        ~session_id
...
let vdi_introduce printer rpc session_id params =
  ...
  let name_description =
    if List.mem_assoc "name-description" params
    then List.assoc "name-description" params
    else ""
  in
  ...
  let virtual_size = 0L
  and physical_utilisation = 0L in
...
let print_assert_exception e =
  ...
  | Api_errors.Server_error (code, [ vcpus; pcpus ])
    when code = Api_errors.host_not_enough_pcpus ->
      Printf.sprintf
        "Not enough CPUs (VM needs %s, but host has %s)"
        vcpus
        pcpus
  | Api_errors.Server_error (code, params)
    ...
    List.map
      (fun host ->
        ...
        , try
          ...
          with
          | e ->
              "Cannot start here [" ^ print_assert_exception e ^ "]" ) )
      hosts
  in
  ...
  let vm_fields =
    List.filter
      (fun x ->
        List.mem
          x.name
          [ "uuid"; "name-label"; "power-state"; "possible-hosts" ] )
      vm_r.fields
  in
  ...
  printer
    (Cli_printer.PList
       [ ( try
           ...
           with
         | e ->
             "VM is not agile because: " ^ print_assert_exception e )
       ] )
  ...
  let _type =
    ...
    then
      ...
      | x ->
          failwith
            (Printf.sprintf
               {|"Unknown type: %s (should be "cd", "disk" or "floppy"|}
               x )
    else `Disk
  in
  let unpluggable = get_bool_param params ~default:true "unpluggable" in
  if _type = `Disk && empty
  then failwith "Empty VBDs can only be made for type=CD" ;
  let vbd =
    Client.VBD.create
      ~rpc
      ~session_id
...
let vbd_unplug printer rpc session_id params =
  ...
  let timeout =
    ...
    then
      try float_of_string (List.assoc "timeout" params) with
      | _ ->
...
let sr_create fd printer rpc session_id params =
  ...
  let device_config =
    List.map
      (fun (k, v) ->
        if Astring.String.is_suffix ~affix:suffix k
        then (
    ...
    List.fold_left
      (fun (srs, complete_configs, incomplete_configs) x ->
        ...
        | None ->
            ...
            then (srs, x :: complete_configs, incomplete_configs)
            else (srs, complete_configs, x :: incomplete_configs) )
      ([], [], [])
      results
  in
  let print_sr x =
...
let vif_configure_ipv4 printer rpc session_id params =
  ...
  then failwith "Required parameter not found: address" ;
...
let net_create printer rpc session_id params =
  ...
  let mtu =
    if List.mem_assoc "MTU" params
    then Int64.of_string (List.assoc "MTU" params)
    else 1500L
  in
...
let vm_destroy printer rpc session_id params =
  ...
  if Client.VM.get_is_control_domain rpc session_id vm
  then
    raise
      (Api_errors.Server_error
         ( Api_errors.operation_not_allowed
         , [ "You cannot destroy a control domain via the CLI" ] ) )
  else Client.VM.destroy rpc session_id vm
...
let event_wait printer rpc session_id params =
  ...
  let filter_params =
    List.map
      (fun (key, value) ->
        if Astring.String.is_prefix ~affix:"/=" value
        then
          let key' = key in
          let value' = String.sub value 2 (String.length value - 2) in
          (`NotEquals, key', value')
        else (`Equals, key, value) )
      filter_params
  in
...
let select_vms
    ?(include_control_vms = false)
    ?(include_template_vms = false)
    rpc
    session_id
    params
    ignore_params =
  ...
  let params =
    if not include_control_vms
    then ("is-control-domain", "false") :: params
    else params
  in
  let params =
    if not include_template_vms
    then ("is-a-template", "false") :: params
    else params
  in
  let vm_name_or_ref =
  ...
  let params, where_clause =
    ...
    | Some v ->
        ...
        , Printf.sprintf
            "(field \"uuid\"=\"%s\") or (field \"name__label\"=\"%s\")"
            v
            v )
  in
...
let format_message msg =
  Printf.sprintf
    "Message: time=%s priority=%Ld name='%s'"
    (Date.to_string msg.API.message_timestamp)
    msg.API.message_priority
...
let do_vm_op
  ...
  try
    ...
    | 1 ->
        [ op (List.hd vms) ]
    | _ ->
        if multiple && get_bool_param params "multiple"
        then do_multiple op vms
        else
          failwith
            ( if not multiple
            then
              "Multiple matches VMs found. Operation can only be performed on \
               one VM at a time"
            else
...
let do_host_op rpc session_id op params ?(multiple = true) ignore_params =
  ...
  | _ ->
      if multiple && get_bool_param params "multiple"
      then do_multiple (op (List.length hosts)) hosts
      else
        failwith
          ( if not multiple
          then
            "Multiple matching hosts found. Operation can only be performed on \
             one host at a time"
          else
...
let hook_no_hosts_available printer rpc session_id vm f =
  ...
  | Api_errors.Server_error (code, params) as e ->
      if code = Api_errors.no_hosts_available
      then (
        printer
          (Cli_printer.PList
...
let vm_memory_shadow_multiplier_set printer rpc session_id params =
  let multiplier =
    try float_of_string (List.assoc "multiplier" params) with
    | _ ->
         ...
         let task =
           ...
           then
             ...
             Client.Async.VM.start_on
               ...
               paused
               force
           else Client.Async.VM.start rpc session_id vm paused force
         in
...
let is_recommended recommendations_xml fieldname =
  ...
  try seek_recommendation i with
  | Xmlm.Error ((line, col), err) ->
      debug
        "Invalid VM.recommendations xml at line %d, column %d: %s"
        line
        col
...
let vm_install_real printer rpc session_id template name description params =
            ...
            then sr_ref
              ...
              failwith
                "SR specified via sr-uuid doesn't have the name specified via \
                 sr-name-label"
        | None ->
            if List.length sr_list > 1
            then failwith "Multiple SRs with that name-label found"
  ...
  try
    ...
    in
    ( try Client.VM.set_has_vendor_device rpc session_id new_vm want_dev with
    | e when e = licerr ->
        let msg =
          Printf.sprintf
    ...
    ( if List.mem_assoc "copy-bios-strings-from" params
    then
      let host =
        Client.Host.get_by_uuid
          rpc
          session_id
...
let vm_install printer rpc session_id params =
    ...
    then
      ...
      then
        let template_name =
          ...
          else List.assoc "template-name" params
        in
  ...
  in
  if not (Client.VM.get_is_a_template rpc session_id template)
  then failwith "Can only install from templates" ;
  ...
  let choose_objects_to_delete vm =
    ...
    let string_of_vdi vdi =
      ...
      Printf.sprintf
        "VDI: %s (%s) %s"
        r.API.vDI_uuid
        r.API.vDI_name_label
        ( if List.length r.API.vDI_VBDs <= 1
        then ""
        else " ** WARNING: disk is shared by other VMs" )
    in
    let string_of_vm vm =
    ...
    let destroy () =
      ...
      then Client.VM.hard_shutdown rpc session_id vm ;
      Client.VM.destroy rpc session_id vm ;
      List.iter (fun vdi -> Client.VDI.destroy rpc session_id vdi) vdis ;
      if suspend_VDI <> Ref.null
      then try Client.VDI.destroy rpc session_id suspend_VDI with _ -> ()
    in
...
let get_templateVM_by_uuid rpc session_id template_uuid =
  ...
  if not (Client.VM.get_is_a_template rpc session_id template_ref)
  then
    failwith
      (Printf.sprintf
...
let vm_clone_aux
    clone_op cloned_string printer include_template_vms rpc session_id params =
  let new_name = List.assoc "new-name-label" params in
  let desc =
...
let get_snapshotVM_by_uuid rpc session_id snap_uuid =
  ...
  if not (Client.VM.get_is_a_snapshot rpc session_id snap_ref)
  then
    failwith
      (Printf.sprintf
...
let snapshot_op op printer rpc session_id params =
  let new_name = List.assoc "new-name-label" params in
  let desc =
    if List.mem_assoc "new-name-description" params
    then Some (List.assoc "new-name-description" params)
    else None
  in
...
let snapshot_destroy printer rpc session_id params =
  let snap_uuid = get_snapshot_uuid params in
  let snap_ref = get_snapshotVM_by_uuid rpc session_id snap_uuid in
  if Client.VM.get_power_state rpc session_id snap_ref <> `Halted
  then Client.VM.hard_shutdown ~rpc ~session_id ~vm:snap_ref ;
    ...
    in
      ...
      (fun () ->
        ...
        let network, network_record =
          ...
          if List.mem_assoc "remote-network" params
          then
            ...
            let management_pifs =
              List.filter
                ...
                pifs
            in
            if List.length management_pifs = 0
            then
              failwith
                (Printf.sprintf
                   "Could not find management PIF on host %s"
            ...
            let sr, _ =
              List.fold_left
                (fun (sr, free_space) ((_, sr_rec') as sr') ->
                  if sr_rec'.API.sR_content_type = "iso"
                  then (sr, free_space)
                  else
                    let free_space' =
                      Int64.sub
                        sr_rec'.API.sR_physical_size
...
let vm_disk_list_aux vm is_cd_list printer rpc session_id params =
  ...
  let vdirecords =
    List.map
      (fun vbd ->
        if not (Client.VBD.get_empty rpc session_id vbd)
        then
          Some
  ...
  let op vm =
    ...
    let vdi =
      Client.VDI.create
        ...
        ~session_id
        ~name_label:vdi_name
        ~name_description:vdi_name
        ~sR:sr
        ~virtual_size:vdi_size
        ~_type:`user
        ~sharable:false
        ~read_only:false
        ~other_config:[]
...
let vm_disk_remove printer rpc session_id params =
  ...
  let op vm =
    ...
    in
    if List.length vbd_to_remove < 1
    then failwith "Disk not found"
    else
    ...
    let vbd_to_remove =
      List.filter
        (fun x ->
          ...
          with
          | _ (* VDI handle invalid *) ->
              disk_name = "<EMPTY>" )
        vm_record.API.vM_VBDs
    in
    if List.length vbd_to_remove < 1
    then raise (failwith "Disk not found")
    else
      let vbd = List.nth vbd_to_remove 0 in
      Client.VBD.destroy rpc session_id vbd
  in
...
let vm_cd_eject printer rpc session_id params =
  let op vm =
    ...
    if List.length cdvbds > 1
    then failwith "Two or more CDs found. Please use vbd-eject" ;
  ...
  let op vm =
    ...
    let cdvbds =
      List.filter
        ...
        vbds
    in
    if List.length cdvbds = 0
    then
      raise
        (Api_errors.Server_error
           (Api_errors.vm_no_empty_cd_vbd, [ Ref.string_of (vm.getref ()) ]) ) ;
    if List.length cdvbds > 1
    then failwith "Two or more empty CD devices found. Please use vbd-insert" ;
    let cd = List.hd cdvbds in
...
let host_license_add fd printer rpc session_id params =
  let host =
    ...
    else get_host_from_session rpc session_id
  in
...
let with_license_server_changes printer rpc session_id params hosts f =
  ...
  ( if List.mem_assoc "license-server-address" params
  then
  ...
  ( if List.mem_assoc "license-server-port" params
  then
...
let host_get_vms_which_prevent_evacuation printer rpc session_id params =
  ...
  let op (vm, error) =
    ...
    let selected =
      List.hd
        (select_fields params [ record ] [ "uuid"; "name-label"; "reason" ])
    in
...
let vdi_import fd printer rpc session_id params =
  ...
  let make_command task_id =
    ...
    let uri =
      Printf.sprintf
        "%s%s?session_id=%s&task_id=%s&vdi=%s%s"
        prefix
...
let vdi_export fd printer rpc session_id params =
  ...
  let make_command task_id =
    ...
    let uri =
      Printf.sprintf
        "%s%s?session_id=%s&task_id=%s&vdi=%s%s%s"
        prefix
...
let download_file rpc session_id task fd filename uri label =
  ...
  let ok =
    ...
    | Response Failed ->
        ...
        then Client.Task.set_status rpc session_id task `failure ;
        false
    | _ ->
        false
  in
  ...
  | `success ->
      ...
      then (
        if filename <> ""
        then marshal fd (Command (Print (Printf.sprintf "%s succeeded" label)))
        )
      else (
        marshal
          fd
...
let vm_import fd printer rpc session_id params =
  ...
  in
  let _type =
    if List.mem_assoc "type" params then List.assoc "type" params else "default"
  in
  ...
  if List.mem_assoc "url" params && List.mem_assoc "filename" params
  then (
    marshal
      fd
      (Command
         (PrintStderr
            "Invalid arguments. The 'url' and 'filename' parameters should not \
             both be specified.\n" ) ) ;
    raise (ExitWithError 1) ) ;
  if Vpx_types.of_string _type <> Vpx_types.XenServer
  then
    ...
    Client.VM.import_convert
      ...
      sr
      remote_config
  else if List.mem_assoc "url" params
  then
    ...
    let filename = List.assoc "filename" params in
    if (not vm_metadata_only) && dry_run
    then (
      marshal
        fd
    ...
    let make_command task_id =
      ...
      let uri =
        Printf.sprintf
          ...
          prefix
          ( if vm_metadata_only
          then Constants.import_metadata_uri
          else Constants.import_uri )
          (Ref.string_of session_id)
          (Ref.string_of task_id)
          full_restore
          force
    ...
    in
    let importtask =
      if List.mem_assoc "task-uuid" params
      then
        Some
          (Client.Task.get_by_uuid
             rpc
             session_id
    ...
    let uuids =
      if uuids = [] && dry_run
      then [ "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx" ]
      else uuids
    in
    marshal fd (Command (Print (String.concat "," uuids)))
    ...
    (fun () ->
      ...
      let ok =
        ...
        | Response Failed ->
            ...
            then Client.Task.set_status rpc session_id blobtask `failure ;
            false
        | _ ->
            false
      in
...
let vm_export fd printer rpc session_id params =
  ...
  let task_uuid =
    if List.mem_assoc "task-uuid" params
    then Some (List.assoc "task-uuid" params)
    else None
  in
  let num = ref 1 in
  let op vm =
...
let vm_is_bios_customized printer rpc session_id params =
  let op vm =
    ...
    in
    if List.length bios_strings = 0
    then
      printer
        (Cli_printer.PMsg "The BIOS strings of this VM have not yet been set.")
    else if bios_strings = Constants.generic_bios_strings
    then printer (Cli_printer.PMsg "This VM is BIOS-generic.")
    else printer (Cli_printer.PMsg "This VM is BIOS-customized.")
  in
  ignore (do_vm_op printer rpc session_id op params [])
...
let vm_vcpu_hotplug printer rpc session_id params =
  ...
  let nvcpu =
    try Int64.of_string vcpus with
    | _ ->
...
let with_database_vdi rpc session_id params f =
  let database_params = read_map_params "database" params in
  let database_uuid =
    if List.mem_assoc "vdi-uuid" database_params
    then List.assoc "vdi-uuid" database_params
    else
      failwith
...
let vm_assert_can_be_recovered printer rpc session_id params =
  ...
  with_database_vdi rpc session_id params (fun database_session ->
      let vm = Client.VM.get_by_uuid ~rpc ~session_id:database_session ~uuid in
      Client.VM.assert_can_be_recovered
        ~rpc
        ~session_id:database_session
        ~self:vm
        ~session_to:session_id )
...
let validate_and_get_vlan params =
  try Int64.of_string (List.assoc "vlan" params) with
  | _ ->
      failwith "Failed to parse parameter 'vlan': expecting an integer"
...
let license_of_host rpc session_id host =
  ...
  let expiry =
    if List.mem_assoc "expiry" params
    then Date.to_float (Date.of_string (List.assoc "expiry" params))
    else 0.
  in
...
let host_enable printer rpc session_id params =
  ignore
    (do_host_op
       ...
       session_id
       (fun _ host ->
         Client.Host.remove_from_other_config
           rpc
           session_id
           (host.getref ())
           "MAINTENANCE_MODE" ;
  ...
  ignore
    (do_host_op
       ...
       (fun _ host ->
         let sr_uuid = List.assoc "sr-uuid" params in
         let sr = Client.SR.get_by_uuid rpc session_id sr_uuid in
         Client.Host.enable_local_storage_caching
           rpc
           session_id
           (host.getref ())
           sr )
...
let host_disable_local_storage_caching printer rpc session_id params =
  ignore
    (do_host_op
       ...
       session_id
       (fun _ host ->
         Client.Host.disable_local_storage_caching
           rpc
           session_id
           (host.getref ()) )
       params
...
let host_backup fd printer rpc session_id params =
  let op _ host =
    ...
    let prefix =
      ...
      let someone =
        try SpecificHost (Client.Host.get_by_uuid rpc session_id uuid) with
        | _ ->
            Master
      in
...
let pool_dump_db fd printer rpc session_id params =
  ...
  let make_command task_id =
    ...
    let uri =
      Printf.sprintf
        "https://%s%s?session_id=%s&task_id=%s"
        master_address
        Constants.pool_xml_db_sync
        (Ref.string_of session_id)
        (Ref.string_of task_id)
    in
    debug "%s" uri ;
...
let pool_restore_db fd printer rpc session_id params =
  ...
  let make_command task_id =
    ...
    let uri =
      Printf.sprintf
        "%s%s?session_id=%s&task_id=%s&dry_run=%b"
        prefix
        Constants.pool_xml_db_sync
        (Ref.string_of session_id)
        (Ref.string_of task_id)
...
let pool_enable_external_auth printer rpc session_id params =
  ...
  Client.Pool.enable_external_auth
    rpc
    session_id
    pool
    config
  ...
  printer
    (Cli_printer.PList
       (do_host_op
          ...
          session_id
          (fun _ host ->
            Client.Host.get_system_status_capabilities
              ~rpc
              ~session_id
              ~host:(host.getref ()) )
          params
...
let host_get_system_status fd printer rpc session_id params =
  ...
  let op n host =
    let doit task_id =
      ...
      let url =
        Printf.sprintf
          "%s%s?session_id=%s&entries=%s&output=%s&task_id=%s"
          prefix
  ...
  in
  ignore
    (do_host_op rpc session_id op params [ "filename"; "entries"; "output" ])
...
let host_cpu_info printer rpc session_id params =
  let host =
    if List.mem_assoc "uuid" params
    then Client.Host.get_by_uuid rpc session_id (List.assoc "uuid" params)
    else get_host_from_session rpc session_id
  in
...
let patch_upload fd printer rpc session_id params =
  ...
  let make_command task_id =
    ...
    let uri =
      Printf.sprintf
        "%s%s?session_id=%s&sr_id=%s&task_id=%s"
        prefix
        Constants.pool_patch_upload_uri
        (Ref.string_of session_id)
        (Ref.string_of default_sr)
        (Ref.string_of task_id)
    in
...
let update_upload fd printer rpc session_id params =
  ...
  let update_ref =
    try Client.Pool_update.introduce rpc session_id vdi_ref with
    | e ->
...
let host_logs_download fd printer rpc session_id params =
  let op n host =
    let filename =
      if List.mem_assoc "file-name" params
      then List.assoc "file-name" params
      else
        let tm = Unix.gmtime (Unix.time ()) in
        Printf.sprintf
          "logs-%d-%d-%dT%02d%02d%02dZ"
    ...
    let filesuffix =
      if n = 1
      then ""
      else "-" ^ safe_get_field (field_lookup host.fields "name-label")
    in
    let make_command task_id =
      let uri =
        Printf.sprintf
          "%s%s?session_id=%s&task_id=%s"
          prefix
          Constants.host_logs_download_uri
          (Ref.string_of session_id)
          (Ref.string_of task_id)
      in
...
let audit_log_get fd printer rpc session_id params =
  ...
  in
  let query =
    if since = "" then "" else Printf.sprintf "since=%s" (Http.urlencode since)
  in
  download_file_with_task
    fd
    rpc
...
let session_subject_identifier_logout_all printer rpc session_id params =
  ...
  List.iter
    (fun subject_identifier ->
      Client.Session.logout_subject_identifier
        ~rpc
        ~session_id
        ~subject_identifier )
    subject_identifiers
...
let vm_appliance_shutdown printer rpc session_id params =
  ...
  let ref = Client.VM_appliance.get_by_uuid ~rpc ~session_id ~uuid in
  if force
  then Client.VM_appliance.hard_shutdown ~rpc ~session_id ~self:ref
  else Client.VM_appliance.clean_shutdown ~rpc ~session_id ~self:ref
...
let vm_appliance_assert_can_be_recovered printer rpc session_id params =
  ...
  with_database_vdi rpc session_id params (fun database_session ->
      let appliance =
        Client.VM_appliance.get_by_uuid ~rpc ~session_id:database_session ~uuid
      in
      Client.VM_appliance.assert_can_be_recovered
        ~rpc
        ~session_id:database_session
        ~self:appliance
        ~session_to:session_id )
...
let gpu_group_get_remaining_capacity printer rpc session_id params =
  ...
  in
  let result =
    Client.GPU_group.get_remaining_capacity
      ~rpc
      ~session_id
      ~self:gpu_group
      ~vgpu_type
...
let dr_task_create printer rpc session_id params =
  ...
  let device_config = read_map_params "device-config" params in
  let whitelist =
    if List.mem_assoc "sr-whitelist" params
    then String.split_on_char ',' (List.assoc "sr-whitelist" params)
    else []
  in
  let dr_task =
  ...
  ignore
    (do_host_op
       ...
       (fun _ host ->
         ...
         let sr_ref = Client.SR.get_by_uuid rpc session_id sr_uuid in
         Client.LVHD.enable_thin_provisioning
           rpc
           session_id
           host_ref
           sr_ref
    ...
    ignore
      (do_host_op
         ...
         (fun _ host ->
           ...
           in
           let ref =
             Client.PVS_cache_storage.create
               ~rpc
               ~session_id
               ~host:(host.getref ())
               ~sR
...
module SDN_controller = struct
  let introduce printer rpc session_id params =
    let port =
      ...
      then
        try Int64.of_string (List.assoc "tcp-port" params) with
        | _ ->
            failwith "port field should be an integer"
      else 0L
    in
    let protocol =
      if List.mem_assoc "protocol" params
      then Record_util.sdn_protocol_of_string (List.assoc "protocol" params)
      else `ssl
    in
    let address =
      if List.mem_assoc "address" params
      then List.assoc "address" params
      else ""
    in
    let sdn =
