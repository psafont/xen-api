  ...
  Arg.parse
    (Arg.align
       [ ( "-template"
         ...
         , Printf.sprintf
             " Clone VMs from named base template (default is %s)"
             !template_name )
       ; ( "-scenario"
         , Arg.Set_string scenario
         , Printf.sprintf
             " Choose scenario (default is %s; possibilities are %s"
             !scenario
       ...
       ; ( "-ipbase"
         , Arg.Set_int ipbase
         , Printf.sprintf
             " Choose base IP address (default is %d for 192.168.%d.1)"
             !ipbase
             !ipbase )
       ; ( "-xenrtoutput"
       ...
       ; ( "-runall"
         , Arg.Set run_all
         , Printf.sprintf
             " Run tests %s (tests run by default are %s)"
    ...
    (fun x ->
      if !mode = ""
      then mode := x
      else debug ~out:stderr "Ignoring unexpected argument: %s" x )
    (Printf.sprintf
       "Configure and run a simulated test\nUsage: %s -key <pool_name> %s"
       Sys.argv.(0)
       (string_of_set possible_modes) ) ;
  if not (List.mem !mode possible_modes)
  then (
    debug
      ~out:stderr
      "Unknown mode: \"%s\" (possibilities are %s)"
      !mode
      (string_of_set possible_modes) ;
    exit 1 ) ;
  if not (List.mem !scenario (Scenario.get_all ()))
  then (
    debug
      ~out:stderr
      "Unknown scenario: \"%s\" (possibilities are %s)"
      !scenario
      (string_of_set (Scenario.get_all ())) ;
    exit 1 ) ;
  if !key = ""
    ...
    | _ ->
        ...
        Pervasiveext.finally
          (fun () ->
            ...
            | "run" ->
                let newrpc =
                  if pool.Scenario.sdk
                  then remoterpc (Printf.sprintf "192.168.%d.1" !ipbase)
                  else rpc
                in
                let session =
                  if pool.Scenario.sdk
                ...
                Pervasiveext.finally
                  ...
                  (fun () ->
                    if pool.Scenario.sdk
                    then Client.Session.logout newrpc session )
            | _ ->
