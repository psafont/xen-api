    ...
    for j = 0 to vm.num - 1 do
      let newname =
        Printf.sprintf
          "VM %d%s%s"
          j
          ( if Array.length storages > 1
          then Printf.sprintf " in SR %d" i
          else "" )
      ...
      for userdevice = 0 to vm.vbds - 1 do
        Printf.printf
          " - creating VDI %d for VM %d on SR %d of %d\n%!"
          userdevice
          j
          i
          (Array.length storages) ;
        let newdisk =
          Client.VDI.create
            ~rpc
            ~session_id
            ~name_label:"Guest disk"
            ~name_description:""
            ~sR:storages.(i)
            ~virtual_size:4194304L
            ~_type:`user
            ~sharable:false
            ~read_only:false
            ~xenstore_data:[]
        ...
        ignore
          (Client.VBD.create
             ...
             ~vM:clone
             ~vDI:newdisk
             ~userdevice:(string_of_int userdevice)
             ~bootable:false
             ~mode:`RW
             ~_type:`Disk
             ~unpluggable:true
