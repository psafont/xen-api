...
let get_network_num_from_interface pool i =
  if i < pool.bonds * 2 then i / 2 else i - pool.bonds
...
let initialise session_id template pool =
  ...
  let newdisk =
    Client.VDI.create
      ~rpc
      ~session_id
      ~name_label:"SDK storage"
      ~name_description:""
      ~sR:defaultsr
      ~virtual_size:sr_disk_size
      ~_type:`user
      ~sharable:false
      ~read_only:false
      ~xenstore_data:[]
  ...
  Client.VM.add_to_xenstore_data
    ~rpc
    ~session_id
    ~self:template
    ~key:"vm-data/provision/interfaces/0/admin"
    ~value:"true" ;
  Array.iteri
    (fun i net ->
      Client.VM.add_to_xenstore_data
...
let reset_template session_id template =
  ...
  List.iter
    (fun vif ->
      try
        if List.mem_assoc
             oc_key
             (Client.VIF.get_other_config rpc session_id vif)
        then Client.VIF.destroy rpc session_id vif
      with
      | _ ->
          () )
    vifs ;
  ...
  try Client.VM.remove_from_other_config rpc session_id template oc_key with
  | _ ->
      ()
...
let uninitialise session_id template key =
  ...
  List.iter
    (fun vm ->
      ...
      if (not is_a_template) && (not is_control_domain) && is_managed
      then (
  ...
  List.iter
    (fun netdev ->
      if not
           (List.exists (fun (_, net) -> net.API.network_bridge = netdev) nets)
      then (
        ignore
...
let iscsi_vm_iso_must_exist session_id =
  ...
  then
    failwith
      (Printf.sprintf
         "The iSCSI target VM iso could not be found (%s)"
...
let create_sdk_pool session_id sdkname pool_name key ipbase =
  ...
  let template =
    try List.hd (Client.VM.get_by_name_label rpc session_id sdkname) with
    | _ ->
  ...
  ignore
    (Sys.command
       (Printf.sprintf
          "ifconfig %s 192.168.%d.200 up"
          (Client.Network.get_bridge
             rpc
             session_id
  ...
  let has_host_booted rpc session_id i host =
    try
      ...
      then true
        ...
        if live.(i) <> live' || enabled.(i) <> enabled'
        then debug "Individual host status: %s" (string_of_status ()) ;
        live.(i) <- live' ;
        enabled.(i) <- enabled' ;
        live' && enabled'
    with
  ...
  let xml =
    try
      Client.SR.probe
        ...
        ~_type:"lvmoiscsi"
    with
    | Api_errors.Server_error ("SR_BACKEND_FAILURE_96", [ a; b; xml ]) ->
        xml
  in
  ...
  let xml =
    try
      Client.SR.probe
        ...
        ~_type:"lvmoiscsi"
    with
    | Api_errors.Server_error ("SR_BACKEND_FAILURE_107", [ a; b; xml ]) ->
        xml
  in
  (* Create an SR for each LUN found *)
  Printf.printf
    "Creating LVMoISCSI SRs (one for each of %d LUNs)\n%!"
    pool.iscsi_luns ;
  let scsiids = Array.of_list (parse_sr_probe_for_scsiids xml) in
  if Array.length scsiids <> pool.iscsi_luns
  then
    failwith
      (Printf.sprintf
         "We created %d VDIs on the iSCSI target VM but found %d LUNs"
         pool.iscsi_luns
         (Array.length scsiids) ) ;
  let lun_srs =
      ...
      (fun i host_uuid ->
        ...
        Array.init pool.bonds (fun bnum ->
            ...
            Client.PIF.reconfigure_ip
              ...
              ""
              "" ;
            if Client.PIF.get_management poolrpc poolses master
            then (
              ( try
                  Client.Host.management_reconfigure poolrpc poolses bondpif
                with
              | _ ->
                  () ) ;
...
let create_pool session_id sdkname pool_name key ipbase =
  ...
  let storages =
    ...
    | [] ->
        [| Client.SR.create
             ...
             ~session_id
             ~_type:"ext"
             ~name_label:"Local vhd"
             ~name_description:""
             ~device_config:[ ("device", "/dev/sda3") ]
             ~host
             ~physical_size:Scenario.sr_disk_size
             ~shared:true
             ~sm_config:[]
             ~content_type:""
        |]
  ...
  Client.Pool.set_crash_dump_SR
    ~rpc
    ~session_id
    ~self:pool_ref
    ~value:storages.(0) ;
  Client.Pool.set_suspend_image_SR
    ~rpc
    ~session_id
    ~self:pool_ref
    ~value:storages.(0) ;
