...
let is_flag_set ~xs ~flag ~domid ~vm =
  try xs.Xenstore.Xs.read (control_path ~xs ~domid flag) = "1" with
  | e ->
      debug
        "Exception while reading %s flag of VM %s (domain %i): %s"
        flag
        (Ref.string_of vm)
        domid
...
let checkpoint ~__context ~vm ~new_name =
  ...
  then
    try
      ...
      let sr_records =
        List.map (fun self -> Db.SR.get_record_internal ~__context ~self) vdi_sr
      in
  ...
  let snap, err =
    if not (TaskHelper.is_cancelling ~__context)
    then
      try
        ( Some
            (Xapi_vm_clone.clone
               Xapi_vm_clone.Disk_op_checkpoint
               ~__context
               ~vm
               ~new_name
               ~snapshot_info_record:!snapshot_info )
        , None )
      with
      | e ->
          (None, Some e)
...
let copy_vm_fields ~__context ~metadata ~dst ~do_not_copy ~overrides =
  ...
  List.iter
    (fun (key, value) ->
      let value =
        if List.mem_assoc key overrides then List.assoc key overrides else value
      in
...
let safe_destroy_vbd ~__context ~rpc ~session_id vbd =
  if Db.is_valid_ref __context vbd then Client.VBD.destroy rpc session_id vbd


let safe_destroy_vif ~__context ~rpc ~session_id vif =
  if Db.is_valid_ref __context vif then Client.VIF.destroy rpc session_id vif


let safe_destroy_vgpu ~__context ~rpc ~session_id vgpu =
  if Db.is_valid_ref __context vgpu then Client.VGPU.destroy rpc session_id vgpu
...
let safe_destroy_vdi ~__context ~rpc ~session_id vdi =
  ...
  then
    ...
    if not (Db.SR.get_content_type ~__context ~self:sr = "iso")
    then Client.VDI.destroy rpc session_id vdi


let safe_destroy_vusb ~__context ~rpc ~session_id vusb =
  if Db.is_valid_ref __context vusb then Client.VUSB.destroy rpc session_id vusb
...
let update_vifs_vbds_vgpus_and_vusbs ~__context ~snapshot ~vm =
  ...
  Helpers.call_api_functions ~__context (fun rpc session_id ->
      ...
      let cloned_disks =
        Xapi_vm_clone.safe_clone_disks
          rpc
          session_id
      ...
      let cloned_suspend_VDI =
        ...
        then Ref.null
          ...
          Xapi_vm_clone.clone_single_vdi
            rpc
            session_id
            Xapi_vm_clone.Disk_op_clone
            ~__context
...
let update_guest_metrics ~__context ~vm ~snapshot =
  ...
  if Db.is_valid_ref __context vm_gm
  then Db.VM_guest_metrics.destroy ~__context ~self:vm_gm ;
  if Db.is_valid_ref __context snap_gm
  then
...
let update_metrics ~__context ~vm ~snapshot =
  ...
  if Db.is_valid_ref __context vm_m
  then Db.VM_metrics.destroy ~__context ~self:vm_m ;
  if Db.is_valid_ref __context snap_m
  then
    ...
    |> if_none (fun () ->
           ...
           raise
             Api_errors.(
               Server_error
                 ( invalid_value
                 , [ "snapshot_metadata:HVM__boot_policy"; "null" ] )) )
    |> Option.get
  in
...
let revert_vm_fields ~__context ~snapshot ~vm =
  ...
  debug
    "Reverting the fields of %s to the ones of %s (%s)"
    (Ref.string_of vm)
  ...
  let snap_metadata =
    ...
    else Helpers.vm_string_to_assoc (Helpers.vm_to_string __context snapshot)
  in
  let do_not_copy = if post_MNR then do_not_copy else extended_do_not_copy in
  let snap_metadata =
