...
let rec update_table
    ...
    vm =
  let add r =
    if not (Hashtbl.mem table (Ref.string_of r))
    then Hashtbl.add table (Ref.string_of r) (make_id ())
  in
  let rec add_vdi v =
    add v ;
    let r = Db.VDI.get_record ~__context ~self:v in
    add r.API.vDI_SR ;
    if include_vhd_parents
    then
      let sm_config = r.API.vDI_sm_config in
      if List.mem_assoc Xapi_globs.vhd_parent sm_config
      then
        ...
        | _ ->
            warn
              "VM.export_metadata: lookup of parent VDI %s failed"
              parent_uuid
  in
  ...
  then (
    ...
    List.iter
      (fun vbd ->
        if Db.is_valid_ref __context vbd
        then (
          add vbd ;
    ...
    List.iter
      (fun vgpu ->
        if Db.is_valid_ref __context vgpu
        then (
          add vgpu ;
...
let lookup table r =
  if not (Hashtbl.mem table r)
  then Ref.null
  else Ref.of_string (Hashtbl.find table r)
...
let make_vm
    ?(with_snapshot_metadata = false) ~preserve_power_state table __context self
    =
  ...
  let vm =
    ...
    ; API.vM_suspend_VDI =
        ( if preserve_power_state
        then lookup table (Ref.string_of vm.API.vM_suspend_VDI)
        else Ref.null )
    ...
    ; API.vM_transportable_snapshot_id =
        ( if with_snapshot_metadata
        then vm.API.vM_transportable_snapshot_id
        else "" )
    ; API.vM_parent =
        ( if with_snapshot_metadata
    ...
    ; API.vM_guest_metrics =
        lookup table (Ref.string_of vm.API.vM_guest_metrics)
  ...
  let vgpu =
    { vgpu with
      API.vGPU_currently_attached =
        ( if preserve_power_state
        then vgpu.API.vGPU_currently_attached
        else false )
...
let vm_metadata
  ...
  List.iter
    (update_table
       ~__context
       ~include_snapshots:with_snapshot_metadata
       ~preserve_power_state
       ~include_vhd_parents
       ~table )
    vms ;
...
let export
    refresh_session __context rpc session_id s vm_ref preserve_power_state =
  info
    "VM.export: VM = %s; preserve_power_state = '%s'"
    (string_of_vm ~__context vm_ref)
    (string_of_bool preserve_power_state) ;
  let table, ova_xml =
    vm_metadata
      ~with_snapshot_metadata:false
      ~preserve_power_state
...
let metadata_handler (req : Request.t) s _ =
  ...
  Xapi_http.with_context "VM.export_metadata" req s (fun __context ->
      ...
      let vm_refs =
        ...
        else [ vm_from_request ~__context req ]
      in
      if (not export_all)
                 ...
                 let locked_vms = ref [] in
                   ...
                   (fun () ->
                     List.iter
                       ...
                       vm_refs ;
                     export_metadata
                       ~with_snapshot_metadata:export_snapshots
  ...
  Server_helpers.exec_with_new_task "VM.export" (fun __context ->
      ...
      then (
        ...
        | Api_errors.Server_error (a, b) as e ->
            error
              "Caught exception in export handler: %s"
