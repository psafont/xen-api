...
let local_assert_healthy ~__context =
  ...
  | Pool_role.Slave _ ->
      if !Xapi_globs.slave_emergency_mode
...
let assert_safe_to_reenable ~__context ~self =
  ...
  let host_disabled_until_reboot =
    try bool_of_string (Localdb.get Constants.host_disabled_until_reboot) with
    | _ ->
        false
  in
  ...
  if Db.Pool.get_ha_enabled ~__context ~self:(Helpers.get_pool ~__context)
  then (
...
let pool_size_is_restricted ~__context =
  let cvm_exception =
    ...
    |> List.exists (fun (vmref, vmrec) ->
           vmref <> dom0
           && Xapi_stdext_std.Xstringext.String.endswith
                "-CVM"
...
let assert_bacon_mode ~__context ~host =
  ...
  in
  if List.length control_domain_vbds > 0
  then
    raise
      (Api_errors.Server_error
         ...
         , [ selfref
           ; "vbd"
           ; List.hd (List.map Ref.string_of control_domain_vbds)
           ] ) ) ;
  ...
  then (
    ...
    let protected_vms, unprotected_vms =
      ...
      then
        List.partition
          (fun (vm, record) ->
            Helpers.vm_should_always_run
              record.API.vM_ha_always_run
    ...
    List.iter
      (fun (vm, _) ->
        match
          Xapi_vm_lifecycle.get_operation_error
            ~__context
            ~self:vm
            ~op:`pool_migrate
            ~strict:true
    ...
    List.iter
      (fun (vm, host) ->
        ...
        | Api_errors.Server_error (code, params) ->
            Hashtbl.replace plans vm (Error (code, params)) ) ;
        if not (Hashtbl.mem plans vm)
        then Hashtbl.replace plans vm (Migrate host) )
      plan ;
    plans
...
let compute_evacuation_plan_wlb ~__context ~self =
  ...
  *)
  ...
  List.iter
    (fun (v, detail) ->
      debug
        "WLB recommends VM evacuation: %s to %s"
      ...
      *)
      ...
      if Db.Host.get_control_domain ~__context ~self:target_host <> v
  ...
  Hashtbl.iter
    (fun v detail ->
      ...
      | Error _ as p ->
          debug
            "VM preventing evacuation: %s because %s"
            (Db.VM.get_name_label ~__context ~self:v)
  ...
  Hashtbl.iter
    (fun vm detail ->
      debug
        "compute_evacuation_plan_wlb: Key: %s Value %s"
    ...
    | Migrate host ->
        ( try
            ...
            let with_network_option =
              if network <> Ref.null
              then [ ("network", Ref.string_of network) ]
              else []
            in
            let options = ("live", "true") :: with_network_option in
            Helpers.call_api_functions ~__context (fun rpc session_id ->
                Client.Client.VM.pool_migrate
                  ~rpc
                  ~session_id
                  ~vm
                  ~host
    ...
    raise
      Api_errors.(
        Server_error
          ( internal_error
          , [ Printf.sprintf
                "evacuate: %d VMs are still resident on %s"
                remainder
                (Ref.string_of host)
            ] ))
...
let restart_agent ~__context ~host =
  ...
  in
  debug
    "Created process with pid: %d to perform xe-toolstack-restart"
...
let enable ~__context ~host =
  if not (Db.Host.get_enabled ~__context ~self:host)
  then (
    assert_safe_to_reenable ~__context ~self:host ;
    Xapi_host_helpers.user_requested_host_disable := false ;
    info
      "Host.enabled: setting host %s (%s) to enabled because of user request"
      (Ref.string_of host)
      (Db.Host.get_hostname ~__context ~self:host) ;
...
let power_on ~__context ~host =
  let result =
    Xapi_plugins.call_plugin
      ...
      Constants.power_on_fn
      [ ("remote_host_uuid", Db.Host.get_uuid ~__context ~self:host) ]
  in
  if result <> "True"
  then failwith (Printf.sprintf "The host failed to power on.")
...
let ask_host_if_it_is_a_slave ~__context ~host =
  ...
  Server_helpers.exec_with_subtask
    ~__context
    "host.ask_host_if_it_is_a_slave"
...
let create
  ...
  if List.length (Db.Host.get_all ~__context) >= Xapi_globs.restricted_pool_size
  ...
  Db.Host.create
    ...
    ~aPI_version_vendor_implementation:
      Datamodel_common.api_version_vendor_implementation
    ~name_description
    ~name_label
    ~uuid
    ~other_config:[]
    ...
    ~suspend_image_sr:Ref.null
    ~crash_dump_sr:Ref.null
    ~logging:[]
    ~hostname
    ~address
    ~metrics
    ...
    ~virtual_hardware_platform_versions:
      ( if host_is_us
      then Xapi_globs.host_virtual_hardware_platform_versions
      else [ 0L ] )
  ...
  Db.Host_metrics.set_last_updated
    ~__context
    ~self:metrics
...
let precheck_destroy_declare_dead ~__context ~self call =
  ...
  if is_host_alive ~__context ~host:self
  then (
    error
...
let request_config_file_sync ~__context ~host ~hash =
  ...
  let master_address = Helpers.get_main_ip_address () in
  Config_file_sync.fetch_config_files
    ~master_address
    ~pool_secret:(Xapi_globs.pool_secret ())
...
let local_management_reconfigure ~__context ~interface =
  ...
  change_management_interface
    ~__context
    interface
    (Record_util.primary_address_type_of_string
       (Xapi_inventory.lookup
...
let management_reconfigure ~__context ~pif =
  ...
  in
  if Db.PIF.get_managed ~__context ~self:pif = true
  then (
    ...
    try
      ...
      in
      if primary_address_type <> mgmt_address_type
      then
        raise
          (Api_errors.Server_error
  ...
  then
    debug
      "PIF %s is already marked as a management PIF; taking no action"
      (Ref.string_of pif)
  else (
    Xapi_network.attach_internal
      ~management_interface:true
      ~__context
      ~self:net
      () ;
    change_management_interface ~__context bridge primary_address_type ;
    Xapi_pif.update_management_flags
      ~__context
      ~host:(Helpers.get_localhost ~__context) )
...
let set_hostname_live ~__context ~host ~hostname =
  Mutex.execute serialize_host_enable_disable_extauth (fun () ->
      ...
      ( if current_auth_type = Xapi_globs.auth_type_AD
      then
      ...
      if String.length hostname > 255
      then
        raise
          (Api_errors.Server_error
             (Api_errors.host_name_invalid, [ "hostname is too long" ]) ) ;
      if is_invalid_hostname hostname
      then
        raise
          (Api_errors.Server_error
...
let set_ssl_legacy ~__context ~self ~value =
  ...
  else D.info "set_ssl_legacy: called with value: %b - doing nothing" value
...
let call_extauth_plugin_nomutex ~__context ~host ~fn ~args =
  ...
  debug
    "Calling extauth plugin %s in host %s with event %s"
    plugin
    (Db.Host.get_name_label ~__context ~self:host)
    fn ;
  ...
  then response.Rpc.contents
    ...
    let protocol_failure () =
      raise
        Api_errors.(
          Server_error
            (extension_protocol_failure, [ Jsonrpc.to_string failure ]))
    in
    match failure with
    | Rpc.Enum xs ->
...
let replace_host_certificate
    ~__context ~type' ~host (write_cert_fs : unit -> X509.Certificate.t) : unit
    =
...
let install_server_certificate
  ...
  if Db.Pool.get_ha_enabled ~__context ~self:(Helpers.get_pool ~__context)
  then raise Api_errors.(Server_error (ha_is_enabled, [])) ;
...
let refresh_server_certificate ~__context ~host =
  ...
  | host ->
      debug
        "Host.refresh_server_certificates - host %s was refrehsed"
        (Ref.string_of host) ) ;
...
let detect_nonhomogeneous_external_auth_in_host ~__context ~host =
  Helpers.call_api_functions ~__context (fun rpc session_id ->
      ...
      then
        Client.Client.Pool.detect_nonhomogeneous_external_auth
          rpc
          session_id
          pool
      else
  ...
  Mutex.execute serialize_host_enable_disable_extauth (fun () ->
      ...
      then (
        ...
        try
          ...
          in
          ignore
            (Extauth.call_extauth_hook_script_in_host_wrapper
               ~__context
               host
               Extauth.event_name_after_extauth_enable
               ~call_plugin_fn ) ;
          debug
        ...
        | Extauth.Unknown_extauth_type msg ->
            ...
            Db.Host.set_external_auth_type
              ~__context
              ~self:host
...
let disable_external_auth_common
  ...
  Mutex.execute serialize_host_enable_disable_extauth (fun () ->
      ...
      then
        ...
        debug
          "Failed to disable external authentication in host %s: %s"
          host_name_label
          msg
        ...
        ignore
          (Extauth.call_extauth_hook_script_in_host_wrapper
             ~__context
             host
             Extauth.event_name_before_extauth_disable
             ~call_plugin_fn ) ;
        ...
        debug
          "external authentication service disabled successfully in host %s"
          host_name_label ;
        ...
        then Extauth_ad.stop_backend_daemon ~wait_until_success:false ;
...
let attach_static_vdis ~__context ~host ~vdi_reason_map =
  ...
  List.iter
    (function
      | vdi, _ ->
          if Db.VDI.get_type ~__context ~self:vdi = `cbt_metadata
          then (
            error
  ...
  let attach (vdi, reason) =
    ...
    in
    if not (List.exists check static_vdis)
    then
...
let detach_static_vdis ~__context ~host ~vdis =
  let detach vdi =
    ...
    let check v =
      v.Static_vdis_list.uuid = Db.VDI.get_uuid ~__context ~self:vdi
    in
    if List.exists check static_vdis
    then Static_vdis.permanent_vdi_detach ~__context ~vdi
  in
...
let copy_license_to_db ~__context ~host ~features ~additional =
  ...
  Helpers.call_api_functions ~__context (fun rpc session_id ->
      ...
      Client.Client.Host.set_license_params
        rpc
        session_id
        !Xapi_globs.localhost_ref
        license_params )
...
let apply_edition_internal ~__context ~host ~edition ~additional =
  ...
  let current_license_params =
    Xapi_stdext_std.Listext.List.replace_assoc
      "sockets"
      socket_count
  ...
  let rec remove_obsolete_features_from_db l =
    ...
    | (rf, r) :: tl ->
        ...
        then (rf, r) :: remove_obsolete_features_from_db tl
        else (
          destroy_feature rf ;
...
let license_add ~__context ~host ~contents =
  let license =
    try Base64.decode_exn contents with
    | _ ->
...
let reset_networking ~__context ~host =
  ...
  List.iter
    (fun self ->
      ...
      ( if Db.PIF.get_physical ~__context ~self = true
...
let enable_local_storage_caching ~__context ~host ~sr =
  ...
  in
  debug
    "shared: %b. List.length pbds: %d. has_required_capability: %b"
    shared
    (List.length pbds)
        ...
        raise
          Api_errors.(
            Server_error
              ( internal_error
              , [ Printf.sprintf
                    "Cannot find vlan from a vlan master PIF:%s"
        ...
        raise
          Api_errors.(
            Server_error
              ( internal_error
              , [ Printf.sprintf
                    "PIF %s has no tunnel_access_PIF_of"
  ...
  let maybe_create_tunnel_for_me (master_pif_ref, master_pif_rec) =
    ...
    then
      ...
      | [ (pif_ref, _) ] ->
          ...
          ignore
            (Xapi_tunnel.create_internal
               ~__context
               ~transport_PIF:pif_ref
               ~network:master_pif_rec.API.pIF_network
               ~host
               ~protocol )
      | _ ->
...
let sync_pif_currently_attached ~__context ~host ~bridges =
  ...
  let pif_to_bridge =
    ...
    List.map
      (fun (pif, pif_r) ->
        ...
        let bridge =
          if List.mem_assoc net network_to_bridge
  ...
  List.iter
    (fun (pif, pif_r) ->
      ...
      in
      if pif_r.API.pIF_currently_attached <> currently_attached
      then (
        Db.PIF.set_currently_attached
          ~__context
          ~self:pif
          ~value:currently_attached ;
        debug
          "PIF %s currently_attached <- %b"
          (Ref.string_of pif)
          currently_attached ) )
    pifs
...
let migrate_receive ~__context ~host ~network ~options =
  ...
  let pif =
    try List.find (fun x -> host = Db.PIF.get_host ~__context ~self:x) pifs with
    | Not_found ->
        raise
          (Api_errors.Server_error
  ...
  let sm_url =
    Printf.sprintf
      "http://%s/services/SM?session_id=%s"
      (Http.Url.maybe_wrap_IPv6_literal ip)
      new_session_id
  in
  let xenops_url =
    Printf.sprintf
      "http://%s/services/xenops?session_id=%s"
      (Http.Url.maybe_wrap_IPv6_literal ip)
      new_session_id
  in
  let master_address =
    try Pool_role.get_master_address () with
    | Pool_role.This_host_is_a_master ->
...
let set_uefi_certificates ~__context ~host ~value =
  ...
  let pool = Helpers.get_pool ~__context in
  if value <> "" then Db.Pool.set_uefi_certificates ~__context ~self:pool ~value


let set_iscsi_iqn ~__context ~host ~value =
  if value = ""
  then raise Api_errors.(Server_error (invalid_value, [ "value"; value ])) ;
...
let set_sched_gran ~__context ~self ~value =
  if Helpers.get_localhost ~__context <> self
  then failwith "Forwarded to the wrong host" ;
  if not !Xapi_globs.allow_host_sched_gran_modification
  then
    raise
      Api_errors.(
...
let get_sched_gran ~__context ~self =
  ...
  | e ->
      ...
      raise
        Api_errors.(
          Server_error (internal_error, [ "Failed to get sched-gran" ]))
...
let emergency_disable_tls_verification ~__context =
  ...
  | e ->
      info
        "Failed to update database after TLS verication was disabled: %s"
...
let alert_if_tls_verification_was_emergency_disabled ~__context =
  ...
  then
    ...
    then
      ...
      Xapi_alert.add
        ~msg:Api_messages.tls_verification_emergency_disabled
        ~cls:`Host
...
let get_host_updates_handler (req : Http.Request.t) s _ =
  ...
  Xapi_http.with_context
    "Getting available updates on host"
    req
    s
    (fun __context ->
      let installed =
