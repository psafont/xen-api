...
let valid_operations ~__context record _ref' : table =
  ...
  let set_errors
      (code : string) (params : string list) (ops : API.vif_operations_set) =
    List.iter
      (fun op ->
  ...
  then (
    debug
      "No operations are valid because current-operations = [ %s ]"
      (String.concat
         "; "
         (List.map
  ...
  if Xapi_network_sriov_helpers.is_sriov_network ~__context ~self:network
  then
    set_errors
  ...
  List.iter
    (fun (task, op) ->
      if List.mem op [ `clean_shutdown; `hard_shutdown; `suspend; `pause ]
      then
        let current_op_str =
...
let clear_current_operations ~__context ~self =
  if Db.VIF.get_current_operations ~__context ~self <> []
  then (
...
let gen_mac (dev, seed) =
  ...
  let rec chain n f acc =
    if n = 0 then Digest.string acc else chain (n - 1) f (f acc)
  in
...
let create
    ...
    ~ipv6_addresses
    ~ipv6_gateway : API.ref_VIF =
  let () = debug "VIF.create running" in
  if Xapi_network_sriov_helpers.is_sriov_network ~__context ~self:network
  then Pool_features.assert_enabled ~__context ~f:Features.Network_sriov ;
  if locking_mode = `locked || ipv4_allowed <> [] || ipv6_allowed <> []
  then assert_locking_licensed ~__context ;
  ...
  let mAC, mAC_autogenerated =
    ...
    | Some seed ->
        ...
        if mAC = ""
        then (gen_mac (int_of_string device, seed), true)
        else (mAC, false)
    | None ->
  ...
  then (
    warn
      "Creating VIF %s makes VM %s not agile"
      (Ref.string_of ref)
      (Ref.string_of vM) ;
    raise
      (Api_errors.Server_error
  ...
  debug
    "VIF ref='%s' created (VM = '%s'; MAC address = '%s')"
    (Ref.string_of ref)
    (Ref.string_of vM)
    mAC ;
  ref
...
let destroy ~__context ~self =
  ...
  Helpers.log_exn_continue
    ...
    (fun self ->
      ...
      then Db.VIF_metrics.destroy ~__context ~self )
    metrics ;
...
let copy ~__context ~vm ~preserve_mac_address vif =
  ...
  let result =
    create
      ...
      ~mAC:
        ...
        then all.API.vIF_MAC
        else "" (* leave blank = generate new mac from vm random seed *) )
      ~mTU:all.API.vIF_MTU
  ...
  in
  let proxies =
    Db.PVS_proxy.get_records_where
      ~__context
