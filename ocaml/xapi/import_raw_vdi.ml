...
let localhost_handler
    rpc session_id vdi_opt (req : Request.t) (s : Unix.file_descr) =
        ...
        | `Ok format ->
            ...
            | None ->
                ...
                | Raw | Vhd ->
                    let prezeroed =
                      not
                        (Sm_fs_ops.must_write_zeroes_into_new_vdi
                           ~__context
                           vdi )
                    in
                ...
                | Tar ->
                    ...
                    Stream_vdi.recv_all_vdi
                      refresh_session
                      s
                      __context
                      rpc
                      session_id
                      ~has_inline_checksums:true
                      ~force:false
...
let import vdi (req : Request.t) (s : Unix.file_descr) _ =
  Xapi_http.assert_credentials_ok
    "VDI.import"
    ~http_action:"put_import_raw_vdi"
    req
    s ;
  ...
  Server_helpers.exec_with_new_task "VDI.import" (fun __context ->
      try
        Helpers.call_api_functions ~__context (fun rpc session_id ->
            ...
            | Some sr ->
                debug
                  "Checking whether localhost can see SR: %s"
                  (Ref.string_of sr) ;
      ...
      | e ->
          error
            "Caught exception in import handler: %s"
