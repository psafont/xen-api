...
module D = Debug.Make (struct
  let name = "export_raw_vdi"
end)

open D

let localhost_handler
    rpc session_id vdi (req : Http.Request.t) (s : Unix.file_descr) =
      ...
      | `Ok format ->
          ...
          ( try
              ...
              | Tar ->
                  ...
                  let size =
                    Client.Client.VDI.get_virtual_size
                      ~rpc
                      ~session_id
                      ~self:vdi
                  in
...
let handler (req : Http.Request.t) (s : Unix.file_descr) _ =
  ...
  Xapi_http.assert_credentials_ok
    "VDI.export_raw"
    ~http_action:"get_export_raw_vdi"
    req
    s ;
  Server_helpers.exec_with_new_task "VDI.export_raw_vdi" (fun __context ->
