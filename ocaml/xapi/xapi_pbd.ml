...
let create_common
    ~__context ~host ~sR ~device_config ~currently_attached ~other_config =
...
let check_sharing_constraint ~__context ~sr =
  if not (Db.SR.get_shared ~__context ~self:sr)
  then
    let pbds = Db.SR.get_PBDs ~__context ~self:sr in
    ...
    Xapi_clustering.with_clustering_lock_if_needed
      ...
      (fun () ->
        ...
        then (
          ...
          Xapi_dr.remove_vdis_from_cache
            ~__context
          ...
          List.iter
            (fun vdi ->
              debug
                "Automatically disabling database replication to VDI %s"
                (Ref.string_of vdi) ;
