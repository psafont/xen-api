...
module Pkg = struct
  ...
  let compare_epoch e1 e2 =
    match (e1, e2) with
    | Some i1, Some i2 ->
        if i1 < i2 then LT else if i1 = i2 then EQ else GT
    | Some _, None ->
        GT
    | None, Some _ ->
        LT
  ...
  let compare_version_strings s1 s2 =
    ...
    let normalize v =
      let split_letters_and_numbers s =
        ...
        if Re.Str.string_match r s 0
        then [ Re.Str.replace_first r "\\1" s; Re.Str.replace_first r "\\2" s ]
        else [ s ]
      in
      ...
      |> List.map (fun s ->
             ...
             if Re.Str.string_match r s 0
             then match int_of_string s with i -> Int i | exception _ -> Str s
             else Str s )
    in
    let rec compare_segments l1 l2 =
      ...
      | c1 :: t1, c2 :: t2 ->
        ...
        | Str s1, Str s2 ->
            ...
            if r < 0 then LT else if r > 0 then GT else compare_segments t1 t2
        )
      | _ :: _, [] ->
          GT
      | [], _ :: _ ->
...
module GuidanceSet = struct
  ...
  let precedences =
    [ ( RebootHost
      , of_list [ RestartToolstack; EvacuateHost; RestartDeviceModel ] )
    ...
    let gs' =
      List.fold_left
        (fun acc (higher, lowers) ->
          if mem higher acc then diff acc lowers else acc )
        gs
        precedences
    in
        ...
        List.fold_left
          (fun a n ->
            ...
            | Xml.Element ("inequality", _, [ Xml.PCData v ]) ->
                { a with
                  inequality =
                    ( try Some (inequality_of_string v) with
                    | Invalid_inequality ->
                        None )
                }
            | Xml.Element ("epoch", _, [ Xml.PCData v ]) ->
              ( try { a with epoch = Epoch.of_string v } with
              | e ->
                  let msg =
            ...
            List.fold_left
              (fun md n ->
                ...
                | Xml.Element ("location", attrs, _) ->
                  ( try { md with location = List.assoc "href" attrs } with
                  | _ ->
...
module UpdateInfo = struct
  ...
  let assert_no_dup_update_id l =
    ...
    if List.length (List.sort_uniq comp l) = List.length (List.sort comp l)
    then l
    else (
                  ...
                  List.fold_left
                    (fun acc node ->
                      ...
                      | Xml.Element ("recommended_guidance", _, [ Xml.PCData v ])
                        ->
                          { acc with
                            rec_guidance = Some (Guidance.of_string v)
                          }
                      | Xml.Element ("absolute_guidance", _, [ Xml.PCData v ])
                        ->
                          { acc with
                            abs_guidance = Some (Guidance.of_string v)
                          }
  ...
  try
    ...
    | Some "http" | Some "https" ->
      ...
      | None ->
          raise
            Api_errors.(
              Server_error (internal_error, [ "invalid host in url" ])) )
    | _ ->
        raise
          Api_errors.(
            Server_error (internal_error, [ "invalid scheme in url" ]))
  with
  | e ->
...
let is_local_pool_repo_enabled () =
  if Mutex.try_lock exposing_pool_repo_mutex
  then (
    ...
    (fun () ->
      Xapi_stdext_pervasives.Pervasiveext.finally
        (fun () ->
          ...
          | e ->
              error
                "Failed to decompress updateinfo.xml.gz: %s"
...
let clean_yum_cache name =
  ...
  | e ->
      warn
        "Unable to clean YUM cache for %s: %s"
        name
        (ExnHelper.string_of_exn e)
...
let write_yum_config ?(source_url = None) binary_url repo_name =
  ...
  let gpgkey_path =
    Filename.concat
      !Xapi_globs.rpm_gpgkey_dir
      !Xapi_globs.repository_gpgkey_name
  in
  if !Xapi_globs.repository_gpgcheck
  then (
    if not (Sys.file_exists gpgkey_path)
    then
      raise
        Api_errors.(
          Server_error (internal_error, [ "gpg key file does not exist" ])) ;
    if not ((Unix.lstat gpgkey_path).Unix.st_kind = Unix.S_REG)
    then
      raise
        Api_errors.(
  ...
  let content_of_binary =
    ...
    ; ( if !Xapi_globs.repository_gpgcheck
      then Printf.sprintf "gpgkey=file://%s" gpgkey_path
      else "" )
    ]
  in
...
let assert_yum_error output =
  ...
  List.iter
    (fun err ->
      if Astring.String.is_infix ~affix:err output
      then (
...
let eval_guidance_for_one_update ~updates_info ~update ~kind =
  ...
  | Some uid ->
    ...
    | Some updateinfo ->
        let is_applicable (a : Applicability.t) =
          ...
          | true, true ->
            ...
            | _ ->
                warn
                  "No installed epoch, version or release for package %s.%s"
                  update.name
                  update.arch ;
                false )
          | _ ->
    ...
    | None ->
        warn
          "Can't find update ID %s from updateinfo.xml for update %s.%s"
          uid
          update.name
          update.arch ;
        None )
  | None ->
      warn
        "Ignore evaluating against package %s.%s as its update ID is missing"
        update.name
        update.arch ;
      None
...
let parse_line_of_list_updates (buff, acc) line =
  ...
  | (Some name_arch, Some epoch_ver_rel, None), _ ->
      warn
        "Can't parse '%s'. name_arch %s and epoch_ver_rel %s are in buff."
        line
        name_arch
        epoch_ver_rel ;
      (empty_buff, acc)
...
let prune_accumulative_updates
    ~accumulative_updates ~latest_updates ~installed_pkgs =
  List.filter_map
    (fun (pkg, uid) ->
      let open Rresult.R.Infix in
...
let consolidate_updates_of_host
    ~repository_name ~updates_info host updates_of_host =
  ...
  let vals, others =
    List.fold_left
      (fun (acc_vals_of_k, acc_others) (x, y) ->
        if x = k
        then (List.rev_append y acc_vals_of_k, acc_others)
