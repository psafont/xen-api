...
module Vm_memory_constraints : T = struct
  ...
  let assert_valid_for_current_context ~__context ~vm ~constraints =
    ...
    if not (Pool_features.is_enabled ~__context Features.DMC)
    then
      assert_valid_and_pinned_at_static_max ~constraints ~reason:"DMC disabled"
    else if is_control_domain
    then
      assert_valid_and_pinned_at_static_max
        ~constraints
        ~reason:"control domain"
    else if nested_virt ~__context vm
    then
      assert_valid_and_pinned_at_static_max ~constraints ~reason:"nested virt"
    else assert_valid ~constraints
