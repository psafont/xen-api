...
module Repeat_with_uniform_backoff : POLICY = struct
  ...
  let is_shorter ~than t = Mtime.Span.compare t than < 0

  let wait ~__context (state : t) (e : exn) =
    if not (is_shorter ~than:state.wait_so_far state.max_total_wait) then
      raise e ;
    let delay =
    ...
    | _ ->
        Thread.delay Scheduler.(span_to_s delay)
    ) ;
    {
      state with
