...
let call_script ?(log_output = Always) ?env ?stdin script args =
    ...
    let output, _ =
      ...
      | Some stdin ->
          Forkhelpers.execute_command_get_output_send_stdin
            ...
            stdin
    in
    if should_log_output_on_success
    then
      debug
        "%s %s succeeded [ output = '%s' ]"
        script
        (String.concat " " args)
        output ;
    output
  ...
  | Unix.Unix_error _ as e ->
      debug
        "Assuming script %s doesn't exist: caught %s"
        script
  ...
  | Forkhelpers.Spawn_internal_error (stderr, stdout, status) as e ->
      ...
      in
      if should_log_output_on_failure
      then
        debug
          "%s %s %s [stdout = '%s'; stderr = '%s']"
          script
          (String.concat " " args)
          message
          stdout
...
let determine_gateway_and_dns_ifs
    ~__context ?(management_interface : API.ref_PIF option) () =
  ...
  then (None, None)
    ...
    let dns_pif, dns_rc =
      ...
      | (ref, rc) :: tl ->
          ...
          then
            warn
              "multiple PIFs with other_config:peerdns=true - choosing %s"
              rc.API.pIF_device ;
          (ref, rc)
      | [] ->
        ( match management_interface with
...
let update_pif_address ~__context ~self =
  ...
  let dbg = Context.string_of_task __context in
  try
    if Net.Interface.exists dbg bridge
    then (
      ...
      | (addr, plen) :: _ ->
          ...
          if ip <> Db.PIF.get_IP ~__context ~self
             || netmask <> Db.PIF.get_netmask ~__context ~self
          then (
            debug
              "PIF %s bridge %s IP address changed: %s/%s"
      ...
      let ipv6_addr' =
        List.map
          ...
          ipv6_addr
      in
      if ipv6_addr' <> Db.PIF.get_IPv6 ~__context ~self
      then (
        debug
          "PIF %s bridge %s IPv6 address changed: %s"
          (Db.PIF.get_uuid ~__context ~self)
          bridge
          (String.concat "; " ipv6_addr') ;
        Db.PIF.set_IPv6 ~__context ~self ~value:ipv6_addr' ) )
    else debug "Bridge %s is not up; not updating IP" bridge
  with
  | _ ->
...
let make_remote_rpc_of_url ~srcstr ~dststr (url, pool_secret) call =
  ...
  in
  XMLRPC_protocol.rpc
    ~transport:(transport_of_url url)
    ~srcstr
    ~dststr
    ~http
    call
...
let call_api_functions_internal ~__context f =
  ...
  let do_master_login () =
    let session =
      Client.Client.Session.slave_login
        rpc
        (get_localhost ~__context)
  ...
  (* let () = debug "login done" in *)
    ...
    (fun () ->
      ...
      then
        try Client.Client.Session.logout rpc session_id with
        | e ->
            debug
              "Helpers.call_api_functions failed to logout: %s (ignoring)"
    ...
    (fun () ->
      ...
      | None ->
          ...
          ( try
              ...
              if not (is_domain_zero ~__context vm)
              then (
                error
                  "VM uuid %s is not domain zero but the uuid is in my \
                   inventory file"
                  uuid ;
...
let update_domain_zero_name ~__context host hostname =
  ...
  if dom0_host <> host
  then error "Unexpectedly incorrect dom0 record in update_domain_zero_name"
  else
    let current_name = Db.VM.get_name_label ~__context ~self:dom0 in
    let is_default =
      try String.sub current_name 0 (String.length stem) = stem with
      | _ ->
          false
    in
    if is_default && current_name <> full_name
    then Db.VM.set_name_label ~__context ~self:dom0 ~value:full_name
...
let check_domain_type : API.domain_type -> [ `hvm | `pv_in_pvh | `pv ] =
  function
  | `hvm ->
      `hvm
  | `pv_in_pvh ->
  ...
  | `unspecified ->
      raise
        Api_errors.(
          Server_error (internal_error, [ "unspecified domain type" ]))
...
let has_qemu_currently ~__context ~self =
  Db.VM_metrics.get_current_domain_type
    ~__context
...
let get_special_network other_config_key ~__context =
  ...
  let findfn net =
    ...
    try bool_of_string (List.assoc other_config_key other_config) with
    | _ ->
        false
  in
...
let group_by f list =
  ...
  let rec take_while p ac = function
    | [] ->
        (ac, [])
    | x :: xs ->
        if p x then take_while p (x :: ac) xs else (ac, x :: xs)
  in
  ...
  try
    let software_version =
      ...
      | RemoteObject (rpc, session_id, host_ref) ->
          Client.Client.Host.get_software_version
            ~rpc
            ~session_id
            ~self:host_ref
    in
...
let is_platform_version_same_on_master ~__context ~host =
  if is_pool_master ~__context ~host
  then true
  else
    let master = get_master ~__context in
    compare_host_platform_versions
      ~__context
      (LocalObject master)
      (LocalObject host)
    = 0
...
let assert_platform_version_is_same_on_master ~__context ~host ~self =
  if not (is_platform_version_same_on_master ~__context ~host)
  then
    raise
...
let assert_rolling_upgrade_not_in_progress : __context:Context.t -> unit =
 fun ~__context ->
  if rolling_upgrade_in_progress ~__context
  then
...
let check_sr_exists_for_host ~__context ~self ~host =
  if host_has_pbd_for_sr ~__context ~host ~sr:self then Some self else None
...
let assert_is_valid_ip kind field address =
  if not (is_valid_ip kind address)
  then raise Api_errors.(Server_error (invalid_ip_address_specified, [ field ]))
...
let this_is_my_address ~__context address =
  ...
  let inet_addrs =
    ...
    | Some x when x = Unix.PF_INET ->
        Net.Interface.get_ipv4_addr
          dbg
          (Xapi_inventory.lookup Xapi_inventory._management_interface)
    | Some x when x = Unix.PF_INET6 ->
        Net.Interface.get_ipv6_addr
          dbg
          (Xapi_inventory.lookup Xapi_inventory._management_interface)
    | _ ->
        []
...
let gethostbyname_family host family =
  ...
  in
  if List.length he = 0 then throw_resolve_error () ;
...
let rec bisect f lower upper =
  ...
  then if f upper then upper else lower
    ...
    if f mid then bisect f mid upper else bisect f lower mid
    ...
    call_api_functions ~__context (fun rpc session_id ->
        let result =
          Client.Client.Host.call_plugin
            ...
            ~session_id
            ~host
            ~plugin:"openvswitch-config-update"
            ~fn:"update"
            ~args:[]
        in
        debug
          "openvswitch-config-update(on %s): %s"
          (Db.Host.get_name_label ~__context ~self:host)
          result )
  with
  | e ->
      debug
        "Got '%s' while trying to update the vswitch configuration on host %s"
        (Printexc.to_string e)
        (Db.Host.get_name_label ~__context ~self:host)
...
let assert_vswitch_controller_not_active ~__context =
  ...
  if sdn_controllers <> [] && backend = Network_interface.Openvswitch
  then
    raise
      (Api_errors.Server_error
...
let assert_using_vswitch ~__context =
  ...
  let using_vswitch =
    ...
    | Not_found ->
        false
  in
  if not using_vswitch
  then raise Api_errors.(Server_error (openvswitch_not_active, []))
...
let rmtree path =
  ...
  let rec rm path =
    ...
    | _ ->
        Unix.unlink path
  in
  try if Sys.file_exists path then rm path with
  | exn ->
...
let resolve_uri_path ~root ~uri_path =
  ...
  | _ ->
      let msg =
        Printf.sprintf
          "Failed to resolve uri path '%s' under '%s': %s"
          uri_path
          root
          x
      in
...
let retry ~__context ~doc ?(policy = Policy.standard) f =
  ...
  let rec loop state =
    ...
    | Api_errors.Server_error (code, objref :: _) as e
      when code = Api_errors.other_operation_in_progress ->
        debug
          "%s locking failed: caught transient failure %s"
          doc
          (ExnHelper.string_of_exn e) ;
  ...
  | false ->
      ...
      then
        raise
          Api_errors.(
            Server_error
              (internal_error, [ Printf.sprintf "retry %s failed" doc ])) ;
      Thread.delay interval ;
      retry_until_timeout ~interval:next_interval ~timeout:next_timeout doc f


let get_first_pusb ~__context usb_group =
  try List.hd (Db.USB_group.get_PUSBs ~__context ~self:usb_group) with
  | _ ->
      raise
        Api_errors.(
          Server_error
...
let get_first_vusb ~__context usb_group =
  try List.hd (Db.USB_group.get_VUSBs ~__context ~self:usb_group) with
  | _ ->
      raise
        Api_errors.(
          Server_error
...
module Task : sig
  ...
  let wait_for_ ~__context ~tasks ~propagate_cancel cb =
    ...
    let maybe_cancel_children () =
      ...
      else D.warn "wait_for_: not cancelling tasks [%s]" tasks_str
    in
    let rec process token =
      let () =
        try TaskHelper.exn_if_cancelling ~__context with
        | e ->
    ...
    let mirror =
      ...
      then Stdlib.Fun.id
        ...
        let mirror_progress () =
          ...
          if Float.compare 0.001 (Float.abs (new_progress -. current_progress))
             >= 0
          then ()
          else
        ...
        let mirror_other_config () =
          ...
          in
          if new_other_config = current_other_config
          then ()
          else
            Db.Task.set_other_config
              ~__context
              ~self:our_task
  ...
  let to_result ~__context ~of_rpc ~t =
    ...
    let res =
      ...
      | `success ->
        ...
        | s ->
          ( try Xmlrpc.of_string s |> of_rpc with
          | e ->
              fail
                (Printf.sprintf
...
let try_internal_async
    ~__context
    (marshaller : Rpc.t -> 'b)
    (internal_async_fn : unit -> API.ref_task)
    (sync_fn : unit -> 'b) : 'b =
  let res =
    try `internal_async_task (internal_async_fn ()) with
  ...
  | `internal_async_task t ->
      let ref = Ref.string_of t in
        ...
        (fun () ->
          info
            "try_internal_async: waiting for task to complete: t = ( %s )"
            ref ;
...
module PoolSecret : sig
  ...
  let to_secret x =
    ...
    *)
    ...
    if has_valid_chars && sufficiently_secret |> not
    then
      raise
        Api_errors.(
          Server_error
  ...
  let _make () =
    ...
    let make_script () =
      try
        call_script ~log_output:Never !Xapi_globs.gen_pool_secret_script []
      with
      | _ ->
          info
            "helpers.ml:PoolSecret._make: script failed, using urandom instead" ;
          make_urandom ()
    in
    let use_script =
      try
      ...
      | _ ->
          false
    in
    if use_script then make_script () else make_urandom ()
...
module FileSys : sig
  ...
  let redirect blob ~fname =
    ...
    try Unixext.write_string_to_file fname blob with
    | e ->
