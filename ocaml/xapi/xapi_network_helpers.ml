...
let assert_network_compatible_with_tunnel ~__context ~network =
  if is_sriov_based_network ~__context ~network
  then
    raise
      Api_errors.(
        Server_error
          (network_incompatible_with_tunnel, [ Ref.string_of network ]))
...
let assert_network_compatible_with_vlan_on_sriov
    ~__context ~network ~sriov ~tagged_PIF =
  match Db.Network.get_PIFs ~__context ~self:network with
  | [] ->
      ()
  | existing_pif :: _ ->
      ...
      | VLAN_untagged _ :: Network_sriov_logical existing_sriov :: _ ->
          ...
          in
          if not
               (is_device_underneath_same_type
                  ~__context
                  existing_phy_pif
                  candidate_phy_pif )
          then
...
let assert_vlan_network_compatible_with_pif
    ~__context ~network ~tagged_PIF ~pif_topo =
  match pif_topo with
  | Network_sriov_logical sriov :: _ ->
      assert_network_compatible_with_vlan_on_sriov
        ~__context
        ~network
        ~sriov
        ~tagged_PIF
  ...
  | logical_pif :: _ ->
    ...
    | sriov :: _ ->
        let existing_pif =
          Db.Network_sriov.get_physical_PIF ~__context ~self:sriov
        in
        if not (is_device_underneath_same_type ~__context pif existing_pif)
        then
          raise
            Api_errors.(
              Server_error
