...
let first_xml_element_with_name elements name =
  try Some (List.find (xml_element_has_name name) elements) with
  | Not_found ->
      None
  ...
  module Host = struct
    ...
    let of_xml_element = function
      | Xml.Element ("host", _, children) ->
          ...
          in
          let bool s =
            try bool_of_string (String.lowercase_ascii s) with
            | Invalid_argument _ ->
                invalid_arg
                  (Printf.sprintf
                     "Invalid boolean value '%s' within 'host' element"
                     s )
          in
      ...
      | Xml.Element ("host_raw_data", _, children) ->
          ...
          let find x =
            ...
            | Not_found ->
                invalid_arg
                  (Printf.sprintf
                     "Missing entry '%s' within 'host_raw_data' element"
                     x )
          in
          let int s =
            try int_of_string (String.lowercase_ascii s) with
            | Invalid_argument _ ->
                invalid_arg
                  (Printf.sprintf
