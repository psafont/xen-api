...
let apply_upgrade_rules ~__context rules previous_version =
  ...
  List.iter
    (fun r ->
      ...
      try r.fn ~__context with
      | exn ->
          error
            "Database upgrade rule '%s' failed: %s"
            r.description
        ...
        List.iter
          (fun r ->
            try
              ...
              if List.mem_assoc m.API.message_name !Api_messages.msgList
              then
                let prio =
                  List.assoc m.API.message_name !Api_messages.msgList
                in
                if prio <> m.API.message_priority
                then (
                  ...
                  | Some _ ->
                      debug
                        "Update message %s with new priority %Ld"
                        (Ref.string_of r)
                        prio
                  | None ->
                      () )
            ...
            | e ->
                warn
                  "Update message %s failed due to exception %s"
                  (Ref.string_of r)
          ...
          (fun self ->
            ...
            then (
              try
                ...
                debug
                  "Upgrade pool's other-config:mail-min-priority: %d -> %d"
                  prio
                  prio'
              with
              | e ->
  ...
  ; fn =
      (fun ~__context ->
        ...
        let wlb_passwd_ref =
          ...
          if old_wlb_pwd = ""
          then Ref.null
          else if Astring.String.is_prefix ~affix:"OpaqueRef:" old_wlb_pwd
  ...
  ; fn =
      (fun ~__context ->
        ...
        let update_vm (vm_ref, vm_rec) =
          if vm_rec.API.vM_is_control_domain
          then (
            let target = vm_rec.API.vM_memory_target in
            debug
              "VM %s (%s) dynamic_{min,max} <- %Ld"
              vm_rec.API.vM_uuid
            ...
            in
            debug
              "VM %s (%s) dynamic_{min,max},target <- %Ld"
              vm_rec.API.vM_uuid
              vm_rec.API.vM_name_label
              safe_constraints.VMC.static_max ;
            Db.VM.set_memory_static_min
              ~__context
              ~self:vm_ref
  ...
  ; fn =
      (fun ~__context ->
        ...
        | Some oem ->
            ...
            if Astring.String.is_infix ~affix:"HP" oem
            then (
              debug "Using old HP BIOS strings" ;
              update_vms Constants.old_hp_bios_strings )
            else if Astring.String.is_infix ~affix:"Dell" oem
            then (
  ...
  ; fn =
      (fun ~__context ->
        ...
        let update_vdi vdi =
          ...
          then Db.VDI.set_type ~__context ~self:vdi ~value:`redo_log
        in
  ...
  ; fn =
      (fun ~__context ->
        ...
        let update_vm vm =
          ...
          let auto_poweron =
            if List.mem_assoc "auto_poweron" other_config
            then List.assoc "auto_poweron" other_config = "true"
            else false
          in
          let restart_priority =
            Db.VM.get_ha_restart_priority ~__context ~self:vm
          in
          if auto_poweron && restart_priority = ""
          then
            Db.VM.set_ha_restart_priority
              ~__context
              ~self:vm
          ...
          (fun self ->
            ...
            let rec pull i o depth =
              ...
              | `El_start ((_, name), attrs) as el ->
                  ...
                  let field =
                    if List.mem_assoc "field" attrs
                    then Some (List.assoc "field" attrs)
                    else None
                  in
                  let value =
                    if List.mem_assoc "value" attrs
                    then Some (List.assoc "value" attrs)
                    else None
                  in
                  if name = "restriction"
                  ...
                  then (
                    ...
                    updated := true )
                  else Xmlm.output o el ;
                  pull i o (depth + 1)
              | el ->
                  Xmlm.output o el ;
                  if el = `El_end
                  then if depth = 1 then () else pull i o (depth - 1)
                  else pull i o depth
            in
            try
              pull i o 0 ;
...
let upgrade_domain_type =
  ...
  ; fn =
      (fun ~__context ->
        List.iter
          (fun (vm, vmr) ->
            if vmr.API.vM_domain_type = `unspecified
            then (
              let domain_type =
                if Helpers.is_domain_zero_with_record ~__context vm vmr
                then Xapi_globs.domain_zero_domain_type
                else
              ...
              if vmr.API.vM_power_state <> `Halted
              then
    ...
    fn =
      (fun ~__context ->
        ...
        |> List.iter (fun self ->
               ...
               in
               update_milliseconds
                 Db.Cluster.get_token_timeout
                 Db.Cluster.set_token_timeout ;
               update_milliseconds
                 Db.Cluster.get_token_timeout_coefficient
  ...
  ; fn =
      (fun ~__context ->
        ...
        |> List.iter (fun self ->
               ...
               in
               if dconf <> new_dconf
               then Db.PBD.set_device_config ~__context ~self ~value:new_dconf )
        )
  }
...
let maybe_upgrade ~__context =
  ...
  in
  debug
    "Database schema version is %s; binary schema version is %s"
    previous_string
    latest_string ;
  ...
  then
    warn
      ...
      previous_string
      previous_string
  else if previous_vsn < latest_vsn
  then (
    apply_upgrade_rules ~__context rules previous_vsn ;
    debug
      "Upgrade rules applied, bumping schema version to %d.%d"
      latest_major_vsn
      latest_minor_vsn ;
    let ( ++ ) f g h = f (g h) in
  ...
  else debug "Database schemas match, no upgrade required"
...
let hi_level_db_upgrade_rules ~__context () =
  try maybe_upgrade ~__context with
  | e ->
      error
