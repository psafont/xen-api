...
let data_from_leaf element =
  ...
  | _ ->
      raise_malformed_response
        "unknown"
        "Expected element to be leaf node"
        element
...
let filtered_headers headers =
  List.map
    (fun s ->
      if String.startswith "Authorization:" s
      then "Authorization: Basic <password>"
      else s )
    headers
...
let parse_result_code meth xml_data response initial_error enable_log =
  let code =
    try data_from_leaf (descend_and_match [ "ResultCode" ] xml_data) with
    | Xml_parse_failure error ->
        raise_malformed_response'
          ...
          ( if enable_log
          then response
          else "Logging output disabled for this call." )
  in
  let message =
    try data_from_leaf (descend_and_match [ "ErrorMessage" ] xml_data) with
...
let retrieve_inner_xml meth response enable_log =
  try descend_and_match (path_to_inner meth) response with
  | Xml_parse_failure error ->
    ( try
...
let wlb_request
  ...
  let timeout =
    try
      if List.mem_assoc timeout_key pool_other_config
      then float_of_string (List.assoc timeout_key pool_other_config)
  ...
  then
    debug
      ...
      body ;
  try
    Remote_requests.perform_request
      ~__context
      ~timeout
      ~verify_cert
      ~host
...
let perform_wlb_request
    ?auth ?url ?enable_log ~meth ~params ~handle_response ~__context () =
  ...
  let check_response response s =
    let response =
      try Xmlrpc_client.XML_protocol.read_response response s with
      | Xml.Error err ->
      ...
      Some
        (let code =
         ...
         then
           ...
           try handle_response inner_xml with
           | Xml_parse_failure error ->
               parse_result_code
                 meth
                 inner_xml
...
let float_or_raise i =
  ...
  | Failure _ ->
      raise_malformed_response'
        "unknown"
        "Could not parse star rating/recID/optID"
        "unknown"
        ...
        then
          raise_malformed_response
            ...
            "List of returned reccomendations is not equal to the number of \
             hosts in pool"
            inner_xml
        else List.map (fun x -> extract_data x) children
    | _ ->
        []
...
let init_wlb
    ...
    sprintf
      ...
      (generate_safe_param
         ...
         (let address_type =
          ...
          then sprintf "http://%s:80/" master_address
          else
  ...
  in
  Locking_helpers.Named_mutex.execute
    request_mutex
    (perform_wlb_request
       ~enable_log:false
       ~meth:"AddXenServer"
...
let decon_wlb ~__context =
  ...
  let handle_response inner_xml =
    ...
    then clear_wlb_config ~__context ~pool
    else
  ...
  let handle_response inner_xml =
    ...
    | _ ->
        raise_malformed_response
          ...
          "Expected children to OptimizationParms node"
          inner_xml
  in
  perform_wlb_request
    ~meth:"GetXenPoolConfiguration"
    ~params
    ~handle_response
    ~__context
    ()
...
let get_opt_recommendations ~__context =
  ...
  let handle_response inner_xml =
    ...
    in
    if is_childless inner_xml
    then ([], "") (*No recommendations to give. *)
    else
  ...
  let result_map =
    perform_wlb_request
      ~meth:"GetOptimizationRecommendations"
      ~params
      ~handle_response
      ~__context
      ()
    ...
    | (k, v) :: vs ->
        ...
        then
          remap
            ...
            opt_id
            vs
        else if k = "MoveToHostUuid"
        then remap vm hostfrom (Some v) reason rec_id opt_id vs
        else if k = "MoveFromHostUuid"
        then remap vm (Some v) hostto reason rec_id opt_id vs
        else if k = "Reason"
        then remap vm hostfrom hostto (Some v) rec_id opt_id vs
        else if k = "RecommendationId"
        then remap vm hostfrom hostto reason (Some v) opt_id vs
        else remap vm hostfrom hostto reason rec_id opt_id vs
    | [] ->
      ...
      | _ ->
          raise_malformed_response'
            "GetOptimizationRecommendations"
            "Missing VmToMoveUuid, RecID, MoveToHostUuid, or Reason"
            "unknown" )
  in
  try
...
let get_evacuation_recoms ~__context ~uuid =
  ...
  let handle_response inner_xml =
    ...
    then []
      ...
      | Xml.PCData _ ->
          unexpected_data
            "HostGetRecommendations"
            "HostGetRecommendationsResult"
            inner_xml
  in
  let result_map =
    perform_wlb_request
      ~meth:"HostGetRecommendations"
      ~params
      ~handle_response
      ~__context
      ()
  ...
  let rec remap vm host rec_id = function
    | (k, v) :: vs ->
        if k = "VmUuid" && v <> ""
        then remap (Some (Db.VM.get_by_uuid ~__context ~uuid:v)) host rec_id vs
        else if k = "HostUuid"
        then remap vm (Some v) rec_id vs
        else if k = "RecommendationId"
        then remap vm host (Some v) vs
        else remap vm host rec_id vs
    | [] ->
      ( match (vm, host, rec_id) with
      | Some vm', Some host', Some rec_id' ->
          (vm', [ "WLB"; host'; float_or_raise rec_id' ])
      | None, Some host', Some rec_id' ->
          ( get_dom0_vm ~__context host'
          , [ "WLB"; host'; float_or_raise rec_id' ] )
      | _ ->
          raise_malformed_response'
            "HostGetRecommendations"
            "Missing VmUuid, RecID or HostUuid"
            "unknown" )
  in
  try List.map (fun kvs -> remap None None None kvs) result_map with
  | Db_exn.Read_missing_uuid (_, _, _) | Db_exn.Too_many_values (_, _, _) ->
      raise_malformed_response'
        "HostGetRecommendations"
        "Invalid VM or host UUID"
