...
let tapdisk_of_attach_info (backend : Storage_interface.backend) =
  ...
  | [] ->
      debug
        "No XenDisk implementation in backend: %s"
...
let perform_cleanup_actions =
  List.iter (fun f ->
      ...
      | e ->
          error
            "Caught %s while performing cleanup actions"
...
let copy' ~task ~dbg ~sr ~vdi ~url ~dest ~dest_vdi =
  ...
  in
  debug
    "copy local=%s/%s url=%s remote=%s/%s"
    (Storage_interface.Sr.string_of sr)
    (Storage_interface.Vdi.string_of vdi)
    url
  ...
  then
    failwith
      (Printf.sprintf
         "Remote SR %s not found"
         (Storage_interface.Sr.string_of dest) ) ;
  let vdis = Remote.SR.scan dbg dest in
  let remote_vdi =
    try List.find (fun x -> x.vdi = dest_vdi) vdis with
    | Not_found ->
        failwith
          (Printf.sprintf
             "Remote VDI %s not found"
             (Storage_interface.Vdi.string_of dest_vdi) )
  in
  ...
  let base_vdi =
    ...
    | _ ->
        debug
          "no local VDI has content_id = %s; we will perform a full copy"
          dest_content_id ;
        None
  in
  try
    ...
    SMPERF.debug
      "mirror.copy: copy initiated local_vdi:%s dest_vdi:%s"
      (Storage_interface.Vdi.string_of vdi)
      (Storage_interface.Vdi.string_of dest_vdi) ;
    finally
      (fun () ->
            ...
            with_activated_disk ~dbg ~sr ~vdi:(Some vdi) ~dp:leaf_dp (fun src ->
                let dd =
                  Sparse_dd_wrapper.start
                    ...
                    remote_vdi.virtual_size
                in
                Storage_task.with_cancel
                  task
                  (fun () -> Sparse_dd_wrapper.cancel dd)
                  (fun () ->
                    try Sparse_dd_wrapper.wait dd with
                    | Sparse_dd_wrapper.Cancelled ->
...
let start' ~task ~dbg ~sr ~vdi ~dp ~url ~dest =
  debug
    "Mirror.start sr:%s vdi:%s url:%s dest:%s"
    (Storage_interface.Sr.string_of sr)
    (Storage_interface.Vdi.string_of vdi)
    url
    (Storage_interface.Sr.string_of dest) ;
  SMPERF.debug
    "mirror.start called sr:%s vdi:%s url:%s dest:%s"
    (Storage_interface.Sr.string_of sr)
    (Storage_interface.Vdi.string_of vdi)
    url
    (Storage_interface.Sr.string_of dest) ;
  ...
  try
    ...
    (let rec inner () =
       ...
       | Some alm ->
           ...
           if stats.Tapctl.Stats.nbd_mirror_failed = 1
           then (
    ...
    in
    debug
      "Local VDI %s = remote VDI %s"
      (Storage_interface.Vdi.string_of snapshot.vdi)
      (Storage_interface.Vdi.string_of new_parent.vdi) ;
    Remote.VDI.compose
      dbg
      dest
      result.Mirror.copy_diffs_to
      result.Mirror.mirror_vdi.vdi ;
    Remote.VDI.remove_from_sm_config
      dbg
      dest
      result.Mirror.mirror_vdi.vdi
      "base_mirror" ;
    debug
      "Local VDI %s now mirrored to remote VDI: %s"
      (Storage_interface.Vdi.string_of local_vdi.vdi)
      (Storage_interface.Vdi.string_of result.Mirror.mirror_vdi.vdi) ;
  ...
  | Storage_error (Sr_not_attached sr_uuid) ->
      error
        " Caught exception %s:%s. Performing cleanup."
    ...
    (fun (id, copy_state) ->
      ...
      List.iter
        ...
        ; (fun () ->
            Remote.VDI.destroy
              dbg
              copy_state.State.Copy_state.dest_sr
...
let receive_start ~dbg ~sr ~vdi_info ~id ~similar =
  ...
  try
    ...
    debug
      "Created dummy snapshot for mirror receive: %s"
    ...
    in
    debug
      "Nearest VDI: content_id=%s vdi=%s"
    ...
    let parent =
      ...
      | Some vdi ->
          ...
          ( if vdi_clone.virtual_size <> vdi_info.virtual_size
          then
            let new_size =
              Local.VDI.resize dbg sr vdi_clone.vdi vdi_info.virtual_size
            in
            debug
              "Resize local clone VDI to %Ld: result %Ld"
...
let post_detach_hook ~sr ~vdi ~dp =
  ...
  |> Option.iter (fun r ->
         ...
         debug
           "Created thread %d to call receive finalize and dp destroy"
           (Thread.id t) )
    ...
    try
      ...
      let remote_base =
        ...
        | Some vdi ->
            ...
            debug "Clone: %s" (Storage_interface.Vdi.string_of vdi_clone.vdi) ;
            ( if vdi_clone.virtual_size <> local_vdi.virtual_size
            then
              let new_size =
                Remote.VDI.resize dbg dest vdi_clone.vdi local_vdi.virtual_size
              in
              debug
                "Resize remote clone VDI to %Ld: result %Ld"
                local_vdi.virtual_size
                new_size ) ;
            vdi_clone
        | None ->
...
let update_snapshot_info_src ~dbg ~sr ~vdi ~url ~dest ~dest_vdi ~snapshot_pairs
  ...
  let snapshot_pairs_for_remote =
    List.map
      ...
      snapshot_pairs
  in
  Remote.SR.update_snapshot_info_dest
    dbg
    dest
    dest_vdi
