...
let call_slave_with_session
    remote_rpc_fn __context host (task_opt : API.ref_task option) f =
  let hostname = Db.Host.get_address ~__context ~self:host in
  let session_id =
    Xapi_session.login_no_password
      ~__context
      ~uname:None
      ~host
      ~pool:true
...
let call_slave_with_local_session
    remote_rpc_fn __context host (task_opt : API.ref_task option) f =
  let hostname = Db.Host.get_address ~__context ~self:host in
  let session_id =
...
let check_live ~__context h =
  ...
  then
    raise
      (Api_errors.Server_error (Api_errors.host_offline, [ Ref.string_of h ]))
...
let map_with_drop ?(doc = "performing unknown operation") f xs =
  let one x =
    ...
    | e ->
        debug
          "Caught exception while %s in message forwarder: %s"
          doc
...
let log_exn_ignore ?(doc = "performing unknown operation") f x =
  ...
  | e ->
      debug
        "Ignoring exception while %s in message forwarder: %s"
        doc
        (ExnHelper.string_of_exn e)
...
let choose_pbds_for_sr ~consider_unplugged_pbds ~__context ~self () =
  ...
  let pbd_candidates =
    if Helpers.is_sr_shared ~__context ~self
    then
      ...
      in
      if PBDSet.is_empty sr_master_pbds
      then Error `Sr_no_pbds
      else
        Ok
          (List.rev_append
             (PBDSet.elements sr_master_pbds)
             (PBDSet.elements rest_pbds) )
    else Ok pbds_to_consider
  in
...
let loadbalance_host_operation
    ~__context ~hosts ~doc ~op (f : API.ref_host -> unit) =
  ...
  let choice =
    Helpers.retry_with_global_lock ~__context ~doc (fun () ->
        ...
        Db.Host.add_to_current_operations
          ~__context
          ~self:choice
          ~key:task_id
          ~value:op ;
  ...
  (* Then do the action with the lock released *)
    ...
    (* Make sure to clean up at the end *)
      (fun () ->
      try
        Db.Host.remove_from_current_operations
          ~__context
          ~self:choice
          ~key:task_id ;
        Xapi_host_helpers.update_allowed_operations ~__context ~self:choice ;
...
functor
  ...
  struct
    ...
    let add_brackets s = if s = "" then "" else Printf.sprintf " (%s)" s
      ...
      try
        ...
        then
          ...
          Printf.sprintf
            ...
            (add_brackets name)
        else Ref.string_of vm_appliance
      with
      | _ ->
          "invalid"
    ...
    let vdi_uuid ~__context vdi =
      try
        if Pool_role.is_master ()
        then Db.VDI.get_uuid __context vdi
        else Ref.string_of vdi
      with
      | _ ->
          "invalid"
    ...
    let vif_uuid ~__context vif =
      try
        if Pool_role.is_master ()
        then Db.VIF.get_uuid __context vif
        else Ref.string_of vif
      with
      | _ ->
          "invalid"
    ...
    let vlan_uuid ~__context vlan =
      try
        if Pool_role.is_master ()
        then Db.VLAN.get_uuid __context vlan
        else Ref.string_of vlan
      with
      | _ ->
          "invalid"
    ...
    let tunnel_uuid ~__context tunnel =
      try
        if Pool_role.is_master ()
        then Db.Tunnel.get_uuid __context tunnel
        else Ref.string_of tunnel
      with
      | _ ->
          "invalid"
    ...
    let bond_uuid ~__context bond =
      try
        if Pool_role.is_master ()
        then Db.Bond.get_uuid __context bond
        else Ref.string_of bond
      with
      | _ ->
          "invalid"
    ...
    let network_sriov_uuid ~__context sriov =
      try
        if Pool_role.is_master ()
        then Db.Network_sriov.get_uuid __context sriov
        else Ref.string_of sriov
      with
      | _ ->
          "invalid"
    ...
    let pif_uuid ~__context pif =
      try
        if Pool_role.is_master ()
        then Db.PIF.get_uuid __context pif
        else Ref.string_of pif
      with
      | _ ->
          "invalid"
    ...
    let vbd_uuid ~__context vbd =
      try
        if Pool_role.is_master ()
        then Db.VBD.get_uuid __context vbd
        else Ref.string_of vbd
      with
      | _ ->
          "invalid"
    ...
    let pbd_uuid ~__context pbd =
      try
        if Pool_role.is_master ()
        then Db.PBD.get_uuid __context pbd
        else Ref.string_of pbd
      with
      | _ ->
          "invalid"
    ...
    let task_uuid ~__context task =
      try
        if Pool_role.is_master ()
        then Db.Task.get_uuid __context task
        else Ref.string_of task
      with
      | _ ->
          "invalid"
    ...
    let crashdump_uuid ~__context cd =
      try
        if Pool_role.is_master ()
        then Db.Crashdump.get_uuid __context cd
        else Ref.string_of cd
      with
      | _ ->
          "invalid"
    ...
    let host_crashdump_uuid ~__context hcd =
      try
        if Pool_role.is_master ()
        then Db.Host_crashdump.get_uuid __context hcd
        else Ref.string_of hcd
      with
      | _ ->
          "invalid"
    ...
    let network_uuid ~__context network =
      try
        if Pool_role.is_master ()
        then Db.Network.get_uuid __context network
        else Ref.string_of network
      with
      | _ ->
          "invalid"
    ...
    let host_patch_uuid ~__context patch =
      try
        if Pool_role.is_master ()
        then Db.Host_patch.get_uuid __context patch
        else Ref.string_of patch
      with
      | _ ->
          "invalid"
    ...
    let pool_patch_uuid ~__context patch =
      try
        if Pool_role.is_master ()
        then Db.Pool_patch.get_uuid __context patch
        else Ref.string_of patch
      with
      | _ ->
          "invalid"
    ...
    let pool_update_uuid ~__context update =
      try
        if Pool_role.is_master ()
        then Db.Pool_update.get_uuid __context update
        else Ref.string_of update
      with
      | _ ->
          "invalid"
    ...
    let pci_uuid ~__context pci =
      try
        if Pool_role.is_master ()
        then Db.PCI.get_uuid __context pci
        else Ref.string_of pci
      with
      | _ ->
          "invalid"
    ...
    let pgpu_uuid ~__context pgpu =
      try
        if Pool_role.is_master ()
        then Db.PGPU.get_uuid __context pgpu
        else Ref.string_of pgpu
      with
      | _ ->
          "invalid"
    ...
    let gpu_group_uuid ~__context gpu_group =
      try
        if Pool_role.is_master ()
        then Db.GPU_group.get_uuid __context gpu_group
        else Ref.string_of gpu_group
      with
      | _ ->
          "invalid"
    ...
    let vgpu_uuid ~__context vgpu =
      try
        if Pool_role.is_master ()
        then Db.VGPU.get_uuid __context vgpu
        else Ref.string_of vgpu
      with
      | _ ->
          "invalid"
    ...
    let vgpu_type_uuid ~__context vgpu_type =
      try
        if Pool_role.is_master ()
        then Db.VGPU_type.get_uuid __context vgpu_type
        else Ref.string_of vgpu_type
      with
      | _ ->
          "invalid"
    ...
    let sdn_controller_uuid ~__context sdn_controller =
      try
        if Pool_role.is_master ()
        then Db.SDN_controller.get_uuid __context sdn_controller
        else Ref.string_of sdn_controller
      with
      | _ ->
          "invalid"
    ...
    let pusb_uuid ~__context pusb =
      try
        if Pool_role.is_master ()
        then Db.PUSB.get_uuid __context pusb
        else Ref.string_of pusb
      with
      | _ ->
          "invalid"
    ...
    let usb_group_uuid ~__context usb_group =
      try
        if Pool_role.is_master ()
        then Db.USB_group.get_uuid __context usb_group
        else Ref.string_of usb_group
      with
      | _ ->
          "invalid"
    ...
    let vusb_uuid ~__context vusb =
      try
        if Pool_role.is_master ()
        then Db.VUSB.get_uuid __context vusb
        else Ref.string_of vusb
      with
      | _ ->
          "invalid"
    ...
    let repository_uuid ~__context repository =
      try
        if Pool_role.is_master ()
        then Db.Repository.get_uuid __context repository
        else Ref.string_of repository
      with
      | _ ->
          "invalid"
    ...
    module Session = struct
      ...
      let login_with_password ~__context ~uname ~pwd ~version ~originator =
        Xapi_session.record_login_failure
          ~__context
          ~uname:(Some uname)
          ~originator:(Some originator)
          ~record:`log_and_alert
          (fun () ->
            Local.Session.login_with_password
              ~__context
              ~uname
              ~pwd
              ~version
      ...
      let slave_login ~__context ~host ~psecret =
        Xapi_session.record_login_failure
          ...
          ~record:`log_and_alert
          ~originator:None
          (fun () -> Local.Session.slave_login ~__context ~host ~psecret)
      ...
      let slave_local_login ~__context ~psecret =
        Xapi_session.record_login_failure
          ~__context
          ~uname:None
          ~originator:(Some "localhost")
          ~record:`log_only
          (fun () -> Local.Session.slave_local_login ~__context ~psecret)
    ...
    module VM_appliance = struct
      ...
      let with_vm_appliance_operation ~__context ~self ~doc ~op f =
        ...
        Helpers.retry_with_global_lock ~__context ~doc (fun () ->
            ...
            Db.VM_appliance.add_to_current_operations
              ~__context
              ~self
              ~key:task_id
              ~value:op ;
        ...
        (* Then do the action with the lock released *)
            ...
            try
              Db.VM_appliance.remove_from_current_operations
                ~__context
                ~self
                ~key:task_id ;
              Xapi_vm_appliance.update_allowed_operations ~__context ~self ;
      ...
      let start ~__context ~self ~paused =
        info
          "VM_appliance.start: VM_appliance = '%s'"
          (vm_appliance_uuid ~__context self) ;
        with_vm_appliance_operation
          ~__context
          ~self
          ~doc:"VM_appliance.start"
          ~op:`start
          (fun () -> Local.VM_appliance.start ~__context ~self ~paused)


      let clean_shutdown ~__context ~self =
        info
          "VM_appliance.clean_shutdown: VM_appliance = '%s'"
          (vm_appliance_uuid ~__context self) ;
        with_vm_appliance_operation
          ~__context
          ~self
          ~doc:"VM_appliance.clean_shutdown"
          ~op:`clean_shutdown
          (fun () -> Local.VM_appliance.clean_shutdown ~__context ~self)


      let hard_shutdown ~__context ~self =
        info
          "VM_appliance.hard_shutdown: VM_appliance = '%s'"
          (vm_appliance_uuid ~__context self) ;
        with_vm_appliance_operation
          ~__context
          ~self
          ~doc:"VM_appliance.hard_shutdown"
          ~op:`hard_shutdown
          (fun () -> Local.VM_appliance.hard_shutdown ~__context ~self)


      let shutdown ~__context ~self =
        info
          "VM_appliance.shutdown: VM_appliance = '%s'"
          (vm_appliance_uuid ~__context self) ;
        with_vm_appliance_operation
          ~__context
          ~self
          ~doc:"VM_appliance.shutdown"
          ~op:`shutdown
          (fun () -> Local.VM_appliance.shutdown ~__context ~self)


      let assert_can_be_recovered ~__context ~self ~session_to =
        info
          "VM_appliance.assert_can_be_recovered: VM_appliance = '%s'"
          (vm_appliance_uuid ~__context self) ;
      ...
      let get_SRs_required_for_recovery ~__context ~self ~session_to =
        info
          "VM_appliance.get_SRs_required_for_recovery: VM_appliance = '%s'"
          (vm_appliance_uuid ~__context self) ;
        Local.VM_appliance.get_SRs_required_for_recovery
          ~__context
          ~self
          ~session_to
      ...
      let recover ~__context ~self ~session_to ~force =
        info
          "VM_appliance.recover: VM_appliance = '%s'"
          (vm_appliance_uuid ~__context self) ;
    ...
    module Pool = struct
      ...
      let eject ~__context ~host =
        ...
        |> List.iter (fun h ->
               ...
               | e ->
                   D.warn
                     "Pool.eject: while ejecting host=%s, we failed to clean \
                      up on host=%s. ignoring error: %s"
                     (Ref.short_string_of host)
                     (Ref.short_string_of h)
      ...
      let designate_new_master ~__context ~host =
        info
          "Pool.designate_new_master: pool = '%s'; host = '%s'"
          (current_pool_uuid ~__context)
          (host_uuid ~__context host) ;
        Xapi_pool_helpers.with_pool_operation
          ~__context
          ~self:(Helpers.get_pool ~__context)
          ~doc:"Pool.designate_new_master"
          ~op:`designate_new_master
          (fun () ->
      ...
      let management_reconfigure ~__context ~network =
        info
          "Pool.management_reconfigure: pool = '%s'; network = '%s'"
          (current_pool_uuid ~__context)
      ...
      let disable_ha ~__context =
        ...
        Xapi_pool_helpers.with_pool_operation
          ...
          ~self:pool
          ~op:`ha_disable
          (fun () -> Local.Pool.disable_ha __context)


      let ha_prevent_restarts_for ~__context ~seconds =
        info
          "Pool.ha_prevent_restarts_for: pool = '%s'; seconds = %Ld"
          (current_pool_uuid ~__context)
          seconds ;
      ...
      let ha_failover_plan_exists ~__context ~n =
        info
          "Pool.ha_failover_plan_exists: pool = '%s'; n = %Ld"
          (current_pool_uuid ~__context)
          n ;
      ...
      let ha_compute_max_host_failures_to_tolerate ~__context =
        Xapi_clustering.with_clustering_lock_if_cluster_exists
          ...
          (fun () ->
            info
              "Pool.ha_compute_max_host_failures_to_tolerate: pool = '%s'"
              (current_pool_uuid ~__context) ;
      ...
      let ha_compute_vm_failover_plan ~__context ~failed_hosts ~failed_vms =
        ...
        Local.Pool.ha_compute_vm_failover_plan
          ~__context
          ~failed_hosts
          ~failed_vms
      ...
      let ha_schedule_plan_recomputation ~__context =
        info
          "Pool.ha_schedule_plan_recomputation: pool = '%s'"
          (current_pool_uuid ~__context) ;
      ...
      let enable_external_auth ~__context ~pool ~config ~service_name ~auth_type
        ...
        info
          ...
          service_name
          auth_type ;
        Local.Pool.enable_external_auth
          ~__context
          ~pool
          ~config
          ~service_name
      ...
      let disable_external_auth ~__context ~pool =
        info
          "Pool.disable_external_auth: pool = '%s'"
          (pool_uuid ~__context pool) ;
      ...
      let enable_redo_log ~__context ~sr =
        info
          "Pool.enable_redo_log: pool = '%s'; sr_uuid = '%s'"
          (current_pool_uuid ~__context)
      ...
      let set_vswitch_controller ~__context ~address =
        info
          "Pool.set_vswitch_controller: pool = '%s'; address = '%s'"
          (current_pool_uuid ~__context)
          address ;
      ...
      let apply_edition ~__context ~self ~edition =
        info
          "Pool.apply_edition: pool = '%s'; edition = '%s'"
          (pool_uuid ~__context self)
          edition ;
      ...
      let set_igmp_snooping_enabled ~__context ~self ~value =
        info
          "Pool.set_igmp_snooping_enabled: Pool = '%s', value = %b"
          (pool_uuid ~__context self)
          value ;
      ...
      let has_extension ~__context ~self ~name =
        info
          "Pool.has_extension: pool = '%s'; name = '%s'"
          (pool_uuid ~__context self)
          name ;
      ...
      let remove_from_guest_agent_config ~__context ~self ~key =
        info
          "Pool.remove_from_guest_agent_config: pool = '%s'; key = '%s'"
          (pool_uuid ~__context self)
          key ;
      ...
      let enable_tls_verification ~__context =
        info
          "Pool.enable_tls_verification: pool = '%s'"
          (current_pool_uuid ~__context) ;
        ...
        Xapi_pool_helpers.with_pool_operation
          ~__context
          ~doc:"Pool.enable_tls_verification"
          ~self
          ~op:`tls_verification_enable
          (fun () ->
            ...
            |> List.iter (fun host ->
                   ...
                   debug
                     "Pool.enable_tls_verification enabling on host %s"
                     (Ref.string_of host) ) ;
      ...
      let set_repositories ~__context ~self ~value =
        info
          "Pool.set_repositories : pool = '%s'; value = [ %s ]"
          (pool_uuid ~__context self)
      ...
      let add_repository ~__context ~self ~value =
        info
          "Pool.add_repository : pool = '%s'; repo_uuid = %s"
          (pool_uuid ~__context self)
      ...
      let remove_repository ~__context ~self ~value =
        info
          "Pool.remove_repository : pool = '%s'; repo_uuid = %s"
          (pool_uuid ~__context self)
          (repository_uuid ~__context value) ;
      ...
      let check_update_readiness ~__context ~self ~requires_reboot =
        info
          "Pool.check_update_readiness: pool = '%s'; requires_reboot = %s"
          (pool_uuid ~__context self)
      ...
      let enable_client_certificate_auth ~__context ~self ~name =
        info
          "Pool.enable_client_certificate_auth: pool = '%s'; name = '%s'"
          (pool_uuid ~__context self)
          name ;
      ...
      let disable_client_certificate_auth ~__context ~self =
        info
          "Pool.disable_client_certificate_auth: pool = '%s'"
          (pool_uuid ~__context self) ;
    ...
    module VM = struct
        ...
        let cancelled =
          List.filter_map
            (fun (task, op) ->
              ...
              then (
                info
                  "Cancelling VM.%s for VM.hard_shutdown/reboot"
      ...
      let unmark_vbds ~__context ~vbds ~doc ~op =
        ...
        iter_with_drop
          ...
          (fun self ->
            ...
            then (
              Db.VBD.remove_from_current_operations
                ~__context
                ~self
                ~key:task_id ;
      ...
      let mark_vbds ~__context ~vm ~doc ~op : API.ref_VBD list =
        ...
        | e ->
            debug
              "Caught exception marking VBD for %s on VM %s: %s"
              doc
              (Ref.string_of vm)
              (ExnHelper.string_of_exn e) ;
      ...
      let with_vbds_marked ~__context ~vm ~doc ~op f =
        ...
        let vbds =
          Helpers.retry_with_global_lock
            ~__context
            ~doc
            ~policy:Helpers.Policy.fail_quickly
            (fun () -> mark_vbds ~__context ~vm ~doc ~op)
        in
        finally
          (fun () -> f vbds)
          (fun () ->
      ...
      let unmark_vifs ~__context ~vifs ~doc ~op =
        ...
        iter_with_drop
          ...
          (fun self ->
            if Db.is_valid_ref __context self
            then (
              Db.VIF.remove_from_current_operations
                ~__context
                ~self
                ~key:task_id ;
      ...
      let mark_vifs ~__context ~vm ~doc ~op : API.ref_VIF list =
        ...
        if List.length marked <> List.length vifs
        then (
      ...
      let forward_vm_op ~local_fn ~__context ~vm op =
        ...
        if List.mem power_state [ `Running; `Paused ]
        then
          do_op_on
            ~local_fn
            ~__context
      ...
      let clear_scheduled_to_be_resident_on ~__context ~vm =
        Db.VM.set_scheduled_to_be_resident_on
          ~__context
          ~self:vm
          ~value:Ref.null ;
        List.iter
      ...
      let clear_reserved_netsriov_vfs_on ~__context ~vm =
        ...
        |> List.iter (fun vif ->
               ...
               then
                 Db.PCI.set_scheduled_to_be_attached_to
                   ~__context
                   ~self:vf
                   ~value:Ref.null )
      ...
      let allocate_vm_to_host ~__context ~vm ~host ~snapshot ?host_op () =
        info
          "Reserve resources for VM %s on host %s"
          (Ref.string_of vm)
          (Ref.string_of host) ;
        ( match host_op with
        | Some x ->
      ...
      let check_vm_preserves_ha_plan ~__context ~vm ~snapshot ~host =
        ...
        then Xapi_ha_vm_failover.assert_new_vm_preserves_ha_plan ~__context vm
        else
          Xapi_ha_vm_failover.assert_vm_placement_preserves_ha_plan
            ~__context
      ...
      *)
      ...
      let forward_to_suitable_host
          ~local_fn ~__context ~vm ~snapshot ?host_op op =
        let suitable_host =
          Helpers.with_global_lock (fun () ->
              let host =
      ...
      let reserve_memory_for_vm ~__context ~vm ~snapshot ~host ?host_op f =
        Helpers.with_global_lock (fun () ->
            Xapi_vm_helpers.assert_can_boot_here
              ~__context
              ~self:vm
              ~host
              ~snapshot
     ...
     *)
      let reserve_memory_for_dynamic_change
          ~__context ~vm new_dynamic_min new_dynamic_max f =
        ...
        Helpers.with_global_lock (fun () ->
            ...
            if dynamic_min_change < 0L
            then (
              restore_old_values_on_error := true ;
              Db.VM.set_memory_dynamic_min
                ~__context
                ~self:vm
                ~value:new_dynamic_min ;
              Db.VM.set_memory_dynamic_max
                ~__context
                ~self:vm
                ~value:new_dynamic_max ) ) ;
        try f () with
        | exn ->
            if !restore_old_values_on_error
            then (
              Db.VM.set_memory_dynamic_min
                ~__context
                ~self:vm
      ...
      let create ~__context ~name_label ~name_description =
        info
          "VM.create: name_label = '%s' name_description = '%s'"
          name_label
          name_description ;
      ...
      let set_ha_always_run ~__context ~self ~value =
        info
          "VM.set_ha_always_run: VM = '%s'; value = '%b'"
      ...
      let set_ha_restart_priority ~__context ~self ~value =
        info
          "VM.set_ha_restart_priority: VM = '%s'; value = '%s'"
          (vm_uuid ~__context self)
          value ;
      ...
      let set_is_a_template ~__context ~self ~value =
        ...
        then
          with_vm_operation
            ~__context
            ~self
            ~doc:"VM.set_is_a_template"
            ~op:`make_into_template
            (fun () -> Local.VM.set_is_a_template ~__context ~self ~value:true)
     ...
     *)
      ...
      let set_is_default_template ~__context ~vm ~value =
        info
          "VM.set_is_default_template: VM = %s; value = '%s'"
        ...
        then
          with_vm_operation
            ~__context
            ~self:vm
            ~doc:"VM.set_is_default_template"
            ~op:`make_into_template
            (fun () ->
      ...
      let maximise_memory ~__context ~self ~total ~approximate =
        info
          "VM.maximise_memory: VM = '%s'; total = '%Ld'; approximate = '%b'"
          (vm_uuid ~__context self)
          total
          approximate ;
      ...
      let update_snapshot_metadata
          ~__context ~vm ~snapshot_of ~snapshot_time ~transportable_snapshot_id
          =
      ...
      let snapshot ~__context ~vm ~new_name =
        ...
        let message_body =
          Printf.sprintf
            "VM '%s' (uuid: %s) snapshotted, snapshot name: %s"
      ...
      let checkpoint ~__context ~vm ~new_name =
        ...
        let result =
          with_vm_operation
            ...
            ~op:`checkpoint
            (fun () ->
              if Db.VM.get_power_state __context vm = `Running
              then forward_vm_op ~local_fn ~__context ~vm forward_fn
              else forward_to_access_srs ~local_fn ~__context ~vm forward_fn )
        in
        let message_body =
          Printf.sprintf
            "VM '%s' (uuid: %s) checkpointed, snapshot name: %s"
            (Db.VM.get_name_label ~__context ~self:vm)
            (Db.VM.get_uuid ~__context ~self:vm)
            new_name
        in
                ...
                let srs_in_provision_spec =
                  ...
                  | Some template ->
                      ...
                      let srs =
                        List.map
                          (fun sr ->
                            try Db.SR.get_by_uuid ~__context ~uuid:sr with
                      ...
                      srs
                in
                Xapi_vm_helpers.possible_hosts
                  ~__context
                  ~vm
                  ~choose_fn:
                    (Xapi_vm_helpers.assert_can_see_specified_SRs
      ...
      let query_services ~__context ~self =
        ...
        with_vm_operation
          ~__context
          ~self
          ~doc:"VM.query_services"
          ~op:`query_services
          (fun () -> Local.VM.query_services ~__context ~self)
      ...
      let start ~__context ~vm ~start_paused ~force =
              ...
              with_vbds_marked
                ...
                (fun vbds ->
                  with_vifs_marked
                    ...
                    ~op:`attach
                    (fun vifs ->
                      Xapi_vm_helpers.ensure_domain_type_is_specified
                        ~__context
                        ~self:vm ;
                      ...
                      Xapi_vm_helpers.consider_generic_bios_strings
                        ~__context
                        ~vm ;
        ...
        let message_body =
          Printf.sprintf
            "VM '%s' (uuid: %s) started on host: %s (uuid: %s)"
            (Db.VM.get_name_label ~__context ~self:vm)
            (Db.VM.get_uuid ~__context ~self:vm)
      ...
      let start_on ~__context ~vm ~host ~start_paused ~force =
        ...
        then Helpers.assert_host_has_highest_version_in_pool ~__context ~host ;
        Xapi_vm_helpers.assert_matches_control_domain_affinity
          ~__context
          ~self:vm
          ~host ;
        ...
        List.iter
          (fun op ->
            ...
            | _, `evacuate ->
                raise
                  (Api_errors.Server_error
                     ( Api_errors.host_evacuate_in_progress
                     , [ Ref.string_of host ] ) )
            | _ ->
                () )
          (Db.Host.get_current_operations ~__context ~self:host) ;
        info
          "VM.start_on: VM = '%s'; host '%s'"
          (vm_uuid ~__context vm)
          (host_uuid ~__context host) ;
      ...
      let call_plugin ~__context ~vm ~plugin ~fn ~args =
        ...
        with_vm_operation
          ...
          ~doc:"VM.call_plugin"
          ~op:`call_plugin
          ~policy:Helpers.Policy.fail_immediately
          (fun () ->
            forward_vm_op ~local_fn ~__context ~vm (fun session_id rpc ->
      ...
      let set_has_vendor_device ~__context ~self ~value =
        info
          "VM.set_has_vendor_device: VM = '%s' to %b"
          (vm_uuid ~__context self)
          value ;
      ...
      let revert ~__context ~snapshot =
        ...
        let vm =
          if Db.is_valid_ref __context vm
          then vm
          else Xapi_vm_snapshot.create_vm_from_snapshot ~__context ~snapshot
        in
        ...
        let message_body =
          Printf.sprintf
            "VM '%s' (uuid: %s) reverted to snapshot: %s (uuid %s)"
            (Db.VM.get_name_label ~__context ~self:vm)
            (Db.VM.get_uuid ~__context ~self:vm)
      ...
      let csvm ~__context ~vm =
        ...
        let result =
          with_vm_operation
            ...
            ~op:`csvm
            (fun () ->
              forward_to_access_srs_and
                ~extra_sr:suspend_sr
                ~local_fn
                ~__context
                ~vm
                (fun session_id rpc -> Client.VM.csvm rpc session_id vm) )
        in
        let message_body =
          Printf.sprintf
            "VM '%s' (uuid: %s) cloned (new uuid: %s)"
            (Db.VM.get_name_label ~__context ~self:vm)
            (Db.VM.get_uuid ~__context ~self:vm)
      ...
      let resume ~__context ~vm ~start_paused ~force =
        ...
        let message_body =
          Printf.sprintf
            "VM '%s' (uuid: %s) resumed on host: %s (uuid: %s)"
            (Db.VM.get_name_label ~__context ~self:vm)
            (Db.VM.get_uuid ~__context ~self:vm)
            (Db.Host.get_name_label ~__context ~self:host)
            (Db.Host.get_uuid ~__context ~self:host)
      ...
      let pool_migrate_complete ~__context ~vm ~host =
        info
          "VM.pool_migrate_complete: VM = '%s'; host = '%s'"
          (vm_uuid ~__context vm)
          (host_uuid ~__context host) ;
      ...
      let pool_migrate ~__context ~vm ~host ~options =
        ...
        in
        if not force
        then Cpuid_helpers.assert_vm_is_compatible ~__context ~vm ~host () ;
      ...
      let assert_can_migrate_sender
          ~__context ~vm ~dest ~live ~vdi_map ~vif_map ~vgpu_map ~options =
        info "VM.assert_can_migrate_sender: VM = '%s'" (vm_uuid ~__context vm) ;
        let local_fn =
          Local.VM.assert_can_migrate_sender
            ~vm
            ~dest
            ~live
            ~vdi_map
        ...
        try
          forward_vm_op ~local_fn ~__context ~vm (fun session_id rpc ->
              Client.VM.assert_can_migrate_sender
                rpc
                session_id
                vm
                dest
      ...
      let assert_can_migrate
          ~__context ~vm ~dest ~live ~vdi_map ~vif_map ~options ~vgpu_map =
          ...
          let forward_internal_async () =
            forward_vm_op ~local_fn ~__context ~vm (fun session_id rpc ->
                ...
                Helpers.try_internal_async
                  ...
                  API.ref_VM_of_rpc
                  (fun () ->
                    Client.InternalAsync.VM.migrate_send
                      rpc
                      session_id
                      vm
                      dest
      ...
      let send_trigger ~__context ~vm ~trigger =
        info
          "VM.send_trigger: VM = '%s'; trigger = '%s'"
          (vm_uuid ~__context vm)
          trigger ;
      ...
      let set_VCPUs_number_live ~__context ~self ~nvcpu =
        info
          "VM.set_VCPUs_number_live: VM = '%s'; number_of_VCPU = %Ld"
          (vm_uuid ~__context self)
          nvcpu ;
        ...
        with_vm_operation
          ~__context
          ~self
          ~doc:"VM.set_VCPUs_number_live"
          ~op:`changing_VCPUs_live
          (fun () ->
            forward_vm_op ~local_fn ~__context ~vm:self (fun session_id rpc ->
      ...
      let add_to_VCPUs_params_live ~__context ~self ~key ~value =
        info
          "VM.add_to_VCPUs_params_live: VM = '%s'; params = ('%s','%s')"
          (vm_uuid ~__context self)
          key
          value ;
        ...
        with_vm_operation
          ~__context
          ~self
          ~doc:"VM.add_to_VCPUs_params_live"
          ~op:`changing_VCPUs_live
          (fun () ->
            forward_vm_op ~local_fn ~__context ~vm:self (fun session_id rpc ->
      ...
      let set_NVRAM ~__context ~self ~value =
        ...
        with_vm_operation
          ...
          ~doc:"VM.set_NVRAM"
          ~op:`changing_NVRAM
          (fun () -> Local.VM.set_NVRAM ~__context ~self ~value)


      let remove_from_NVRAM ~__context ~self ~key =
        info
          "VM.remove_from_NVRAM: self='%s', key='%s'"
          (vm_uuid ~__context self)
          key ;
        with_vm_operation
          ~__context
          ~self
          ~doc:"VM.remove_from_NVRAM"
          ~op:`changing_NVRAM
          (fun () -> Local.VM.remove_from_NVRAM ~__context ~self ~key)


      let add_to_NVRAM ~__context ~self ~key ~value =
        info
          "VM.add_to_NVRAM: self='%s', key='%s'"
          (vm_uuid ~__context self)
          key ;
        with_vm_operation
          ~__context
          ...
          ~doc:"VM.add_to_NVRAM"
          ~op:`changing_NVRAM
          (fun () -> Local.VM.add_to_NVRAM ~__context ~self ~key ~value)


      let set_VCPUs_max ~__context ~self ~value =
        info
          "VM.set_VCPUs_max: self = %s; value = %Ld"
          (vm_uuid ~__context self)
          value ;
        with_vm_operation
          ~__context
          ~self
          ~doc:"VM.set_VCPUs_max"
          ~op:`changing_VCPUs
      ...
      let set_VCPUs_at_startup ~__context ~self ~value =
        info
          "VM.set_VCPUs_at_startup: self = %s; value = %Ld"
          (vm_uuid ~__context self)
          value ;
      ...
      let set_memory_dynamic_range ~__context ~self ~min ~max =
        info
          "VM.set_memory_dynamic_range: VM = '%s'; min = %Ld; max = %Ld"
          (Ref.string_of self)
          min
          max ;
        ...
        with_vm_operation
          ~__context
          ~self
          ~doc:"VM.set_memory_dynamic_range"
          ...
          (fun () ->
            ...
            | `Running ->
                ...
                reserve_memory_for_dynamic_change
                  ~__context
                  ~vm:self
                  min
                  max
      ...
      let set_memory_dynamic_max ~__context ~self ~value =
        info
          "VM.set_memory_dynamic_max: VM = '%s'; value = %Ld"
          (vm_uuid ~__context self)
          value ;
      ...
      let set_memory_dynamic_min ~__context ~self ~value =
        info
          "VM.set_memory_dynamic_min: VM = '%s'; value = %Ld"
          (vm_uuid ~__context self)
          value ;
        set_memory_dynamic_range
          ~__context
      ...
      let set_memory_static_range ~__context ~self ~min ~max =
        info
          "VM.set_memory_static_range: self = %s; min = %Ld; max = %Ld"
          (vm_uuid ~__context self)
          min
          max ;
        with_vm_operation
          ~__context
          ~self
          ~doc:"VM.set_memory_static_range"
      ...
      let set_memory_static_max ~__context ~self ~value =
        info
          "VM.set_memory_static_max: VM = '%s'; value = %Ld"
          (vm_uuid ~__context self)
          value ;
      ...
      let set_memory_static_min ~__context ~self ~value =
        info
          "VM.set_memory_static_min: VM = '%s'; value = %Ld"
          (vm_uuid ~__context self)
          value ;
        set_memory_static_range
          ~__context
      ...
      let set_memory_limits
          ~__context ~self ~static_min ~static_max ~dynamic_min ~dynamic_max =
        info
        ...
        with_vm_operation
          ~__context
          ~self
          ~doc:"VM.set_memory_limits"
          ~op:`changing_memory_limits
          (fun () ->
            forward_vm_op ~local_fn ~__context ~vm:self (fun session_id rpc ->
                Client.VM.set_memory_limits
                  rpc
                  session_id
                  self
                  static_min
      ...
      let set_memory_target_live ~__context ~self ~target =
        info
          "VM.set_memory_target_live: VM = '%s'; min = %Ld"
          (vm_uuid ~__context self)
          target ;
      ...
      let wait_memory_target_live ~__context ~self =
        ...
        with_vm_operation
          ~__context
          ~self
          ~doc:"VM.wait_memory_target_live"
      ...
      let set_HVM_shadow_multiplier ~__context ~self ~value =
        info
          "VM.set_HVM_shadow_multiplier: self = %s; multiplier = %f"
          (vm_uuid ~__context self)
          value ;
        with_vm_operation
          ~__context
          ~self
          ~doc:"VM.set_HVM_shadow_multiplier"
      ...
      let set_shadow_multiplier_live ~__context ~self ~multiplier =
        info
          "VM.set_shadow_multiplier_live: VM = '%s'; min = %f"
          (vm_uuid ~__context self)
          multiplier ;
        ...
        with_vm_operation
          ~__context
          ~self
          ~doc:"VM.set_shadow_multiplier_live"
          ~op:`changing_shadow_memory_live
          (fun () ->
            forward_vm_op ~local_fn ~__context ~vm:self (fun session_id rpc ->
                (* No need to perform a memory calculation here: the real code will tell us if the
                   							   new value is too big. *)
                Client.VM.set_shadow_multiplier_live
                  rpc
                  session_id
                  self
                  multiplier ) )
      ...
      let get_boot_record ~__context ~self =
        ...
        with_vm_operation
          ~__context
          ~self
          ~doc:"VM.get_boot_record"
          ~op:`get_boot_record
          (fun () -> Local.VM.get_boot_record ~__context ~self)
      ...
      let record_data_source ~__context ~self ~data_source =
        info
          "VM.record_data_source: VM = '%s'; data source = '%s'"
          (vm_uuid ~__context self)
          data_source ;
      ...
      let query_data_source ~__context ~self ~data_source =
        info
          "VM.query_data_source: VM = '%s'; data source = '%s'"
          (vm_uuid ~__context self)
          data_source ;
        Xapi_vm_lifecycle.assert_initial_power_state_in
          ~__context
          ~self
      ...
      let forget_data_source_archives ~__context ~self ~data_source =
        info
          "VM.forget_data_source_archives: VM = '%s'; data source = '%s'"
          (vm_uuid ~__context self)
          data_source ;
        let local_fn =
        ...
        with_vm_operation
          ~__context
          ~self
          ~doc:"VM.forget_data_source_archives"
          ~op:`data_source_op
          (fun () ->
            forward_vm_op ~local_fn ~__context ~vm:self (fun session_id rpc ->
                Client.VM.forget_data_source_archives
                  rpc
                  session_id
                  self
                  data_source ) )
      ...
      let assert_can_boot_here ~__context ~self ~host =
        info
          "VM.assert_can_boot_here: VM = '%s'; host = '%s'"
          (vm_uuid ~__context self)
          (host_uuid ~__context host) ;
      ...
      let retrieve_wlb_recommendations ~__context ~vm =
        info
          "VM.retrieve_wlb_recommendations: VM = '%s'"
          (vm_uuid ~__context vm) ;
      ...
      let set_bios_strings ~__context ~self ~value =
        info
          "VM.set_bios_strings: self = '%s'; value = '%s'"
          (vm_uuid ~__context self)
      ...
      let copy_bios_strings ~__context ~vm ~host =
        info
          "VM.copy_bios_strings: VM = '%s'; host = '%s'"
          (vm_uuid ~__context vm)
          (host_uuid ~__context host) ;
      ...
      let assert_can_be_recovered ~__context ~self ~session_to =
        info
          "VM.assert_can_be_recovered: self = '%s';"
          (vm_uuid ~__context self) ;
      ...
      let get_SRs_required_for_recovery ~__context ~self ~session_to =
        info
          "VM.get_SRs_required_for_recovery: self = '%s';"
          (vm_uuid ~__context self) ;
      ...
      let recover ~__context ~self ~session_to ~force =
        ...
        if Db.is_valid_ref __context appliance
        then
          raise
            (Api_errors.Server_error
      ...
      let set_appliance ~__context ~self ~value =
        info
          "VM.set_appliance: self = '%s'; value = '%s';"
          (vm_uuid ~__context self)
      ...
      let import_convert
          ~__context ~_type ~username ~password ~sr ~remote_config =
        info
          "VM.import_convert: type = '%s'; remote_config = '%s;'"
          _type
          (String.concat
             ","
             (List.map (fun (k, v) -> k ^ "=" ^ v) remote_config) ) ;
        Local.VM.import_convert
          ~__context
          ~_type
          ~username
          ~password
      ...
      let import ~__context ~url ~sr ~full_restore ~force =
        ...
        let host = Db.PBD.get_host ~__context ~self:pbd in
        do_op_on
          ~local_fn:(Local.VM.import ~url ~sr ~full_restore ~force)
          ~__context
          ~host
          (fun session_id rpc ->
      ...
      let set_domain_type ~__context ~self ~value =
        info
          "VM.set_domain_type: self = '%s'; value = '%s';"
          (vm_uuid ~__context self)
      ...
      let set_HVM_boot_policy ~__context ~self ~value =
        info
          "VM.set_HVM_boot_policy: self = '%s'; value = '%s';"
          (vm_uuid ~__context self)
          value ;
    ...
    module Diagnostics = struct
      ...
      let network_stats ~__context ~host ~params =
        info
          "Diagnostics.network_stats: host = '%s'"
          (host_uuid ~__context host) ;
    ...
    module Host = struct
      let create
        ...
        info
          "Host.create: uuid='%s' name_label='%s' hostname='%s' address='%s'"
          uuid
          name_label
          hostname
          address ;
      ...
      let set_license_params ~__context ~self ~value =
        info
          "Host.set_license_params: host = '%s'; license_params = [ %s ]"
          (host_uuid ~__context self)
          (String.concat ", " (List.map (fun (k, v) -> k ^ "=" ^ v) value)) ;
      ...
      let set_ssl_legacy ~__context ~self ~value =
        info
          "Host.set_ssl_legacy: host = '%s'; value = %b"
          (host_uuid ~__context self)
          value ;
      ...
      let ha_disable_failover_decisions ~__context ~host =
        info
          "Host.ha_disable_failover_decisions: host = '%s'"
          (host_uuid ~__context host) ;
      ...
      let ha_release_resources ~__context ~host =
        info
          "Host.ha_release_resources: host = '%s'"
          (host_uuid ~__context host) ;
      ...
      let ha_wait_for_shutdown_via_statefile ~__context ~host =
        info
          "Host.ha_wait_for_shutdown_via_statefile: host = '%s'"
          (host_uuid ~__context host) ;
      ...
      let request_config_file_sync ~__context ~host ~hash =
        debug
          "Host.request_config_file_sync: host = '%s'"
          (host_uuid ~__context host) ;
      ...
      let propose_new_master ~__context ~address ~manual =
        info
          "Host.propose_new_master: type = '%s'; host address = '%s'"
      ...
      let management_reconfigure ~__context ~pif =
        info
          "Host.management_reconfigure: management PIF = '%s'"
      ...
      let get_management_interface ~__context ~host =
        info
          "Host.get_management_interface: host = '%s'"
          (host_uuid ~__context host) ;
      ...
      let disable ~__context ~host =
        ...
        Xapi_ha_vm_failover.assert_host_disable_preserves_ha_plan
          ~__context
          host ;
      ...
      let prepare_for_poweroff ~__context ~host =
        info
          "Host.prepare_for_poweroff: host = '%s'"
          (host_uuid ~__context host) ;
      ...
      let bugreport_upload ~__context ~host ~url ~options =
        ...
        in
        info
          "Host.bugreport_upload: host = '%s'; url = '%s'; options = [ %s ]"
          (host_uuid ~__context host)
          "(url filtered)"
          (String.concat
             "; "
      ...
      let send_debug_keys ~__context ~host ~keys =
        info
          "Host.send_debug_keys: host = '%s'; keys = '%s'"
          (host_uuid ~__context host)
          keys ;
      ...
      let get_vms_which_prevent_evacuation ~__context ~self =
        info
          "Host.get_vms_which_prevent_evacuation: host = '%s'"
          (host_uuid ~__context self) ;
      ...
      let evacuate ~__context ~host ~network =
        ...
        Xapi_host_helpers.with_host_operation
          ...
          ~doc:"Host.evacuate"
          ~op:`evacuate
          (fun () -> Local.Host.evacuate ~__context ~host ~network)


      let retrieve_wlb_evacuate_recommendations ~__context ~self =
        info
          "Host.retrieve_wlb_evacuate_recommendations: host = '%s'"
          (host_uuid ~__context self) ;
      ...
      let update_master ~__context ~host ~master_address =
        info
          "Host.update_master: host = '%s'; master = '%s'"
          (host_uuid ~__context host)
          master_address ;
      ...
      let get_system_status_capabilities ~__context ~host =
        info
          "Host.get_system_status_capabilities: host = '%s'"
          (host_uuid ~__context host) ;
      ...
      let get_diagnostic_timing_stats ~__context ~host =
        info
          "Host.get_diagnostic_timing_stats: host = '%s'"
          (host_uuid ~__context host) ;
      ...
      let set_hostname_live ~__context ~host ~hostname =
        info
          "Host.set_hostname_live: host = '%s'; hostname = '%s'"
          (host_uuid ~__context host)
          hostname ;
      ...
      let record_data_source ~__context ~host ~data_source =
        info
          "Host.record_data_source: host = '%s';  data source = '%s'"
          (host_uuid ~__context host)
          data_source ;
      ...
      let query_data_source ~__context ~host ~data_source =
        info
          "Host.query_data_source: host = '%s'; data source = '%s'"
          (host_uuid ~__context host)
          data_source ;
      ...
      let forget_data_source_archives ~__context ~host ~data_source =
        info
          "Host.forget_data_source_archives: host = '%s'; data source = '%s'"
          (host_uuid ~__context host)
          data_source ;
        let local_fn =
        ...
        in
        do_op_on ~local_fn ~__context ~host (fun session_id rpc ->
            Client.Host.forget_data_source_archives
              rpc
              session_id
              host
              data_source )
      ...
      let call_extension ~__context ~host ~call =
        info
          "Host.call_extension host = '%s'; call = '%s'"
          (host_uuid ~__context host)
          call ;
      ...
      let has_extension ~__context ~host ~name =
        info
          "Host.has_extension: host = '%s'; name = '%s'"
          (host_uuid ~__context host)
          name ;
      ...
      let compute_memory_overhead ~__context ~host =
        info
          "Host.compute_memory_overhead: host = '%s'"
          (host_uuid ~__context host) ;
      ...
      let enable_binary_storage ~__context ~host =
        info
          "Host.enable_binary_storage: host = '%s'"
          (host_uuid ~__context host) ;
      ...
      let disable_binary_storage ~__context ~host =
        info
          "Host.disable_binary_storage: host = '%s'"
          (host_uuid ~__context host) ;
      ...
      let enable_external_auth ~__context ~host ~config ~service_name ~auth_type
        ...
        if auth_type = Xapi_globs.auth_type_AD
        then Pool_features.assert_enabled ~__context ~f:Features.AD ;
        let local_fn =
          Local.Host.enable_external_auth ~host ~config ~service_name ~auth_type
        in
        do_op_on ~local_fn ~__context ~host (fun session_id rpc ->
            Client.Host.enable_external_auth
              rpc
              session_id
              host
              config
      ...
      let disable_external_auth ~__context ~host ~config =
        info
          "Host.disable_external_auth: host = '%s'"
          (host_uuid ~__context host) ;
      ...
      let install_ca_certificate ~__context ~host ~name ~cert =
        info
          "Host.install_ca_certificate: host = '%s'; name = '%s'"
          (host_uuid ~__context host)
          name ;
      ...
      let uninstall_ca_certificate ~__context ~host ~name =
        info
          "Host.uninstall_ca_certificate: host = '%s'; name = '%s'"
          (host_uuid ~__context host)
          name ;
      ...
      let crl_install ~__context ~host ~name ~crl =
        info
          "Host.crl_install: host = '%s'; name = '%s'"
          (host_uuid ~__context host)
          name ;
      ...
      let crl_uninstall ~__context ~host ~name =
        info
          "Host.crl_uninstall: host = '%s'; name = '%s'"
          (host_uuid ~__context host)
          name ;
      ...
      let get_server_certificate ~__context ~host =
        info
          "Host.get_server_certificate: host = '%s'"
          (host_uuid ~__context host) ;
      ...
      let refresh_server_certificate ~__context ~host =
        info
          "Host.refresh_server_certificate: host = '%s'"
          (host_uuid ~__context host) ;
        let pool = Helpers.get_pool ~__context in
        ...
        Xapi_pool_helpers.with_pool_operation
          ~__context
          ~doc:"Host.refresh_server_certificate"
          ~self:pool
          ~op:`cert_refresh
        @@ fun () ->
      ...
      let install_server_certificate
          ~__context ~host ~certificate ~private_key ~certificate_chain =
        info
          "Host.install_server_certificate: host = '%s'"
          (host_uuid ~__context host) ;
        let local_fn =
      ...
      let reset_server_certificate ~__context ~host =
        info
          "Host.reset_server_certificate: host = '%s'"
          (host_uuid ~__context host) ;
      ...
      let copy_primary_host_certs ~__context ~host =
        ...
        Xapi_pool_helpers.with_pool_operation
          ...
          ~doc:"Host.copy_primary_host_certs"
          ~self:(Helpers.get_pool ~__context)
      ...
      let attach_static_vdis ~__context ~host ~vdi_reason_map =
        info
          "Host.attach_static_vdis: host = '%s'; vdi/reason pairs = [ %s ]"
          (host_uuid ~__context host)
          (String.concat
             "; "
      ...
      let detach_static_vdis ~__context ~host ~vdis =
        info
          "Host.detach_static_vdis: host = '%s'; vdis =[ %s ]"
          (host_uuid ~__context host)
      ...
      let set_localdb_key ~__context ~host ~key ~value =
        info
          "Host.set_localdb_key: host = '%s'; key = '%s'; value = '%s'"
          (host_uuid ~__context host)
          key
          value ;
      ...
      let apply_edition ~__context ~host ~edition ~force =
        info
          "Host.apply_edition: host = '%s'; edition = '%s'; force = '%s'"
          (host_uuid ~__context host)
          edition
          (string_of_bool force) ;
      ...
      let sync_pif_currently_attached ~__context ~host ~bridges =
        info
          "Host.sync_pif_currently_attached: host = '%s'"
          (host_uuid ~__context host) ;
      ...
      let migrate_receive ~__context ~host ~network ~options =
        info
          "Host.migrate_receive: host = '%s'; network = '%s'"
          (host_uuid ~__context host)
          (network_uuid ~__context network) ;
      ...
      let apply_guest_agent_config ~__context ~host =
        info
          "Host.apply_guest_agent_config: host = '%s'"
          (host_uuid ~__context host) ;
      ...
      let nvidia_vf_setup ~__context ~host ~pf ~enable =
        info
          "Host.nvidia_vf_setup: host = '%s' pf = '%s' enable = %b"
          (host_uuid ~__context host)
          (pci_uuid ~__context pf)
          enable ;
      ...
      let allocate_resources_for_vm ~__context ~self ~vm ~live =
        info
          "Host.host_allocate_resources_for_vm: host = %s; VM = %s"
          (host_uuid ~__context self)
          (vm_uuid ~__context vm) ;
        let snapshot = Db.VM.get_record ~__context ~self:vm in
      ...
      let set_iscsi_iqn ~__context ~host ~value =
        info
          "Host.set_iscsi_iqn: host='%s' iqn='%s'"
          (host_uuid ~__context host)
          value ;
      ...
      let set_multipathing ~__context ~host ~value =
        info
          "Host.set_multipathing: host='%s' value='%s'"
          (host_uuid ~__context host)
          (string_of_bool value) ;
      ...
      let set_uefi_certificates ~__context ~host ~value =
        info
          "Host.set_uefi_certificates: host='%s' value='%s'"
          (host_uuid ~__context host)
          value ;
      ...
      let notify_send_new_pool_secret ~__context ~host ~old_ps ~new_ps =
        info
          "Host.notify_send_new_pool_secret: host='%s'"
          (host_uuid ~__context host) ;
        let local_fn =
          Local.Host.notify_send_new_pool_secret ~host ~old_ps ~new_ps
        in
        do_op_on ~__context ~host ~local_fn (fun session_id rpc ->
            Client.Host.notify_send_new_pool_secret
              rpc
              session_id
              host
              old_ps
      ...
      let set_sched_gran ~__context ~self ~value =
        info
          "Host.set_sched_gran: host='%s' sched='%s'"
          (host_uuid ~__context self)
    ...
    module Host_crashdump = struct
      let destroy ~__context ~self =
        info
          "Host_crashdump.destroy: host crashdump = '%s'"
      ...
      let upload ~__context ~self ~url ~options =
        info
          "Host_crashdump.upload: host crashdump = '%s'; url = '%s'"
          (host_crashdump_uuid ~__context self)
          "(url filtered)" ;
    ...
    module Host_patch = struct
      let destroy ~__context ~self =
        info
          "Host_patch.destroy: host patch = '%s'"
    ...
    module Pool_patch = struct
      let apply ~__context ~self ~host =
        info
          "Pool_patch.apply: pool patch = '%s'; host = '%s'"
      ...
      let precheck ~__context ~self ~host =
        info
          "Pool_patch.precheck: pool patch = '%s'; host = '%s'"
          (pool_patch_uuid ~__context self)
          (host_uuid ~__context host) ;
      ...
      let pool_apply ~__context ~self =
        info
          "Pool_patch.pool_apply: pool patch = '%s'"
          (pool_patch_uuid ~__context self) ;
      ...
      let clean_on_host ~__context ~self ~host =
        info
          "Pool_patch.clean_on_host: pool patch = '%s'"
          (pool_patch_uuid ~__context self) ;
      ...
      let pool_clean ~__context ~self =
        info
          "Pool_patch.pool_clean: pool patch = '%s'"
          (pool_patch_uuid ~__context self) ;
      ...
      let destroy ~__context ~self =
        info
          "Pool_patch.destroy: pool patch = '%s'"
          (pool_patch_uuid ~__context self) ;
    ...
    module Network = struct
      ...
      let create
          ...
          ~tags =
        info
          "Network.create: name_label = '%s'; bridge = '%s'; managed = '%b'"
          name_label
          bridge
          managed ;
      ...
      let attach ~__context ~network ~host =
        info
          "Network.attach: network = '%s'; host = '%s'"
          (network_uuid ~__context network)
          (host_uuid ~__context host) ;
      ...
      let set_default_locking_mode ~__context ~network ~value =
        info
          "Network.set_default_locking_mode: network = '%s'; value = %s"
          (network_uuid ~__context network)
      ...
      let attach_for_vm ~__context ~host ~vm =
        info
          "Network.attach_for_vm: host = '%s'; VM = '%s'"
          (host_uuid ~__context host)
          (vm_uuid ~__context vm) ;
      ...
      let detach_for_vm ~__context ~host ~vm =
        info
          "Network.detach_for_vm: host = '%s'; VM = '%s'"
          (host_uuid ~__context host)
          (vm_uuid ~__context vm) ;
      ...
      let add_purpose ~__context ~self ~value =
        info
          "Network.add_purpose: self = '%s'; value = '%s'"
      ...
      let remove_purpose ~__context ~self ~value =
        info
          "Network.remove_purpose: self = '%s'; value = '%s'"
          (network_uuid ~__context self)
          (Record_util.network_purpose_to_string value) ;
    ...
    module VIF = struct
      ...
      let unplug_common ~__context ~self ~force =
        ...
        let local_fn, remote_fn =
          if force
          then (Local.VIF.unplug_force, Client.VIF.unplug_force)
          else (Local.VIF.unplug, Client.VIF.unplug)
        in
      ...
      let set_locking_mode ~__context ~self ~value =
        info
          "VIF.set_locking_mode: VIF = '%s'; value = '%s'"
          (vif_uuid ~__context self)
      ...
      let set_ipv4_allowed ~__context ~self ~value =
        info
          "VIF.set_ipv4_allowed: VIF = '%s'; value = '%s'"
          (vif_uuid ~__context self)
          (String.concat "," value) ;
      ...
      let add_ipv4_allowed ~__context ~self ~value =
        info
          "VIF.add_ipv4_allowed: VIF = '%s'; value = '%s'"
          (vif_uuid ~__context self)
          value ;
      ...
      let remove_ipv4_allowed ~__context ~self ~value =
        info
          "VIF.remove_ipv4_allowed: VIF = '%s'; value = '%s'"
          (vif_uuid ~__context self)
          value ;
      ...
      let set_ipv6_allowed ~__context ~self ~value =
        info
          "VIF.set_ipv6_allowed: VIF = '%s'; value = '%s'"
          (vif_uuid ~__context self)
          (String.concat "," value) ;
      ...
      let add_ipv6_allowed ~__context ~self ~value =
        info
          "VIF.add_ipv6_allowed: VIF = '%s'; value = '%s'"
          (vif_uuid ~__context self)
          value ;
      ...
      let remove_ipv6_allowed ~__context ~self ~value =
        info
          "VIF.remove_ipv6_allowed: VIF = '%s'; value = '%s'"
          (vif_uuid ~__context self)
          value ;
    ...
    module VLAN = struct
      ...
      let create ~__context ~tagged_PIF ~tag ~network =
        info
          "VLAN.create: network = '%s'; VLAN tag = %Ld"
          (network_uuid ~__context network)
          tag ;
    ...
    module Bond = struct
      let create ~__context ~network ~members ~mAC ~mode ~properties =
        info
          "Bond.create: network = '%s'; members = [ %s ]"
          (network_uuid ~__context network)
      ...
      let destroy ~__context ~self =
        ...
        let success () =
          let progress = Db.Task.get_progress ~__context ~self:task in
          debug "Task progress %.1f" progress ;
          if progress = 1.0 then Some () else None
        in
      ...
      let set_property ~__context ~self ~name ~value =
        info
          "Bond.set_property: bond = '%s'; name = '%s'; value = '%s'"
          (bond_uuid ~__context self)
          name
          value ;
        let host =
    ...
    module PIF = struct
      ...
      let create_VLAN ~__context ~device ~network ~host ~vLAN =
        info
          "PIF.create_VLAN: network = '%s'; VLAN tag = %Ld"
          (network_uuid ~__context network)
          vLAN ;
      ...
      let destroy ~__context ~self =
        ...
        do_op_on
          ~local_fn
          ~__context
          ~host:(Db.PIF.get_host ~__context ~self)
      ...
      let reconfigure_ip ~__context ~self ~mode ~iP ~netmask ~gateway ~dNS =
        ...
        let success () =
          ...
          if status <> `pending then Some () else None
        in
        let fn () =
          do_op_on ~local_fn ~__context ~host (fun session_id rpc ->
              Client.PIF.reconfigure_ip
                rpc
                session_id
                self
                mode
      ...
      let set_primary_address_type ~__context ~self ~primary_address_type =
        ...
        do_op_on
          ...
          ~host:(Db.PIF.get_host ~__context ~self)
          (fun session_id rpc ->
            Client.PIF.set_primary_address_type
              rpc
              session_id
              self
      ...
      let set_property ~__context ~self ~name ~value =
        info
          "PIF.set_property: PIF = '%s'; name = '%s'; value = '%s'"
          (pif_uuid ~__context self)
          name
          value ;
        let host = Db.PIF.get_host ~__context ~self in
      ...
      let set_disallow_unplug ~__context ~self ~value =
        info
          "PIF.set_disallow_unplug: PIF uuid = %s; value = %s"
          (pif_uuid ~__context self)
          (string_of_bool value) ;
    ...
    module SR = struct
      ...
      let forward_sr_op
          ?(consider_unplugged_pbds = false) ~local_fn ~__context ~self op =
        let pbd =
      ...
      let forward_sr_multiple_op
          ~local_fn ~__context ~srs ?(prefer_slaves = false) op =
        let choose_fn ~host =
          Xapi_vm_helpers.assert_can_see_specified_SRs
            ~__context
            ~reqd_srs:srs
      ...
      let forward_sr_all_op
        ...
        | Ok pbds ->
            ...
            |> List.iter (fun host ->
                   try do_op_on ~local_fn ~__context ~host op with
      ...
      let introduce
          ~__context ~uuid ~name_label ~name_description ~_type ~content_type =
      ...
      let destroy ~__context ~sr =
        ...
        with_sr_marked ~__context ~sr ~doc:"SR.destroy" ~op:`destroy (fun () ->
            ...
            forward_sr_op
              ~consider_unplugged_pbds:true
              ~local_fn:local_destroy
              ~__context
              ~self:sr
              (fun session_id rpc -> Client.SR.destroy rpc session_id sr) ;
            forward_sr_all_op
              ~consider_unplugged_pbds:true
              ~local_fn:local_forget
              ~__context
              ~self:sr
      ...
      let set_name_label ~__context ~sr ~value =
        info
          "SR.set_name_label: SR = '%s' name-label = '%s'"
          (sr_uuid ~__context sr)
          value ;
      ...
      let set_name_description ~__context ~sr ~value =
        info
          "SR.set_name_description: SR = '%s' name-description = '%s'"
          (sr_uuid ~__context sr)
          value ;
      ...
      let assert_can_host_ha_statefile ~__context ~sr =
        info
          "SR.assert_can_host_ha_statefile: SR = '%s'"
          (sr_uuid ~__context sr) ;
      ...
      let assert_supports_database_replication ~__context ~sr =
        info
          "SR.assert_supports_database_replication: SR '%s'"
          (sr_uuid ~__context sr) ;
      ...
      let disable_database_replication ~__context ~sr =
        info
          "SR.disable_database_replication: SR = '%s'"
          (sr_uuid ~__context sr) ;
      ...
      let record_data_source ~__context ~sr ~data_source =
        info
          "SR.record_data_source: SR = '%s';  data source = '%s'"
          (sr_uuid ~__context sr)
          data_source ;
      ...
      let query_data_source ~__context ~sr ~data_source =
        info
          "SR.query_data_source: SR = '%s'; data source = '%s'"
          (sr_uuid ~__context sr)
          data_source ;
      ...
      let forget_data_source_archives ~__context ~sr ~data_source =
        info
          "SR.forget_data_source_archives: sr = '%s'; data source = '%s'"
          (sr_uuid ~__context sr)
          data_source ;
    ...
    module VDI = struct
      let unmark_vdi ~__context ~vdi ~doc ~op =
        ...
        log_exn_ignore
          ...
          (fun self ->
            if Db.is_valid_ref __context self
            then (
              Db.VDI.remove_from_current_operations
                ~__context
                ~self
                ~key:task_id ;
      ...
      let set_name_label ~__context ~self ~value =
        info
          "VDI.set_name_label: VDI = '%s' name-label = '%s'"
          (vdi_uuid ~__context self)
          value ;
      ...
      let set_name_description ~__context ~self ~value =
        info
          "VDI.set_name_description: VDI = '%s' name-description = '%s'"
          (vdi_uuid ~__context self)
          value ;
      ...
      let introduce
          ...
          ~snapshot_of =
        info
          "VDI.introduce: SR = '%s'; name label = '%s'"
          (sr_uuid ~__context sR)
          name_label ;
        let local_fn =
          Local.VDI.introduce
      ...
      let copy ~__context ~vdi ~sr ~base_vdi ~into_vdi =
        info
          "VDI.copy: VDI = '%s'; SR = '%s'; base_vdi = '%s'; into_vdi = '%s'"
      ...
      let pool_migrate ~__context ~vdi ~sr ~options =
        ...
        in
        info
          "VDI.pool_migrate: VDI = '%s'; SR = '%s'; VM = '%s'"
          (vdi_uuid ~__context vdi)
          (sr_uuid ~__context sr)
          (vm_uuid ~__context vm) ;
        VM.with_vm_operation
          ~__context
          ~self:vm
          ~doc:"VDI.pool_migrate"
          ~op:`migrate_send
          (fun () ->
            let snapshot, host =
              if Xapi_vm_lifecycle_helpers.is_live ~__context ~self:vm
      ...
      let generate_config ~__context ~host ~vdi =
        info
          "VDI.generate_config: VDI = '%s'; host = '%s'"
          (vdi_uuid ~__context vdi)
          (host_uuid ~__context host) ;
      ...
      let checksum ~__context ~self =
        VM.forward_to_access_srs_and
          ~local_fn:(Local.VDI.checksum ~self)
          ~__context
      ...
      let set_cbt_enabled ~__context ~self ~value =
        info
          "VDI.set_cbt_enabled: VDI = '%s'; value = '%b'"
          (vdi_uuid ~__context self)
          value ;
        let sr = Db.VDI.get_SR ~__context ~self in
        Sm.assert_session_has_internal_sr_access ~__context ~sr ;
      ...
      let list_changed_blocks ~__context ~vdi_from ~vdi_to =
        info
          "VDI.list_changed_blocks: vdi_from  = '%s'; vdi_to = '%s'"
        ...
        with_sr_andor_vdi
          ...
          (fun () ->
            forward_vdi_op
              ...
              ~self:vdi_to
              (fun session_id rpc ->
                Client.VDI.list_changed_blocks
                  ~rpc
                  ~session_id
                  ~vdi_from
                  ~vdi_to ) )
    ...
    module VBD = struct
      ...
      let insert ~__context ~vbd ~vdi =
        info
          "VBD.insert: VBD = '%s'; VDI = '%s'"
          (vbd_uuid ~__context vbd)
          (vdi_uuid ~__context vdi) ;
      ...
      let unplug_force_no_safety_check ~__context ~self =
        warn
          "VBD.unplug_force_no_safety_check: VBD = '%s'"
          (vbd_uuid ~__context self) ;
    ...
    module PBD = struct
      ...
      let sanitize (k, v) =
        if Astring.String.is_suffix ~affix:"transformed" k
        then k ^ "=undisclosed"
        else k ^ "=" ^ v
      ...
      let set_device_config ~__context ~self ~value =
        ...
        info
          "PBD.set_device_config: PBD = '%s'; device_config = [ %s ]"
      ...
      let plug ~__context ~self =
        ...
        then
          Server_helpers.exec_with_new_task
            "PBD.plug initial SR scan"
            (fun __context ->
              let should_handle_metadata_vdis = is_shared_sr in
              if should_handle_metadata_vdis
              then Xapi_dr.signal_sr_is_processing ~__context ~sr ;
    ...
    module GPU_group = struct
      ...
      let update_enabled_VGPU_types ~__context ~self =
        info
          "GPU_group.update_enabled_VGPU_types: gpu_group = '%s'"
          (gpu_group_uuid ~__context self) ;
      ...
      let update_supported_VGPU_types ~__context ~self =
        info
          "GPU_group.update_supported_VGPU_types: gpu_group = '%s'"
          (gpu_group_uuid ~__context self) ;
    ...
    module VGPU = struct
      ...
      let atomic_set_resident_on ~__context ~self ~value =
        info
          "VGPU.atomic_set_resident_on: VGPU = '%s'; PGPU = '%s'"
    ...
    module Pool_update = struct
      ...
      let pool_apply ~__context ~self =
        info
          "Pool_update.pool_apply: pool update = '%s'"
      ...
      let destroy ~__context ~self =
        info
          "Pool_update.destroy: pool update = '%s'"
          (pool_update_uuid ~__context self) ;
      ...
      let attach ~__context ~self ~use_localhost_proxy =
        info
          "Pool_update.attach: pool update = '%s'"
          (pool_update_uuid ~__context self) ;
      ...
      let detach ~__context ~self =
        info
          "Pool_update.detach: pool update = '%s''"
          (pool_update_uuid ~__context self) ;
    ...
    module SDN_controller = struct
      let introduce ~__context ~protocol ~address ~port =
        info
          "SDN_controller.introduce: protocol='%s', address='%s', port='%Ld'"
      ...
      let forget ~__context ~self =
        info
          "SDN_controller.forget: sdn_controller = '%s'"
      ...
      let unmark_vusb ~__context ~vusb ~doc ~op =
        ...
        log_exn
          ...
          (fun self ->
            if Db.is_valid_ref __context self
            then (
              Db.VUSB.remove_from_current_operations
                ~__context
                ~self
                ~key:task_id ;
      ...
      let mark_vusb ~__context ~vusb ~doc ~op =
        ...
        log_exn
          ...
          (fun self ->
            ...
            Db.VUSB.add_to_current_operations
              ~__context
              ~self
              ~key:task_id
              ~value:op ;
    ...
    module Network_sriov = struct
      let create ~__context ~pif ~network =
        info
          "Network_sriov.create : pif = '%s' , network = '%s' "
          (pif_uuid ~__context pif)
          (network_uuid ~__context network) ;
      ...
      let destroy ~__context ~self =
        info
          "Network_sriov.destroy : network_sriov = '%s'"
      ...
      let get_remaining_capacity ~__context ~self =
        info
          "Network_sriov.get_remaining_capacity : network_sriov = '%s'"
          (network_sriov_uuid ~__context self) ;
    ...
    module Cluster = struct
          ...
          (fun () ->
            let cluster =
              Local.Cluster.create
                ...
                ~token_timeout_coefficient
            in
            Xapi_cluster_helpers.update_allowed_operations
              ~__context
              ~self:cluster ;
            cluster )
      ...
      let destroy ~__context ~self =
        ...
        Xapi_cluster_helpers.with_cluster_operation
          ~__context
          ~self
          ~doc:"Cluster.destroy"
          ~op:`destroy
    ...
    module Cluster_host = struct
      let create ~__context ~cluster ~host ~pif =
        info
          "Cluster_host.create with cluster:%s, host:%s, pif:%s"
        ...
        Xapi_cluster_helpers.with_cluster_operation
          ...
          (fun () ->
            ...
            in
            Xapi_cluster_host_helpers.update_allowed_operations
              ~__context
              ~self:cluster_host ;
            cluster_host )
      ...
      let force_destroy ~__context ~self =
        ...
        Xapi_cluster_helpers.with_cluster_operation
          ~__context
          ~self:cluster
          ~doc:"Cluster_host.force_destroy"
          ~op:`remove
          (fun () ->
            do_op_on ~__context ~local_fn ~host (fun session_id rpc ->
      ...
      let enable ~__context ~self =
        ...
        Xapi_cluster_helpers.with_cluster_operation
          ...
          ~op:`enable
          (fun () ->
            Xapi_cluster_host_helpers.with_cluster_host_operation
              ~__context
              ~self
              ~doc:"Cluster_host.enable"
              ~op:`enable
              (fun () ->
                do_op_on ~__context ~local_fn ~host (fun session_id rpc ->
