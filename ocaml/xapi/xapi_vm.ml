...
let create ~__context ~name_label ~name_description ~power_state ~user_version
    ...
    ~blocked_operations:_ ~snapshot_schedule:_ ~is_vmss_snapshot:_ ~appliance
  ...
  *)
  ...
  Db.VM_metrics.create ~__context ~ref:metrics ~uuid:metrics_uuid
    ...
    ~last_updated:Date.epoch ~other_config:[] ~hvm:false ~nested_virt:false
  ...
  Db.VM.create ~__context ~ref:vm_ref ~uuid:(Uuidx.to_string uuid)
    ...
    ~ha_always_run ~tags ~bios_strings:[] ~snapshot_schedule:Ref.null
