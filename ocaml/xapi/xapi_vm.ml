      ...
      List.iter
        (fun vbd ->
          ...
          try
            ...
            then
              Xapi_vdi.update_allowed_operations
                ~__context
...
let assert_can_boot_here ~__context ~self ~host =
  ...
  then Helpers.assert_platform_version_is_same_on_master ~__context ~host ~self ;
...
let retrieve_wlb_recommendations ~__context ~vm =
  let compat_string_of_float f =
    ...
    if Astring.String.is_suffix ~affix:"." repr then repr ^ "0" else repr
  in
...
let validate_restart_priority priority =
  if not (List.mem priority Constants.ha_valid_restart_priorities)
  then
    raise
...
let set_ha_always_run ~__context ~self ~value =
  if value
  then set_ha_restart_priority ~__context ~self ~value:Constants.ha_restart
  else set_ha_restart_priority ~__context ~self ~value:""
...
let set_memory_static_range ~__context ~self ~min ~max =
  ...
  let constraints =
    { constraints with
      Vm_memory_constraints.static_min = min
    ; static_max = max
    }
  in
  ...
  if Helpers.is_domain_zero ~__context self
  then set_dom0_memory ~__context ~self ~bytes:max ;
  (* It is allowed to update the memory settings of a running control domain,
     	 ...
     	 * the client to do so. *)
  if is_control_domain && power_state = `Running
  then Db.VM.set_requires_reboot ~__context ~self ~value:true
...
let set_memory_limits
  ...
  update_memory_overhead ~__context ~vm:self ;
  if Helpers.is_domain_zero ~__context self
  then set_dom0_memory ~__context ~self ~bytes:static_max ;
  (* It is allowed to update the memory settings of a running control domain,
     	 * but it needs to be rebooted for the changes to take effect. We signal
     	 * the client to do so. *)
  if is_control_domain && power_state = `Running
...
let assert_memory_constraints ~__context ~vm platformdata =
  ...
  if List.exists (is_sriov_vgpu ~__context) vgpus
  then
...
let save_uefi_certificates_to_dir ~__context ~pool ~vm =
  ...
  if not (Sys.file_exists uefi_key)
  then
    if Sys.file_exists !Xapi_globs.varstore_dir
...
let start ~__context ~vm ~start_paused ~force =
  ...
  then (
    ...
    debug
      "Setting ha_always_run on vm=%s as true during VM.start"
      (Ref.string_of vm) ) ;
  let pool = Helpers.get_pool ~__context in
  let default_value =
  ...
  let platform =
    ...
    |> Xapi_vm_helpers.ensure_device_model_profile_present
         ~__context
         ~domain_type:vmr.API.vM_domain_type
         ~default_value
         ~is_a_template:false
    |> update_platform_secureboot ~__context ~self:pool
  in
  let vmr =
    if platform <> vmr.API.vM_platform
    then (
  ...
  if vusbs <> []
  then Vm_platform.check_restricted_device_model ~__context vmr.API.vM_platform ;
  let sriov_networks =
    Xapi_network_sriov_helpers.get_sriov_networks_from_vm __context vm
  in
  if sriov_networks <> []
  then Pool_features.assert_enabled ~__context ~f:Features.Network_sriov ;
  ...
  | _ ->
      Xapi_gpumon.with_gpumon_stopped
        ~timeout:!Xapi_globs.gpumon_stop_timeout
...
let hard_shutdown ~__context ~vm =
  ...
  debug
    "Setting ha_always_run on vm=%s as false during VM.hard_shutdown"
    (Ref.string_of vm) ;
...
let clean_shutdown_with_timeout ~__context ~vm timeout =
  Db.VM.set_ha_always_run ~__context ~self:vm ~value:false ;
  debug
    "Setting ha_always_run on vm=%s as false during VM.clean_shutdown"
    (Ref.string_of vm) ;
...
let power_state_reset ~__context ~vm =
  ...
  then (
    error
      "VM.power_state_reset vm=%s blocked because VM is a control domain"
      (Ref.string_of vm) ;
    raise
      (Api_errors.Server_error
    ...
    then (
      ...
      let running =
        try
          ...
          if s.Vm.power_state = Running
          then (
            debug
              "VM.power_state_reset vm=%s xenopsd reports running;"
              (Ref.string_of vm) ;
            true )
          else (
      ...
      then
        raise
          (Api_errors.Server_error
             (Api_errors.domain_exists, [ Ref.string_of vm ]) ) )
    else (
...
let suspend ~__context ~vm =
  Db.VM.set_ha_always_run ~__context ~self:vm ~value:false ;
  debug
    "Setting ha_always_run on vm=%s as false during VM.suspend"
    (Ref.string_of vm) ;
  ...
  |> List.iter (fun (vgpu, pgpu) ->
         debug
           "VM.suspend checking vgpu/pgpu support for suspend: %s/%s"
           (Ref.string_of vgpu)
           (Ref.string_of pgpu) ;
         try
...
let resume ~__context ~vm ~start_paused ~force =
  ...
  then (
    Db.VM.set_ha_always_run ~__context ~self:vm ~value:true ;
    debug
      "Setting ha_always_run on vm=%s as true during VM.resume"
      (Ref.string_of vm) ) ;
  let host = Helpers.get_localhost ~__context in
  if not force
...
let create
  ...
  *)
  ...
  Db.VM_metrics.create
    ...
    ~current_domain_type ;
  let domain_type =
    if domain_type = `unspecified
    then derive_domain_type ~hVM_boot_policy
    else domain_type
  in
  ...
  Db.VM.create
    ...
    ~last_booted_record:_last_booted_record
    ~xenstore_data
    ~recommendations
    ~blobs:[]
    ~ha_restart_priority
...
let clone ~__context ~vm ~new_name =
  ...
  then hard_shutdown ~__context ~vm:new_vm ;
  new_vm
  ...
  Helpers.call_api_functions ~__context (fun rpc session_id ->
      ...
      then (
        ...
        try
          ...
          Xapi_templates_install.post_install_script
            rpc
            session_id
            __context
            dom0
        ...
        | e ->
            ...
            List.iter
              (Helpers.log_exn_continue
                 "deleting auto-provisioned VBD"
                 (fun self -> Client.VBD.destroy rpc session_id self) )
              vbds ;
            List.iter
              (Helpers.log_exn_continue
                 "deleting auto-provisioned VDI"
...
let set_VCPUs_max ~__context ~self ~value =
  if Helpers.is_domain_zero ~__context self
  then failwith "set_VCPUs_max is not allowed on dom0" ;
  let is_control_domain = Db.VM.get_is_control_domain ~__context ~self in
  let power_state = Db.VM.get_power_state ~__context ~self in
  ...
  if value < 1L || value < vcpus_at_startup
  then
    invalid_value
...
let set_memory_dynamic_range ~__context ~self ~min ~max =
  ...
  let constraints =
    ...
    else Vm_memory_constraints.get ~__context ~vm_ref:self
  in
  let constraints =
    { constraints with
...
let request_rdp ~__context ~vm ~enabled =
  ...
  let vm_gmr =
    try
      Some (Db.VM_guest_metrics.get_record_internal ~__context ~self:vm_gm)
    with
    | _ ->
        None
  in
  ...
  then Xapi_xenops.request_rdp ~__context ~self:vm enabled
...
let record_call_plugin_latest vm =
  ...
  Xapi_stdext_threads.Threadext.Mutex.execute call_plugin_latest_m (fun () ->
      ...
      let to_wait =
        if Hashtbl.mem call_plugin_latest vm
        then
      ...
      else Hashtbl.replace call_plugin_latest vm now )


(* this is the generic plugin call available to xapi users *)
let call_plugin ~__context ~vm ~plugin ~fn ~args =
  if plugin <> "guest-agent-operation"
  then
    raise
      (Api_errors.Server_error (Api_errors.xenapi_missing_plugin, [ plugin ])) ;
  ...
  try
    ...
    | _ ->
        let msg =
          Printf.sprintf
            "The requested fn \"%s\" could not be found in plugin \"%s\"."
            fn
            plugin
        in
        raise
...
let send_trigger ~__context ~vm ~trigger =
  raise
    (Api_errors.Server_error (Api_errors.not_implemented, [ "send_trigger" ]))
...
let maximise_memory ~__context ~self ~total ~approximate =
  ...
  let r =
    { r with
      API.vM_VCPUs_max = (if approximate then 64L else r.API.vM_VCPUs_max)
    }
  in
  ...
  let will_fit static_max =
    ...
    let normal, shadow =
      Memory_check.vm_compute_start_memory
        ~__context
        ~policy:Memory_check.Static_max
        r
    in
...
let set_bios_strings ~__context ~self ~value =
  ...
  if Db.VM.get_bios_strings ~__context ~self <> []
  then
...
let set_snapshot_schedule ~__context ~self ~value =
  ...
  if value <> Ref.null && is_vmss_valid_ref
  then (
    ...
    then
      ...
      raise
        (Api_errors.Server_error
           (Api_errors.invalid_value, [ Ref.string_of value ]) ) ;
    if Db.VM.get_is_a_template ~__context ~self
    then
    ...
    if snapshot_type = `snapshot_with_quiesce
    then (
...
let set_start_delay ~__context ~self ~value =
  if value < 0L
  then invalid_value "start_delay must be non-negative" (Int64.to_string value) ;
...
let set_order ~__context ~self ~value =
  if value < 0L
  then invalid_value "order must be non-negative" (Int64.to_string value) ;
...
let set_suspend_VDI ~__context ~self ~value =
  Xapi_vm_lifecycle.assert_initial_power_state_is
    ...
    ~self
    ~expected:`Suspended ;
  if Db.VDI.get_type ~__context ~self:value = `cbt_metadata
  then (
    error
      "VM.set_suspend_VDI: the given VDI has type cbt_metadata (at %s)"
      __LOC__ ;
    raise
      Api_errors.(
        Server_error
  ...
  then (
    ...
    then Db.VM.set_suspend_VDI ~__context ~self ~value
    else
      raise
        (Api_errors.Server_error
...
let set_appliance ~__context ~self ~value =
  ...
  if Db.is_valid_ref __context previous_value
  then
  ...
  then
    ...
    try
      Open_uri.with_open_uri uri (fun fd ->
          let module Request =
            Cohttp.Request.Make (Cohttp_posix_io.Unbuffered_IO)
          in
          let module Response =
            Cohttp.Response.Make (Cohttp_posix_io.Unbuffered_IO)
          in
...
let set_domain_type ~__context ~self ~value =
  ...
  then invalid_value "domain_type" (Record_util.domain_type_to_string value) ;
  Db.VM.set_domain_type ~__context ~self ~value ;
  Db.VM.set_HVM_boot_policy
    ~__context
    ~self
