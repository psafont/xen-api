      ...
      let create_session () =
        ...
        let session =
          Xapi_session.login_no_password
            ...
            ~subject:Ref.null
            ~auth_user_sid:""
            ~auth_user_name:sm_username
            ~rbac_permissions:[]
        in
        (* Give this session access to this particular SR *)
        Option.iter
          (fun sr ->
            Db.Session.add_to_other_config
              ~__context
              ~self:session
              ~key:Xapi_globs._sm_session
              ~value:(Ref.string_of sr) )
          sr ;
        session
      in
    ...
    let xml, stderr =
      Stats.time_this name (fun () ->
          ...
          try
            ...
            in
            try (Xml.parse_string output, stderr) with
            | e ->
                error
...
let rethrow_parse_failures xml f =
  ...
  | XMLRPC.RunTimeTypeError (s, x) ->
      xmlrpc_parse_failure
        xml
        (Printf.sprintf
           "XMLRPC unmarshall RunTimeTypeError: looking for %s found %s"
           s
           (Xml.to_string_fmt x) )
  | e ->
...
let parse_attach_result (xml : Xml.xml) =
  rethrow_parse_failures (Xml.to_string_fmt xml) (fun () ->
      ...
      in
      let o_direct_reason =
        try XMLRPC.From.string (safe_assoc "o_direct_reason" info) with
        | _ ->
            ""
      in
      let xenstore_data =
...
let get_supported add_fn =
  let check_driver entry =
    if Astring.String.is_suffix ~affix:"SR" entry
    then
      ...
      then
        ...
        | e ->
            error
              "Rejecting SM plugin: %s because of exception: %s (executable)"
              driver
              (Printexc.to_string e)
  in
  List.iter
    (fun (f, dir) ->
      ...
      then (
        ...
        try Array.iter f (Sys.readdir dir) with
        | e ->
            error
              "Error checking directory %s for SM backends: %s"
              dir
              (ExnHelper.string_of_exn e) )
      else error "Not scanning %s for SM backends: directory does not exist" dir
      )
