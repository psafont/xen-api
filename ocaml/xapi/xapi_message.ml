...
let of_xml input =
  ...
  let rec f () =
    ...
    | `Data dat ->
        ...
        | "priority" ->
            message :=
              { !message with API.message_priority = Int64.of_string dat }
        | "cls" ->
            message :=
              { !message with
                API.message_cls = Record_util.string_to_class dat
              }
        | "obj_uuid" ->
            message := { !message with API.message_obj_uuid = dat }
        | "timestamp" ->
            message :=
              { !message with API.message_timestamp = Date.of_string dat }
        | "uuid" ->
...
let handle_message ~__context message =
  try
    ...
    else if Sys.file_exists !Xapi_globs.xapi_message_script
    then
      let output, log =
    ...
    else info "%s not found, skipping" !Xapi_globs.xapi_message_script
  with
  | e ->
      error
        "Unexpected exception in message hook %s: %s"
        !Xapi_globs.xapi_message_script
    ...
    try
      Mutex.execute event_mutex (fun () ->
          let fd, basefilename, filename =
            ...
            let rec doit n =
              if n > 10
              then failwith "Couldn't create a file"
              else
...
let create ~__context ~name ~priority ~cls ~obj_uuid ~body =
  ...
  let priority =
    ...
    then List.assoc name !Api_messages.msgList
    else priority
  in
  let message =
...
let destroy ~__context ~self =
  ...
  let fullpath =
    try Unix.readlink symlinkfname with
    | _ ->
        let allfiles =
          List.map
  ...
  then
    let allmsg =
      List.filter_map
        ...
        (Array.to_list (Sys.readdir message_dir))
    in
    if List.length allmsg > Xapi_globs.message_limit
    then (
      ...
      let rec reap_one i msgs =
        ...
        then ()
          ...
          ( try destroy_real __context (snd (List.hd msgs)) with
          | e ->
...
let get ~__context ~cls ~obj_uuid ~since =
  ...
  if not (check_uuid ~__context ~cls ~uuid:obj_uuid)
  then raise (Api_errors.Server_error (Api_errors.uuid_invalid, [])) ;
  let msg =
    get_real_inner
      class_symlink
        ...
        | (last_in_memory, _, _) :: _ when last_in_memory < since ->
            Some
              (List.filter_map
                 (fun (gen, _ref, msg) ->
                   if gen > since
                   then Some (gen, Xapi_event.Message.Create (_ref, msg))
                   else None )
                 !in_memory_cache )
...
let get_by_uuid ~__context ~uuid =
  ...
  | _ ->
      raise
        (Api_errors.Server_error (Api_errors.uuid_invalid, [ "message"; uuid ]))
...
let repopulate_cache () =
  Mutex.execute in_memory_cache_mutex (fun () ->
      ...
      in
      debug
        "Constructing in-memory-cache: most length=%d"
...
let handler (req : Http.Request.t) fd _ =
  ...
  let check_query param =
    if not (List.mem_assoc param query)
    then (
      error
        "Xapi_message.handler: HTTP request for message lacked %s parameter"
        param ;
    ...
    (fun __context ->
      try
        ...
        then
          ...
          let cls =
            try Record_util.string_to_class cls with
            | _ ->
                failwith ("Xapi_message.handler: Bad class " ^ cls)
          in
          if not (check_uuid ~__context ~cls ~uuid)
          then failwith ("Xapi_message.handler: Bad uuid " ^ uuid) ;
      ...
      | e ->
          error
            "Xapi_message.handler: caught exception '%s'"
            (ExnHelper.string_of_exn e) )
