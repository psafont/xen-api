    ...
    (fun () ->
      ignore
        (Server_helpers.exec_with_new_task
           ...
           (fun __context ->
             Extauth.call_extauth_hook_script_in_pool
               ~__context
...
let asynchronously_run_hook_script_after_subject_add =
  At_least_once_more.make
    "running after-subject-add hook script"
    run_hook_script_after_subject_add
...
let create ~__context ~subject_identifier ~other_config =
  ...
  in
  if List.exists (fun x -> x = Xapi_globs.auth_type_AD) auth_types
...
let asynchronously_run_hook_script_after_subject_remove =
  At_least_once_more.make
    "running after-subject-remove hook script"
    run_hook_script_after_subject_remove
...
let update ~__context ~self =
  ...
  let subject_info =
    Xapi_auth.get_subject_information_from_identifier
      ...
      ~subject_identifier
  in
  if Db.Subject.get_other_config ~__context ~self <> subject_info
  then (
    ...
    let need_update_config =
      List.fold_left
        (fun updated subj ->
          ...
          try update ~__context ~self:subj || updated with
          | e ->
              debug
                "Error trying to update subject %s: %s"
...
let remove_from_roles ~__context ~self ~role =
  ...
  then (
    ...
    debug
      "subject %s does not have role %s"
      (Db.Subject.get_subject_identifier ~__context ~self)
      (Ref.string_of role) ;
    raise (Api_errors.Server_error (Api_errors.role_not_found, [])) )
...
let get_subject_information_from_identifier ~__context ~cache identifier =
  let open Extauth in
  if cache
  then query_subject_information_from_db ~__context identifier
  else (Ext_auth.d ()).query_subject_information identifier
