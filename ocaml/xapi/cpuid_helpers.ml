...
let features_of_string str =
  ...
  with
  | _ ->
      raise (InvalidFeatureString str)
...
let upgrade_features ~__context ~vm host_features vm_features =
  ...
  let upgraded_features =
    ...
    else zero_extend vm_features (Array.length host_features)
  in
  if vm_features <> upgraded_features
  then
    debug
      "VM featureset upgraded from %s to %s"
...
let update_cpu_flags ~__context ~vm ~host =
  ...
  try
    ...
    then set_flags ~__context vm host_vendor new_features
  with
  | Not_found ->
      (* pre-Dundee? *)
...
let assert_vm_is_compatible ~__context ~vm ~host ?remote () =
  ...
  then
    try
      ...
      if List.mem_assoc cpu_info_vendor_key vm_cpu_info
      then (
        ...
        debug
          "VM last booted on CPU of vendor %s; host CPUs are of vendor %s"
          vm_cpu_vendor
          host_cpu_vendor ;
        if vm_cpu_vendor <> host_cpu_vendor
        then
          fail
      ...
      then (
        ...
        in
        if not (is_subset vm_cpu_features' host_cpu_features')
        then (
          debug
            ...
            (string_of_features host_cpu_features') ;
          debug
            "Host is missing these CPU features required by the VM: %s"
