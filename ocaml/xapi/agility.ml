  ...
  then false
    ...
    let enabled_hosts =
      List.filter
        ...
        all_hosts
    in
    if not (Listext.subset enabled_hosts plugged_hosts)
    then (
      warn
...
let is_network_properly_shared ~__context ~self =
  ...
  if not properly_shared
  then
    warn
      "Network %s not shared properly: Not all hosts have PIFs"
      (Ref.string_of self) ;
  properly_shared
...
let caching_vm_t_assert_agile ~__context (ok_srs, ok_networks) vm vm_t =
  ...
  if vm_t.API.vM_VGPUs <> []
  then raise Api_errors.(Server_error (vm_has_vgpu, [ Ref.string_of vm ])) ;
  (* Any kind of VUSB means that the VM is not agile. *)
  if vm_t.API.vM_VUSBs <> []
  then raise Api_errors.(Server_error (vm_has_vusbs, [ Ref.string_of vm ])) ;
  ...
  let check_vbd ok_srs vbd =
    ...
    then ok_srs
      ...
      then ok_srs
      else if not (is_sr_properly_shared ~__context ~self:sr)
      then
        raise
          Api_errors.(
            Server_error
              (ha_constraint_violation_sr_not_shared, [ Ref.string_of sr ]))
      else SRSet.add sr ok_srs
  in
  ...
  let check_vif ok_networks vif =
    ...
    then ok_networks
    else if not (is_network_properly_shared ~__context ~self:network)
    then
      raise
        Api_errors.(
          Server_error
...
let partition_vm_ps_by_agile ~__context vm_ps =
  let distinguish_vm (agile_vm_ps, not_agile_vm_ps, cache) ((vm, vm_t) as vm_p)
    ...
    with
    | _ ->
        (agile_vm_ps, vm_p :: not_agile_vm_ps, cache)
  in
