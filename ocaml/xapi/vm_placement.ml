...
let drop n list =
  ...
  let rec drop n list = if n = 0 then list else drop (n - 1) (List.tl list) in
  drop n list
...
let take n list =
  ...
  let rec take n list acc =
    if n = 0
    then List.rev acc
    else take (n - 1) (List.tl list) (List.hd list :: acc)
  in
  take n list []
...
let take_nth n list =
  ...
  let rec take_nth n list1 list2 =
    if n = 0
    then (List.hd list2, List.rev list1 @ List.tl list2)
...
let affinity_host_category affinity_host_ids : host_category =
 fun host ->
  ...
  then possible_host_category host
  else -1L


(* === Selection functions ================================================== *)

let select_host_from_category
    (category : host_category) hosts validate_host generate_random_value =
...
let select_host_from_categories
    categories hosts validate_host generate_random_value =
...
let select_host_from_summary
    pool affinity_host_ids validate_host generate_random_value =
