...
(* write info to both info and debug log *)

module L = Debug.Make (struct
  let name = "license"
end)

module W = Debug.Make (struct
  let name = "watchdog"
end)
...
let populate_db backend =
  ...
  in
  debug
    "Attempting to populate database from one of these locations: [%s]"
    (String.concat
       "; "
...
let register_callback_fns () =
  ...
  let stunnel_destination_is_ok addr =
    Server_helpers.exec_with_new_task
      "check_stunnel_destination"
      (fun __context ->
        let hosts =
          Db.Host.get_refs_where
            ~__context
...
let check_no_other_masters () =
  Server_helpers.exec_with_new_task
    "checking no other known hosts are masters"
    (fun __context ->
      let assert_is_slave href =
        ...
        | e ->
            ...
            debug
              "Couldn't contact slave on startup check: %s"
              (Printexc.to_string e)
      in
...
let on_master_restart ~__context =
  ...
  Helpers.log_exn_continue
    "sending an immediate heartbeat"
    (fun () ->
  ...
  let am_i_missing_certs () =
    ...
    try
      if Db.Pool.get_tls_verification_enabled
           ~__context
    ...
    | e ->
        D.error
          "am_i_missing_certs: exception (ignoring): %s"
          (Printexc.to_string e)
  in
  am_i_missing_certs ()
...
let init_local_database () =
  ...
  then Localdb.put Constants.master_scripts "false" ;
  (* We've just rebooted, so we clear the flag that stops the host being disabled during the reboot *)
  if !Xapi_globs.on_system_boot
  then Localdb.put Constants.host_disabled_until_reboot "false"
...
let bring_up_management_if ~__context () =
  ...
  | e ->
      debug
        "Caught exception bringing up management interface: %s"
        (ExnHelper.string_of_exn e)
...
let wait_for_management_ip_address ~__context =
  ...
  | Pool_role.Slave masters_ip ->
      if masters_ip = "127.0.0.1" || masters_ip = ip
      then (
...
let attempt_pool_hello my_ip =
  ...
  | Api_errors.Server_error (code, params) as exn ->
      debug
        "Caught exception: %s during Pool.hello"
...
let start_ha () =
  try Xapi_ha.on_server_restart () with
  | e ->
      (* Critical that we don't continue as a master and use shared resources *)
      debug
        "Caught exception starting HA system: %s"
        (ExnHelper.string_of_exn e)
...
let start_redo_log () =
  try
    ...
    then (
      ...
      Redo_log_usage.read_from_redo_log
        Xapi_ha.ha_redo_log
        Db_globs.gen_metadata_db
        db_ref )
  with
  | e ->
      debug
        "Caught exception starting non-HA redo log: %s"
        (ExnHelper.string_of_exn e)
...
let resynchronise_ha_state () =
  ...
  | e ->
      (* Critical that we don't continue as a master and use shared resources *)
      error
        "Caught exception resynchronising state of HA system: %s"
        (ExnHelper.string_of_exn e)
...
let check_network_reset () =
  try
    ...
    Server_helpers.exec_with_new_task
      "Performing emergency network reset"
      (fun __context ->
        let host = Helpers.get_localhost ~__context in
        (* Parse reset file *)
        ...
        let create_vlan pif vlan =
          let network =
            ...
            | None ->
                let name_label =
                  Printf.sprintf
                    "Pool-wide network associated with %s on VLAN%s"
                    device
                    vlan
                in
                Xapi_network.create
...
let server_init () =
  ...
  let call_extauth_hook_script_after_xapi_initialize ~__context =
    ...
    then
      try
        ...
        ignore
          (Extauth.call_extauth_hook_script_in_host
             ~__context
             host
    ...
    then (
      ...
      while not !Xapi_globs.event_hook_auth_on_xapi_initialize_succeeded do
        try
          ...
          try call_extauth_hook_script_after_xapi_initialize ~__context with
          | e ->
              ()
          (* CP-709 *)
        with
        | e ->
            ...
            debug
              "Failed initializing external authentication system \
               auth_type=%s, service_name=%s: %s"
              auth_type
              service_name
              (ExnHelper.string_of_exn e) ;
            last_error := Some e ;
      ...
      (fun __context ->
        Startup.run
          ...
          ; ( "updating the vswitch controller"
            , []
            , fun () ->
                Helpers.update_vswitch_controller
                  ~__context
          ...
          ; ( "listening to events from xapi"
            ...
            , fun () ->
                if not !noevents
                then ignore (Thread.create Xapi_xenops.events_from_xapi ()) )
        ...
        let maybe_wait_for_clustering_ip () =
          ...
          | Some self ->
              ...
              in
              debug
                "Got clustering IP %s, resyncing cluster_host %s"
                ip
                (Ref.string_of self) ;
          ...
          ; (* CA-290237, CA-290473: Create cluster objects after network objects and management IP initialised *)
            ...
            , fun () ->
                log_and_ignore_exn (fun () ->
                    Xapi_cluster_host.create_as_necessary
                      __context
  ...
  | Unix.Unix_error (e, s1, s2) as exn ->
      ...
      debug
        "xapi top-level caught Unix_error: %s, %s, %s"
