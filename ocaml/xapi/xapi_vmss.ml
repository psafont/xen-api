...
let create
    ...
    ~retained_snapshots
    ~frequency
    ~schedule : API.ref_VMSS =
  assert_licensed ~__context ;
  if _type = `snapshot_with_quiesce
  then Pool_features.assert_enabled ~__context ~f:Features.VSS ;
...
let destroy ~__context ~self =
  ...
  then
    (* we can't delete a VMSS that contains VMs *)
...
let show_task_in_xencenter ~__context ~vm =
  ...
  then
    ...
    | e ->
        debug
          "Error adding other_config:show_in_xencenter to task %s: %s"
          (Ref.string_of task)
