...
let get_remaining_capacity_internal
    ~__context ~self ~vgpu_type ~pre_allocate_list =
  try
...
let assert_destination_pgpu_is_compatible_with_vm
    ~__context ~vm ~vgpu ~pgpu ~host ?remote () =
  ...
  let test_nvidia_compatibility vgpu pgpu =
    let pgpu_metadata =
      try
        get_compatibility_metadata pgpu |> List.assoc Xapi_gpumon.Nvidia.key
      with
      | Not_found ->
          debug
            "Key %s is missing from the compatibility_metadata for pgpu %s on \
             the host %s."
            Xapi_gpumon.Nvidia.key
            (Ref.string_of pgpu)
            (Ref.string_of host) ;
          raise
            Api_errors.(
    ...
    in
    Xapi_gpumon.Nvidia.assert_pgpu_is_compatible_with_vm
      ~__context
      ~vm
      ~vgpu
      ~dest_host:host
      ~encoded_pgpu_metadata:pgpu_metadata
  in
...
let assert_destination_has_pgpu_compatible_with_vm
    ~__context ~vm ~vgpu_map ~host ?remote () =
  let module XenAPI = Client.Client in
  ...
  let rec get_first_suitable_pgpu pgpu_types vgpu = function
    | [] ->
        debug
          "No compatible pGPU found on host %s, needed by VM %s"
          (Ref.string_of host)
          (Ref.string_of vm) ;
  ...
  let test_compatibility vgpu pgpus =
    ...
    | `nvidia | `nvidia_sriov ->
        ...
        |> fun self ->
        Db.GPU_group.get_GPU_types ~__context ~self
        |> fun pgpu_types ->
        get_first_suitable_pgpu pgpu_types vgpu pgpus
        |> fun pgpu ->
        assert_destination_pgpu_is_compatible_with_vm
          ~__context
          ~vm
          ~vgpu
          ~pgpu
