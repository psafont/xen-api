...
let plug ~__context ~self =
  ...
  if System_domains.storage_driver_domain_of_vbd ~__context ~vbd:self = vm
     && not force_loopback_vbd
  then (
    debug "VBD.plug of loopback VBD '%s'" (Ref.string_of self) ;
    Storage_access.attach_and_activate
      ~__context
      ~vbd:self
      ~domid
      (fun attach_info ->
        ...
        let device_path =
          ...
          | _, _, nbd :: _ ->
              debug
                "Using nbd-client for VBD.plug of VBD '%s'"
                (Ref.string_of self) ;
    ...
    if hvm && dev_type <> `CD && mode = `RO
    then
      raise
        (Api_errors.Server_error
           ( Api_errors.disk_vbd_must_be_readwrite_for_hvm
           , [ Ref.string_of self ] ) ) ;
  ...
  *)
  ...
  | _ ->
      Mutex.execute autodetect_mutex (fun () ->
          ...
          in
          if (not (valid_device userdevice ~_type))
             || (userdevice = "autodetect" && possibilities = [])
          then
            raise
              (Api_errors.Server_error
                 (Api_errors.invalid_device, [ userdevice ]) ) ;
          ...
          let ref = Ref.make () in
          debug
            "VBD.create (device = %s; uuid = %s; ref = %s)"
            userdevice
          ...
          in
          if Xapi_stdext_std.Listext.List.intersect userdevices existing_devices
             <> []
          then
            raise
              (Api_errors.Server_error
          ...
          if not empty
          then assert_doesnt_make_vm_non_agile ~__context ~vm:vM ~vdi:vDI ;
