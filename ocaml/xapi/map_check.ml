...
let validate_kvpair field_name requirements (key, value) =
  ...
  let requirement =
    try List.find (fun requirement -> requirement.key = key) requirements with
    | Not_found ->
        fail ()
  in
      ...
      List.fold_right
        (fun v acc ->
          ...
          | Some v ->
              if acc = ""
              then v
              else if Xapi_stdext_std.Xstringext.String.has_substr acc v
              then err value
              else v ^ "," ^ acc )
        vs
        ""
...
let assert_key ~field ~ks ~key ~value =
  ...
  else assert_value ~field ~key ~attr:(List.assoc key ks) ~value
        ...
        Some
          ( assert_req_values ~field ~ks ~vs:value ;
            ...
            let value =
              List.map
                (fun (k, (kt, default)) ->
                  if List.mem_assoc k value
                  then (k, List.assoc k value)
                  else (k, default) )
                ks
            in
