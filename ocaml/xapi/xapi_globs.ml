...
let session_revalidation_interval = ref Mtime.Span.(5 * min)

(* CP-820: other-config field in subjects should be periodically refreshed *)
let update_all_subjects_interval = ref Mtime.Span.(15 * min)
...
let winbind_dns_sync_interval = ref Mtime.Span.hour
...
let xapi_globs_spec =
  ...
  ; ( "session_revalidation_interval"
    , LongDurationFromSeconds session_revalidation_interval
    )
  ; ( "update_all_subjects_interval"
    , LongDurationFromSeconds update_all_subjects_interval
    )
  ...
  ; ( "winbind_dns_sync_interval"
    , LongDurationFromSeconds winbind_dns_sync_interval
    )
