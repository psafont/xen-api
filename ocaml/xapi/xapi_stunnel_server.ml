...
let sync ~__context ipv6_enabled =
  try
    ...
    let client_auth_name =
      ...
      then Some (Db.Pool.get_client_certificate_auth_name ~__context ~self:pool)
      else None
    in
    info
      "Synchronising stunnel (accept = %s, client_auth_name = %s)"
      accept
    ...
    else debug "No configuration changes: not restarting stunnel"
  with
  | e ->
