...
let fix_bonds ~__context () =
  ...
  List.iter
    (fun (rf, rc) ->
      if not (Db.is_valid_ref __context rc.API.pIF_bond_slave_of)
      then Db.PIF.set_bond_slave_of ~__context ~self:rf ~value:Ref.null )
    my_slave_pifs ;
  let my_bond_pifs =
    Db.PIF.get_records_where
      ~__context
...
let copy_bonds_from_master ~__context () =
  ...
  let maybe_create_bond_for_me bond =
    ...
    | [], [] ->
        ...
        warn
          "Cannot create bond %s at all: no PIFs exist on slave"
          bond.API.bond_uuid
    | [], _ ->
  ...
  List.iter
    (Helpers.log_exn_continue
       "resynchronising bonds on slave"
  ...
  let maybe_create_sriov_for_me (master_pif_ref, master_pif_rec) =
    ...
    then
      ...
      | (pif_ref, pif_rec) :: _ ->
        ...
        | exn ->
            error
              "Error occurs when syncing network sriov for PIF %s: %s"
              pif_rec.API.pIF_uuid
              (Printexc.to_string exn) )
  in
  List.iter
    (Helpers.log_exn_continue
       "resynchronising network sriov on slave"
