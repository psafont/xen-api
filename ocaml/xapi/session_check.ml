    ...
    (fun __context ->
      ...
      then () (* debug "Session is in the local database" *)
        ...
        try
          ...
          if (not pool) && not (Pool_role.is_master ())
          then raise Non_master_login_on_slave ;
          if Pool_role.is_master ()
          then
            Db_actions.DB_Action.Session.set_last_active
              ~__context
              ~self:session_id
        ...
        | Db_exn.DBCache_NotFound (_, tblname, reference) ->
            debug
              "Session check failed: missing reference; tbl = %s, ref = %s"
              tblname
              reference ;
            raise
              (Api_errors.Server_error
        ...
        | Non_master_login_on_slave ->
            let master =
              Db_actions.DB_Action.Pool.get_master
                ~__context
        ...
        | Api_errors.Server_error (code, params) as e ->
            debug
              "Session check failed: unexpected exception %s %s"
              code
              (String.concat " " params) ;
            raise e
        | exn ->
            debug
              "Session check failed: unexpected exception '%s'"
