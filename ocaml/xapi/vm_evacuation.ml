...
let estimate_evacuate_timeout ~__context ~host =
  ...
  let memory_used =
    Int64.sub
      metrics.API.host_metrics_memory_total
...
let ensure_no_vms ~__context ~rpc ~session_id ~evacuate_timeout =
  ...
  let self_managed_poweroff vm =
    ...
    then
      debug
        "Skip running VM %s: has self-managed poweroff"
  ...
  let evacuate () =
    ...
    let timeout =
      if evacuate_timeout > 0.
      then evacuate_timeout
      else estimate_evacuate_timeout ~__context ~host
    in
    let tasks =
      [ Client.Async.Host.evacuate ~rpc ~session_id ~host ~network:Ref.null ]
    in
    if not (Tasks.with_tasks_destroy ~rpc ~session_id ~timeout ~tasks)
    then get_running_domains () |> List.iter cancel_vm_tasks
  in
  ...
  log_and_ignore_exn (fun () ->
      ...
      |> List.filter_map (fun (vm, _) ->
             if self_managed_poweroff vm then None else Some vm )
      |> shutdown ;
      evacuate () ) ;
