...
let import_vdi_url
    ~__context ?(prefer_slaves = false) rpc session_id task_id vdi =
  ...
  in
  Printf.sprintf
    "https://%s%s?vdi=%s&session_id=%s&task_id=%s"
    address
...
let copy_vdi ~__context ?base vdi_src vdi_dst =
  ...
  Helpers.call_api_functions ~__context (fun rpc session_id ->
      ...
      let can_local_copy =
        Importexport.check_sr_availability
          ~__context
      ...
      in
      debug
        "Sm_fs_ops.copy_vdi: %s-copying %Ld%s preserving sparseness %s"
                  ...
                  (fun ~__context ->
                    ...
                    try
                      ...
                      | _ ->
                        ...
                        | code :: params ->
                            debug
                              "Copy of VDI to remote failed: %s [ %s ]"
                              code
