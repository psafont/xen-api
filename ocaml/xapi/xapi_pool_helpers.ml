...
let valid_operations ~__context record (pool : API.ref_pool) =
  ...
  then (
    List.iter
      (fun (blocking_op, err) ->
        if List.mem blocking_op current_ops
        then (
    ...
    List.iter
      (fun wait_op ->
        if List.mem wait_op current_ops
        then
  ...
  else set_errors Api_errors.ha_not_enabled [] [ `ha_disable ] ;
  ...
  | [] ->
      ...
      then
        set_errors
          Api_errors.incompatible_cluster_stack_active
          [ current_stack ]
          [ `cluster_create ] ) ;
  table
...
let assert_operation_valid ~__context ~self ~(op : API.pool_allowed_operations)
  ...
  then raise Api_errors.(Server_error (pool_secret_rotation_pending, [])) ;
...
let call_fn_on_hosts ~__context hosts f =
  Helpers.call_api_functions ~__context (fun rpc session_id ->
      ...
      List.iter
        (fun (host, exn) ->
          warn
            {|Exception raised while performing operation on host "%s": %s|}
            (Ref.string_of host)
...
let apply_guest_agent_config ~__context =
  let f ~rpc ~session_id ~host =
    try Client.Host.apply_guest_agent_config ~rpc ~session_id ~host with
    | e ->
        error
          "Failed to apply guest agent config to host %s: %s"
