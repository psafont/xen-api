...
let create_from_query_result ~__context q =
  let r = Ref.make ()
  and u = Uuid.string_of_uuid (Uuid.make_uuid ()) in
  ...
  then (
    ...
    info
      "Registering SM plugin %s (version %s)"
...
let update_from_query_result ~__context (self, r) q_result =
  ...
  then (
    ...
    then
      Db.SM.set_required_api_version
        ~__context
        ~self
...
let unregister_plugin ~__context q_result =
  _serialize_reg (fun () ->
      ...
      if is_v1 q_result.required_api_version
      then
        info
          "Not unregistering SM plugin %s (required_api_version %s < 2.0)"
          driver
          q_result.required_api_version
      else
        List.iter
          (fun (rf, rc) ->
            if rc.API.sM_type = driver
            then
              try
                info
                  "Unregistering SM plugin %s (version %s)"
                  driver
                  q_result.version ;
              ...
              | e ->
                  warn
                    "Ignore unregistering SM plugin failure: %s"
...
let register_plugin ~__context q_result =
  _serialize_reg (fun () ->
      ...
      then
        info
          "Not registering SM plugin %s (required_api_version %s < 2.0)"
          driver
          q_result.required_api_version
      else (
