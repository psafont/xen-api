...
let wait_for_subtask ?progress_minmax ~__context task =
  Helpers.call_api_functions ~__context (fun rpc session ->
      ...
      let cancel_task () =
        ...
        Db_actions.DB_Action.Task.set_current_operations
          ~__context
          ~self:task
      ...
      let process_copy_task task_rec =
        ...
        Option.iter
          (fun value ->
            Db_actions.DB_Action.Task.set_progress
              ~__context
              ~self:main_task
              ~value )
          myprogress ;
      ...
      let process_main_task task_rec =
        ...
        if List.exists (fun (_, x) -> x = `cancel) current_ops
        then cancel_task ()
      in
      ...
      while not !finished do
        ...
        let checkevent ev =
          ...
          | Event_helper.Task (r, Some x) ->
              if r = task
              then process_copy_task x
              else if r = main_task
              then process_main_task x
          | _ ->
              ()
      ...
      debug
        "Finished listening for events relating to tasks %s and %s"
        (Ref.string_of task)
        (Ref.string_of main_task) ;
...
let wait_for_clone ?progress_minmax ~__context task =
  ...
  try result |> Xmlrpc.of_string |> API.ref_VDI_of_rpc with
  | parse_error ->
      raise
        Api_errors.(
...
let clone_single_vdi
    ?progress rpc session_id disk_op ~__context vdi driver_params =
  let task =
    match disk_op with
    | Disk_op_clone ->
    ...
    try
      ...
      let newvdi, on_error_delete =
        ...
        then (vbd_r.API.vBD_VDI, false) (* don't delete the original CD *)
          ...
          ( clone_single_vdi
              ~progress:(done_so_far, size, total)
              rpc
              session_id
              disk_op
              ~__context
...
let snapshot_info ~power_state ~is_a_snapshot =
  ...
  in
  if is_a_snapshot
  then (disk_snapshot_type, crash_consistent) :: power_state_info
  else []


let snapshot_metadata ~__context ~vm ~is_a_snapshot =
  if is_a_snapshot then Helpers.vm_to_string __context vm else ""
...
let copy_vm_record
    ?snapshot_info_record ~__context ~vm ~disk_op ~new_name ~new_power_state =
  ...
  let other_config =
    if all.Db_actions.vM_is_a_template
  ...
  let generation_id =
    Xapi_vm_helpers.fresh_genid
      ~current_genid:all.Db_actions.vM_generation_id
      ()
  in
  ...
  Db.VM.create
    ...
    ~snapshot_time:
      ( if is_a_snapshot
      then Date.of_float (Unix.gettimeofday ())
      else Date.never )
    ~snapshot_info:
        ...
        try
          ...
          List.iter
            (fun (vbd, newvdi, _) ->
              ...
              if Db.VBD.get_type ~__context ~self:vbd = `Disk
              then
                let other_config =
                ...
                then
                  Db.VBD.add_to_other_config
                    ~__context
                    ~self:vbd
                    ~key:Constants.owner_key
                    ~value:"" )
            cloned_disks ;
          (* copy VIFs *)
            ...
            List.map
              (fun vif ->
                Xapi_vif_helpers.copy
                  ~__context
                  ~vm:ref
                  ~preserve_mac_address:is_a_snapshot
                  vif )
              vifs
          in
          (* copy VGPUs *)
          ...
          then Xapi_xenops.nvram_post_clone ~__context ~self:ref ~uuid ;
