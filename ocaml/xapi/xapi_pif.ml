    ...
    raise
      Api_errors.(
        Server_error
          ( internal_error
          , [ Printf.sprintf
                "refresh: Host mismatch, expected %s but got %s"
                (Ref.string_of host)
...
let assert_not_bond_slave_of ~__context ~self =
  if Db.PIF.get_bond_slave_of ~__context ~self <> Ref.null
  then
    raise
...
let assert_valid_ip_configuration ~__context ~self =
  if Db.PIF.get_ip_configuration_mode ~__context ~self = `None
  then
    raise
      (Api_errors.Server_error
...
let assert_valid_ipv6_configuration ~__context ~self =
  if Db.PIF.get_ipv6_configuration_mode ~__context ~self = `None
  then
    raise
      (Api_errors.Server_error
...
let assert_usable_for_management ~__context ~primary_address_type ~self =
  ...
  if primary_address_type = `IPv4
  then assert_valid_ip_configuration ~__context ~self
  else if primary_address_type = `IPv6
  then assert_valid_ipv6_configuration ~__context ~self
  else ()
...
let assert_not_management_pif ~__context ~self =
  if Db.PIF.get_management ~__context ~self
  then
    raise
      (Api_errors.Server_error
...
let assert_no_protection_enabled ~__context ~self =
  ...
  then
    ...
    then raise (Api_errors.Server_error (Api_errors.redo_log_is_enabled, []))
...
let assert_no_sriov ~__context ~self =
  ...
  | Physical _ :: _, _ :: _ ->
      raise
        Api_errors.(
          Server_error (pif_sriov_still_exists, [ Ref.string_of self ]))
  | _ ->
      ()
...
let set_default_properties ~__context ~self =
  if pif_has_properties ~__context ~self
  then Db.PIF.set_properties ~__context ~self ~value:default_properties
  else Db.PIF.set_properties ~__context ~self ~value:[]
...
let forget_internal ~t ~__context ~self =
  ...
  then Nm.bring_pif_down ~__context self ;
...
let introduce ~__context ~host ~mAC ~device ~managed =
  ...
  *)
  let mAC =
    if mAC = ""
    then Listext.assoc_default device t.device_to_mac_table ""
    else mAC
  in
  if not (Helpers.is_valid_MAC mAC)
  then raise (Api_errors.Server_error (Api_errors.mac_invalid, [ mAC ])) ;
...
let scan ~__context ~host =
  ...
  let non_managed_devices, disallow_unplug_devices =
    if Sys.file_exists !Xapi_globs.non_managed_pifs
    then (
      try
        let output, _ =
      ...
      | e ->
          warn
            "Error when executing script %s: %s; ignoring"
...
let set_property ~__context ~self ~name ~value =
  ...
  in
  if not (List.mem_assoc name property_names_and_values)
  then fail ()
  else if not (List.mem value (List.assoc name property_names_and_values))
  then fail () ;
  (* Only bond masters and unbonded physical PIFs can be configured *)
  if (not (pif_has_properties ~__context ~self))
  ...
  List.iter
    (fun pif ->
      if Db.PIF.get_currently_attached ~__context ~self
      then Nm.bring_pif_up ~__context pif )
    (self :: vlan_pifs)
...
let rec unplug ~__context ~self =
  ...
  if Db.PIF.get_capabilities ~__context ~self |> List.mem "fcoe"
  then assert_fcoe_not_in_use ~__context ~self ;
  List.iter
    (fun tunnel ->
  ...
  | Bond_master bond :: _ ->
      List.iter
        (fun slave ->
          if Db.PIF.get_sriov_physical_PIF_of ~__context ~self:slave <> []
          then (
            debug
  ...
  if pif_rec.API.pIF_bond_slave_of = Ref.null
  then Nm.bring_pif_down ~__context self
...
let rec plug ~__context ~self =
  ...
  let () =
    ...
    | Tunnel_access tunnel ->
        ...
        in
        if Db.PIF.get_ip_configuration_mode ~__context ~self:transport_PIF
           = `None
        then
          raise
            Api_errors.(
    ...
    | Physical pif_rec ->
        ...
        if bond <> Ref.null
        then
          if pif_rec.API.pIF_sriov_physical_PIF_of <> []
          then (
  ...
  if pif_rec.API.pIF_bond_slave_of = Ref.null
  then Nm.bring_pif_up ~__context ~management_interface:false self
