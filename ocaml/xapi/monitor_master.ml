...
let update_configuration_from_master () =
  Server_helpers.exec_with_new_task
    "update_configuration_from_master"
    (fun __context ->
      let oc =
...
let set_pif_metrics
    ~__context ~self ~vendor ~device ~carrier ~speed ~duplex ~pcibuspath pmr =
  ...
  if pmr.API.pIF_metrics_carrier <> carrier
  then Db.PIF_metrics.set_carrier ~__context ~self ~value:carrier ;
  if pmr.API.pIF_metrics_speed <> speed
  then Db.PIF_metrics.set_speed ~__context ~self ~value:speed ;
  if pmr.API.pIF_metrics_duplex <> duplex
  then Db.PIF_metrics.set_duplex ~__context ~self ~value:duplex ;
  if pmr.API.pIF_metrics_pci_bus_path <> pcibuspath
  then Db.PIF_metrics.set_pci_bus_path ~__context ~self ~value:pcibuspath ;
  Db.PIF_metrics.set_last_updated
    ~__context
    ~self
          ...
          try
            ...
            then (
              ...
              | e ->
                  log_backtrace () ;
                  error
                    "Failed to update VIF carrier flags for PIF: %s"
                    (ExnHelper.string_of_exn e) ) ;
            (* 2. Update database *)
            let metrics =
              (* If PIF metrics don't exist then create them *)
              if Db.is_valid_ref __context pifrec.API.pIF_metrics
              then pifrec.API.pIF_metrics
              else
                let ref = Ref.make () in
