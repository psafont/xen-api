...
let assert_network_is_managed ~__context ~self =
  if not (Db.Network.get_managed ~__context ~self)
  then
    raise
...
let create_internal_bridge ~__context ~bridge ~uuid ~persist =
  ...
  then
    ...
    Mutex.execute internal_bridge_m (fun () ->
        let current = Net.Bridge.get_all dbg () in
        if not (List.mem bridge current)
        then (
...
let set_himn_ip ~__context bridge other_config =
  ...
  | Not_found ->
      error
        "Cannot setup host internal management network: no \
         other-config:ip_begin or other-config:netmask"


let is_himn net =
  List.assoc_opt
    Xapi_globs.is_guest_installer_network
...
let check_himn ~__context =
  ...
  | (_, rc) :: _ ->
      ...
      if List.mem rc.API.network_bridge bridges
      then
        set_himn_ip ~__context rc.API.network_bridge rc.API.network_other_config


let attach_internal
    ?(management_interface = false) ?(force_bringup = false) ~__context ~self ()
    =
  ...
  if not net.API.network_managed
  then (
    let dbg = Context.string_of_task __context in
    let bridges = Net.Bridge.get_all dbg () in
    if not (List.mem net.API.network_bridge bridges)
    then
      raise
        (Api_errors.Server_error
    ...
    if List.length local_pifs = 0
    then
      create_internal_bridge
        ~__context
        ~bridge:net.API.network_bridge
        ~uuid:net.API.network_uuid
        ~persist ;
...
let detach ~__context net =
  ...
  then (
    List.iter
      (fun iface ->
        D.warn
          "Untracked interface %s exists on bridge %s: deleting"
          iface
          bridge_name ;
...
let deregister_vif ~__context vif =
  ...
  Mutex.execute active_vifs_to_networks_m (fun () ->
      ...
      then (
        ...
        in
        debug
          "deregister_vif vif=%s network=%s remaining vifs = [ %s ]"
          (Ref.string_of vif)
          (Ref.string_of network)
        ...
        then
          ...
          if ifs = []
          then detach ~__context network_rc
          else
            error
...
let rec choose_bridge_name bridges =
  ...
  incr counter ;
  if List.mem name bridges then choose_bridge_name bridges else name
  ...
  Mutex.execute mutex (fun () ->
      ...
      let bridge =
        ...
        then choose_bridge_name bridges
          ...
          if List.mem bridge bridges
          then
            raise
              (Api_errors.Server_error
                 (Api_errors.bridge_name_exists, [ bridge ]) ) ;
          bridge )
      in
      let r = Ref.make ()
      and uuid = Uuid.make_uuid () in
