...
let make
  ...
  in
  let (_ : unit) =
    Db_actions.DB_Action.Task.create
      ~ref
      ~__context
...
let get_name ~__context =
  ...
  else Db.Task.get_name_label ~__context ~self:task_id
...
let set_external_pid ~__context pid =
  operate_on_db_task ~__context (fun self ->
      Db_actions.DB_Action.Task.set_externalpid
        ~__context
        ~self
...
let set_result_on_task ~__context task_id result =
  ...
  | None ->
      ()
  | Some x ->
      Db_actions.DB_Action.Task.set_result
        ~__context
        ~self:task_id
...
let complete ~__context result =
  operate_on_db_task ~__context (fun self ->
      ...
      if status = `pending
      then (
        Db_actions.DB_Action.Task.set_allowed_operations
          ~__context
          ~self
          ~value:[] ;
        Db_actions.DB_Action.Task.set_finished
          ~__context
          ~self
        ...
        debug
          "the status of %s is: %s; cannot set it to `success"
...
let is_cancelling ~__context =
  ...
  &&
  let l =
    Db_actions.DB_Action.Task.get_current_operations
      ~__context
...
let cancel_this ~__context ~self =
  ...
  then (
    ...
    debug
      "the status of %s is %s; cannot set it to `cancelled"
      (Ref.really_pretty_and_small self)
      (status_to_string status)
...
let failed ~__context exn =
  ...
  operate_on_db_task ~__context (fun self ->
      ...
      then (
        Db_actions.DB_Action.Task.set_progress ~__context ~self ~value:1. ;
        Db_actions.DB_Action.Task.set_error_info
          ~__context
          ~self
          ~value:(code :: params) ;
        Db_actions.DB_Action.Task.set_backtrace
          ~__context
          ~self
        ...
        then
          Db_actions.DB_Action.Task.set_status
            ~__context
            ~self
            ~value:`cancelled
        else
        ...
        debug
          "the status of %s is %s; cannot set it to %s"
          (Ref.really_pretty_and_small self)
          (status_to_string status)
...
let register_task __context ?(cancellable = true) id =
  ...
  if cancellable
  then set_cancellable ~__context
