...
module D = Debug.Make (struct
  let name = "xapi_pv_driver_version"
end)
...
let get_drivers_version os_version drivers_version =
  try
    ...
    let lookup_driver_key_with_default key default =
      if not (List.mem_assoc key drivers_version)
      then default
      else int_of_string (List.assoc key drivers_version)
    in
    ...
    if is_windows
    then Windows (major, minor, micro, build)
    else Linux (major, minor, micro, build)
  with
  | _ ->
      Unknown
