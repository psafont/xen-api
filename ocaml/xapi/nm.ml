...
let determine_mtu pif_rc net_rc =
  ...
  if List.mem_assoc "mtu" pif_rc.API.pIF_other_config
  then (
...
let determine_ethtool_settings properties oc =
  let proc key =
    if List.mem_assoc ("ethtool-" ^ key) oc
    then
      ...
      then [ (key, "off") ]
        ...
        debug
          "Invalid value for ethtool-%s = %s. Must be on|true|off|false."
          key
          value ;
        [] )
    else if List.mem_assoc key properties
    then [ (key, List.assoc key properties) ]
    else []
  in
  let speed =
    if List.mem_assoc "ethtool-speed" oc
    then
  ...
  in
  let duplex =
    if List.mem_assoc "ethtool-duplex" oc
    then
...
let create_bond ~__context bond mtu persistent =
  ...
  let props =
    let rec get_prop p =
      if List.mem_assoc p props then List.assoc p props else ""
    and get_prop_assoc_if_mode m p =
      if mode = m
      then if List.mem_assoc p props then [ (p, List.assoc p props) ] else []
      else []
    in
    if List.length slaves > 1
    then
      let hashing_algorithm = get_prop "hashing_algorithm"
      and rebalance_interval =
        if mode = `lacp then [] else [ ("rebalance-interval", "1800000") ]
      ...
      let overrides =
        List.filter_map
          (fun (k, v) ->
            if String.startswith "bond-" k
            then Some (String.sub_to_end k 5, v)
            else None )
  ...
  let igmp_snooping =
    Some
      (Db.Pool.get_igmp_snooping_enabled
         ~__context
...
let rec create_bridges ~__context pif_rc net_rc =
  ...
  | Physical _ ->
      let cleanup =
        if pif_rc.API.pIF_bond_slave_of <> Ref.null
        then destroy_bond ~__context ~force:true pif_rc.API.pIF_bond_slave_of
        else []
      in
      let ethtool_settings, ethtool_offload =
  ...
  | Network_sriov_logical _ ->
      raise
        Api_errors.(
          Server_error
            ( internal_error
            , [ "Should not create bridge for SRIOV logical PIF" ] ))
...
let rec destroy_bridges ~__context ~force pif_rc bridge =
  ...
  | VLAN_untagged vlan ->
      ...
      if not rc.API.pIF_currently_attached
      then
        let bridge =
  ...
  | Network_sriov_logical _ ->
      raise
        Api_errors.(
          Server_error
            ( internal_error
            , [ "Should not destroy bridge for SRIOV logical PIF" ] ))


let determine_static_routes net_rc =
  if List.mem_assoc "static-routes" net_rc.API.network_other_config
  then
    try
      let routes =
        String.split
...
let maybe_update_master_pif_mac
    ~__context bond_record (bond_master_record : API.pIF_t) bond_master =
      ...
      | _ ->
          ...
          Network.transform_networkd_exn pif (fun () ->
              ...
              let gateway_if, dns_if =
                Helpers.determine_gateway_and_dns_ifs
                  ~__context
              ...
              let ipv4_conf, ipv4_gateway, dns =
                ...
                | `Static ->
                    ...
                    in
                    let gateway =
                      if rc.API.pIF_gateway <> ""
                      then Some (Unix.inet_addr_of_string rc.API.pIF_gateway)
                      else None
                    in
                    let dns =
                      if rc.API.pIF_DNS <> ""
          ...
          if rc.API.pIF_currently_attached = false || management_interface
          then (
          ...
          ( try
              ...
              if mtu <> rc.API.pIF_MTU
              then Db.PIF.set_MTU ~__context ~self:pif ~value:mtu
            with
          | _ ->
              warn "could not update MTU field on PIF %s" rc.API.pIF_uuid ) ;
          (* sync igmp_snooping_enabled *)
          ( if rc.API.pIF_VLAN = -1L
          then
            let igmp_snooping =
              Db.Pool.get_igmp_snooping_enabled
                ~__context
            ...
            in
            if igmp_snooping' <> rc.API.pIF_igmp_snooping_status
            then
              Db.PIF.set_igmp_snooping_status
                ~__context
                ~self:pif
                ~value:igmp_snooping' ) ;
