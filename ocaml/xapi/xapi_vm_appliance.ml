...
let perform_operation ~__context ~self ~operation ~ascending_priority =
  ...
  | [] ->
      debug
        "Operation %s on appliance with uuid %s completed successfully"
        operation.name
        appliance_uuid
  | _ ->
      debug
        "Operation %s on appliance with uuid %s partially failed"
        operation.name
        appliance_uuid ;
      raise
...
let get_SRs_required_for_recovery ~__context ~self ~session_to =
  ...
  |> List.to_seq
  |> Seq.flat_map (fun vm ->
         Xapi_vm_helpers.get_SRs_required_for_recovery
           ~__context
           ~self:vm
           ~session_to
         |> List.to_seq )
...
let recover ~__context ~self ~session_to ~force =
  Xapi_dr.assert_session_allows_dr
  ...
  Server_helpers.exec_with_new_task
    ~session_id:session_to
    "Recreating VM appliance object"
    (fun __context_to ->
      let recovered_appliance =
        try
          ...
          let existing_appliance =
            Db.VM_appliance.get_by_uuid
              ~__context:__context_to
              ~uuid:old_appliance.API.vM_appliance_uuid
          in
          debug
            "An appliance with UUID %s already exists - reusing it."
            old_appliance.API.vM_appliance_uuid ;
          let vms =
            Db.VM_appliance.get_VMs
              ~__context:__context_to
        ...
        | Db_exn.Read_missing_uuid ("VM_appliance", _, _) ->
            ...
            let new_appliance =
              create
                ...
                ~name_description:
                  old_appliance.API.vM_appliance_name_description
            in
            Db.VM_appliance.set_uuid
              ~__context:__context_to
              ~self:new_appliance
      ...
      List.iter
        (fun vm ->
          if not (Db.VM.get_is_a_template ~__context:__context_to ~self:vm)
          then
            Db.VM.set_appliance
              ~__context:__context_to
              ~self:vm
