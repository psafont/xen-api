...
let valid_operations ~expensive_sharing_checks ~__context record _ref' : table =
  ...
  let set_errors
      (code : string) (params : string list) (ops : API.vbd_operations_set) =
    List.iter
      (fun op ->
  ...
  if Listext.List.set_difference current_ops (`attach :: safe_to_parallelise)
     <> []
  then
    set_errors
      Api_errors.other_operation_in_progress
  ...
  then
    set_errors
      Api_errors.vbd_not_unpluggable
      [ _ref ]
      [ `unplug; `unplug_force ] ;
  ...
  | `Running, false ->
      set_errors
        Api_errors.device_already_detached
        [ _ref ]
        [ `unplug; `unplug_force ]
  | _, _ ->
  ...
  if power_state = `Running && domain_type = `hvm
  then (
  ...
  ( if record.Db_actions.vBD_type = `CD && power_state = `Suspended
  then
    let expected =
      String.concat
        ", "
  ...
  then (
    ...
    in
    ( if vdi_operations_besides_copy
    then
      let concurrent_op =
      ...
      set_errors
        ...
        [ `attach; `plug; `insert ] ) ;
    if (not record.Db_actions.vBD_currently_attached)
       && expensive_sharing_checks
    then (
      ...
      let vbd_records =
        ...
        List.concat
          (List.map
             (fun self ->
               try [ Db.VBD.get_record_internal ~__context ~self ] with
               | _ ->
                   [] )
             vbds )
      in
      ...
      let pointing_to_a_system_domain vbd =
        System_domains.get_is_system_domain
          ~__context
      ...
      if (not vdi_record.Db_actions.vDI_sharable)
...
let assert_doesnt_make_vm_non_agile ~__context ~vm ~vdi =
  ...
  then (
    warn
      "Attaching VDI %s makes VM %s not agile"
      (Ref.string_of vdi)
      (Ref.string_of vm) ;
    raise
      (Api_errors.Server_error
...
let clear_current_operations ~__context ~self =
  if Db.VBD.get_current_operations ~__context ~self <> []
  then (
