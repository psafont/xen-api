...
let scan_start ~__context usbs =
  ...
  *)
    ...
    (fun s ->
      let self =
        create
          ...
          ~other_config:[]
          ~path:s.USB.path
          ~vendor_id:s.USB.vendor_id
          ~vendor_desc:s.USB.vendor_desc
          ~product_id:s.USB.product_id
          ~product_desc:s.USB.product_desc
          ~serial:s.USB.serial
          ~version:s.USB.version
          ~description:s.USB.description
          ~speed:s.USB.speed
      in
  ...
  |> List.iter (fun (self, _) ->
         try Xapi_pusb_helpers.destroy_pusb ~__context self with
         | e ->
             error
               "Caught exception while removing PUSB %s: %s"
               (Ref.string_of self)
    ...
    (Thread.create
       (fun () ->
         while true do
           ...
           | e ->
               Printf.printf
                 "Caught exception from scan_start '%s' \n%!"
                 (Printexc.to_string e) ;
               ()
         done )
       () )
      ...
      | true ->
          ...
          List.iter
            (fun sr ->
              ...
              |> List.iter (fun rf ->
                     try
                       if get_sm_usb_path ~__context rf = pusb_path
                       then (
                  ...
                  raise
                    Api_errors.(
                      Server_error
                        ( internal_error
                        , [ Printf.sprintf
                              "too many vusb on the USB_group: %s"
