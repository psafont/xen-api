...
let try_create_sr_from_record
    ~__context ~_type ~device_config ~dr_task ~sr_record =
  Helpers.call_api_functions ~__context (fun rpc session_id ->
      ...
      | e ->
          ...
          warn
            "Could not successfully attach SR %s - caught %s"
            sr_record.uuid
...
let create ~__context ~_type ~device_config ~whitelist =
  ...
  if not (List.mem_assoc Smint.Sr_metadata (Sm.features_of_driver _type))
  then
    raise
      (Api_errors.Server_error
         ( Api_errors.operation_not_allowed
         , [ Printf.sprintf
               "Disaster recovery not supported on SRs of type %s"
               _type
           ] ) ) ;
  ...
  let sr_records =
    try parse_sr_probe probe_result with
    | Failure msg ->
        raise
          (Api_errors.Server_error
