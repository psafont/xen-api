...
let validate_params ~token_timeout ~token_timeout_coefficient =
  ...
  in
  if token_timeout < Constants.minimum_token_timeout_s
  then invalid_value "token_timeout" token_timeout ;
  if token_timeout_coefficient < Constants.minimum_token_timeout_coefficient_s
  then invalid_value "token_timeout_coefficient" token_timeout_coefficient
...
let create
  ...
  with_clustering_lock __LOC__ (fun () ->
      ...
      | Ok cluster_token ->
          ...
          D.debug
            "Created Cluster: %s and Cluster_host: %s"
...
let pool_create
    ~__context ~network ~cluster_stack ~token_timeout ~token_timeout_coefficient
    =
  validate_params ~token_timeout ~token_timeout_coefficient ;
  let master = Helpers.get_master ~__context in
      ...
      (fun host ->
        ...
        Helpers.call_api_functions ~__context (fun rpc session_id ->
            ...
            let cluster_host_ref =
              Client.Client.Cluster_host.create
                ~rpc
                ~session_id
                ~cluster
                ~host
  ...
  | e ->
      ...
      info
        "pool_create attempting cleanup of cluster=%s"
        (Ref.short_string_of cluster) ;
      ( try pool_force_destroy ~__context ~self:cluster with
      | e ->
          error
            "pool_create attempt to clean up cluster=%s failed. ex='%s'"
            (Ref.short_string_of cluster)
