...
let register sr rpc d info =
  Mutex.execute m (fun () ->
      ...
      debug
        "register SR %s (currently-registered = [ %s ])"
        (s_of_sr sr)
        (String.concat
           ", "
...
let unregister sr =
  Mutex.execute m (fun () ->
      ...
      debug
        "unregister SR %s (currently-registered = [ %s ])"
        (s_of_sr sr)
        (String.concat
           ", "
           (Hashtbl.fold (fun sr _ acc -> s_of_sr sr :: acc) plugins []) ) )
...
let query_result_of_sr sr =
  try
    Mutex.execute m (fun () -> Some (Hashtbl.find plugins sr).query_result)
  with
  | _ ->
      None
...
let of_sr sr =
  Mutex.execute m (fun () ->
      if not (Hashtbl.mem plugins sr)
      then (
        error
          "No storage plugin for SR: %s (currently-registered = [ %s ])"
          (s_of_sr sr)
          (String.concat
             ", "
             (Hashtbl.fold (fun sr _ acc -> s_of_sr sr :: acc) plugins []) ) ;
        raise (Storage_error (No_storage_plugin_for_sr (s_of_sr sr))) )
      else (Hashtbl.find plugins sr).processor )
...
module Mux = struct
  ...
  module SR = struct
    ...
    let create
        () ~dbg ~sr ~name_label ~name_description ~device_config ~physical_size
        =
      let module C =
        StorageAPI (Idl.Exn.GenClient (struct
          let rpc = of_sr sr
  ...
  module VDI = struct
    ...
    let snapshot () ~dbg ~sr ~vdi_info =
      ...
      in
      try C.VDI.snapshot dbg sr vdi_info with
      | Storage_interface.Storage_error (Activated_on_another_host uuid) ->
          Server_helpers.exec_with_new_task
            "smapiv2.snapshot.activated"
            ...
            (fun __context ->
              ...
              debug
                "SM reports the VDI is activated elsewhere on %s, redirecting \
                 to IP %s"
                uuid
                addr ;
  ...
  module Policy = struct
    let get_backend_vm () ~dbg ~vm ~sr ~vdi =
      ...
      then (
        ...
        raise (Storage_error (No_storage_plugin_for_sr (s_of_sr sr))) )
      else (Hashtbl.find plugins sr).backend_domain
  end
