...
module D = Debug.Make (struct
  let name = "xapi_pvs_proxy_control"
end)
...
let find_cache_vdi ~__context ~host ~site =
  ...
  let pcs' =
    Db.PVS_cache_storage.get_refs_where
      ~__context
      ~expr:
        (And
  ...
  | e ->
      let reason =
        ...
        | No_cache_sr_available ->
            ...
            let body =
              Printf.sprintf
                "Unable to setup PVS-proxy %s for VIF %s: no cache storage \
                 found on PVS-site %s for host %s."
                proxy_uuid
                (Db.VIF.get_uuid ~__context ~self:vif)
                (Db.PVS_site.get_name_label
                   ~__context
                   ~self:(Db.PVS_proxy.get_site ~__context ~self:proxy) )
                (Db.Host.get_name_label
                   ~__context
        ...
        | Helpers.No_pvs_server_available ->
            ...
            let body =
              Printf.sprintf
                "Unable to setup PVS-proxy %s for VIF %s: no PVS-server found \
...
let stop_proxy ~__context vif proxy =
  ...
  | e ->
      ...
      in
      error
        "Unable to disable PVS proxy for VIF %s: %s."
        (Ref.string_of vif)
        reason
