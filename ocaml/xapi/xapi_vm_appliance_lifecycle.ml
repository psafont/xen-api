...
let check_operation_error ~__context record self op =
  ...
  then Some (Api_errors.other_operation_in_progress, [ "VM_appliance"; _ref ])
  else
    let vms = Db.VM_appliance.get_VMs ~__context ~self in
    if List.length vms = 0
    then Some (Api_errors.operation_not_allowed, [ "Appliance has no VMs." ])
    else
      ...
      in
      if List.exists predicate power_states
      then None
      else Some (Api_errors.operation_not_allowed, [ error ])
