...
let host_pre_declare_dead ~__context ~host ~reason =
  ...
  execute_host_hook
    ~__context
    ~script_name:scriptname__host_pre_declare_dead
    ~reason
    ~host ;
  if String.equal reason reason__dbdestroy
  then
    log_and_ignore_exn (fun () ->
        ...
        | Some self ->
            info
              "Declaring cluster host %s as permanently dead"
              (Ref.string_of self) ;
...
let host_post_declare_dead ~__context ~host ~reason =
  ...
  execute_host_hook
    ~__context
    ~script_name:scriptname__host_post_declare_dead
    ~reason
    ~host
...
let pool_ha_overcommitted_hook ~__context =
  execute_pool_hook
    ~__context
    ~script_name:scriptname__pool_ha_overcommitted
    ~reason:reason__none
...
let pool_pre_ha_vm_restart_hook ~__context =
  execute_pool_hook
    ~__context
    ~script_name:scriptname__pool_pre_ha_vm_restart
    ~reason:reason__none
...
let pool_join_hook ~__context =
  execute_pool_hook
    ~__context
    ~script_name:scriptname__pool_join
    ~reason:reason__none
...
let pool_eject_hook ~__context =
  execute_pool_hook
    ~__context
    ~script_name:scriptname__pool_eject
    ~reason:reason__none
