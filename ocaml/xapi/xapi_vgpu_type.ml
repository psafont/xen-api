...
let create
  ...
  debug
    "VGPU_type ref='%s' created (vendor_name = '%s'; model_name = '%s')"
    (Ref.string_of ref)
    vendor_name
    model_name ;
  ref
...
let find_and_update ~__context vgpu_type =
  ...
  | [ (vgpu_type_ref, rc) ] ->
      (* If looking up by identifier succeeds, update that VGPU_type's vendor_name
         		 * and model_name. *)
      if vgpu_type.vendor_name <> rc.Db_actions.vGPU_type_vendor_name
      then
        Db.VGPU_type.set_vendor_name
          ~__context
          ~self:vgpu_type_ref
          ~value:vgpu_type.vendor_name ;
      if vgpu_type.model_name <> rc.Db_actions.vGPU_type_model_name
      then
        Db.VGPU_type.set_model_name
          ~__context
          ~self:vgpu_type_ref
  ...
  | [] ->
    (* If looking up by identifier fails, try to the old method (vendor name
       		 ...
       		 * field. *)
    ( match
        Db.VGPU_type.get_internal_records_where ~__context ~expr:old_expr
      with
    | [ (vgpu_type_ref, rc) ] ->
        Db.VGPU_type.set_identifier
          ~__context
          ~self:vgpu_type_ref
...
let find_or_create ~__context vgpu_type =
  ...
  | Some (vgpu_type_ref, rc) ->
      ...
      then
        Db.VGPU_type.set_framebuffer_size
          ~__context
          ~self:vgpu_type_ref
          ~value:vgpu_type.framebuffer_size ;
      if vgpu_type.max_heads <> rc.Db_actions.vGPU_type_max_heads
      then
        Db.VGPU_type.set_max_heads
          ~__context
          ~self:vgpu_type_ref
          ~value:vgpu_type.max_heads ;
      if vgpu_type.max_resolution_x <> rc.Db_actions.vGPU_type_max_resolution_x
      then
        Db.VGPU_type.set_max_resolution_x
          ~__context
          ~self:vgpu_type_ref
          ~value:vgpu_type.max_resolution_x ;
      if vgpu_type.max_resolution_y <> rc.Db_actions.vGPU_type_max_resolution_x
      then
        Db.VGPU_type.set_max_resolution_y
          ~__context
          ~self:vgpu_type_ref
          ~value:vgpu_type.max_resolution_y ;
      if vgpu_type.size <> rc.Db_actions.vGPU_type_size
      then
        Db.VGPU_type.set_size
          ~__context
          ~self:vgpu_type_ref
          ~value:vgpu_type.size ;
      if vgpu_type.internal_config <> rc.Db_actions.vGPU_type_internal_config
      then
        Db.VGPU_type.set_internal_config
          ~__context
          ~self:vgpu_type_ref
          ~value:vgpu_type.internal_config ;
      if implementation <> rc.Db_actions.vGPU_type_implementation
      then
        Db.VGPU_type.set_implementation
          ~__context
          ~self:vgpu_type_ref
          ~value:implementation ;
      if vgpu_type.experimental <> rc.Db_actions.vGPU_type_experimental
      then
        Db.VGPU_type.set_experimental
          ~__context
          ~self:vgpu_type_ref
          ~value:vgpu_type.experimental ;
      let compatible_types_in_vm =
        if vgpu_type.compatible_model_names_in_vm = []
        then []
        else [ vgpu_type_ref ]
      in
      if compatible_types_in_vm
         <> rc.Db_actions.vGPU_type_compatible_types_in_vm
      then
        Db.VGPU_type.set_compatible_types_in_vm
          ~__context
          ~self:vgpu_type_ref
          ~value:compatible_types_in_vm ;
      let compatible_types_on_pgpu =
        if vgpu_type.compatible_model_names_on_pgpu = []
        then []
        else [ vgpu_type_ref ]
      in
      if compatible_types_on_pgpu
         <> rc.Db_actions.vGPU_type_compatible_types_on_pgpu
      then
        Db.VGPU_type.set_compatible_types_on_pgpu
          ~__context
          ~self:vgpu_type_ref
...
let read_whitelist_line_by_line
    ~whitelist ~device_id ~parse_line ~device_id_of_conf =
...
module Vendor_nvidia = struct
  ...
  let get_host_driver_version () =
    try
      Scanf.sscanf
        (Unix.readlink !Xapi_globs.nvidia_host_driver_file)
        "libnvidia-vgpu.so.%s"
        (fun x -> x)
    with
    | _ ->
        info
          "Can not get the file version of %s, use the default value"
    ...
    *)
    List.filter_map
      (fun pgpu ->
        ...
        if int_of_string (get_attr "deviceId" devid) = device_id
        then
          let psubdev_id =
  ...
  let extract_conf whitelist device_id vgpu_types vgpu_ids =
    ...
    *)
    ...
    let driver_supports_multiple_vgpus =
      host_driver_supports_multi_vgpu
        ~host_driver_version
        ~supported_versions:
    ...
    List.filter_map
      (fun vgpu_type ->
        ...
        if List.mem_assoc id vgpu_ids
        then (
          let { max = max_instance; psubdev_id; sriov } =
            List.assoc id vgpu_ids
          in
  ...
  let vgpu_type_of_conf pci_access vendor_name _ conf =
    ...
    in
    let model_name =
      Pci.lookup_subsystem_device_name
        pci_access
        vendor_id
        conf.identifier.vdev_id
        vendor_id
...
module Vendor_intel = struct
  ...
  let read_whitelist_line ~line =
    try
      Some
        (Scanf.sscanf
           ...
           ->
             { identifier =
                 Identifier.{ pdev_id; low_gm_sz; high_gm_sz; fence_sz }
    ...
    | e ->
        error
          "Failed to read whitelist line: '%s' %s"
          line
          (Printexc.to_string e) ;
        None
    ...
    try
      ...
      let pdev_id, psubdev_id =
        ...
        | Scanf.Scan_failure _ ->
            Scanf.sscanf pdev_id_data {|"0x%x"|} (fun pdev_id ->
                (pdev_id, None) )
      in
      ...
      Scanf.sscanf
        (List.assoc "plugin0.vdev_id" args)
        {|"0x%x:0x%x"|}
  ...
  let read_config_dir conf_dir =
    let rec read_configs ac = function
      | [] ->
          ac
      | conf_file :: tl ->
        ( try read_configs ((conf_file, of_conf_file conf_file) :: ac) tl with
        | Parse_error e ->
            error
              "Ignoring error parsing %s: %s\n%s\n"
              conf_file
    ...
    debug
      "Reading NVIDIA vGPU config files %s/{%s}"
      conf_dir
  ...
  let create_compat_config_file __context =
    try
      ...
      let host_driver_supports_multiple =
        Vendor_nvidia.host_driver_supports_multi_vgpu
          ~host_driver_version
          ~supported_versions:
            !Xapi_globs.nvidia_multi_vgpu_enabled_driver_versions
      in
      if not host_driver_supports_multiple
      then
    ...
    | e ->
        error
          "Failed to create NVidia compat config_file: %s\n%s\n"
          (Printexc.to_string e)
          (Printexc.get_backtrace ())
end
