      ...
      then Some (Api_errors.sr_no_pbds, [ Ref.string_of sr ])
        ...
        let my_active_vbd_records =
          ...
          | None ->
              List.map
                snd
                (Db.VBD.get_internal_records_where
                   ~__context
                   ~expr:
                     (And
        ...
        *)
        ...
        let blocked_by_attach =
          let blocked_by_attach =
            ...
            then false
            else if operation_can_be_performed_with_ro_attach
        ...
        then
          Some
            ...
            , [ _ref; Record_util.vdi_operation_to_string op ] )
        else if (* data_destroy first waits for all the VBDs to disappear in its
                ...
                my_has_current_operation_vbd_records <> []
                && op <> `data_destroy
        then Some (Api_errors.other_operation_in_progress, [ "VDI"; _ref ])
        else
          let sm_features =
          ...
          then sm_feature_error
            ...
            let allowed_for_cbt_metadata_vdi =
              ...
              | `update ->
                  true
            in
            if (not allowed_for_cbt_metadata_vdi)
            ...
            then
              ...
              let allowed_when_cbt_enabled =
                ...
                | `update ->
                    true
              in
              if (not allowed_when_cbt_enabled)
...
let update_allowed_operations_internal
    ~__context ~self ~sr_records ~pbd_records ?vbd_records () =
  let pool = Helpers.get_pool ~__context in
  let ha_enabled = Db.Pool.get_ha_enabled ~__context ~self:pool in
  ...
  let allowed =
    if Helpers.rolling_upgrade_in_progress ~__context
    then
      Xapi_stdext_std.Listext.List.intersect
        allowed
...
let update_allowed_operations ~__context ~self : unit =
  update_allowed_operations_internal
    ~__context
    ~self
    ~sr_records:[]
    ~pbd_records:[]
...
let update_vdi_db ~__context ~sr newvdi =
  ...
  | [] ->
      failwith
        (Printf.sprintf
           "newvdi failed to create a VDI for %s"
...
let create
  ...
  then
    info
      "sr:%s vdi:%s requested virtual size %Ld < actual virtual size %Ld"
      (Ref.string_of sR)
...
let introduce
  ...
  let open Storage_access in
  debug
    "introduce uuid=%s name_label=%s sm_config=[ %s ]"
    uuid
    name_label
  ...
  if name_label <> ""
  then Db.VDI.set_name_label ~__context ~self:ref ~value:name_label ;
  if name_description <> ""
  then Db.VDI.set_name_description ~__context ~self:ref ~value:name_description ;
  update_allowed_operations ~__context ~self:ref ;
  ref
...
let snapshot ~__context ~vdi ~driver_params =
  ...
  let newvdi =
    Storage_access.transform_storage_exn (fun () ->
        ...
        with
        | Storage_interface.Storage_error (Unimplemented _) ->
            debug
...
let wait_for_vbds_to_be_unplugged_and_destroyed ~__context ~self ~timeout =
  ...
  let next_token_and_vbds ~token ~timeout =
    ...
    List.iter
      (fun event ->
        debug
          "wait_for_vbds_to_be_unplugged_and_destroyed: got event %s"
  ...
  let rec loop ~token ~remaining_vbds =
    ...
    if remaining_vbds <> [] && Mtime.is_earlier now ~than:finish
    then (
      debug
...
let destroy_and_data_destroy_common
    ~__context ~self ~(operation : [ `destroy | `data_destroy of _ ]) =
...
let _data_destroy ~__context ~self ~timeout =
  ...
  then (
    ...
    try
      destroy_and_data_destroy_common
        ~__context
        ~self
  ...
  Storage_access.transform_storage_exn (fun () ->
      ...
      | Storage_interface.Storage_error (Unimplemented _) ->
          ...
          ( try
              ...
              Db.SR.remove_from_current_operations
                ~__context
                ~self:a.Db_actions.vDI_SR
                ~key:task_id ;
              Xapi_sr_operations.update_allowed_operations
                ~__context
                ~self:a.Db_actions.vDI_SR ;
...
let copy ~__context ~vdi ~sr ~base_vdi ~into_vdi =
  ...
  try
        ...
        Helpers.call_api_functions ~__context (fun rpc session_id ->
            let new_vdi =
              Client.VDI.create
                ...
                ~sm_config:[]
                ~tags:src.API.vDI_tags
            in
            vdi_to_cleanup := Some new_vdi ;
            ( if src.API.vDI_on_boot = `reset
            then
              try
    ...
    if dst_r.API.vDI_virtual_size < src.API.vDI_virtual_size
    then (
      error
        "VDI.copy: cannot copy a VDI (%s) of size %Ld into a VDI (%s) of size \
         %Ld"
        (Ref.string_of vdi)
        src.API.vDI_virtual_size
        (Ref.string_of dst)
    ...
    let base =
      if Db.is_valid_ref __context base_vdi then Some base_vdi else None
    in
  ...
  | e ->
      ...
      | Some vdi ->
          error
            "Caught %s during VDI.copy; cleaning up created VDI %s"
...
let set_metadata_of_pool ~__context ~self ~value =
  if Db.VDI.get_type ~__context ~self = `cbt_metadata
  then (
    error
...
let checksum ~__context ~self =
  ...
  Helpers.call_api_functions ~__context (fun rpc session_id ->
      Sm_fs_ops.with_block_attached_device
        __context
        rpc
        session_id
        self
...
let open_database ~__context ~self =
  ...
  try
    ...
    Xapi_session.create_readonly_session
      ~__context
      ~uname:"disaster-recovery"
      ~db_ref
  with
  | e ->
      ...
      raise
        (Api_errors.Server_error
           (Api_errors.could_not_import_database, [ reason ]) )
...
let change_cbt_status ~__context ~self ~new_cbt_enabled ~caller_name =
  if Db.VDI.get_cbt_enabled ~__context ~self <> new_cbt_enabled
  then (
    let task = Context.get_task_id __context in
    let sr = Db.VDI.get_SR ~__context ~self in
    let sr' =
    ...
    debug
      "%s: Not doing anything, CBT is already %s"
      caller_name
...
let set_cbt_enabled ~__context ~self ~value =
  if Db.VDI.get_cbt_enabled ~__context ~self <> value
  then (
    Db.VDI.set_cbt_enabled ~__context ~self ~value ;
    update_allowed_operations ~__context ~self )
  else
    debug
      "VDI.set_cbt_enabled: Not doing anything, CBT is already %s"
...
let _get_nbd_info ~__context ~self ~get_server_certificate =
  ...
  then (
    error
      "VDI.get_nbd_info: called with a VDI of type cbt_metadata (at %s)"
      __LOC__ ;
    raise
      (Api_errors.Server_error
         ( Api_errors.vdi_incompatible_type
