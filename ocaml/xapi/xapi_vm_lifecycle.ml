...
let allowed_power_states ~__context ~vmr ~(op : API.vm_operations) =
  ...
  | `changing_VCPUs | `changing_static_range | `changing_memory_limits ->
      `Halted
      :: (if vmr.Db_actions.vM_is_control_domain then [ `Running ] else [])
 ...
 *  react helpfully. *)
let check_op_for_feature
    ~__context ~vmr ~vmmr ~vmgmr ~power_state ~op ~ref ~strict =
...
let check_vgpu ~__context ~op ~ref_str ~vgpus ~power_state =
  let is_migratable vgpu =
    try
      ...
      let host_from =
        Db.VGPU.get_VM ~__context ~self:vgpu
        |> fun vm ->
...
let nomigrate ~__context vm metrics =
  try Db.VM_metrics.get_nomigrate ~__context ~self:metrics with
  | _ ->
...
let nested_virt ~__context vm metrics =
  try Db.VM_metrics.get_nested_virt ~__context ~self:metrics with
  | _ ->
      let platformdata = Db.VM.get_platform ~__context ~self:vm in
...
let nvidia_sriov_pcis ~__context vgpus =
  ...
  |> List.filter_map (fun vgpu ->
         Db.VGPU.get_type ~__context ~self:vgpu
         |> fun typ ->
         Db.VGPU_type.get_implementation ~__context ~self:typ
         |> function
         | `nvidia_sriov ->
...
let check_operation_error ~__context ~ref =
  ...
  fun ~op ~strict ->
    ...
    let current_error =
      check current_error (fun () ->
          if is_snapshot then check_snapshot ~vmr ~op ~ref_str else None )
    in
    ...
    let current_error =
      check current_error (fun () ->
          ...
          in
          if (op = `changing_VCPUs || op = `destroy)
...
let force_state_reset_keep_current_operations ~__context ~self ~value:state =
  ...
  then (
    ...
    List.iter
      (fun vgpu ->
        ...
        if List.mem_assoc Xapi_gpumon.Nvidia.key keys
        then (
          warn
            "Found unexpected Nvidia compat metadata on vGPU %s / VM %s"
            (Ref.string_of vgpu)
            (Ref.string_of self) ;
          let v =
  ...
  then (
    ...
    |> List.iter (fun pci ->
           if Db.is_valid_ref __context pci
           then Db.PCI.remove_attached_VMs ~__context ~self:pci ~value:self
           else
    ...
    List.iter
      (fun pci ->
        ...
        if List.mem self (Db.PCI.get_attached_VMs ~__context ~self:pci)
        then Db.PCI.remove_attached_VMs ~__context ~self:pci ~value:self ;
...
let force_state_reset ~__context ~self ~value:state =
  if Db.VM.get_current_operations ~__context ~self <> []
  then Db.VM.set_current_operations ~__context ~self ~value:[] ;
