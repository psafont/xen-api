...
let assert_sr_can_host_statefile ~__context ~sr ~cluster_stack =
  ...
  then (
    error
      "Cannot place statefile in SR %s: some hosts lack a PBD: [ %s ]"
      (Ref.string_of sr)
      (String.concat
         "; "
         (List.map
  ...
  List.iter
    (fun self ->
      ...
      then (
        error
          "Cannot place statefile in SR %s: PBD %s is not plugged"
          (Ref.string_of sr)
          (Ref.string_of self) ;
