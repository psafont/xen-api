...
let host_install_server_certificate fd printer rpc session_id params =
  ...
  in
  let private_key = List.assoc "private_key" params
  |> get_file_or_fail fd "private key"
  in
  let certificate_chain = List.assoc_opt "certificate_chain" params
  ...
  ignore (do_host_op rpc session_id ~multiple:false (fun _ host ->
    ...
    ) params ["certificate"; "private_key"; "certificate_chain"]
  )
