...
let create ~__context ~site ~vIF =
  ...
  then
    if Pvs_proxy_control.start_proxy ~__context vIF pvs_proxy
    then (
      try
          ...
          Helpers.call_api_functions ~__context (fun rpc session_id ->
              ignore
                (Client.Client.Message.create
                   ...
                   ~body ) ) ;
          error
            "Unable to setup PVS proxy for vif %s: %s."
            (Ref.string_of vIF)
...
let destroy ~__context ~self =
  if Db.PVS_proxy.get_currently_attached ~__context ~self
  then (
