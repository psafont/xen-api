...
let get_changes rrd_files =
  List.iter
    (fun filename ->
      ...
      | e ->
          ...
          then (
            error
              "Unable to read memory usage for VM %s: %s"
              filename
  ...
  Mcache.get_updates_map
    ~before:Mcache.vm_memory_cached
      ...
      List.iter
        (fun (vm_uuid, memory) ->
          ...
          | e ->
              keeps := vm_uuid :: !keeps ;
              error
                "Unable to update memory usage for VM %s: %s"
                vm_uuid
                (Printexc.to_string e) )
