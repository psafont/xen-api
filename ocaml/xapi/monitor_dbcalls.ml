  ...
  Server_helpers.exec_with_new_task "updating PIFs" (fun __context ->
          ...
          try
            ...
            let my_bonds =
              List.map
                ...
                my_bond_pifs
            in
            if List.length my_bonds <> 1
            then debug "Error: bond %s cannot be found" bond
            else
              Db.Bond.set_links_up
                ~__context
                ~self:(List.hd my_bonds)
      ...
      List.iter
        (function
          ...
          | e ->
              error
                "pifs_and_memory_update issue: %s"
...
let monitor_dbcall_thread () =
  while true do
    ...
    | e ->
        info
          "monitor_dbcall_thread would have died from: %s; restarting in 30s."
          (ExnHelper.string_of_exn e) ;
        Thread.delay 30.
  done
