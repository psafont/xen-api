...
let handle_request req =
  ...
  | exn ->
      ...
      then
        warn
          "Exception handling remote request %s: %s"
...
let handle_requests () =
  while Mutex.execute request_mutex (fun () -> not !shutting_down) do
    ...
    | exn ->
        error
          "Exception in handle_requests thread!  %s"
          (ExnHelper.string_of_exn exn) ;
        Thread.delay 30.
  done
...
let perform_request
    ~__context ~timeout ~verify_cert ~host ~port ~request ~handler ~enable_log =
...
let read_response result response s =
  try result := Unixext.string_of_fd s with
...
let send_test_post ~__context ~host ~port ~body =
  try
    ...
    perform_request
      ...
      ~verify_cert
      ~host
      ~port:(Int64.to_int port)
      ~request
      ~handler:(read_response result)
      ~enable_log:true ;
    !result
  with
  | Timed_out ->
