...
let permanent_vdi_attach ~__context ~vdi ~reason =
  info
    "permanent_vdi_attach: vdi = %s; sr = %s"
    (Ref.string_of vdi)
...
let permanent_vdi_detach ~__context ~vdi =
  info
    "permanent_vdi_detach: vdi = %s; sr = %s"
    (Ref.string_of vdi)
    (Ref.string_of (Db.VDI.get_SR ~__context ~self:vdi)) ;
  let vdi_uuid = Db.VDI.get_uuid ~__context ~self:vdi in
  log_and_ignore_exn (fun () ->
      ignore
        (Helpers.call_script !Xapi_globs.static_vdis [ "detach"; vdi_uuid ]) ) ;
...
let reattempt_on_boot_attach () =
  ...
  try ignore (Helpers.call_script "/sbin/service" [ script; "start" ]) with
  | e ->
      warn
        "Attempt to reattach static VDIs via '%s start' failed: %s"
        script
        (ExnHelper.string_of_exn e)
