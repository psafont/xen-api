  ...
  let compare x y =
    ...
    then
      ...
      then
        ...
        then compare x.serial y.serial
        else compare x.product_id y.product_id
      else compare x.vendor_id y.vendor_id
    else compare x.path y.path
end
...
let extract_member json member =
  let safe_hd = function
    ...
    | _ ->
        failwith
          (Printf.sprintf
             "Internal error: Json from scan script missing element: %s"
             member )
  in
