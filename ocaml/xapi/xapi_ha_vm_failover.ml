  ...
  let parallel ~__context ~rpc ~session_id lst =
    let rec loop completed lst =
      ...
      | completed, pending, tasks ->
          ...
          debug
            "Waited for %d tasks, evaluating next actions"
            (List.length tasks) ;
          pending
...
let compute_evacuation_plan
    ~__context total_hosts remaining_hosts vms_and_snapshots =
  let hosts =
    List.map
      (fun host ->
        ( host
  ...
  debug
    "Planning configuration for offline agile VMs = %s"
    (Binpack.string_of_configuration
       (fun x ->
         Printf.sprintf
           "%s (%s)"
           (Ref.short_string_of x)
...
let host_of_non_agile_vm ~__context all_hosts_and_snapshots_sorted (vm, snapshot)
  ...
  | (host, host_snapshot) :: _ ->
      ...
      debug
        "Non-agile VM %s (%s) considered pinned to Host %s (%s)"
        (Ref.short_string_of vm)
        snapshot.API.vM_name_label
        (Ref.short_string_of host)
        host_snapshot.API.host_hostname ;
      [ (vm, host) ]
  | [] ->
      warn
        "No host could support protected xHA VM: %s (%s)"
        (Ref.short_string_of vm)
        snapshot.API.vM_name_label ;
      []
  ...
  let live_hosts =
    List.filter
      (fun (rf, r) ->
        r.API.host_enabled
        &&
        try Db.Host_metrics.get_live ~__context ~self:r.API.host_metrics with
        | _ ->
            false )
      all_hosts
  in
...
*)
let compute_restart_plan
    ~__context
    ~all_protected_vms
    ~live_set
    ?(change = no_configuration_change)
    num_failures =
  ...
  let vm_accounted_to_host vm =
    ...
    then Some vm_t.API.vM_resident_on
      ...
      in
      if List.mem scheduled live_hosts then Some scheduled else None
  in
  let string_of_vm vm =
    Printf.sprintf
      "%s (%s)"
  ...
  let hosts_and_memory =
    List.map
      (fun host ->
        ...
        let currently_free =
          Memory_check.host_compute_free_memory_with_policy
            ~__context
            summary
            Memory_check.Static_max
        in
  ...
  in
  debug
    "Restart plan for non-agile offline VMs: [ %s ]"
  ...
  debug
    "Computing a specific plan for the failure of VMs: [ %s ]"
  ...
  debug
    "Restart plan for agile offline VMs: [ %s ]"
  ...
  then
    warn
      "Some protected VMs could not be restarted: [ %s ]"
  ...
  Binpack.check_configuration config ;
  debug
    "Planning configuration for future failures = %s"
    (Binpack.string_of_configuration string_of_host string_of_vm config) ;
...
let plan_for_n_failures
  ...
  | e ->
      error
        "Unexpected error in HA VM failover planning function: %s"
...
let compute_max_host_failures_to_tolerate ~__context ?live_set ?protected_vms ()
  ...
  Helpers.bisect
    (fun n ->
      plan_for_n_failures
        ~__context
        ~all_protected_vms:protected_vms
        ?live_set
        (Int64.to_int n)
      = Plan_exists_for_all_VMs )
    0L
...
let mark_pool_as_overcommitted ~__context ~live_set =
  Xapi_clustering.with_clustering_lock_if_cluster_exists
    ...
    (fun () ->
      ...
      in
      if planned_for <> max_failures
      then (
        Db.Pool.set_ha_plan_exists_for
          ~__context
          ~self:pool
...
let assert_configuration_change_preserves_ha_plan ~__context c =
  debug
    "assert_configuration_change_preserves_ha_plan c = %s"
...
let assert_host_disable_preserves_ha_plan ~__context host =
  assert_configuration_change_preserves_ha_plan
    ~__context
    { no_configuration_change with hosts_to_disable = [ host ] }


let assert_vm_placement_preserves_ha_plan
    ~__context ?(leaving = []) ?(arriving = []) () =
  assert_configuration_change_preserves_ha_plan
    ~__context
...
let restart_auto_run_vms ~__context live_set n =
  ...
  *)
          ...
          then (
            ...
            let shutting_down =
              Xapi_stdext_threads.Threadext.Mutex.execute
                ...
                (fun () -> !Xapi_globs.hosts_which_are_shutting_down)
            in
            ( if not (List.exists (fun x -> x = h) shutting_down)
            then
              let obj_uuid = Db.Host.get_uuid ~__context ~self:h in
              let host_name = Db.Host.get_name_label ~__context ~self:h in
              Xapi_alert.add
                ~msg:Api_messages.ha_host_failed
                ~cls:`Host
                ~obj_uuid
  ...
  let plan, plan_is_complete =
    try
      if Xapi_fist.simulate_planner_failure ()
      then failwith "fist_simulate_planner_failure" ;
      ...
      let plan, config, vms_not_restarted, non_agile_protected_vms_exist =
        ...
        then (
          ...
          ( try Xapi_hooks.pool_pre_ha_vm_restart_hook ~__context with
          | e ->
              error
                "pool-pre-ha-vm-restart-hook failed: %s: continuing anyway"
                (ExnHelper.string_of_exn e) ) ;
          debug
            "Recomputing restart plan to take into account new state of the \
             world after running the script" ;
          compute_restart_plan ~__context ~all_protected_vms ~live_set n )
        else (plan, config, vms_not_restarted, non_agile_protected_vms_exist)
    ...
    | e ->
        error
          "Caught unexpected exception in HA planner: %s"
          (ExnHelper.string_of_exn e) ;
        ([], false)
  in
  ...
  let consider_sending_failed_alert_for vm =
    debug
      "We failed to restart protected VM %s: considering sending an alert"
      (Ref.string_of vm) ;
    if not (Hashtbl.mem restart_failed vm)
    then (
      Hashtbl.replace restart_failed vm () ;
      let obj_uuid = Db.VM.get_uuid ~__context ~self:vm in
      Xapi_alert.add
        ~msg:Api_messages.ha_protected_vm_restart_failed
        ~cls:`VM
        ~obj_uuid
        ~body:"" )
  in
  ...
  Helpers.call_api_functions ~__context (fun rpc session_id ->
      ...
      let restart_vm vm ?host () =
        let go () =
          ( if Xapi_fist.simulate_restart_failure ()
          then
          ...
          then (
            ...
            failwith
              (Printf.sprintf
                 "VM: %s restart attempt delayed for 120s"
                 (Ref.string_of vm) )
        in
        TaskChains.task go
        >>= function
        | Error (Api_errors.Server_error (code, params))
          when code = Api_errors.ha_operation_would_break_failover_plan ->
            ...
            | Error e ->
                error
                  "Caught exception trying to restart VM %s: %s"
                  (Ref.string_of vm)
                  (ExnHelper.string_of_exn e) ;
                TaskChains.fail e )
        | Error e ->
      ...
      *)
          ...
          map_parallel
            ...
            (fun (vm, vmr) ->
              ...
              , if List.mem_assoc vm plan
                ...
                else TaskChains.fail (Failure "VM has no plan") ) )
            all
      in
      ...
      let gc_table tbl =
        ...
        List.iter
          (fun vm ->
            if not (List.mem vm vms_we_know_about)
            then (
        ...
        (fun vm ->
          ...
          , if Db.VM.get_power_state ~__context ~self:vm = `Halted
            ...
            then
              TaskChains.task (fun () ->
                  Client.Client.Async.VM.start rpc session_id vm false true )
            else TaskChains.ok Rpc.Null ) )
        !reset_vms
      |> List.iter (fun (vm, result) ->
             match result with
             | Error e ->
                 error
                   "Failed to restart best-effort VM %s (%s): %s"
