...
let has_vbd_leaked __context vbd =
  ...
  then (
    info
      "Ignoring orphaned disk attached to control domain (device = %s)"
      device ;
    false )
  else
    let has_valid_task =
    ...
    let has_valid_related =
      ...
      try Db.VBD.get_currently_attached ~__context ~self:related with
      | _ ->
          false
      (* VBD record gone *)
    in
