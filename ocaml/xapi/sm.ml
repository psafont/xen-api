...
let vdi_create
  ...
  debug
    ...
    driver
    (sprintf
       "sr=%s sm_config=[%s] type=[%s] size=%Ld"
       (Ref.string_of sr)
...
let vdi_introduce dconf driver sr new_uuid sm_config location =
  debug
    ...
    driver
    (sprintf
       "sr=%s new_uuid=%s sm_config=[%s] location=%s"
       (Ref.string_of sr)
       new_uuid
       (String.concat "; " (List.map (fun (k, v) -> k ^ "=" ^ v) sm_config))
       location ) ;
...
let vdi_attach dconf driver sr vdi writable =
  ...
  try Sm_exec.parse_attach_result result with
  | _ ->
...
let vdi_list_changed_blocks dconf driver sr ~vdi_from ~vdi_to =
  debug
    ...
    driver
    (sprintf
       "sr=%s vdi_from=%s vdi_to=%s"
       (Ref.string_of sr)
       (Ref.string_of vdi_from)
