...
module Ip = struct
  ...
  let rec first a b f =
    if gt a b then None else if f a then Some a else first (succ a) b f
end
  ...
  let update_network net =
    ...
    List.iter
      (fun new_lease_vif ->
        ...
        in
        Db.Network.add_to_assigned_ips
          ~__context
          ~self:(Ref.of_string net)
          ~key:new_lease_vif
          ~value:(Ip.string_of lease.ip) )
      new_lease_vifs ;
    List.iter
      (fun released_lease_vif ->
        Db.Network.remove_from_assigned_ips
          ~__context
          ~self:(Ref.of_string net)
...
module Udhcpd_conf = struct
  ...
  let to_string ~__context t =
    ...
    let other_config = Db.Network.get_other_config ~__context ~self:network in
    let include_gw =
      try not (List.assoc ip_disable_gw_key other_config = "true") with
      | Not_found ->
          true
    in
    let include_pxe =
      try List.assoc pxe_server_key other_config = "true" with
      | Not_found ->
          false
    in
    let config_list =
...
let restart_nolock () =
  ...
  let rec wait_for_pid n =
    ...
    | None ->
        if n > 0 && n mod 10 = 0
        then debug "Continuing to wait for the pidfile from udhcpd" ;
        Thread.delay 0.1 ;
        wait_for_pid (n + 1)
  in
  wait_for_pid 0
...
let gc_leases_nolock ~__context =
  ...
  List.iter
    (fun l ->
      info
        "Host internal management network removing lease for VIF %s -> %s"
        l.vif
        (Ip.string_of l.ip) )
    bad ;
  assigned := good
...
let maybe_add_lease_nolock ~__context vif =
  ...
  then (
    let other_config = Db.Network.get_other_config ~__context ~self:network in
    if (not (List.mem_assoc ip_begin_key other_config))
    ...
    | Some l ->
        info
          "VIF %s on host-internal management network already has lease: %s"
          (Ref.string_of vif)
          (Ip.string_of l.ip) ;
        restart_nolock ()
    | None ->
