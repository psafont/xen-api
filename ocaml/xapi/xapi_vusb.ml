...
let create ~__context ~vM ~uSB_group ~other_config =
  ...
  Mutex.execute m (fun () ->
      ...
      if List.length attached_vusbs > 5
      then raise (Api_errors.Server_error (Api_errors.too_many_vusbs, [ "6" ])) ;
...
let destroy ~__context ~self =
  ...
  if r.Db_actions.vUSB_currently_attached
  then
    raise
      (Api_errors.Server_error
         ( Api_errors.operation_not_allowed
         , [ Printf.sprintf
               "VUSB '%s' still attached to '%s'"
