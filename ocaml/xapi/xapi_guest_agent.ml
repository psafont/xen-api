...
let networks path vif_type (list : string -> string list) =
    ...
    let extract_network_keys eth =
      ...
      let rec extract prefixes eth =
        ...
        | [] ->
            None
        | prefix :: rest ->
            if Astring.String.is_prefix ~affix:prefix eth
            then
              let n = string_after_prefix ~prefix eth in
              Some (extend path eth, n)
            else extract rest eth
      in
  ...
  in
  let cmp a b =
    try compare (int_of_string a) (int_of_string b) with
    | Failure _ ->
        error
          "String (\"%s\" or \"%s\") can't be converted into an integer as \
           index of IP"
          a
          b ;
...
let get_initial_guest_metrics
    (lookup : string -> string option) (list : string -> string list) =
  ...
  let pv_drivers_version =
    if pv_drivers_version = [] || List.mem_assoc "micro" pv_drivers_version
    then pv_drivers_version (* already there; do nothing *)
...
let create_and_set_guest_metrics
...
let all
  ...
  let guest_metrics_cached =
    Mutex.execute mutex (fun () ->
        try Hashtbl.find cache domid with
        | _ ->
            ...
            in
            if live
            then dead_domains := IntSet.remove domid !dead_domains
            else dead_domains := IntSet.add domid !dead_domains ;
            { pv_drivers_version = []
            ; os_version = []
            ; networks = []
            ; other = []
  ...
  then (
    ...
    if guest_metrics_cached.pv_drivers_version <> pv_drivers_version
    then
      Db.VM_guest_metrics.set_PV_drivers_version
        ~__context
        ~self:gm
        ~value:pv_drivers_version ;
    if guest_metrics_cached.os_version <> os_version
    then
      Db.VM_guest_metrics.set_os_version ~__context ~self:gm ~value:os_version ;
    if guest_metrics_cached.networks <> networks
    then Db.VM_guest_metrics.set_networks ~__context ~self:gm ~value:networks ;
    if guest_metrics_cached.other <> other
    then (
    ...
    if guest_metrics_cached.can_use_hotplug_vbd <> can_use_hotplug_vbd
    then
      Db.VM_guest_metrics.set_can_use_hotplug_vbd
        ~__context
        ~self:gm
        ~value:can_use_hotplug_vbd ;
    if guest_metrics_cached.can_use_hotplug_vif <> can_use_hotplug_vif
    then
      Db.VM_guest_metrics.set_can_use_hotplug_vif
        ~__context
        ~self:gm
    ...
    Db.VM_guest_metrics.set_last_updated
      ~__context
      ~self:gm
      ~value:(Date.of_float last_updated) ;
    if guest_metrics_cached.device_id <> device_id
    then
      if List.mem_assoc Xapi_globs.device_id_key_name device_id
      then (
        let value = List.assoc Xapi_globs.device_id_key_name device_id in
        let platform = Db.VM.get_platform ~__context ~self in
        info
          "Updating VM %s platform:%s <- %s"
          (Ref.string_of self)
          Xapi_globs.device_id_key_name
          value ;
        ( if List.mem_assoc Xapi_globs.device_id_key_name platform
        then
          try
    ...
    ( if Xapi_pv_driver_version.is_windows_and_orlando_or_newer gmr
    then
      let platform = Db.VM.get_platform ~__context ~self in
      if not (List.mem_assoc Xapi_globs.viridian_key_name platform)
      then (
        info
          "Setting VM %s platform:%s <- %s"
          (Ref.string_of self)
          Xapi_globs.viridian_key_name
