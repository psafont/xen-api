...
let reset_vms_running_on_missing_hosts ~__context =
  List.iter
    (fun vm ->
      ...
      in
      if (not valid_resident_on) && vm_r.API.vM_power_state = `Running
      then (
        let msg =
          Printf.sprintf
...
let create_tools_sr
    __context name_label name_description sr_introduce maybe_create_pbd =
  ...
  in
  let destroy self =
    try Db.SR.destroy ~__context ~self with
    | e ->
        warn
          "failed to destroy redundant tools SR %s: %s"
          (Ref.string_of self)
...
let ensure_vm_metrics_records_exist __context =
  List.iter
    (fun vm ->
      ...
      then (
        info
          "Regenerating missing VM_metrics record for VM %s"
          (Ref.string_of vm) ;
        let m = Ref.make () in
        ...
        Db.VM_metrics.create
          ...
          ~install_time:Xapi_stdext_date.Date.never
          ~state:[]
...
let ensure_vm_metrics_records_exist_noexn __context =
  Helpers.log_exn_continue
    "ensuring VM_metrics flags exist"
    ensure_vm_metrics_records_exist
    __context
