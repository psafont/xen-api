...
let read_backups () =
  ...
  | e ->
      ...
      raise
        Api_errors.(Server_error (internal_error, [ "failed to read backups" ]))
...
let load_psr_pool_secrets () =
  ...
  | false, true | true, false ->
      D.error
        "expected both files to exist: %s , %s"
        old_pool_secret_backup_path
        new_pool_secret_backup_path ;
      raise
        Api_errors.(
  ...
  | true, true ->
      ...
      if SecretString.equal (Xapi_globs.pool_secret ()) old_pool_secret
      then Xapi_globs.pool_secrets := [ old_pool_secret; new_pool_secret ]
      else Xapi_globs.pool_secrets := [ new_pool_secret; old_pool_secret ]
