...
let create_session_permissions_tbl ~session_id ~rbac_permissions =
  if use_efficient_permission_set && Pool_role.is_master ()
     (* Create this structure on the master only, *)
     (* so as to avoid heap-leaking on the slaves *)
  then (
    debug
      "Creating permission-set tree for session %s"
...
let destroy_session_permissions_tbl ~session_id =
  ...
  then Hashtbl.remove session_permissions_tbl session_id
...
let permission_of_action ?args ~keys _action =
  ...
  then
    ...
    (* only actions with rbac-guarded map keys fall here *)
    match args with
    | None ->
                  ...
                  | Rpc.String key_name_in_args ->
                    ( try
                        ...
                        let key_name =
                          List.find
                            (fun key_name ->
                              if Astring.String.is_suffix ~affix:"*" key_name
                              then
...
let is_permission_in_session ~session_id ~permission ~session =
  ...
  then
    ...
    let permission_tree =
      try Some (Hashtbl.find session_permissions_tbl session_id) with
      | Not_found ->
    ...
    (* use linear look-up of permissions *)
    find_linear permission session.API.session_rbac_permissions
...
let is_access_allowed ~__context ~session_id ~permission =
  ...
  then true (* normal user session *)
    ...
    else is_permission_in_session ~session_id ~permission ~session
...
let check
  ...
  then (
    ...
    | error ->
        ...
        Rbac_audit.allowed_post_fn_error
          ~__context
          ~session_id
          ~action
          ~permission
    ...
    let allowed_roles_string =
      ...
      | e ->
          debug
            "Could not obtain allowed roles for %s (%s)"
            permission
...
let get_session_of_context ~__context ~permission =
  try Context.get_session_id __context with
  | Failure _ ->
      raise
        (Api_errors.Server_error
