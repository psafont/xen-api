...
let valid_operations ~__context record _ref' =
  ...
  let set_errors
    ...
    List.iter
      (fun op ->
        ...
        then Hashtbl.replace table op (Some (code, params)) )
      ops
  in
  ...
  if List.mem `apply_updates current_ops
  then
    set_errors
      Api_errors.other_operation_in_progress
  ...
  ( try
      if Db.Host_metrics.get_live
           ~__context
           ~self:record.Db_actions.host_metrics
  ...
  | _ ->
      set_errors
        Api_errors.xenapi_missing_plugin
        [ Constants.power_on_plugin ]
        [ `power_on ] ) ;
  ...
  in
  if plugged_clustered_srs <> []
  then (
    let hosts_down =
      Db.Host_metrics.get_refs_where
        ~__context
...
let with_host_operation ~__context ~self ~doc ~op f =
  ...
  (* Then do the action with the lock released *)
      ...
      try
        ...
        else update_allowed_operations ~__context ~self
      with
      | _ ->
          () )
...
let mark_host_as_dead ~__context ~host ~reason =
  let done_already =
    Mutex.execute Xapi_globs.hosts_which_are_shutting_down_m (fun () ->
        if List.mem host !Xapi_globs.hosts_which_are_shutting_down
        then true
        else (
  ...
  then (
    ...
    | e ->
        info
          "Caught and ignoring exception setting host %s to dead: %s"
          (Ref.string_of host)
...
let assert_startup_complete () =
  Mutex.execute startup_complete_m (fun () ->
      ...
      then raise (Api_errors.Server_error (Api_errors.host_still_booting, [])) )
...
let consider_enabling_host_nolock ~__context =
  ...
  let all_pbds_ok =
    List.fold_left
      ...
      (List.map
         ...
         pbds )
  in
  if (not !user_requested_host_disable)
  ...
  then (
    ...
    Db.Host.remove_pending_guidances
      ~__context
      ~self:localhost
    ...
    then (
      ...
      Local_work_queue.start_vm_lifecycle_queue () )
              ...
              bool_of_string (Localdb.get Constants.host_disabled_until_reboot)
            with
            | _ ->
                false
    then
    ...
    if Db.Host.get_enabled ~__context ~self:localhost
...
module Configuration = struct
  let make_set_initiator_args iqn hostname =
    ...
    let hostname_chopped =
      if String.length hostname > 30 then String.sub hostname 0 30 else hostname
    in
  ...
  let set_multipathing enabled =
    ...
    if enabled then Unixext.touch_file flag else Unixext.unlink_safe flag
    ...
    let loop (token, was_in_rpu) =
      Helpers.call_api_functions ~__context (fun rpc session_id ->
          ...
          let check_host (host_ref, host_rec) =
            ...
            let multipathing =
              try
                Some (List.assoc "multipathing" oc |> Stdlib.bool_of_string)
              with
              | _ ->
                  None
            in
            ...
            | Some multipathing
              when multipathing <> host_rec.API.host_multipathing ->
                Client.Client.Host.set_multipathing
                  rpc
                  session_id
                  host_ref
                  multipathing
    ...
    Thread.create
      (fun () ->
        ...
        while true do
          ...
          | e ->
              error
                "Caught exception in Configuration.start_watcher_thread: %s"
