...
let get_template_record rpc session_id vm =
  ...
  let disks =
    if List.mem_assoc disks_key other_config
    then disks_of_xml (Xml.parse_string (List.assoc disks_key other_config))
    else []
  in
  let script =
    if List.mem_assoc post_install_key other_config
    then Some (List.assoc post_install_key other_config)
    else None
  in
  if disks = [] && script = None
  then None
  else Some { disks; post_install_script = script }
...
let create_disk rpc session_id vm sm_config disk =
  let sr =
    try Client.SR.get_by_uuid rpc session_id disk.sr with
    | _ ->
        D.error "Unable to find SR (uuid: %s) to provision the disk" disk.sr ;
        raise
          (Api_errors.Server_error (Api_errors.uuid_invalid, [ "sr"; disk.sr ]))
  in
...
let post_install rpc session_id vm =
  ...
  ( try Client.VM.remove_from_other_config rpc session_id vm disks_key with
  | _ ->
      () ) ;
  try Client.VM.remove_from_other_config rpc session_id vm post_install_key with
  | _ ->
      ()
