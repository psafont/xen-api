...
let version_of_rpc = function
  ...
  | rpc ->
      raise
        (Failure
           (Printf.sprintf
              "version_of_rpc: malformed RPC %s"
              (Rpc.to_string rpc) ) )
...
let assert_compatible ~__context other_version =
  ...
  if this_version.xapi_vsn_major <> other_version.xapi_vsn_major
...
let remote_metadata_export_import
    ~__context ~rpc ~session_id ~remote_address ~restore which =
      ...
      in
        ...
        (fun () ->
                       ...
                       (with_http put (fun (_, ofd) ->
                            let (n : int64) =
                              Xapi_stdext_unix.Unixext.copy_file
                                ?limit:r.Http.Response.content_length
                                ifd
                                ofd
                            in
          ...
          | `success ->
              ...
              ( try result |> Xmlrpc.of_string |> API.ref_VM_set_of_rpc with
              | parse_error ->
                  raise
                    Api_errors.(
                      Server_error
...
let base_vdi_of_req ~__context (req : Http.Request.t) =
  let all = req.Http.Request.query @ req.Http.Request.cookie in
  if List.mem_assoc "base" all
  then
    let base = List.assoc "base" all in
    Some
      ( if Db.is_valid_ref __context (Ref.of_string base)
...
let sr_of_req ~__context (req : Http.Request.t) =
  ...
  else if List.mem_assoc "sr_uuid" all
