...
let with_cluster_host_operation
    ~__context ~(self : [ `Cluster_host ] API.Ref.t) ~doc ~op ?policy f =
  let task_id = Ref.string_of (Context.get_task_id __context) in
  Helpers.retry_with_global_lock ~__context ~doc ?policy (fun () ->
      assert_operation_valid ~__context ~self ~op ;
      Db.Cluster_host.add_to_current_operations
        ~__context
        ~self
        ~key:task_id
        ~value:op ;
  ...
  (* Then do the action with the lock released *)
      ...
      try
        Db.Cluster_host.remove_from_current_operations
          ~__context
          ~self
          ~key:task_id ;
        update_allowed_operations ~__context ~self ;
