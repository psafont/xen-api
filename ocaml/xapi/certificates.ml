...
let pem_of_string x =
  ...
  | Error _ ->
      ...
      raise
        Api_errors.(
          Server_error (invalid_value, [ "certificate"; "<omitted>" ]))
  | Ok x ->
      x
...
let not_safe_chars name =
  ...
  let rec f i =
    if i = n then false else if not (safe_char name.[i]) then true else f (i + 1)
  in
  f 0
...
let host_install kind ~name ~cert =
  if is_unsafe kind name then raise_name_invalid kind name ;
...
let host_uninstall kind ~name =
  if is_unsafe kind name then raise_name_invalid kind name ;
  let filename = library_filename kind name in
  if not (Sys.file_exists filename) then raise_does_not_exist kind name ;
...
let get_cert kind name =
  if is_unsafe kind name then raise_name_invalid kind name ;
  let filename = library_filename kind name in
  try Unixext.string_of_file filename with
  | e ->
      warn
        "Exception reading %s %s: %s"
        (to_string kind)
        name
        (ExnHelper.string_of_exn e) ;
      raise_corrupt kind name
...
let sync_all_hosts ~__context hosts =
  ...
  Helpers.call_api_functions ~__context (fun rpc session_id ->
      List.iter
        (fun host ->
          try Client.Host.certificate_sync rpc session_id host with
          | e ->
              exn := Some e )
        hosts ) ;
...
let sync_certs_crls
    kind list_func install_func uninstall_func ~__context master_certs host =
  Helpers.call_api_functions ~__context (fun rpc session_id ->
      let host_certs = list_func rpc session_id host in
      List.iter
        (fun c ->
          if not (List.mem c master_certs)
          then uninstall_func rpc session_id host c )
        host_certs ;
      List.iter
        (fun c ->
          if not (List.mem c host_certs)
          then install_func rpc session_id host c (get_cert kind c) )
        master_certs )
...
let pool_install kind ~__context ~name ~cert =
  ...
  try pool_sync ~__context with
  | exn ->
      ( try host_uninstall kind ~name with
      | e ->
          warn
            "Exception unwinding install of %s %s: %s"
            (to_string kind)
            name
            (ExnHelper.string_of_exn e) ) ;
      raise exn
...
let install_server_certificate
    ?(pem_chain = None) ~pem_leaf ~pkcs8_private_key ~path =
  let installation =
