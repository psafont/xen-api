...
let with_clustering_lock where f =
  ...
  Locking_helpers.Named_mutex.execute clustering_lock_m (fun () ->
      Xapi_stdext_pervasives.Pervasiveext.finally
        (fun () ->
          debug
            "Grabbed host-local clustering lock; executing function... (%s)"
            where ;
          f () )
        (fun () ->
          debug
...
let pif_of_host ~__context (network : API.ref_network) (host : API.ref_host) =
  ...
  | _ ->
      let msg =
        Printf.sprintf
          "No PIF found for host:%s and network:%s"
          (Ref.string_of host)
...
let assert_pif_prerequisites pif =
  ...
  let assert_pif_permaplugged (pif_ref, record) =
    if not record.API.pIF_disallow_unplug
    then
      raise
        Api_errors.(Server_error (pif_allows_unplug, [ Ref.string_of pif_ref ])) ;
    if not record.pIF_currently_attached
    then
      raise
        Api_errors.(
...
let assert_pif_attached_to ~__context ~host ~pIF =
  if not (List.mem pIF (Db.Host.get_PIFs ~__context ~self:host))
  then
    raise
      Api_errors.(
        Server_error
...
let assert_cluster_host_can_be_created ~__context ~host =
  match
    Db.Cluster_host.get_refs_where
      ~__context
...
let assert_cluster_host_enabled ~__context ~self ~expected =
  ...
  then
    ...
    | true ->
        raise
          Api_errors.(
            Server_error (clustering_disabled, [ Ref.string_of self ]))
    | false ->
        raise
...
let assert_operation_host_target_is_localhost ~__context ~host =
  if host <> Helpers.get_localhost ~__context
  then
    raise
      Api_errors.(
        Server_error
...
let assert_cluster_host_quorate ~__context ~self =
   ...
   * *)
  let result =
    Cluster_client.LocalClient.diagnostics
      (rpc ~__context)
  ...
  | Ok diag ->
      debug
        "Local cluster host is quorate: %b"
        diag.Cluster_interface.is_quorate ;
      if not diag.Cluster_interface.is_quorate
      then
        raise
          Api_errors.(
