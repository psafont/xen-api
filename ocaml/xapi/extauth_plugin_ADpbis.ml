...
module D = Debug.Make (struct
  let name = "extauth_plugin_ADpbis"
end)
...
let start_damon () =
  try Lwsmd.start ~timeout:5. ~wait_until_success:true with
  | _ ->
      raise
...
module AuthADlw : Auth_signature.AUTH_MODULE = struct
  ...
  let pbis_common_with_password
      (password : string) (pbis_cmd : string) (pbis_args : string list) =
    let debug_cmd =
    ...
    | Forkhelpers.Spawn_internal_error (stderr, stdout, Unix.WEXITED n) ->
        error
          "execute %s exited with code %d [stdout = '%s'; stderr = '%s']"
          debug_cmd
          n
          stdout
          stderr ;
  ...
  let pbis_common
      ?(stdin_string = "") (pbis_cmd : string) (pbis_args : string list) =
    let debug_cmd =
      pbis_cmd ^ " " ^ List.fold_left (fun p pp -> p ^ " " ^ pp) " " pbis_args
    in
    let debug_cmd =
      if String.has_substr debug_cmd "--password"
      then "(omitted for security)"
      else debug_cmd
    in
    ...
    let unlink_file filename =
      if List.mem filename !files_to_unlink
      then (
    ...
    finally_finalize (fun () ->
        let _ =
          try
            ...
            in
              ...
              (fun () ->
                debug
                  "Created process pid %s for cmd %s"
                ...
                | e ->
                    ...
                    debug
                      "Error writing to stdin for cmd %s: %s"
                      debug_cmd
                      (ExnHelper.string_of_exn e) ;
                    raise
                      (Auth_signature.Auth_service_error
        ...
        then (
          error
            "execute '%s': exit_code=[%d] output=[%s]"
            debug_cmd
            !exited_code
            (String.replace "\n" ";" !output) ;
          ...
          in
          debug
            "Pbis raised an error for cmd %s: (%s) %s"
            debug_cmd
            errcode
            errmsg ;
          match errcode with
  ...
  *)
  ...
  let get_full_subject_name ?(use_nt_format = true) subject_name =
    ...
    | Not_found ->
      ( try
        ...
        with
      | Not_found ->
          if (* if neither the UPN separator @ nor the NT username separator \ was found *)
             use_nt_format
          then
  ...
  let pbis_get_sid_bygid gid =
    ...
    if List.mem_assoc "SID" subject_attrs
    then List.assoc "SID" subject_attrs (* OK, return SID *)
    else
  ...
  let pbis_get_sid_byname _subject_name cmd =
    ...
    then List.assoc "SID" subject_attrs (* OK, return SID *)
      ...
      let msg =
        Printf.sprintf
          "Pbis didn't return an SID field for user %s"
          subject_name
      in
  ...
  *)
  let query_subject_information subject_identifier =
    let unmap_lw_space_chars lwname =
      ...
      String.iteri
        (fun i c ->
          if c = current_lw_space_replacement
          then Bytes.set defensive_copy i ' '
          else () )
        lwname ;
    ...
    then
      ...
      ; ( "subject-displayname"
        , if subject_gecos = "" || subject_gecos = "<null>"
          then subject_name
          else subject_gecos )
  ...
  *)
  let query_group_membership subject_identifier =
    ...
    if List.assoc "subject-is-group" subject_info = "true"
    ...
    then
      ...
      in
      debug
        "Resolved %i group sids for subject %s (%s): %s"
  ...
  let _is_pbis_server_available max_tries =
    ...
    let try_clear_cache () =
      ...
      | full_username ->
        ...
        | exception e ->
            debug
              "Failed to remove user %s from cache: %s"
              full_username
              (ExnHelper.string_of_exn e) ;
            Error () )
    in
      ...
      then
        raise
          (Auth_signature.Auth_service_error
             ( Auth_signature.E_GENERIC
             , Printf.sprintf
                 "hostname '%s' cannot contain only digits."
                 hostname ) )
      else
        let domain =
          let service_name =
            Server_helpers.exec_with_new_task
              ...
              (fun __context ->
                ...
                Db.Host.get_external_auth_service_name ~__context ~self:host )
          in
          if List.mem_assoc "domain" config_params
        ...
        let ou_conf, ou_params =
          if List.mem_assoc "ou" config_params
          then
        ...
        try
          ...
          if not (is_pbis_server_available max_tries)
          then (
            let errmsg =
              Printf.sprintf
          ...
          Server_helpers.exec_with_new_task
            "storing external_auth_configuration"
            (fun __context ->
              let host = Helpers.get_localhost ~__context in
              Db.Host.set_external_auth_configuration
                ~__context
                ~self:host
                ~value:extauthconf ;
              debug
                "added external_auth_configuration for host %s"
                (Db.Host.get_name_label ~__context ~self:host) ) ;
          Xapi_stdext_threads.Threadext.Mutex.execute
            cache_of_pbis_common_m
        ...
        | e ->
            ...
            debug
              "Error enabling external authentication for domain %s and user \
               %s: %s"
              domain
              user
              (ExnHelper.string_of_exn e) ;
  ...
  let on_disable config_params =
    ...
    let pbis_failure =
      ...
      | e ->
          ...
          debug
            "Internal Pbis error when disabling external authentication: %s"
            (ExnHelper.string_of_exn e) ;
    ...
    Server_helpers.exec_with_new_task
      ...
      (fun __context ->
        ...
        debug
          "removed external_auth_configuration for host %s"
          (Db.Host.get_name_label ~__context ~self:host) ) ;
