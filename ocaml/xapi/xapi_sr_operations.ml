...
let features_of_sr_internal ~__context ~_type =
  ...
  | (_, sm) :: _ ->
      List.filter_map
        (fun (name, v) ->
          try Some (List.assoc name Smint.string_to_capability_table, v) with
          | Not_found ->
              None )
...
let valid_operations ~__context ?op record _ref' : table =
  ...
  let set_errors
      (code : string) (params : string list) (ops : API.storage_operations_set)
      =
    List.iter
      (fun op ->
  ...
  *)
  ...
  let check_any_attached_pbds ~__context record =
    ...
    then set_errors Api_errors.sr_has_pbd [ _ref ] [ `destroy; `forget ]
    else ()
  in
  let check_no_pbds ~__context record =
    (* If the SR has no PBDs, destroy is not allowed. *)
    if Db.SR.get_PBDs ~__context ~self:_ref' = []
    then set_errors Api_errors.sr_no_pbds [ _ref ] [ `destroy ]
  in
  let check_any_managed_vdis ~__context record =
    ...
    if List.exists
         ...
         vdis
    then set_errors Api_errors.sr_not_empty [] [ `destroy ]
  in
  ...
  let check_cluster_stack_compatible ~__context record =
    ...
    with
    | Api_errors.Server_error (e, args) ->
...
let sr_health_check ~__context ~self =
  if Helpers.i_am_srmaster ~__context ~sr:self
  then
    ...
    in
    if info.Storage_interface.clustered
