...
let compare_dates
    (a : Xapi_stdext_date.Date.iso8601 option)
...
let get_lowest_edition_with_expiry ~__context ~hosts ~edition_to_int =
  ...
  let pool_edition, _ =
    List.filter_map
      (fun (edition, _) ->
        if List.mem_assoc edition edition_to_int
        then Some (edition, List.assoc edition edition_to_int)
        else None )
