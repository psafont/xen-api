...
let string_of_origin = function
  | Http (req, fd) ->
      ...
      Printf.sprintf
        "HTTP request from %s with User-Agent: %s"
        peer
...
let default_database () =
  if Pool_role.is_master () then Db_backend.make () else Db_ref.Remote
...
let _clean_addr_of_string ip =
  ...
  ( if Astring.String.is_prefix ~affix:ipv4_affix ip
  then Astring.String.drop ~max:(String.length ipv4_affix) ip
  else ip )
  |> Ipaddr.of_string
...
let from_forwarded_task
    ?(http_other_config = []) ?session_id ?(origin = Internal) task_id =
...
let make
  ...
  then
    ...
    info
      "task %s%s created%s%s"
      (* CP-982: promote tracking debug line to info status *) dbg
      task_uuid
      (trackid_of_session ~with_brackets:true ~prefix:" " session_id)
...
let of_http_req
  ...
  let new_task_context () =
    ...
    make
      ...
      label
  in
  if internal_async_subtask
  then new_task_context ()
  else
