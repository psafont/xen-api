    ...
    let extras =
      ...
      while !pos < String.length extra_args do
        ...
        then (
          incr pos ;
          i := !pos )
        else if !i >= String.length extra_args
...
let with_open_tcp_ssl server f =
  ...
  Stunnel.with_connect
    ...
    ~verify_cert:None
    ~extended_diagnosis:(!debug_file <> None)
    server
    port
  @@ fun x ->
...
let with_open_tcp server f =
  if !xeusessl && not (is_localhost server)
  then (* never use SSL on-host *)
...
let with_open_channels f =
  ...
  let result =
    ...
    else with_open_tcp !xapiserver wrap
  in
...
let copy_with_heartbeat ?(block = 65536) in_ch out_ch heartbeat_fun =
  ...
  while not !finish do
    ...
    if bytes <> 0
    then output out_ch buf 0 bytes
    else (
      flush out_ch ;
      finish := true ) ;
...
let main_loop ifd ofd permitted_filenames =
  ...
  let msg =
    Printf.sprintf
      "Server has protocol version %d.%d. Client has %d.%d"
      major'
      minor'
      major
      minor
  ...
  in
  let heartbeat_fun =
    if with_heartbeat then fun () -> marshal ofd (Response Wait) else ignore
  in
  ...
  while !exit_code = None do
    ...
    *)
    while
      match Unix.select [ ifd ] [] [] 5.0 with
      | _ :: _, _, _ ->
          false
      | _ ->
          true
    ...
    | Command (Load x) ->
        ...
        ( try
            ...
            while !left > 0 do
              ...
              Xapi_stdext_unix.Unixext.really_write
                ofd
      ...
      ( try
          let rec doit url =
            ...
            | 200 ->
                let file_ch =
                  if filename = ""
                  then Unix.out_channel_of_descr (Unix.dup Unix.stdout)
                  else (
                    assert_filename_permitted
                      ~permit_cwd:true
                      permitted_filenames
                      filename ;
                    try
                    ...
                    with
                    | e ->
                        raise (ClientSideError (Printexc.to_string e)) )
                in
...
let main () =
  ...
  List.iter
    (fun (x, ic, oc) ->
      close_out_noerr oc ;
      close_in_noerr ic ;
      if Sys.file_exists x.Stunnel.logfile
      then (
        if !exit_status <> 0
        then (
          debug "\nStunnel diagnosis:\n\n" ;
          try Stunnel.diagnose_failure x with
          | e ->
              debug "%s\n" (Printexc.to_string e) ) ;
