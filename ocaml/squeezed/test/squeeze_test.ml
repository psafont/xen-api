...
class virtual vm initial_domain =
  object (self)
    ...
    method set_target new_target_kib =
      ...
      then
        debug
          "domid %d: target change was %Ld now %Ld (max %Ld)"
          domain.domid
          domain.target_kib
          new_target_kib
  ...
  object
    ...
    method compute_memory_actual_delta time_passed =
      compute_memory_actual_delta
        domain
        balloon_rate_kib_per_unit_time
        time_passed
  end
...
class idealised_vm_with_limit
  initial_domain balloon_rate_kib_per_unit_time minimum_memory =
  object
    inherit vm initial_domain
...
class intermittently_stuck_vm
  initial_domain balloon_rate_kib_per_unit_time interval =
  object
    inherit vm initial_domain
    ...
    method compute_memory_actual_delta time_passed =
      ...
      in
      debug
        "useful_time = %.2f (current actual = %Ld; target = %Ld)"
        useful_time
...
let simulate scenario =
  ...
  let execute_action (action : action) =
    ...
    if action.new_target_kib > domain#get_domain.memory_max_kib
    then (
  ...
  Xapi_stdext_pervasives.Pervasiveext.finally
    (fun () ->
      ...
      debug
        "%s: attempting to free %Ld KiB"
        scenario.name
        scenario.required_mem_kib ;
      Squeeze.change_host_free_memory
        ~fistpoints:scenario.fistpoints
        io
        scenario.required_mem_kib
        (fun x -> x >= scenario.required_mem_kib) ;
      debug
        "%s: %Ld KiB of memory has been freed"
        scenario.name
        scenario.required_mem_kib ;
      ...
      Squeeze.change_host_free_memory
        ...
        (fun x -> x <= 32L) ;
      debug
        "%s: After rebalancing only %Ld KiB of memory is used"
        scenario.name
        !host_free_mem_kib ;
