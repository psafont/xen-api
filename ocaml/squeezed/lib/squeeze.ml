...
let direction_of_actual inaccuracy_kib memory_actual_kib target_kib =
  ...
  if abs delta_kib <= inaccuracy_kib
  then None
  else if target_kib > memory_actual_kib
  then Some Down
  else Some Up
...
module Proportional = struct
  ...
  let constrain minimum maximum value =
    ...
    then maximum
    else if classify_float value = FP_nan
    then minimum
    else (* arbitrary *)
      value
  ...
  (* Given a set of domains and a host free memory target, return balloon target
     adjustments *)
  let compute_target_adjustments
      ?fistpoints verbose (host : host) host_target_kib =
...
module Squeezer = struct
  ...
  let make () =
    { stuckness = Stuckness_monitor.make (); non_active_domids = [] }
  ...
  let one_iteration
    ...
    let targets =
      Policy.compute_target_adjustments
        ~fistpoints
        verbose
    ...
    then
      debug
        ...
        ( if success
        then " OK"
        else if target_too_big
        then " cannot free enough"
        else if cant_allocate_any_more
        then " cannot allocate enough"
        else "" )
    ...
    let action =
      ...
      then Success
      else Failed non_active_can_balloon_domids
    in
...
module Gnuplot = struct
  ...
  let write_gp stem host cols =
    ...
    Printf.fprintf
      oc
      "plot \"%s.dat\" using 1:2 title \"free host memory\" with lines "
      stem ;
    let col = ref 3 in
    List.iter
      (fun domain ->
        ...
        then (
          Printf.fprintf
            oc
            ", \"%s.dat\" using 1:%d title \"domid %d target\" with lines "
            stem
            !col
            domain.domid ;
          incr col ) )
...
let change_host_free_memory ?fistpoints io required_mem_kib success_condition =
  ...
  while not !finished do
    ...
    let new_target_direction domain =
      let new_target =
        if List.mem_assoc domain.domid new_targets
    ...
    let maxmems =
      IntMap.mapi
        (fun domid domain ->
          ...
          else if List.mem_assoc domid new_targets
          then List.assoc domid new_targets
          else domain.target_kib )
...
let balance_memory ?fistpoints io =
  ...
  with
  | e ->
      debug "balance memory caught: %s" (Printexc.to_string e)
...
let is_host_memory_unbalanced ?fistpoints io =
  ...
  match result with
  | AdjustTargets ts ->
      if List.fold_left ( || ) false (List.map is_new_target ts)
      then (
