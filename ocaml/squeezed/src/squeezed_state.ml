...
let xs_read path =
  try Client.immediate (get_client ()) (fun xs -> Client.read xs path) with
  | Xs_protocol.Enoent _ as e ->
...
let xs_read_option path =
  try
    Some (Client.immediate (get_client ()) (fun xs -> Client.read xs path))
  with
  | Xs_protocol.Enoent _ ->
      None
