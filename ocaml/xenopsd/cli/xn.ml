...
let diagnose_error f =
  ...
  | e ->
      ...
      | Unix.Unix_error (Unix.ENOENT, _, _) ->
          ...
          Printf.fprintf
            stderr
            "1. Start the xenopsd service; it will create the socket when it \
             is started;\n" ;
          Printf.fprintf
            stderr
...
let usage () =
  ...
  Printf.fprintf
    stderr
    "%s add <config> - add a VM from <config>\n"
    Sys.argv.(0) ;
  Printf.fprintf
    stderr
    "%s list [verbose] - query the states of known VMs\n"
    Sys.argv.(0) ;
  Printf.fprintf
    stderr
    "%s remove <name or id> - forget about a VM\n"
    Sys.argv.(0) ;
  Printf.fprintf
    stderr
    "%s start <name or id> [paused] - start a VM\n"
    Sys.argv.(0) ;
  Printf.fprintf stderr "%s pause <name or id> - pause a VM\n" Sys.argv.(0) ;
  Printf.fprintf stderr "%s unpause <name or id> - unpause a VM\n" Sys.argv.(0) ;
  Printf.fprintf
    stderr
    "%s shutdown <name or id> - shutdown a VM\n"
    Sys.argv.(0) ;
  Printf.fprintf stderr "%s reboot <name or id> - reboot a VM\n" Sys.argv.(0) ;
  Printf.fprintf
    stderr
    "%s suspend <name or id> <disk> - suspend a VM\n"
    Sys.argv.(0) ;
  Printf.fprintf
    stderr
    "%s resume <name or id> <disk> - resume a VM\n"
    Sys.argv.(0) ;
  Printf.fprintf
    stderr
  ...
  Printf.fprintf
    stderr
    "%s cd-insert <id> <disk> - insert a CD into a VBD\n"
    Sys.argv.(0) ;
  Printf.fprintf
    stderr
    "%s cd-eject <id> - eject a CD from a VBD\n"
    Sys.argv.(0) ;
  Printf.fprintf
    stderr
  ...
  Printf.fprintf
    stderr
    "%s diagnostics - display diagnostic information\n"
    Sys.argv.(0) ;
  Printf.fprintf
    stderr
    "%s task-list - display the state of all known tasks\n"
    Sys.argv.(0) ;
  Printf.fprintf
    stderr
    "%s shutdown - shutdown the xenops service\n"
    Sys.argv.(0) ;
  ()
...
let parse_vif vm_id (x, idx) =
  ...
  ; rate = None
  ; backend =
      ( if List.mem_assoc _bridge kvpairs
      then Network.Local (List.assoc _bridge kvpairs)
      else Network.Local "xenbr0" )
  ; other_config = []
  ...
  | Some filename ->
      let ic = open_in filename in
        ...
        (fun () ->
          ...
          let uuid =
            if mem _uuid
            then find _uuid |> string
            else Uuidm.to_string (Uuidm.create `V4)
          in
          ...
          let pci_msitranslate =
            if mem _vm_pci_msitranslate
            then find _vm_pci_msitranslate |> bool
            else true
          in
          let pci_power_mgmt =
            if mem _vm_pci_power_mgmt
            then find _vm_pci_power_mgmt |> bool
            else false
          in
          let has_vendor_device =
            if mem _vm_has_vendor_device
            then find _vm_has_vendor_device |> bool
            else false
          in
          let vm =
            { id = uuid
...
let find_by_name x =
  ...
  try List.find this_one all with
  | Not_found ->
...
let export copts metadata xm filename (x : Vm.id option) () =
  ...
  then `Error (false, "Unfortunately I only support metadata import")
    ...
    | Some x ->
      ...
      | Some f ->
          (if xm then export_metadata_xm else export_metadata) copts f x ;
          `Ok () )
...
let trim limit str =
  ...
  if l < limit then str else "..." ^ String.sub str (l - limit + 3) (limit - 3)
...
let raw_console_proxy sockaddr =
  ...
  let proxy fd =
    ...
    while not !finished do
      if !buf_local_start <> !buf_local_end
      then (
        let b =
          Unix.write
            Unix.stdout
        ...
        then (
          buf_local_start := 0 ;
          buf_local_end := 0 ) )
      else if !buf_remote_start <> !buf_remote_end
      then (
        let b =
          Unix.write
            fd
        ...
        if !buf_remote_start = !buf_remote_end
        then (
          buf_remote_start := 0 ;
          buf_remote_end := 0 ) )
      else if !final
...
let unix_proxy path =
  ...
  ( try Unix.connect unix (Unix.ADDR_UNIX path) with
  | e ->
      Unix.close unix ;
      raise e ) ;
...
let find_vbd id =
  ...
  try List.find this_one vbds with
  | Not_found ->
