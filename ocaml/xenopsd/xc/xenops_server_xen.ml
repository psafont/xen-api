...
let choose_alternative kind default platformdata =
  ...
  if List.mem_assoc kind platformdata
  then
    ...
    then Filename.concat dir x
      ...
      error
        "Invalid platform:%s=%s (check execute permissions of %s)"
        kind
        x
  ...
  | xs ->
      ...
      raise
        (Xenopsd_error
           (Internal_error
              (Printf.sprintf
                 "More than one domain with uuid (%s): %s"
                 uuid'
                 domid_list ) ) )
...
let device_kind_of_backend_keys backend_keys =
  try
    Device_common.vbd_kind_of_string (List.assoc "backend-kind" backend_keys)
  with
  | Not_found ->
...
let params_of_backend backend =
  ...
  let xenstore_data =
    ...
    | xendisk :: _ ->
        ...
        if List.mem_assoc backend_kind xenstore_data
        then xenstore_data
        else ("backend-kind", backend_kind) :: xenstore_data
    | [] ->
        raise
          (Xenopsd_error
             (Internal_error
...
let print_fork_error f =
  ...
  | Forkhelpers.Spawn_internal_error (stderr, stdout, status) as e ->
    ...
    | Unix.WEXITED n ->
        error
          "Forkhelpers.Spawn_internal_error(%s, %s, WEXITED %d)"
          stderr
          stdout
          n ;
        raise e
    | Unix.WSIGNALED n ->
        error
          "Forkhelpers.Spawn_internal_error(%s, %s, WSIGNALED %d)"
          stderr
          stdout
          n ;
        raise e
    | Unix.WSTOPPED n ->
        error
          "Forkhelpers.Spawn_internal_error(%s, %s, WSTOPPED %d)"
          stderr
          stdout
          n ;
        raise e )
...
module NbdClient = struct
  ...
  let with_nbd_device ~unix_socket_path ~export_name f =
    let nbd_device = start_nbd_client ~unix_socket_path ~export_name in
      ...
      (fun () -> f nbd_device)
      (fun () ->
        try stop_nbd_client ~nbd_device with
        | e ->
            warn
              "ignoring exception while disconnecting nbd-client from %s: %s"
              nbd_device
        ...
        (fun () ->
          ...
          let device = create_vbd_frontend ~xc ~xs task frontend_domid vdi in
            ...
            (fun () ->
              ...
              | Nbd nbd ->
                  debug
                    "with_disk: using nbd-client for %s"
...
module Mem = struct
    ...
    Option.map
      (fun session_id ->
        let reservation_id, reserved_memory =
          retry (fun () ->
              debug
                "Requesting a host memory reservation between %Ld and %Ld"
                min
                max ;
              ...
              in
              debug
                "Memory reservation size = %Ld (reservation_id = %s)"
                kib
                reservation_id ;
              (reservation_id, kib) )
        in
  ...
  let transfer_reservation_to_domain_exn dbg domid (reservation_id, amount) =
    ...
    | Some session_id ->
      ( try
          Client.transfer_reservation_to_domain
            dbg
            session_id
            reservation_id
            domid
  ...
  let query_reservation_of_domain dbg domid =
    ...
    | Some session_id ->
      ...
      | _ ->
          error
            "Internal error. Cannot query reservation_id for domid = %d"
            domid ;
          raise Memory_interface.(MemoryError No_reservation) )
    | None ->
        info
          "No ballooning daemon. Cannot query reservation_id for domid = %d"
          domid ;
        raise Memory_interface.(MemoryError No_reservation)
  ...
  let get (cache, mutex) fetch_all_f fetch_one_f domid key =
    ...
    Mutex.execute domid_mutex (fun () ->
        ...
        ( try
            ...
            if cached_value <> fetched_value
            then (* force refresh of domid cache *)
              refresh_cache ()
          with
        | _ ->
          (try (* attempt to refresh cache *)
               refresh_cache () with _ -> ()) ) ;
        try PerVMCache.find domid_cache (Some key) with
        | _ ->
            let keys =
              try PerVMCache.fold (fun k v acc -> k :: acc) domid_cache [] with
              | _ ->
                  []
            in
  ...
  | Some frontend_domid ->
      let fetch_all_from_xenstore_f () =
        ...
        let id_of_device device =
          ...
          try Some (xs.Xs.read (Printf.sprintf "%s/%s" path key)) with
          | _ ->
              None
        in
      ...
      let fetch_one_from_xenstore_f cached_device =
        ...
        let xenstored_device =
          Device_common.list_frontends
            ~xs
            ~for_devids:[ cached_frontend_devid ]
            frontend_domid
        in
      ...
      ( try
          DeviceCache.get
            device_cache
            fetch_all_from_xenstore_f
            fetch_one_from_xenstore_f
            frontend_domid
            id
        with
      | DeviceCache.NotFoundIn ids ->
          debug
...
let debug_featuresets xc name featureset featureset_max index_max =
  ...
  if not @@ Array.for_all (fun x -> Int64.equal x 0L) ours_minus_max
  then (
...
module HOST = struct
  ...
  let stat () =
    ...
    let get_cpuinfo () =
      ...
      close_in in_chan ;
      let find key =
        if Hashtbl.mem tbl key then Hashtbl.find tbl key else "unknown"
      in
      ( find "vendor_id"
      , find "model name"
      , find "cpu MHz"
...
module VM = struct
  ...
  let get_domain_type ~xs di =
    ...
    | Xs_protocol.Enoent _ ->
        ...
        if di.Xenctrl.hvm_guest then Domain_HVM else Domain_PV
    ...
    with_xc_and_xs (fun xc xs ->
        ...
        let _ =
          DB.update k (function
              | Some x ->
                  debug
                    "VM = %s; reloading stored domain-level configuration"
                    vm.Vm.id ;
                  Some x
              | None ->
                  debug
          ...
          DB.update k (fun vmextra ->
              ...
              let min_bytes, max_bytes =
                ...
                | None ->
                    ...
                    then (
                      debug
                        "VM = %s; using stored suspend_memory_bytes = %Ld"
                        vm.Vm.id
                  ...
                  then
                    warn
                      "VM = %s; the suppress-spurious-page-faults option is no \
                       longer implemented"
                      vm.Vm.id ;
                  if vm.machine_address_size <> None
                  then
                    warn
  ...
  let on_domain_if_exists f (task : Xenops_task.task_handle) vm =
    try on_domain f task vm with
    | Xenopsd_error (Does_not_exist ("domain", _)) ->
  ...
  let remove vm =
    ...
    try DB.remove vm.Vm.id with
    | Xenopsd_error (Does_not_exist ("extra", _)) ->
        ()
  ...
  let log_exn_continue msg f x =
    ...
    | e ->
        debug
          "Safely ignoring exception: %s while %s"
          (Printexc.to_string e)
          msg
  ...
  let destroy_device_model =
    on_domain_if_exists (fun xc xs task vm di ->
        ...
        log_exn_continue
          "Error stoping device-model, already dead ?"
          (fun () -> Device.Dm.stop ~xs ~qemu_domid ~dm:(dm_of ~vm) domid)
          () ;
        log_exn_continue
          "Error stoping vncterm, already dead ?"
  ...
  let destroy =
    on_domain_if_exists (fun xc xs task vm di ->
          ...
          (fun () ->
            ...
            List.iter
              (fun dp ->
                try Storage.dp_destroy task dp with
                | e ->
                    warn
                      "Ignoring exception in VM.destroy: %s"
                      (Printexc.to_string e) )
              dps )
          (fun () ->
  ...
  let pause =
    on_domain (fun xc xs _ _ di ->
        if di.Xenctrl.total_memory_pages = 0n
  ...
  let set_shadow_multiplier task vm target =
    on_domain
      (fun xc xs _ _ di ->
        if get_domain_type ~xs di = Vm.Domain_PV
        then
          raise
        ...
        debug
          "VM = %s; domid = %d; shadow_allocation_setto %d MiB"
          vm.Vm.id
          domid
          newshadow ;
    ...
    | { VmExtra.build_info = Some build_info
      ...
      ; xen_platform
        ...
        | HVM hvm_info ->
            ...
            in
            let usb_enabled =
              try List.assoc "usb" vm.Vm.platformdata = "true" with
              | Not_found ->
                  true
            in
            let usb_tablet_enabled =
              try List.assoc "usb_tablet" vm.Vm.platformdata = "true" with
              | Not_found ->
                  true
            in
            let usb =
  ...
  let clean_memory_reservation task domid =
    ...
    with
    | Memory_interface.MemoryError Memory_interface.No_reservation ->
        error
  ...
  let build_domain_exn xc xs domid task vm vbds vifs vgpus vusbs extras force =
    ...
    let pvinpvh_xen_cmdline =
      let base =
        try List.assoc "pvinpvh-xen-cmdline" vm.Vm.platformdata with
        | Not_found ->
            !Xenopsd.pvinpvh_xen_cmdline
      in
      let shim_mem =
        let shim_mib =
    ...
    let domid = di.Xenctrl.domid in
      ...
      (fun () ->
        ...
        | Bootloader.Bad_sexpr x ->
            let m =
              Printf.sprintf
                "VM = %s; domid = %d; Bootloader.Bad_sexpr %s"
                vm.Vm.id
                domid
                x
            in
        ...
        | Bootloader.Bad_error x ->
            let m =
              Printf.sprintf
                "VM = %s; domid = %d; Bootloader.Bad_error %s"
                vm.Vm.id
                domid
                x
            in
        ...
        | Bootloader.Unknown_bootloader x ->
            let m =
              Printf.sprintf
                "VM = %s; domid = %d; Bootloader.Unknown_bootloader %s"
                vm.Vm.id
                domid
                x
            in
  ...
  let create_device_model_exn
      vbds vifs vgpus vusbs saved_state vmextra xc xs task vm di =
  ...
  let with_mounted_dir_ro device f =
    ...
    Unix.mkdir mount_point 0o640 ;
      ...
      (fun () ->
        ...
        f mount_point )
      (fun () ->
        ( try umount mount_point with
        | e ->
            debug "Caught %s" (Printexc.to_string e) ) ;
        try Unix.rmdir mount_point with
        | e ->
            debug "Caught %s" (Printexc.to_string e) )
    ...
    | Disk disk ->
        with_disk ~xc ~xs task disk write (fun path ->
            ...
            with_fd_of_path path (fun fd ->
                  ...
                  (fun () -> f fd)
                  (fun () ->
                    try Xapi_stdext_unix.Unixext.fsync fd with
                    | Unix.Unix_error (Unix.EIO, _, _) ->
                        error
                          "Caught EIO in fsync after suspend; suspend image \
                           may be corrupt" ;
    ...
    on_domain
      (fun xc xs (task : Xenops_task.task_handle) vm di ->
        ...
        with_data ~xc ~xs task data true (fun fd ->
            ...
            debug
              "VM = %s; domid = %d; Disk backends have all been flushed"
              vm.Vm.id
              domid ;
            List.iter
              (fun vbds_chunk ->
                      ...
                      | Error (`Msg m) ->
                          raise
                            (Xenopsd_error
                               (Internal_error
                                  (Printf.sprintf
                                     "Failed to unmarshal VBD backend: %s"
                                     m ) ) )
                    in
            ...
            debug
              "VM = %s; domid = %d; Storing final memory usage"
              vm.Vm.id
              domid ;
            let _ =
              DB.update_exn vm.Vm.id (fun d ->
          ...
          (fun () ->
            ...
            let build_info, timeoffset =
              ...
              | { VmExtra.build_info = None } ->
                  error
                    "VM = %s; No stored build_info: cannot safely restore"
                    vm.Vm.id ;
            ...
            Int64.(
              ...
              and max = to_int (div vm.Vm.memory_dynamic_max 1024L) in
              Domain.set_memory_dynamic_range ~xc ~xs ~min ~max domid) ;
            try inject_igmp_query domid vifs |> ignore with
            | e ->
                error
                  "VM %s: inject IGMP query failed: %s"
                  vm.Vm.id
                  (Printexc.to_string e) )
          (fun () -> clean_memory_reservation task di.Xenctrl.domid) )
      task
    ...
    with_xc_and_xs (fun xc xs ->
        ...
        | Some di ->
              ...
              let max_memory_bytes =
                let overhead_bytes =
                  Memory.bytes_of_mib
                    ...
                    then Memory.HVM.xen_max_offset_mib
            ...
            let rec ls_lR
                ...
                (quota, acc)
                dir =
              if quota <= 0 || StringSet.mem dir excludes
              then (quota, acc) (* quota reached, stop listing/reading *)
              else
            ...
            then (
              try
                ...
                then (
                  ...
                  warn
                    "xenstore guest agent quota is still exceeded for domid %d"
                    di.Xenctrl.domid )
              with
              | _ ->
                  warn
            ...
            let shadow_multiplier_target =
              ...
              then 1.
                ...
                try
                  ...
                  then (
                    warn
                      "CA-104562: Got value '%d' from shadow_allocation_get"
                ...
                | e ->
                    warn
                      "Caught exception in getting shadow allocation: %s"
                      (Printexc.to_string e) ;
                    -1.0
            in
  ...
  let run_script task vm script =
    ...
    let domid, path =
      with_xc_and_xs (fun xc xs ->
          ...
          | Some di ->
              ...
              let _ =
                try xs.Xs.read (path ^ "/control/feature-xs-batcmd") with
                | _ ->
                    raise
                      (Xenopsd_error
                         (Unimplemented
                            "run-script is not supported on the given VM (or \
                             it is still booting)" ) )
              in
    ...
    let truncate s =
      ...
      then String.sub s 0 (1024 - String.length mark) ^ mark
      else s
    in
  ...
  let set_internal_state vm state =
    ...
    let persistent =
      ...
      | HVM { timeoffset } ->
        ...
        | Some (HVM hvm_info) ->
            { persistent with
              VmExtra.ty = Some (HVM { hvm_info with timeoffset })
            }
        | _ ->
            persistent )
      | _ ->
    ...
    let profile =
      (* Set the profile correctly if the incoming state does not have a profile
         ...
         `revision_of` below. *)
      if persistent.VmExtra.profile = None && will_be_hvm vm
      then Some Device.Profile.Qemu_trad
      else persistent.VmExtra.profile
    in
    ...
    if state_new <> state
    then debug "vm %s: different original metadata %s" k state ;
    (* Save the state *)
  ...
  let plug task vm pci advertise =
    on_frontend
      (fun xc xs frontend_domid domain_type ->
        ...
        xs.Xs.write
          (Printf.sprintf
             "/local/domain/0/backend/pci/%d/0/msitranslate"
             frontend_domid )
          (if persistent.VmExtra.pci_msitranslate then "1" else "0") ;
        xs.Xs.write
          (Printf.sprintf
             "/local/domain/0/backend/pci/%d/0/power_mgmt"
             frontend_domid )
          (if persistent.VmExtra.pci_power_mgmt then "1" else "0") ;
        if not (Sys.file_exists "/sys/bus/pci/drivers/pciback")
        then (
...
let set_active_device path active =
  with_xs (fun xs -> if active then xs.Xs.write path "1" else safe_rm xs path)
...
module VGPU = struct
  ...
  let set_active task vm vgpu active =
    try set_active_device (active_path vm vgpu) active with
    | e ->
        debug
          "VGPU.set_active %s.%s <- %b failed: %s"
          (fst vgpu.Vgpu.id)
          (snd vgpu.Vgpu.id)
          active
          (Printexc.to_string e)
  ...
  let get_active vm vgpu =
    try with_xs (fun xs -> xs.Xs.read (active_path vm vgpu)) = "1" with
    | _ ->
        false
...
module VBD = struct
      ...
      | Some (Local path) ->
          ...
          ; attach_info =
              Storage_interface.
                { implementations =
                    [ XenDisk
                        { params = path; extra = []; backend_type = "vbd3" }
                    ; BlockDevice { path }
                    ]
                }
          }
  ...
  let set_active task vm vbd active =
    try set_active_device (active_path vm vbd) active with
    | e ->
        debug
          "set_active %s.%s <- %b failed: %s"
          (fst vbd.Vbd.id)
          (snd vbd.Vbd.id)
          active
          (Printexc.to_string e)
  ...
  let get_active vm vbd =
    try with_xs (fun xs -> xs.Xs.read (active_path vm vbd)) = "1" with
    | _ ->
        false
  ...
  let plug task vm vbd =
    ...
    then
      debug
        "VBD %s.%s is not active: not plugging into VM"
        (fst vbd.Vbd.id)
        (snd vbd.Vbd.id)
    else
      on_frontend
    ...
    with_xc_and_xs (fun xc xs ->
        try
          ...
          let dev =
            ...
            | Xenopsd_error (Does_not_exist (_, _)) ->
                debug
                  "VM = %s; VBD = %s; Ignoring missing domain"
                  vm
                  (id_of vbd) ;
                None
            | Xenopsd_error Device_not_connected ->
                debug
                  "VM = %s; VBD = %s; Ignoring missing device"
                  vm
                  (id_of vbd) ;
                None
          in
  ...
  let ionice qos pid =
    try run !Xc_resources.ionice (Ionice.set_args qos pid) |> ignore_string with
    | e ->
        ...
        Option.iter
          (function
            | Ionice qos ->
              ...
              | e ->
                  error
                    "Failed to ionice kthread-pid: %s"
                    (Printexc.to_string e) ) )
          vbd.Vbd.qos )
  ...
  let get_device_action_request vm vbd =
    with_xc_and_xs (fun xc xs ->
          ...
          then
            ...
            then (
              debug
                ...
                vm
                (id_of vbd)
                (string_of_qos qos_target)
                (string_of_qos vbd.Vbd.qos) ;
              Some Needs_set_qos )
            else None
          else (
            debug
              "VM = %s; VBD = %s; VBD_unplug needed, device offline: %s"
              vm
              (id_of vbd)
        ...
        | Xenopsd_error Device_not_connected ->
            debug
              "VM = %s; VBD = %s; Device_not_connected so no action required"
              vm
              (id_of vbd) ;
            None )
end
...
module VIF = struct
  ...
  let set_active task vm vif active =
    try set_active_device (active_path vm vif) active with
    | e ->
        debug
          "set_active %s.%s <- %b failed: %s"
          (fst vif.Vif.id)
  ...
  let get_active vm vif =
    try with_xs (fun xs -> xs.Xs.read (active_path vm vif)) = "1" with
    | _ ->
        false
  ...
  let plug_exn task vm vif =
    ...
    then
      debug
        "VIF %s.%s is not active: not plugging into VM"
        (fst vif.Vif.id)
        (snd vif.Vif.id)
    else
      on_frontend
          ...
          | Network.Local _ | Network.Remote _ ->
              ...
              let destroy device =
                ...
                Xenops_task.with_subtask
                  ...
                  (fun () ->
                    ( if force
                    then Device.hard_shutdown
                    else Device.clean_shutdown )
                      task
                      ~xs
                      device ) ;
                Xenops_task.with_subtask
    ...
    with_xc_and_xs (fun xc xs ->
        ...
        try
          ...
          let _ =
            DB.update_exn vm (fun vm_t ->
                let persistent = vm_t.VmExtra.persistent in
                if List.mem_assoc vif.Vif.id persistent.VmExtra.qemu_vifs
                then
                  ...
                  List.iter
                    (fun key ->
                      if Astring.String.is_prefix
                           ~affix:pvs_proxy_key_prefix
                           key
                      then t.Xs.rm (Printf.sprintf "%s/%s" private_path key) )
                    keys ) )
            else (
              Xs.transaction xs (fun t ->
              ...
              let pv_drivers_detected =
                ...
                | true, false ->
                    (* State "connected" (4) means that PV drivers are present for
                       this device *)
                    debug
                      "VM = %s; found PV driver evidence on %s (value = %s)"
                      vm
                      path
                      value ;
                    true
                | false, true ->
                    ...
                    let found =
                      ...
                      List.exists
                        (fun device ->
                          try
                            xs.Xs.read
                              (Device_common.backend_state_path_of_device
                                 ~xs
                                 device )
                            = xenbus_connected
                          with
                    ...
                    if not found
                    then
                      (* No devices in state "connected" (4) *)
  ...
  let watch_fired xc xs path domains watches =
    ...
    | [ "local"; "domain"; domid; "rrd"; name; "shutdown" ] ->
        ...
        then (
          ...
          try deregister_rrd_plugin ~domid:(int_of_string domid) ~name with
          | e ->
              debug
...
let look_for_forkexec () =
  ...
  | e ->
      error
        "The fork/exec service is not working properly. The raw error was: %s"
        (Printexc.to_string e) ;
...
let look_for_xen () =
  ...
  | None ->
      error
        "The file %s does not exist: you are not running xen."
...
let init () =
  ...
  look_for_xenctrl () ;
  if major < "4"
     || ((major = "4" && minor < "2") && !Xenopsd.run_hotplug_scripts)
  then (
    error
