...
let assert_file_is_readable filename =
  try Unix.access filename [ Unix.F_OK; Unix.R_OK ] with
  | _ ->
...
let wait_xen_free_mem ~xc ?(maximum_wait_time_seconds = 64) required_memory_kib
  ...
  let rec wait accumulated_wait_time_seconds =
    ...
    if is_power_of_2 accumulated_wait_time_seconds
    then
      debug
    ...
    then true
    else if scrub_memory_kib = 0L (* We'll never have enough memory. *)
    then false
    else if accumulated_wait_time_seconds >= maximum_wait_time_seconds
...
let make ~xc ~xs vm_info vcpus domain_config uuid final_uuid no_sharept =
  ...
  let assert_capability cap ~on_error =
    if not (List.mem cap host_info.capabilities)
    then (
  ...
  let hap =
    ...
    then false
      ...
      let hap =
        ...
        | Some unknown ->
            error
              "VM = %s; Unrecognized value platform/hap=\"%s\"."
              (Uuid.to_string uuid)
              unknown ;
      ...
      assert_capability
        ...
        ~on_error:(fun () ->
          sprintf
            "Guest paging mode %s unavailable"
  ...
  try
    ...
    Xs.transaction xs (fun t ->
        ...
        let device_dirs' =
          let xsi_254 =
            try List.assoc "netscaler" vm_info.platformdata = "XSI-254" with
            | Not_found ->
                false
          in
...
let shutdown ~xc ~xs domid req =
  ...
  debug
    "VM = %s; domid = %d; Requesting domain %s"
    (Uuid.to_string uuid)
    domid
...
let shutdown_wait_for_ack
    (t : Xenops_task.task_handle)
    ~timeout
    ~xc
    ~xs
    domid
    (domain_type : [ `pv | `pvh | `hvm ])
    req =
  ...
  then (
    ...
    then
      info
        "VM = %s; domid = %d; Domain acknowledged shutdown request"
        (Uuid.to_string uuid)
        domid
    else
      debug
        "VM = %s; domid = %d; Domain disappeared"
        (Uuid.to_string uuid)
        domid )
...
let destroy (task : Xenops_task.task_handle) ~xc ~xs ~qemu_domid ~dm domid =
  ...
  debug
    "VM = %s; domid = %d; Domain.destroy: all known devices = [ %a ]"
    (Uuid.to_string uuid)
    domid
    (fun () -> String.concat "; ")
  ...
  List.iter
    (fun pcidev ->
      ...
      log_exn_continue
        ...
        (fun () ->
          Xenctrl.domain_deassign_device
            xc
            domid
  ...
  debug
    "VM = %s; domid = %d; Domain.destroy calling Xenctrl.domain_destroy"
    (Uuid.to_string uuid)
    domid ;
  ...
  List.iter
    (fun device ->
      try Device.hard_shutdown task ~xs device with
      | e ->
          (* If this fails we may have a resource leak. We should prevent this
             from happening! *)
          error
            "VM = %s; domid = %d; Caught exception %s while destroying device \
             %s"
            (Uuid.to_string uuid)
            domid
            (Printexc.to_string e)
  ...
  List.iter
    (fun dev ->
      error
        "VM = %s; domid = %d; Domain.destroy failed to release device: %s"
        (Uuid.to_string uuid)
        domid
        (string_of_device dev) )
    failed_devices ;
  ...
  if failed_devices = []
  then
    if List.length other_domains < 1
    then log_exn_rm ~xs (Device_common.get_private_path_by_uuid uuid)
    else log_exn_rm ~xs (Hotplug.get_hotplug_base_by_uuid uuid domid) ;
...
let maybe_ca_140252_workaround ~xc ~vcpus domid =
  ...
  then debug "Allocating %d I/O req evtchns in advance for device model" vcpus ;
  for i = 1 to vcpus do
...
let create_channels ~xc uuid domid =
  ...
  debug
    "VM = %s; domid = %d; store evtchn = %d; console evtchn = %d"
    (Uuid.to_string uuid)
    domid
    store
    console ;
...
let build_pre ~xc ~xs ~vcpus ~memory ~has_hard_affinity domid =
  ...
  let uuid = get_uuid ~xc domid in
  debug
    "VM = %s; domid = %d; waiting for %Ld MiB of free host memory"
    (Uuid.to_string uuid)
    domid
    memory.required_host_free_mib ;
  ...
  then (
    error
      "VM = %s; domid = %d; Failed waiting for Xen to free %Ld MiB"
      (Uuid.to_string uuid)
      domid
      memory.required_host_free_mib ;
  ...
  ( if not Xenctrl.((domain_getinfo xc domid).hvm_guest)
  then
    let kib = Memory.kib_of_mib memory.xen_max_mib in
    log_reraise
      (Printf.sprintf "domain_set_memmap_limit %Ld KiB" kib)
      (fun () -> Xenctrl.domain_set_memmap_limit xc domid kib) ) ;
  log_reraise
    (Printf.sprintf "shadow_allocation_set %d MiB" shadow_mib)
    (fun () -> Xenctrl.shadow_allocation_set xc domid shadow_mib) ;
  if !Xenopsd.numa_placement
  then
    log_reraise (Printf.sprintf "NUMA placement") (fun () ->
        if has_hard_affinity
        then D.debug "VM has hard affinity set, skipping NUMA optimization"
        else
          ...
          in
          if !Xenopsd.numa_placement_strict
          then do_numa_placement ()
          else Xenops_utils.best_effort "NUMA placement" do_numa_placement ) ;
...
let xenguest_args_base
    ~domid ~store_port ~store_domid ~console_port ~console_domid ~memory =
  [ "-domid"
  ; string_of_int domid
  ; "-store_port"
...
let xenguest task xenguest_path domid uuid args =
  ...
  | store_mfn :: console_mfn :: _ ->
      debug
        "VM = %s; domid = %d; store_mfn = %s; console_mfn = %s"
        (Uuid.to_string uuid)
        domid
        store_mfn
        console_mfn ;
...
let restore_libxc_record cnx domid uuid =
  ...
  | Xenguest_result (store, console) ->
      debug
        "VM = %s; domid = %d; store_mfn = %nd; console_mfn = %nd"
        (Uuid.to_string uuid)
        domid
        store
        console ;
...
let consume_qemu_record fd limit domid uuid =
  ...
  in
    ...
    (fun () ->
      debug
        "VM = %s; domid = %d; reading %Ld bytes from %s"
        (Uuid.to_string uuid)
        domid
        limit
        file ;
      let bytes =
        try Unixext.copy_file ~limit fd fd2 with
      ...
      then (
        error
          "VM = %s; domid = %d; qemu save file was truncated"
          (Uuid.to_string uuid)
          domid ;
...
let restore_common
  ...
  | Ok Legacy ->
      ...
      let store_mfn, console_mfn =
        ...
        | Error e ->
            error
              "Caught error when using converison script: %s"
              (Printexc.to_string e) ;
      ...
      then (
        ...
        let length =
          ...
          | Error e ->
              error
                "VM = %s; domid = %d; Error reading QEMU signature: %s"
                (Uuid.to_string uuid)
                domid
                e ;
              raise Suspend_image_failure
      ...
      with_emu_manager_restore
        ...
        (fun cnx ->
          ...
          let restore_and_wait emu =
            ...
            >>= fun () ->
            debug
              "Sent restore:%s to emu-manager. Waiting for result..."
              (string_of_emu emu) ;
          ...
          let rec process_header fd res =
            ...
            | Demu, _ ->
                ...
                restore_and_wait Vgpu
                >>= fun () ->
                debug "Restored DEMU state" ;
                process_header fd res
            | Varstored, len ->
                debug
                  "Read varstored record header (domid=%d length=%Ld)"
                  domid
                  len ;
          ...
          let cancel_on_error result =
            let () =
              ...
              | Error e ->
                  ...
                  warn
                    "Canceling task %s, error during resume: %s"
          ...
          let res =
            emu_manager_results
            >>= fun result -> thread_status >>= fun () -> return result
          in
          match res with
  ...
  | Error e ->
      error
        "VM = %s; domid = %d; Error reading save signature: %s"
        (Uuid.to_string uuid)
        domid
        e ;
      raise Suspend_image_failure
...
let restore
  ...
  let store_port, console_port =
    build_pre
      ...
      ~memory
      ~vcpus
      ~has_hard_affinity:info.has_hard_affinity
      domid
  in
  let store_mfn, console_mfn =
    restore_common
...
let suspend_emu_manager
  ...
  with_connection task manager_path domid args fds (fun cnx ->
      ...
      let callback txt =
        ...
        if Astring.String.is_prefix ~affix:prefix txt
        then
          let rest =
            String.sub
              txt
      ...
      let rec wait_for_message () =
        ...
        | Suspend ->
            ...
            then (
              debug
                "VM = %s; domid = %d; suspending qemu-dm"
                (Uuid.to_string uuid)
                domid ;
              Device.Dm.suspend task ~xs ~qemu_domid ~dm domid ;
              if is_uefi
              then Device.Dm.suspend_varstored task ~xs domid |> ignore ) ;
            send_done cnx ;
            wait_for_message ()
        | Prepare x when x = "xenguest" ->
            debug "Writing Libxc header" ;
            write_header main_fd (Libxc, 0L)
            >>= fun () ->
        ...
        | Result x ->
            debug
              "VM = %s; domid = %d; emu-manager completed successfully"
              (Uuid.to_string uuid)
              domid ;
            return ()
        | Error x ->
            error
              "VM = %s; domid = %d; emu-manager failed: \"%s\""
              (Uuid.to_string uuid)
              domid
              x ;
            Error
        ...
        | _ ->
            error
              "VM = %s; domid = %d; unexpected message from emu-manager"
              (Uuid.to_string uuid)
              domid ;
...
let write_qemu_record domid uuid fd =
  ...
  let fd2 = Unix.openfile file [ Unix.O_RDONLY ] 0o640 in
    ...
    (fun () ->
      ...
      >>= fun () ->
      debug
        "VM = %s; domid = %d; writing %Ld bytes from %s"
        (Uuid.to_string uuid)
        domid
        size
        file ;
...
let write_varstored_record task ~xs domid main_fd =
  ...
  write_header main_fd (Varstored, Int64.of_int varstored_rec_len)
  >>= fun () ->
...
let suspend
  ...
  let res =
    ...
    write_header main_fd (Xenops, Int64.of_int xenops_rec_len)
    >>= fun () ->
    ...
    >>= fun () ->
    ( if is_uefi
    then write_varstored_record task ~xs domid main_fd
    else return () )
    >>= fun () ->
    ...
    ( if domain_type = `hvm
    then write_qemu_record domid uuid main_fd
    else return () )
    >>= fun () ->
...
let vcpu_affinity_set ~xc domid vcpu cpumap =
  let uuid = get_uuid ~xc domid in
  debug
    "VM = %s; domid = %d; vcpu_affinity_set %d <- %s"
    (Uuid.to_string uuid)
    domid
    vcpu
    (String.concat
...
let vcpu_affinity_get ~xc domid vcpu =
  let uuid = get_uuid ~xc domid in
  debug
    "VM = %s; domid = %d; vcpu_affinity_get %d"
    (Uuid.to_string uuid)
    domid
    vcpu ;
...
let set_memory_dynamic_range ~xc ~xs ~min ~max domid =
  ...
  let uuid = get_uuid ~xc domid in
  debug
    "VM = %s; domid = %d; set_memory_dynamic_range min = %d; max = %d"
    (Uuid.to_string uuid)
    domid
    min
    max ;
    ...
    Xs.transaction xs (fun t ->
        ...
        then
          ...
          let rec fixup write path (name, node) =
            ...
            then (
              debug
                "Removing xenstore tree at %s"
                (String.concat "/" path ^ "/" ^ name) ;
              t.Xst.rm (String.concat "/" path ^ "/" ^ name) ) ;
            if node.contents <> fixed_contents || write || changed_name
            then (
              debug
                "About to write to %s (%s)\n%!"
