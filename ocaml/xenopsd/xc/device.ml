...
module Profile = struct
  ...
  let of_string = function
    | x when x = Name.qemu_trad ->
        sprintf
          "unsupported device-model profile %s: use %s"
          x
    ...
    | x ->
        debug
          "unknown device-model profile %s: defaulting to %s"
          x
          Name.qemu_upstream_compat ;
        Qemu_upstream_compat
end
...
module Generic = struct
  ...
  let add_device
      ~xs device backend_list frontend_list private_list xenserver_list =
    Mutex.execute device_serialise_m (fun () ->
        ...
        in
        debug
          "adding device  B%d[%s]  F%d[%s]  H[%s]"
          device.backend.domid
          backend_path
          device.frontend.domid
          frontend_rw_path
          hotplug_path ;
  ...
  let safe_rm ~xs path =
    ...
    with
    | _ ->
        debug "Failed to xenstore-rm %s; continuing" path
  ...
  let assert_exists_t ~xs t (x : device) =
    let backend_stub = backend_path_of_device ~xs x in
    try ignore_string (t.Xst.read backend_stub) with
    | Xs_protocol.Enoent _ ->
  ...
  let clean_shutdown_async ~xs (x : device) =
    ...
    Xs.transaction xs (fun t ->
        ...
        let state =
          try Xenbus_utils.of_string (t.Xst.read state_path) with
          | _ ->
              Xenbus_utils.Closed
        in
        if state <> Xenbus_utils.Closed
        then (
  ...
  let on_backend_closed_unplug ~xs x =
    ...
    |> Watch.map (fun _ ->
           debug
             "Backend closed for %s, deleting hotplug-status"
             (string_of_device x) ;
           (* deleting this key causes the udev rule to fire *)
           safe_rm ~xs (Hotplug.path_written_by_hotplug_scripts x) )


  let clean_shutdown_wait
      (task : Xenops_task.task_handle) ~xs ~ignore_transients (x : device) =
    ...
    if cancellable_watch
         cancel
         [ frontend_closed; unplug ]
  ...
  let hard_shutdown_complete ~xs (x : device) =
    if is_qdisk x then safe_rm ~xs (Hotplug.path_written_by_hotplug_scripts x) ;
    if run_hotplug_scripts x then backend_closed ~xs x else unplug_watch ~xs x
  ...
  let really_kill pid =
    try Unixext.kill_and_wait pid with
...
module Vbd_Common = struct
  ...
  let shutdown_request_clean_shutdown_wait
      (task : Xenops_task.task_handle) ~xs ~ignore_transients (x : device) =
  ...
  let shutdown_request_hard_shutdown
      (task : Xenops_task.task_handle) ~xs (x : device) =
  ...
  let clean_shutdown_wait
      (task : Xenops_task.task_handle) ~xs ~ignore_transients x =
    match shutdown_mode_of_device ~xs x with
    | Classic ->
  ...
  let release (task : Xenops_task.task_handle) ~xc ~xs (x : device) =
    ...
    if x.frontend.domid = 0 && x.backend.domid = 0
    then Hotplug.wait_for_frontend_unplug task ~xs x
  ...
  let add_async ~xs ~hvm x domid =
    let back_tbl = Hashtbl.create 16
    and front_tbl = Hashtbl.create 16 in
    let open Device_number in
    ...
    in
    debug
      "Device.Vbd.add (device_number=%s | params=%s | phystype=%s)"
  ...
  let add_wait (task : Xenops_task.task_handle) ~xc ~xs device =
    ...
    debug
      "Device.Vbd successfully added; device_is_online = %b"
  ...
  let add (task : Xenops_task.task_handle) ~xc ~xs ~hvm x domid =
    let device =
      ...
      while !result = None do
        try result := Some (add_async ~xs ~hvm x domid) with
        | Device_frontend_already_connected _ as e ->
            if x.device_number = None
            then (
  ...
  let qemu_media_change ~xs device _type params =
    ...
    let pathtowrite = if oldval = params then "/" ^ params else params in
  ...
  let add
    ...
    let back_options =
      ...
      | Some (kbytes_per_s, timeslice_us) ->
          ...
          and ( ^/ ) = Int64.div in
          let timeslice_us =
            if timeslice_us > 0L then timeslice_us else 50000L
            (* 50ms by default *)
          in
          let bytes_per_interval =
            ((kbytes_per_s ^* 1024L) ^* timeslice_us) ^/ 1000000L
          in
          if bytes_per_interval > 0L && bytes_per_interval < 0xffffffffL
          then [ ("rate", sprintf "%Lu,%Lu" bytes_per_interval timeslice_us) ]
          else (
            debug
              "VIF qos: invalid value for byte/interval: %Lu"
              bytes_per_interval ;
            [] )
    in
    let back =
    ...
    let front_options =
      ...
      then [ ("protocol", string_of_protocol protocol) ]
      else []
    in
    let front_mtu =
      match mtu with
...
module NetSriovVf = struct
  let add
    ...
    in
    (let ret =
       Network_client.Client.Sriov.make_vf_config
...
module DaemonMgmt (D : DAEMONPIDPATH) = struct
  ...
  let stop ~xs domid =
    ...
    | Some pid ->
        debug
          "%s: stopping %s with SIGTERM (domid = %d pid = %d)"
          D.name
          D.name
          domid
          pid ;
...
module SystemdDaemonMgmt (D : DAEMONPIDPATH) = struct
  ...
  let of_domid domid =
    ...
    if Fe_systemctl.exists key then Some key else None
  ...
  let alive service _ =
    if Fe_systemctl.is_active ~service
    then true
    else
...
module PV_Vnc = struct
  ...
  let get_tc_port ~xs domid =
    ...
    then None
      ...
      try Some (int_of_string (xs.Xs.read (Generic.tc_port_path domid))) with
      | _ ->
          None
  ...
  let load_args = function
    | None ->
        []
    | Some filename ->
        if Sys.file_exists filename then [ "-l"; filename ] else []
  ...
  let save ~xs domid =
    ...
    | Some pid ->
        ...
        if Unix.time () -. start_time > delay
        then
          debug
            "Device.PV_Vnc.save: timeout while waiting for %s to appear"
            filename
        else debug "Device.PV_Vnc.save: %s has appeared" filename
    | None ->
        ()
...
module PCI = struct
  ...
  let () =
    Printexc.register_printer (function
        | Domain_not_running (addr, domid) ->
            Some
              (Printf.sprintf
                 "Domain_not_running(inserting pci=%s,domid=%d)"
  ...
  let _pci_add ~xc ~xs ~hvm domid { host; guest = _, guest_addr; qmp_add } =
    ...
    then
      ...
      then
        ...
        in
        ()
      else raise (Domain_not_running (host, domid)) ;
    let addresses =
    ...
    let apply_io_permission i addr =
      ...
      then
        ...
        then
          ...
          if Nativeint.(logand scan_flags _pci_bar_io > 0n)
          then
            Xenctrl.domain_ioport_permission
              xc
              domid
  ...
  let do_flr device =
    ...
    let callscript s devstr =
      if Sys.file_exists !Xc_resources.pci_flr_script
      then
        try
          ignore
            (Forkhelpers.execute_command_get_output
    ...
    ( if Sys.file_exists device_reset_file
    then try write_string_to_file device_reset_file "1" with _ -> ()
    else try write_string_to_file doflr device with _ -> () ) ;
    ...
    | Some drv ->
        raise
          (Xenopsd_error
             (Internal_error
                (Printf.sprintf
                   "Fail to bind to i915, device is bound to %s"
  ...
  let num_vfs devstr =
    ...
    | exn ->
        let msg =
          Printf.sprintf
            "Can't read %s to reset Nvidia GPU %s: %s"
            path
            devstr
  ...
  let vfs_of devstr =
    let virtfn n =
      ...
      try Some (path |> Unix.readlink |> Filename.basename) with
      | exn ->
          debug
            "Can't read %s of PCI %s: %s"
            path
            devstr
            (Printexc.to_string exn) ;
          None
  ...
  let deactivate_nvidia_sriov devstr =
    ...
    | vfs when Sys.file_exists cmd ->
        ...
        debug
          "Deactivating NVidia vGPUs %s yielded: '%s'"
          devstr
          (String.escaped out)
    | _ ->
        let msg =
  ...
  let unbind_from_nvidia devstr =
    debug
      "pci: attempting to lock device %s before unbinding from nvidia"
      devstr ;
        ...
        List.iter
          (fun device ->
            ...
            | Some (Supported I915), I915 ->
                debug
                  "pci: device %s already bound to i915; doing nothing"
                  devstr
            | Some (Supported Nvidia), Nvidia ->
                debug
                  "pci: device %s already bound to nvidia; doing nothing"
                  devstr
            | Some (Supported Pciback), Pciback ->
                debug
                  "pci: device %s already bound to pciback; doing flr"
                  devstr ;
                do_flr devstr
  ...
  let signal_device_model ~xs domid cmd parameter =
    debug
      "Device.Pci.signal_device_model domid=%d cmd=%s param=%s"
      domid
      cmd
      parameter ;
    let be_domid = 0 in
  ...
  let ensure_device_frontend_exists ~xs backend_domid frontend_domid =
    ...
    let backend_path =
      Printf.sprintf
        "/local/domain/%d/backend/pci/%d/0"
        backend_domid
        frontend_domid
    in
    debug
      "adding PCI frontend: frontend_domid = %d; backend_domid = %d"
      frontend_domid
      backend_domid ;
    Xs.transaction xs (fun t ->
...
module Vfs = struct
  let add ~xc ~xs ?(backend_domid = 0) domid =
    ...
    let request_path =
      Printf.sprintf
        "/local/domain/%d/backend/vfs/exports/requests/%d"
        backend_domid
        domid
    in
...
module Vusb = struct
  let exec_usb_reset_script argv =
    try
      ...
      in
      debug
        "usb_reset script %s returned stdout=%s stderr=%s"
        (String.concat " " argv)
        stdout
        stderr
    with
    | err ->
        error
          "Failed to call usb_reset script with arguments %s"
          (String.concat " " argv) ;
        raise
          (Xenopsd_error
             (Internal_error
                (Printf.sprintf
                   "Call to usb reset failed: %s"
  ...
  let vusb_plug ~xs ~privileged ~domid ~id ~hostbus ~hostport ~version ~speed =
    ...
    let get_bus () =
      let vusb_controller_plug ~driver ~driver_id =
        ...
        if not (List.mem driver_id (qom_list ~xs ~domid))
        then
          qmp_send_cmd
            domid
            Qmp.(
              Device_add
                Device.
                  { driver; device = USB { USB.id = driver_id; params = None } })
          |> ignore
      in
        ...
        Qmp.(
          Device_add
            Device.
              { driver = "usb-host"
              ; device =
                  USB { USB.id; params = Some USB.{ bus; hostbus; hostport } }
              })
      in
  ...
  let vusb_unplug ~xs ~privileged ~domid ~id ~hostbus ~hostport =
    debug "vusb_unplug: unplug VUSB device %s" id ;
      ...
      (fun () ->
        if Qemu.is_running ~xs domid
        then
          try qmp_send_cmd domid Qmp.(Device_del id) |> ignore with
          | QMP_connection_error _ ->
              raise (Xenopsd_error Device_not_connected) )
...
module Serial : sig
  ...
  let update_xenstore ~xs domid =
    ...
    | exception e ->
        debug
          "Can't probe serial0 device for domid %d: %s"
          domid
...
module Dm_Common = struct
  ...
  let qemu_args ~xs ~dm info restore ?(domid_for_vnc = false) domid =
    ...
    let argv =
      List.concat
        ...
        ; ( restore
          |> function false -> [] | true -> [ "-loadvm"; restorefile ] )
        ; info.pci_emulations
        ...
        ; ( Qemu.pidfile_path domid
          |> function None -> [] | Some x -> [ "-pidfile"; x ] )
        ]
    in
    { argv; fd_map = [] }
    ...
    let device_args =
      ...
      |> List.map (fun vgpu ->
             ...
             | Nvidia
                 ...
                 debug
                   "NVidia vGPU config: using config file %s and uuid %s"
                   config_file
                   uuid ;
                 make
                   addr
             ...
             | Nvidia
                 ...
                 debug
                   "NVidia vGPU config: using type id %s and uuid: %s"
                   type_id
                   uuid ;
                 make
                   addr
             ...
             | Nvidia { type_id = None; config_file = None } ->
                 ...
                 raise
                   (Xenopsd_error
                      (Internal_error
                         (Printf.sprintf
                            "NVidia vGPU metadata incomplete (%s)"
                            __LOC__ ) ) )
             | _ ->
                 "" )
    in
  ...
  let mxgpu_device_in_use ~xs physical_function =
    ...
    try
      ...
      |> List.map (fun domid ->
             ...
             try List.map (fun x -> path ^ "/" ^ x) (xs.Xs.directory path) with
             | Xs_protocol.Enoent _ ->
                 [] )
      |> List.concat
  ...
  let call_gimtool args =
    try
      info
        "Initialising MxGPU PF: %s %s"
        !Xc_resources.gimtool
        (String.concat " " args) ;
      ignore (Forkhelpers.execute_command_get_output !Xc_resources.gimtool args)
    with
    | _ ->
        error
          "Failed to initialise MxGPU PF (%s %s)"
          !Xc_resources.gimtool
          (String.concat " " args) ;
  ...
  let configure_gim ~xs physical_function vgpus_per_gpu framebufferbytes =
    ...
    if not (mxgpu_device_in_use ~xs pf)
    then
    ...
    else info "MxGPU PF %s already initialised" pf
  ...
  let wait_path ~pidalive ~task ~name ~domid ~xs ~ready_path ~timeout ~cancel _
    ...
    | Watch.Timeout _ ->
        if pidalive name
        then raise (Ioemu_failed (name, "Timeout reached while starting daemon")) ;
  ...
  let stop ~xs ~qemu_domid domid =
    ...
    let stop_varstored () =
      ...
      Xenops_sandbox.Varstore_guard.stop
        dbg
        ~domid
  ...
  module Config_qemu_upstream_compat = struct
      ...
      let device ~xs ~domid ~info =
        let has_platform_device =
          ...
          | _ ->
              true
        in
        if has_platform_device
        then Dm_Common.xen_platform ~trad_compat:true ~xs ~domid ~info
        else []
    end
    ...
    module XenPV = struct
      let addr ~xs ~domid info ~nics =
        ...
        if has_nvidia_vgpu
        then 2
        else if XenPlatform.device ~xs ~domid ~info = []
        then 3
        else
          nics
  ...
  module QMP_Event = struct
    ...
    let remove domid =
      ...
      | e ->
          debug_exn
            (Printf.sprintf
               "Got exception trying to remove QMP on domain-%d"
               domid )
            e
    ...
    let add domid =
      ...
      | e ->
          debug_exn
            (Printf.sprintf
               "QMP domain-%d: negotiation failed: removing socket"
               domid )
            e ;
          remove domid ;
          raise
    ...
    let update_cant_suspend domid xs =
      ...
      | exception QMP_Error (_, msg) ->
        ...
        | Some _ ->
            debug
              "query-migratable ignoring precheck, qemu too old (domid=%d)"
              domid ;
            Generic.safe_rm ~xs path )
    ...
    let qmp_event_handle domid qmp_event =
      ...
      let rtc_change timeoffset =
        with_xs (fun xs ->
            ...
            | e ->
                error
                  "Failed to process RTC_CHANGE for domain %d: %s"
                  domid
                  (Printexc.to_string e) )
      in
    ...
    let process domid line =
      ...
      | msg ->
          debug
            "Got non-event message, domain-%d: %s"
            domid
    ...
    let qmp_event_thread () =
      let qmp_domid socket =
        try Some (Scanf.sscanf socket "qmp-event-%d" (fun d -> d)) with
        | _ ->
            None
      in
      let add_domain id =
      ...
      | e ->
          error
            "Connecting to existing QMP sockets failed: %s (%s)"
            (Printexc.to_string e)
            __LOC__ ) ;
      let forever = -1 in
      while true do
        try
          ...
          @@ Polly.wait m 10 forever (fun m fd events ->
                 ...
                 if Polly.Events.(test events inp)
                 then (
    ...
    module Vbd = struct
      ...
      let qemu_media_change ~xs device _type params =
        ...
        try
          ...
          if params = ""
          then qmp_send_cmd domid Qmp.(Eject (cd, Some true)) |> ignore
          else
            let as_msg cmd = Qmp.(Success (Some __LOC__, cmd)) in
    ...
    module Vcpu = struct
      ...
      let set ~xs ~devid domid online =
        ...
        | false ->
            ...
            let qom_path =
              qmp_send_cmd domid Qmp.Query_hotpluggable_cpus
              |> function
            ...
            in
            qom_path
            |> fun id -> qmp_send_cmd domid Qmp.(Device_del id) |> ignore
    end
    ...
    module Dm = struct
      ...
      let suspend (task : Xenops_task.task_handle) ~xs ~qemu_domid domid =
        ...
        let save_fd = Unix.openfile save_file perms 0o660 in
          ...
          (fun () ->
            let fd =
              qmp_send_cmd ~send_fd:save_fd domid Qmp.(Add_fd None)
              |> function
              | Qmp.(Fd_info fd) ->
                  fd
              | other ->
          ...
          (fun () ->
            ...
            do
              ...
              then (
                ...
                | e ->
                    debug
                      "QMP event socket for domid %d not yet ready: %s"
                      domid
                      (Printexc.to_string e) ;
                    Thread.delay 0.1 )
      ...
      let qemu_args ~xs ~dm info restore domid =
        ...
        let nic_type =
          try
            xs.Xs.read (sprintf "/local/domain/%d/platform/nic_type" domid)
          with
          | _ ->
              Config.NIC.default
        in
        let disk_type =
          try
            xs.Xs.read (sprintf "/local/domain/%d/platform/disk_type" domid)
          with
          | _ ->
              Config.DISK.default
        in
          ...
          | None ->
              raise
                (Ioemu_failed
                   ...
                   , sprintf
                       "Unknown platform:disk_type=%s in device-model=%s"
                       disk_type
                       Config.name ) )
        in
        if not (Config.Firmware.supported info.firmware)
        then
          ...
          raise
            (Ioemu_failed
               ( sprintf "domid %d" domid
               , sprintf
                   "The firmware doesn't support device-model=%s"
                   Config.name ) ) ;
        let qmp =
          [ "libxl"; "event" ]
          |> List.map (fun x ->
        ...
        let misc =
          List.concat
            ...
            ; ( info.Dm_Common.parallel
              |> function
              | None -> [ "-parallel"; "null" ] | Some x -> [ "-parallel"; x ]
              )
            ; qmp
        ...
        if nic_count > nic_max
        then debug "Limiting the number of emulated NICs to %d" nic_max ;
...
module Dm = struct
  ...
  let start_varstored
      ~xs ~nvram ?(restore = false) (task : Xenops_task.task_handle) domid =
    ...
    let chroot, socket_path =
      Xenops_sandbox.Varstore_guard.start
        (Xenops_task.get_dbg task)
        ~vm_uuid
        ~domid
        ~paths:[ efivars_save_path ]
    in
    let args =
      Add.many
        [ "--domain"
      ...
      >>= fun () ->
      Varstored.pidfile_path domid
      |> (function None -> return () | Some x -> Add.many [ "--pidfile"; x ])
      >>= fun () ->
      Add.many @@ argf "uuid:%s" vm_uuid
      >>= fun () ->
      on reset_on_boot @@ Add.arg "--nonpersistent"
      >>= fun () ->
      on restore @@ Add.arg "--resume"
      >>= fun () ->
      on restore
      @@ Add.many
      @@ argf
    ...
    wait_path
      ~pidalive:(Varstored.alive service)
      ~task
      ~name
      ~domid
      ~xs
  ...
  let start_vgpu ~xc ~xs task ?(restore = false) domid vgpus vcpus profile =
    ...
    | { physical_pci_address = pci; implementation = Nvidia vgpu } :: _ ->
        ...
        if not (Vgpu.is_running ~xs domid)
        then (
          ...
          info
            "Daemon %s is already running for domain %d"
            !Xc_resources.vgpu
            domid ;
        ...
        if cancellable_watch
             ...
             ~timeout:3600.
             ()
        then info "Daemon vgpu is ready"
        else
          let error_code_path =
  ...
  let __start
      ...
      ~xs
      ~dm
      ?(timeout = !Xenopsd.qemu_dm_ready_timeout)
      action
      info
      domid =
    let args =
    ...
    in
    debug
      "Device.Dm.start domid=%d args: [%s]"
      domid
    ...
    | Some _ ->
        ...
        waitpid_async qemu_pid ~callback:(fun qemu_crash ->
            Forkhelpers.(
              debug
                "Finished waiting qemu pid=%d for domid=%d"
                (getpid qemu_pid)
                domid ;
              let crash_reason =
                ...
                | None ->
                    debug
                      "domid=%d qemu-pid=%d: detected exit=0"
                      domid
                      (getpid qemu_pid) ;
                    "exit:0"
                | Some e ->
                  ...
                  | Subprocess_failed x ->
                      (* exit n *)
                      debug
                        "domid=%d qemu-pid=%d: detected exit=%d"
                        domid
                        (getpid qemu_pid)
                        x ;
                  ...
                  | Subprocess_killed x ->
                      ...
                      debug
                        "domid=%d qemu-pid=%d: detected signal=%d"
                        domid
                        (getpid qemu_pid)
                        x ;
              ...
              in
              if not (Qemu.SignalMask.has Qemu.signal_mask domid)
              then
                match Qemu.pid ~xs domid with
                | None ->
  ...
  let restore_vgpu
      (task : Xenops_task.task_handle) ~xc ~xs domid vgpus vcpus profile =
  ...
  let restore_varstored (task : Xenops_task.task_handle) ~xs ~efivars domid =
    ...
    let path =
      Xenops_sandbox.Varstore_guard.prepare
        ~domid
        ~vm_uuid:(Uuidm.to_string (Xenops_helpers.uuid_of_domid ~xs domid))
        efivars_resume_path
    in
...
let get_tc_port ~xs domid =
  (* Check whether a qemu exists for this domain *)
  let qemu_exists = Qemu.is_running ~xs domid in
  if qemu_exists then Dm.get_tc_port ~xs domid else PV_Vnc.get_tc_port ~xs domid
