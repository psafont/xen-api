...
let max_bytes_of_xenstore_entries entries =
  ...
  in
  if not (get "quota-activate" true bool_of_string)
  then (
    warn
  ...
  let call' =
    ...
    | "query", [ debug_info; unit_p ] ->
        ...
        Rpc.
          { name = "query"
          ; params =
              [ Rpc.Dict [ ("debug_info", debug_info); ("unit", unit_p) ] ]
          ; is_notification = false
          }
    | _ ->
        call
  ...
  Xapi_stdext_pervasives.Pervasiveext.finally
    (fun () ->
      ...
      let do_receive fn =
        ...
        fn
          uri
          req.Xenops_migrate.Forwarded_http_request.cookie
          this_connection
          context
      in
      let uri = req.Xenops_migrate.Forwarded_http_request.uri in
      if has_prefix uri memory_prefix
      then do_receive Xenops_server.VM.receive_memory
      else if has_prefix uri migrate_vgpu_prefix
      then do_receive Xenops_server.VM.receive_vgpu
      else (
        error
          "Expected URI prefix %s or %s, got %s"
          memory_prefix
          migrate_vgpu_prefix
          uri ;
        let module Response =
