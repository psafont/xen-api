...
module Handshake = struct
  ...
  let recv ?(verbose = false) (s : Unix.file_descr) : result =
    ...
    if verbose
    then debug "Handshake.recv: about to read result code from remote." ;
    ( try really_read s buf 0 (Bytes.length buf) with
    | _ ->
        raise (Remote_failed "unmarshalling result code from remote") ) ;
    if verbose
    then debug "Handshake.recv: finished reading result code from remote." ;
    let len =
    ...
    then Success
      ...
      if verbose
      then debug "Handshake.recv: about to read error message from remote." ;
      ( try really_read s msg 0 len with
      | _ ->
          raise (Remote_failed "unmarshalling error message from remote") ) ;
      if verbose
      then debug "Handshake.recv: finished reading error message from remote." ;
