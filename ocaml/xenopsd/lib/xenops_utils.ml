...
module MemFS = struct
  ...
  let exists path =
    Mutex.execute m (fun () ->
        try StringMap.mem (filename path) !(dir_locked (dirname path)) with
        | _ ->
            false )
  ...
  let readdir path =
    Mutex.execute m (fun () ->
        try StringMap.fold (fun x _ acc -> x :: acc) !(dir_locked path) [] with
        | _ ->
            [] )
...
let get_network_backend () =
  try String.trim (Unixext.string_of_file !Resources.network_conf) with
  | _ ->
      failwith
        (Printf.sprintf
           "Failed to read network backend from: %s"
...
let detect_hypervisor () =
  if Sys.file_exists _sys_hypervisor_type
  then
    let hypervisor =
...
let char_max_encoded_length =
  Array.init 256 (fun i ->
      if i >= 0x80
      then 3 (* Uutf.u_rep, see utf8_recode *)
      else
