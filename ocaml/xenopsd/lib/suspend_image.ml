...
let write_qemu_header_for_legacy_libxc fd size =
  wrap (fun () -> Io.write fd qemu_save_signature_legacy_libxc)
  >>= fun () ->
  wrap (fun () ->
...
let read_header fd =
  read_int64 fd
  >>= fun x ->
  header_type_of_int64 x
  >>= fun hdr -> read_int64 fd >>= fun len -> return (hdr, len)


let write_header fd (hdr_type, len) =
  write_int64 fd (int64_of_header_type hdr_type)
  >>= fun () -> write_int64 fd len


let check_conversion_script () =
  let open Unix in
  try return (access !Resources.legacy_conv_tool [ X_OK ]) with
  | _ ->
      Error
        (Failure
           (Printf.sprintf
              "Executable not found: %s"
              !Resources.legacy_conv_tool ) )


type 'a thread_status =
  | Running
  | Thread_failure of exn
  | Success of 'a
      ...
      | Forkhelpers.Spawn_internal_error (_, _, status) ->
        ...
        | Unix.WSIGNALED n ->
            Error
              (Failure
                 (Printf.sprintf
                    "Conversion script exited with signal %s"
                    (Unixext.string_of_signal n) ) )
        | Unix.WSTOPPED n ->
            Error
              (Failure
                 (Printf.sprintf
                    "Conversion script stopped with signal %s"
                    (Unixext.string_of_signal n) ) ) )
      | _ ->
          Error
            (Failure
               (Printf.sprintf
                  "Conversion script thread caught exception: %s"
                  (Printexc.to_string e) ) ) )
    | _, Thread_failure e ->
        Error
          (Failure
             (Printf.sprintf
                "Thread executing %s caught exception: %s"
                name
                (Printexc.to_string e) ) )
    | Running, _ | _, Running ->
        Condition.wait c m ;
