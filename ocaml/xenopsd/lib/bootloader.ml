...
let command bootloader q pv_bootloader_args image vm_uuid =
  ...
  let pv_bootloader_args =
    if pv_bootloader_args = ""
    then []
    else Astring.String.cuts ~sep:" " pv_bootloader_args
  in
...
let parse_output_simple x =
  ...
  let content =
    List.fold_left
      parse_line
      { kernel = None; ramdisk = None; args = None }
      linelist
  in
  { kernel_path =
...
let sanity_check_path p =
  ...
  | p ->
      ...
      | _ ->
          raise
            (Bad_error
               ( "Malicious guest? Bootloader returned a kernel or ramdisk \
...
let extract
  ...
  with
  | Forkhelpers.Spawn_internal_error (stderr, _stdout, _) ->
