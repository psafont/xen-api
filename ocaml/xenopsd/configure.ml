...
let find_ocamlfind verbose name =
  ...
  then
    Printf.fprintf
      stderr
      "querying for ocamlfind package %s: %s"
      name
...
let find_ml_val verbose name libs =
  ...
  let found =
    Sys.command
      (Printf.sprintf
         "ocamlfind ocamlc -package %s -c %s %s"
         (String.concat "," libs)
         ml_file
...
let expand start finish input output =
  ...
  then (
    Printf.fprintf
      stderr
      "Failed to expand %s -> %s in %s producing %s\n"
      start
      finish
      input
      output ;
...
let configure
  ...
  let lines =
    ...
    ; Printf.sprintf
        "ENABLE_XENTOOLLOG=--%s-xentoollog"
