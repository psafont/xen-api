  ...
  while not (StringSet.is_empty !ids) do
    ...
    then (
      Printf.fprintf
        ...
        (List.length deltas) ;
      flush stderr ) ;
    if List.length deltas = 0
    then failwith (Printf.sprintf "no deltas, next_id = %d" next_id) ;
    event_id := Some next_id ;
    List.iter
      (function
        | Dynamic.Task id' ->
...
let fail_not_built_task id =
  ...
  | Task.Failed x ->
    ( match
        Rpcmarshal.unmarshal Xenops_interface.Errors.error.Rpc.Types.ty x
      with
...
let vm_assert_equal vm vm' =
  ...
  assert_equal
    ~msg:"suppress_spurious_page_faults"
    ~printer:string_of_bool
    vm.suppress_spurious_page_faults
    vm'.suppress_spurious_page_faults ;
  assert_equal
    ~msg:"machine_address_size"
    ~printer:(function None -> "None" | Some x -> string_of_int x)
    vm.machine_address_size
    vm'.machine_address_size ;
  assert_equal
    ~msg:"memory_static_max"
    ~printer:Int64.to_string
    vm.memory_static_max
    vm'.memory_static_max ;
  assert_equal
    ~msg:"memory_dynamic_max"
    ~printer:Int64.to_string
    vm.memory_dynamic_max
    vm'.memory_dynamic_max ;
  assert_equal
    ~msg:"memory_dynamic_min"
    ~printer:Int64.to_string
    ...
    (fun () ->
      ...
      | Running | Paused ->
          ...
          | e ->
              Printf.fprintf
                stderr
                "Caught failure during with_vm cleanup: %s"
                (Printexc.to_string e) ;
              raise e )
      | _ ->
          () ) ;
      try Client.VM.remove dbg id with
      | e ->
          Printf.fprintf
            stderr
            "Caught failure during with_vm cleanup: %s"
...
module VbdDeviceTests = DeviceTests (struct
  ...
  let assert_equal vbd vbd' =
    ...
    assert_equal
      ...
      vbd'.unpluggable ;
    assert_equal
      ~msg:"extra_backend_keys"
      ~printer:sl
      vbd.extra_backend_keys
      vbd'.extra_backend_keys ;
    assert_equal
      ~msg:"extra_private_keys"
      ~printer:sl
