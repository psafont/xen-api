    ...
    let update generation key default f row =
      ...
      let updatefn () =
        StringMap.update
          ...
          default
          (fun x ->
            { stat = { x.stat with Stat.modified = generation }; v = f x.v } )
          row
      in
      if mem key row
      then
        ...
        let newv = f old in
        if newv = old then row else updatefn ()
      else updatefn ()
...
module Row = struct
  ...
  let find key t =
    try find key t with
    | Not_found ->
        raise (DBCache_NotFound ("missing field", key, ""))
...
module TableSet = struct
  ...
  let find key t =
    try find key t with
    | Not_found ->
        raise (DBCache_NotFound ("missing table", key, ""))
end
  ...
  let notify e db =
    List.iter
      (fun (name, f) ->
        ...
        | e ->
            Printf.printf
              "Caught %s from database callback '%s'\n%!"
              ...
              let vm_to_vbds =
                Table.fold
                  (fun vbd _ row acc ->
                    ...
                    in
                    let existing =
                      if Schema.ForeignMap.mem vm acc
                      then Schema.ForeignMap.find vm acc
                      else []
                    in
...
let update_one_to_many g tblname objref f db =
  if not (is_valid tblname objref db)
  then db
  else
    List.fold_left
