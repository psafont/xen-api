...
module From = struct
  let database schema (input : Xmlm.input) =
    ...
    let maybe_return f accu =
      if Xmlm.eoi input
      then
        if Stack.is_empty tags
        then accu
        else raise (Unmarshall_error "Unexpected end of file")
      else f accu
    in
                ...
                List.fold_left
                  (fun row ((_, k), v) ->
                    ...
                    try
                      ...
                      let value =
                        Schema.Value.unmarshal
                          column_schema.Schema.Column.ty
                    ...
                    | Not_found ->
                        ...
                        let exc =
                          Unmarshall_error
                            (Printf.sprintf
                               "Unexpected column in table %s: %s"
                               tblname
                               k )
                        in
                        raise exc )
