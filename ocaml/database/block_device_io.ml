...
let initialise_redo_log block_dev_fd timer =
  ...
  let deadline = Timer.deadline_of timer in
  Unixext.time_limited_write_substring block_dev_fd magic_size magic deadline ;
  Unixext.time_limited_write_substring block_dev_fd 2 "\0000" deadline
...
let open_block_device block_dev timer =
  ...
  let magic' =
    try Unixext.time_limited_read block_dev_fd 16 Timer.(deadline_of timer)
    with End_of_file -> ""
  in
  if magic' <> magic then (
...
let read_validity_byte block_dev_fd timer =
  ...
  let validity =
    Unixext.time_limited_read block_dev_fd 1 Timer.(deadline_of timer)
  in
  validity
...
let seek_past_last_record block_dev_fd half timer =
  ...
  while not !stop do
    ...
    let len_str =
      Unixext.time_limited_read block_dev_fd size_size Timer.(deadline_of timer)
    in
...
let read_database block_dev_fd timer =
  let read len =
    Unixext.time_limited_read block_dev_fd len Timer.(deadline_of timer)
  in
        ...
        (fun () ->
          ...
          Unixext.read_data_in_chunks
            (fun chunk len ->
              ...
              (* Otherwise write it *)
              Unixext.time_limited_write dest_fd len chunk
                Timer.(deadline_of timer) ;
