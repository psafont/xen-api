...
module Time : sig
  ...
  type t = {mutable timer: timer}

  let start () =
    if !times_out then
      {timer= Finite (Timer.start ~duration:!timeout)}
    else
      {timer= Infinite (Mtime_clock.counter ())}
  ...
  while not !write_ok do
    ...
    | e -> (
        ...
        | elapsed, Some (Expired excess) ->
            debug
              "Connection to master died. Time in this call is %a; timed out \
               %a ago"
        ...
        | elapsed, Some (Remaining spare) ->
            debug
              "Connection to master died. Time in this call is %a; will time \
               out in %a"
