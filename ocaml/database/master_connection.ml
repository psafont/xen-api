...
let retry_timeout = ref Db_globs.master_connection_retry_timeout

(* Whether xapi should restart after waiting to establish a master connection *)
let retry_times_out = ref false
...
module Call : sig
  (** Call tracks the amount of time spent retrying xmlrpc calls. This allows
      the reconnect code to wait indefinitely until the connection is
      established or wait for a certain amount of time when needed. The elapsed
      amount of time builds on Timer.elapsed to provide the amount of time
      since or after the timeout has been reached, if it exists, and it's
  ...
  val since : t -> Mtime.Span.t * Timer.countdown Option.t
end = struct
  ...
  let start () =
    if !retry_times_out then
      Finite (Timer.start ~duration:!retry_timeout)
    else
...
let do_db_xml_rpc_persistent_with_reopen ~host:_ ~path (req : string) :
  ...
  let call_started = Call.start () in
  ...
  let reconnect () =
    ...
    ( match Call.since call_started with
    | elapsed, None ->
