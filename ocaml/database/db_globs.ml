...
(** The maximum time for which we are prepared to wait for a response from the
    block device I/O process before assuming that it has died while emptying *)
let redo_log_max_block_time_empty = ref Mtime.Span.(2 * s)

(** The maximum time for which we are prepared to wait for a response from the
    block device I/O process before assuming that it has died while reading *)
let redo_log_max_block_time_read = ref Mtime.Span.(30 * s)

(** The maximum time for which we are prepared to wait for a response from the
    block device I/O process before assuming that it has died while writing a
    delta *)
...
let redo_log_initial_backoff_delay = Mtime.Span.(2 * s)

(** The maximum permitted backoff delay *)
let redo_log_maximum_backoff_delay = Mtime.Span.(2 * min)
...
(** The maximum time for which we are prepared to wait for a response from the
    block device I/O process before assuming that it has died while initially
    connecting to it *)
...
let master_connection_retry_timeout = Mtime.Span.(10 * s)

(* When we are a slave who has just started up we want to wait forever for
   the master to appear. (See CA-25481) *)
let master_connection_retry_times_out = ref false
