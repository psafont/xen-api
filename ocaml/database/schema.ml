  ...
  let _ =
    Printexc.register_printer (function
        | Error (expected, actual) ->
            Some
              (Printf.sprintf
                 "Schema.Type.Error: expected %s; received %s"
...
module Column = struct
  type t =
    ...
    ; empty : Value.t
          (** fresh value used when loading non-persistent fields *)
...
module Table = struct
  ...
  let find name t =
    try List.find (fun col -> col.Column.name = name) t.columns with
    | Not_found ->
...
module Database = struct
  ...
  let find name t =
    try List.find (fun tbl -> tbl.Table.name = name) t.tables with
    | Not_found ->
