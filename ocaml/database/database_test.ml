...
functor
  ...
  struct
    ...
    let expect_missing_column name f =
      try f () with
      | Db_exn.DBCache_NotFound ("missing column", _, name') when name' = name
        ->
          ()
    ...
    let check_ref_index t tblname key =
      ...
      | Some { Ref_index.name_label = name_label'; uuid; _ref } ->
          ...
          then
            failwith
              (Printf.sprintf
                 "check_ref_index %s key %s: got ref %s uuid %s"
                 tblname
                 key
                 _ref
                 uuid ) ;
          let real_ref =
            if Client.is_valid_ref t key
            then key
            else Client.db_get_by_uuid t tblname key
          in
          let real_name_label =
            try Some (Client.read_field t tblname name_label real_ref) with
            | _ ->
                None
          in
    ...
    let check_events t =
      let dump db g =
        ...
        Db_cache_types.TableSet.fold_over_recent
          g
          (fun name _ table _ ->
              ...
              (fun r { Db_cache_types.Stat.created; modified; deleted } _ _ ->
                let s =
                  try
                    ...
                    let s =
                      Db_cache_types.Row.fold_over_recent
                        g
                        (fun k _ v acc ->
                          Printf.sprintf
                            "%s %s=%s"
    ...
    let main in_process =
      ...
      if Client.is_valid_ref t invalid_ref
      then failwith "is_valid_ref <invalid_ref> = true" ;
      ...
      then
        failwith
          (Printf.sprintf
             "db_get_by_uuid <valid uuid>: got %s; expected %s"
             r
             valid_ref ) ;
      ...
      if Client.read_field t "VM" "protection_policy" valid_ref
         <> "OpaqueRef:NULL"
      then
        failwith
