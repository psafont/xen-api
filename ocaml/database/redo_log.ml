...
let read_separator sock timer =
  let sep = Unixext.time_limited_read sock 1 Timer.(deadline_of timer) in
...
let read_generation_count sock response_timer =
  ...
  let gen_count =
    Unixext.time_limited_read sock generation_size
      Timer.(deadline_of response_timer)
  in
...
let read_length sock timer =
  ...
  let length_str =
    Unixext.time_limited_read sock length_size Timer.(deadline_of timer)
  in
...
let read_length_and_string sock timer =
  ...
  read_separator sock timer ;
  let str = Unixext.time_limited_read sock length Timer.(deadline_of timer) in
  str
...
let rec read_read_response sock fn_db fn_delta expected_gen_count timer
    datasockpath =
  let hdr = Unixext.time_limited_read sock 5 Timer.(deadline_of timer) in
  if hdr <> "read|" then raise (CommunicationsProblem "expected 'read|'") ;
  let kind = Unixext.time_limited_read sock 5 Timer.(deadline_of timer) in
  match kind with
  | "db___" ->
...
let action_read fn_db fn_delta sock datasockpath =
  ...
  Unixext.time_limited_write sock (Bytes.length str) str
    Timer.(deadline_of response_timer) ;
  (* Read response *)
...
let action_write_db marker generation_count write_fn sock datasockpath =
  ...
  let response =
    Unixext.time_limited_read sock response_length
      Timer.(deadline_of response_timer)
  in
...
let reset_backoff_delay log =
  log.backoff := Timer.start ~duration:Db_globs.redo_log_initial_backoff_delay

let shortest a b = if Mtime.Span.compare a b > 0 then b else a

let delay_again log =
  let doubled_delay = Mtime.Span.(2 * Timer.duration !(log.backoff)) in
  let duration =
    shortest doubled_delay Db_globs.redo_log_maximum_backoff_delay
  in
  log.backoff := Timer.start ~duration ;
...
let maybe_retry f log =
  D.debug "Considering whether to attempt to flush the DB... (backoff %a)"
    Debug.Pp.mtime_span
    Timer.(duration !(log.backoff)) ;
  match Timer.remaining !(log.backoff) with
  | Remaining remaining ->
              ...
              try
                ...
                let response =
                  Unixext.time_limited_read s response_length
                    Timer.(deadline_of connect_timer)
                in
                match response with
                | "connect|ack_" ->
...
let create ~name ~state_change_callback ~read_only =
  let instance =
    ...
    ; backoff=
        ref (Timer.start ~duration:Db_globs.redo_log_initial_backoff_delay)
    ; sock= ref None
    ; pid= ref None
