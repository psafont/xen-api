...
module D = Debug.Make (struct
  let name = "parse_db_conf"
end)

open D

type db_connection_mode =
  | Write_limit
  | No_limit
...
let generation_read dbconn =
  ...
  try Generation.of_string (Unixext.string_of_file gencount_fname) with
  | _ ->
      0L
...
let from_block r =
  String.concat
    ...
    ; ( if r.mode = Write_limit
      then
        Printf.sprintf
          "write_limit_period:%d\nwrite_limit_write_cycles:%d\n"
          r.write_limit_period
          r.write_limit_write_cycles
      else "" )
    ; String.concat
        ""
        (List.map
  ...
  try
    ...
    let read_block () =
      ...
      let maybe_put_in key_name default conv_fn =
        if List.mem_assoc key_name !key_values
        then (
    ...
    while !lines <> [] do
      ...
      then connections := read_block () :: !connections
      else consume_line ()
    done ;
