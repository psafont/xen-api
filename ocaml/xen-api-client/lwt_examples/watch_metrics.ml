...
let main () =
  ...
  Session.login_with_password
    ...
    ~pwd:!password
    ~version:"1.0"
    ~originator:"watch_metrics"
  >>= fun session_id ->
  Lwt.finalize
    (fun () ->
      Host.get_all ~rpc ~session_id
      >>= fun hosts ->
      let host = List.hd hosts in
      Host.get_data_sources ~rpc ~session_id ~host
      >>= fun dsl ->
      let rec loop start =
        let open Cohttp_lwt_unix in
        let uri =
          Xen_api_metrics.Updates.uri
            ~host:(Uri.of_string !uri)
            ~authentication:(`UserPassword (!username, !password))
            ~start:(Int64.to_int start)
          ...
          (fun legend ->
            ...
            | `Ok ((_name, _cf, `Host, _uuid) as legend') ->
                ...
                then
                  Printf.fprintf
                    stderr
                    "Failed to find host data source: %s\n"
                    legend
            | `Ok _ ->
                ()
            | `Error (`Msg x) ->
...
let _ =
  Arg.parse
    ...
    ; ( "-start"
      , Arg.Set_int start
      , Printf.sprintf
          "Time since epoc to fetch updates from (default %d)"
          !start )
    ; ( "-interval"
