...
let main filename =
  ...
  Lwt.finalize
    (fun () ->
      ...
      Pool.get_default_SR ~rpc ~session_id ~self:pool
      >>= fun sr ->
      VDI.create
        ~rpc
        ~session_id
        ~name_label:"upload_disk"
        ~name_description:""
        ~sR:sr
        ~virtual_size
        ~_type:`user
      ...
      Lwt.catch
        (fun () ->
          ...
          Disk.start_upload ~chunked:false ~uri
          >>= fun oc ->
          let blocksize = 1024 * 1024 * 2 in
          let block = Cstruct.create blocksize in
          Lwt_unix.openfile filename [ Unix.O_RDONLY ] 0o0
          >>= fun fd ->
          Data_channel.of_fd ~seekable:true fd
          >>= fun ic ->
          let rec copy remaining =
            ...
            then return ()
              ...
              in
              ic.Data_channel.really_read block
              >>= fun () ->
              oc.Data_channel.really_write block
              >>= fun () ->
