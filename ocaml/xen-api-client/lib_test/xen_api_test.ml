...
module Fake_IO = struct
  ...
  let read_line ic =
    if Queue.is_empty ic
    then (*			Printf.fprintf stderr "read_line = None\n%!"; *)
      return None
    else
...
let test_login_fail _ =
  ...
  ( try
      let _session_id =
        C.Session.login_with_password
          ~rpc
          ~uname:"root"
          ~pwd:"password"
          ~version:"1.0"
          ~originator:"xen-api test"
      in
      ()
    with
  | Xen_api.No_response ->
...
let test_login_success _ =
  ...
  let rpc call =
    ...
    >>= function Ok x -> Xmlrpc.response_of_string x | Error e -> raise e
  in
  let session_id' =
    C.Session.login_with_password
      ~rpc
