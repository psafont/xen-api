...
let complete _name _offset op fd buffer =
  ...
  let rec loop acc fd buf ofs len =
    ...
    let len' = len - n in
    let acc' = acc + n in
    if len' = 0 || n = 0 then return acc' else loop acc' fd buf (ofs + n) len'
  in
  loop 0 fd buf ofs len
  >>= fun n -> if n = 0 && len <> 0 then fail End_of_file else return ()
...
let of_seekable_fd fd =
  ...
  >>= fun c ->
  let skip n =
    Lwt_unix.LargeFile.lseek fd n Unix.SEEK_CUR
    >>= fun offset ->
