...
let with_reusable_stunnel
    ?use_fork_exec_helper ?write_to_log ?verify_cert host port f =
...
let with_transport transport f =
  ...
  | SSL
      ...
      debug
        "stunnel pid: %d (cached = %b) connected to %s:%d"
        s_pid
        use_stunnel_cache
        host
        port ;
      ...
      let with_recorded_stunnelpid task_opt s_pid f =
        debug
          "with_recorded_stunnelpid task_opt=%s s_pid=%d"
      ...
      with_recorded_stunnelpid task_id s_pid (fun () ->
            ...
            (fun () ->
              ...
              | e ->
                  ...
                  then
                    if Sys.file_exists st_proc.Stunnel.logfile
            ...
            (fun () ->
              ...
              then (
                ...
                debug
                  "stunnel pid: %d (cached = %b) returned stunnel to cache"
                  s_pid
                  use_stunnel_cache )
              else (
...
let with_http request f s =
  try Http_client.rpc s request (fun response s -> f (response, s)) with
