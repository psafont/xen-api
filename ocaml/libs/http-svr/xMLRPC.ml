...
module To = struct
  ...
  let datetime s =
    value
      (box "dateTime.iso8601" [ pcdata (Xapi_stdext_date.Date.to_string s) ])
  ...
  let structure fields =
    value
      (box
         "struct"
         (List.map (fun (k, v) -> box "member" [ name k; v ]) fields) )
...
module From = struct
    ...
    unbox
      ...
      (function
        ...
        | y ->
            rtte
              (Printf.sprintf
                 "singleton: {%s} should be the singleton {%s}"
  ...
  let pair ok f1 f2 xml =
    unbox
      ok
      (function [ v1; v2 ] -> (f1 v1, f2 v2) | _ -> rtte "pair " xml)
      xml
