...
let tests =
  ...
  let test_roundtrip_conversion () =
    let non_utc = "20201220T18:10:19+02:00" in
    let result = non_utc |> of_iso8601 |> to_rfc3339 in
    Alcotest.(check string) "Roundtrip conversion be consistent" non_utc result
  in
  let test_ca333908 () =
