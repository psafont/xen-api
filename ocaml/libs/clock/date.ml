...
let print_tz tz_s =
  ...
  | Some tz ->
      ...
      let all_tz_minutes = tz / 60 in
      let tz_h = all_tz_minutes / 60 |> Int.abs in
      let tz_min = all_tz_minutes mod 60 |> Int.abs in
...
let _localtime_string tz t = remove_tz tz t |> to_rfc3339

let localtime () =
  remove_tz (Ptime_clock.current_tz_offset_s ()) (Ptime_clock.now ())
...
let equal x y = if x == y then true else try compare x y = 0 with _ -> false
