...
let calibrated_ratio =
  let expected = Mtime.Span.(100 * ms |> to_uint64_ns) |> Int64.to_float in
  let elapsed = Mtime_clock.counter () in
  Thread.delay 0.11 ;
  let actual = Mtime_clock.count elapsed |> Mtime.Span.to_uint64_ns |> Int64.to_float in
...
let wait_is_less =
  let cmp a b =
    let a =
      Mtime.Span.to_uint64_ns a
      |> Int64.to_float
      |> Float.mul calibrated_ratio
      |> Int64.of_float
...
let test_remove_self () =
  ...
  Alcotest.check wait_is_less "small time" expected elapsed
